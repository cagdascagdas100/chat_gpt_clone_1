...
$HeartbeatFile = Join-Path $BridgeRoot 'ai-heartbeat\autopilot-watcher-v4.md'
$LogDir = Join-Path $BridgeRoot 'ai-runner-logs'
$ResultDir = Join-Path $BridgeRoot 'ai-results'
$CacheDir = Join-Path $BridgeRoot 'ai-script-cache-v4'
$LastFile = Join-Path $BridgeRoot 'ai-tasks\.watcher-v4-last-task-id'
...
$WatcherLog = Join-Path $LogDir ('autopilot-watcher-v4-' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.log')
function Log([string]$m){ $line='['+(Get-Date -Format s)+'] '+$m; Write-Output $line; Add-Content -Encoding UTF8 -Path $WatcherLog -Value $line }
function HB([string]$status,[string]$taskId,[string]$msg){ @('# AAYS Autopilot Watcher V4 JSONFIX','',('Time: '+(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')),('Status: '+$status),('TaskId: '+$taskId),('BridgeRoot: '+$BridgeRoot),('TaskSource: git show '+$RepoRef+':'+$TaskPath),('WatcherLog: '+$WatcherLog),('Message: '+$msg),'Mode: fetch-show-json-extract-no-working-tree-reset','Reads: remote current-task via git show; extracts JSON; downloads script via git show; child process with timeout') | Set-Content -Encoding UTF8 -Path $HeartbeatFile }
function Git([string[]]$args){ try{ Push-Location $BridgeRoot; $o=(& git @args 2>&1 | Out-String); Add-Content -Encoding UTF8 -Path $WatcherLog -Value ('> git '+($args -join ' ')); Add-Content -Encoding UTF8 -Path $WatcherLog -Value $o; return $o } catch { return $_.Exception.Message } finally { try{Pop-Location}catch{} } }
function PushResults([string]$taskId){ try{ Push-Location $BridgeRoot; git add ai-results ai-heartbeat ai-tasks/.watcher-v4-last-task-id ai-runner-logs ai-script-cache-v4 2>&1 | Out-String | Add-Content -Encoding UTF8 $WatcherLog; $c=(git commit -m "Watcher V4 result $taskId" 2>&1 | Out-String); Add-Content -Encoding UTF8 -Path $WatcherLog -Value $c; if($c -notmatch 'nothing to commit'){ git pull --rebase origin main 2>&1 | Out-String | Add-Content -Encoding UTF8 $WatcherLog; git push origin main 2>&1 | Out-String | Add-Content -Encoding UTF8 $WatcherLog } } catch { Add-Content -Encoding UTF8 -Path $WatcherLog -Value ('PUSH_ERROR '+$_.Exception.Message) } finally { try{Pop-Location}catch{} } }
...
function ExtractJson([string]$raw){ if([string]::IsNullOrWhiteSpace($raw)){ return $null }; $s=$raw.Trim(); $s=$s -replace [char]0xFEFF,''; $first=$s.IndexOf('{'); $last=$s.LastIndexOf('}'); if($first -lt 0 -or $last -le $first){ return $null }; return $s.Substring($first, $last-$first+1) }
function ReadRemoteTask(){ try{ $raw=RemoteText $TaskPath; $json=ExtractJson $raw; if([string]::IsNullOrWhiteSpace($json)){ Log 'TASK_JSON_EXTRACT_EMPTY'; return $null }; $task=$json | ConvertFrom-Json; return $task } catch { Log ('TASK_READ_ERROR '+$_.Exception.Message); return $null } }
...
function RunTask($task){
  if($null -eq $task){ HB 'polling' '' 'no-remote-script-task'; return }
  $id=[string]$task.id
  if([string]::IsNullOrWhiteSpace($id)){ HB 'polling' '' 'remote task missing id after jsonfix'; return }
  ...
  $out=Join-Path $LogDir ($id+'-v4-stdout.log'); $err=Join-Path $LogDir ($id+'-v4-stderr.log')
  ...
  $res=Join-Path $ResultDir ((Get-Date -Format 'yyyy-MM-dd_HH-mm-ss')+'-'+$id+'-v4-result.md')
  @('# AAYS Autopilot Watcher V4 Result','',('TaskId: '+$id),('Script: '+$scriptName),('ExitCode: '+$exit),('Time: '+(Get-Date -Format s)),'','## STDOUT','```text') | Set-Content -Encoding UTF8 -Path $res
...
Log 'AAYS Autopilot Watcher V4 JSONFIX started.'
HB 'polling' '' 'started'
while($true){ try{ $task=ReadRemoteTask; RunTask $task } catch { Log ('LOOP_ERROR '+$_.Exception.Message); HB 'error' '' $_.Exception.Message }; if($Once){ break }; Start-Sleep -Seconds $PollSeconds }
