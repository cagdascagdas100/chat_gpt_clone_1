...
if (-not $gh) { [void]$blockers.Add('GITHUB_CLI_NOT_FOUND') }
...
try {
  if ($git -and (Test-Path -LiteralPath $RepoRoot -PathType Container)) {
    ...
    if ($before.exit_code -ne 0) { $facts.git_ls_remote_before_error = ($before.stderr -replace '(?i)(token|password|authorization)[:=]\S+','$1=REDACTED') }
  }

  if ($gh -and (Test-Path -LiteralPath $RepoRoot -PathType Container)) {
    ...
    $facts.gh_auth_status_summary = (($ghStatus.stdout + ' ' + $ghStatus.stderr) -replace '(?i)(token|password|authorization)[:=]\S+','$1=REDACTED').Trim()
  ...
  if (-not $Apply) {
    ...
    if ($blockers.Count -gt 0) {
      ...
      if ($setup.exit_code -ne 0) {
        ...
        $facts.gh_auth_setup_git_error = (($setup.stderr + ' ' + $setup.stdout) -replace '(?i)(token|password|authorization)[:=]\S+','$1=REDACTED').Trim()
        $status = 'BLOCKED_NONINTERACTIVE_PUBLISH_AUTH_APPLY'
      } else {
        ...
        if ($after.exit_code -ne 0) {
          ...
          $facts.git_ls_remote_after_error = ($after.stderr -replace '(?i)(token|password|authorization)[:=]\S+','$1=REDACTED')
          $status = 'BLOCKED_NONINTERACTIVE_PUBLISH_AUTH_APPLY'
        } else {
