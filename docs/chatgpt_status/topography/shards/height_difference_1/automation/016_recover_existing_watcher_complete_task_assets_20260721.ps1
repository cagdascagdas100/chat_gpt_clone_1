...
if (-not $OutputPath) {
  $OutputPath = Join-Path $RepoRoot 'docs\chatgpt_status\topography\shards\height_difference_1\runner_outputs\013_existing_watcher_complete_recovery_preflight_latest.json'
}
...
$RequiredPaths = @($QueueRel, $TopographyQueueRel, $MetricScriptRel, $Revision8EntryRel, $Revision8PayloadRel)
$RequiredPresence = [ordered]@{}
foreach ($Rel in $RequiredPaths) { $RequiredPresence[$Rel] = Test-GitPath $SourceRef $Rel }
...
$Blockers = @()
foreach ($Rel in $RequiredPaths) { if (!$RequiredPresence[$Rel]) { $Blockers += "SOURCE_ASSET_MISSING:$Rel" } }
...
$Preflight = [ordered]@{
  ...
  blockers = $Blockers
  task_asset_directories = $TaskAssetDirs
  task_queue_files = $TaskQueueFiles
...
if ($Blockers.Count -gt 0) { $Preflight.status = 'BLOCKED_COMPLETE_RECOVERY_PREFLIGHT'; Emit $Preflight; exit 2 }
if (!$Apply) {
  $Preflight.status = 'READY_FOR_COMPLETE_RECOVERY_APPLY'
...
function ReadTaskId([string]$Path) { try { $j = Get-Content $Path -Raw | ConvertFrom-Json; if ($j.task_id) { return [string]$j.task_id } } catch {}; return [IO.Path]::GetFileNameWithoutExtension($Path) }
function FileMatchesTask([string]$Path,[string]$Id) { try { $j = Get-Content $Path -Raw | ConvertFrom-Json; return ([string]$j.task_id -eq $Id) } catch { return ([IO.Path]::GetFileName($Path) -like "*$Id*") } }
function Known([string]$Id) { foreach ($s in @('pending','running','done','failed','processed','error')) { $d = Join-Path $BridgeRoot "ai-queue\$s"; if (Test-Path $d) { foreach ($f in @(Get-ChildItem $d -File -ErrorAction SilentlyContinue)) { if (FileMatchesTask $f.FullName $Id) { return $true } } } }; return $false }
function SyncDirectory([string]$Rel) { $src = Join-Path $WatchRepo ($Rel -replace '/','\'); $dst = Join-Path $RepoRoot ($Rel -replace '/','\'); if (!(Test-Path $src)) { throw "SOURCE_DIRECTORY_MISSING:$Rel" }; New-Item -ItemType Directory -Force -Path $dst | Out-Null; Copy-Item -Recurse -Force (Join-Path $src '*') $dst }
function SyncFile([string]$Rel) { $src = Join-Path $WatchRepo ($Rel -replace '/','\'); $dst = Join-Path $RepoRoot ($Rel -replace '/','\'); if (!(Test-Path $src)) { throw "SOURCE_FILE_MISSING:$Rel" }; New-Item -ItemType Directory -Force -Path (Split-Path -Parent $dst) | Out-Null; Copy-Item -Force $src $dst }
...
function PushSourceHeartbeat([string]$Text) {
  try {
    $lastPath = Join-Path $StateDir 'last_source_heartbeat_push.txt'; $now = Get-Date; $push = $true
    if (Test-Path $lastPath) { try { $last = [datetime](Get-Content $lastPath -Raw); if (($now - $last).TotalSeconds -lt $HeartbeatPushSeconds) { $push = $false } } catch {} }
    if (!$push) { return }
    ...
    if ($changes) {
      git -C $WatchRepo commit -m 'Update aays1 source-branch watcher heartbeat' | Out-Null
      git -C $WatchRepo pull --rebase origin $SourceBranch | Out-Null
      git -C $WatchRepo push origin "HEAD:$SourceBranch" | Out-Null
    }
    $now.ToString('o') | Set-Content -LiteralPath $lastPath -Encoding UTF8
  } catch { "heartbeat_push_error=$($_.Exception.Message)" | Set-Content -LiteralPath (Join-Path $StateDir 'last_source_push_error.txt') -Encoding UTF8 }
}
while ($true) {
  ...
  try {
    ...
    if (Test-Path $QueueDir) { Get-ChildItem $QueueDir -File -Filter '*.task.json' | Sort-Object LastWriteTime | ForEach-Object { $id = ReadTaskId $_.FullName; if (!(Known $id)) { Copy-Item -Force $_.FullName (Join-Path $PendingDir $_.Name); $copied += 1 } } }
    EnsureSingleRunner
    $hb = "status=WATCHING`npage_key=aays1`nsource_branch=$SourceBranch`nrepo_root=$RepoRoot`nwatch_repo=$WatchRepo`nbridge_root=$BridgeRoot`nqueue_dir=$QueueDir`nbridge_pending=$PendingDir`ntask_id=$TaskId`ncopied_this_loop=$copied`nupdated_at=$stamp`nfinal_ready=false"
    $hb | Set-Content -LiteralPath (Join-Path $StateDir 'heartbeat.txt') -Encoding UTF8
    PushSourceHeartbeat $hb
  } catch {
    $err = "status=WATCH_ERROR`npage_key=aays1`nsource_branch=$SourceBranch`nerror=$($_.Exception.Message)`nupdated_at=$stamp`nfinal_ready=false"
    $err | Set-Content -LiteralPath (Join-Path $StateDir 'last_error.txt') -Encoding UTF8
    PushSourceHeartbeat $err
  }
  Start-Sleep -Seconds 60
}
'@
$WatcherText = $Template.Replace('__REPO_ROOT__',$RepoRoot).Replace('__BRIDGE_ROOT__',$BridgeRoot).Replace('__WATCH_REPO__',$WatchRepo).Replace('__SOURCE_BRANCH__',$SourceBranch).Replace('__RESTORE_RUNNER__',$RestoreLiteral).Replace('__TASK_ID__',$TaskId).Replace('__ASSET_DIRS__',$AssetDirsLiteral).Replace('__QUEUE_FILES__',$QueueFilesLiteral)
...
Emit ([ordered]@{
  ...
  slot_id = 'height_difference_1'
  task_id = $TaskId
  status = if ($RunnerAfter.Count -eq 1) { 'COMPLETE_WATCHER_AND_EXISTING_SINGLE_RUNNER_RECOVERED' } else { 'COMPLETE_WATCHER_RECOVERED_EXISTING_RUNNER_RESTORE_PENDING' }
  source_branch = $SourceBranch
  ...
  complete_task_assets_synced = $true
  source_branch_heartbeat_enabled = $true
