...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    parser.add_argument("--expected-operation-rows", type=int, default=125)
    ...
    try:
        ...
        candidate_count = int(candidates.get("candidate_count", len(candidates.get("candidates", [])) if isinstance(candidates.get("candidates"), list) else 0))
        stages.append({"stage": "CANDIDATES_HTTP", "status": "PASS", "candidate_count": candidate_count, "candidate_status": candidates.get("status"), "sha256": hashlib.sha256(candidate_response.content).hexdigest()})
        payload = {"schema_version": 1, "slot_id": "height_difference_2", "status": "PORT_8012_WEB_ACCEPTANCE_PASSED", "base_url": base, "visible_operation_rows": len(numbers), "candidate_count": candidate_count, "stages": stages, "final_ready": False, "fake_data": False}
        code = 0
    except Exception as exc:
        payload = {"schema_version": 1, "slot_id": "height_difference_2", "status": "BLOCKED_PORT_8012_WEB_ACCEPTANCE", "error": f"{type(exc).__name__}: {exc}", "stages": stages, "final_ready": False, "fake_data": False}
        code = 2
    with open(args.output, "w", encoding="utf-8") as handle:
        json.dump(payload, handle, ensure_ascii=False, indent=2)
        handle.write("\n")
    print(json.dumps({"ok": code == 0, "status": payload["status"], "rows": payload.get("visible_operation_rows", 0)}))
    return code
