...
EXPECTED = {
    ...
    "carrier": "7f18620aa0344246c3dd96faff59737189669af1",
    ...
    "web_verifier": "68a4ee1bbd0976241dc68269ca0f662e31f14fd7",
}
...
def main() -> int:
    ...
    check("carrier_v64_rev13", TASK_VERSION in carrier and f"$pickupRequestRevision = {PICKUP_REVISION}" in carrier and EXPECTED["entrypoint"] in carrier and EXPECTED["numeric_gate"] in carrier and EXPECTED["web_verifier"] in carrier and EXPECTED["terrain_resolver"] in carrier and EXPECTED["terrain_crosschecker"] in carrier and EXPECTED["terrain_wrapper"] in carrier, "Carrier pins revision13 entrypoint, numeric, web and Terrain50 chain.")
    check("carrier_web_transport_contract", all(token in carrier for token in ["PORT_8012_CURRENT_CANDIDATE_LOCAL_SHA256_REQUIRED=true", "PORT_8012_HTTP_CANDIDATE_SHA256_REQUIRED=false", "PORT_8012_BASE_AUTOPROBE_REQUIRED=true", "PORT_8012_OPERATION_FILE_PATH_GUARD_REQUIRED=true", "PORT_8012_EXAMPLE_FILE_PATH_GUARD_REQUIRED=true"]), "Carrier publishes the local-candidate and actual-site web acceptance contract.")
    ...
    check("numeric_root_web_default", "http://127.0.0.1:8012/" in numeric_gate and "web_base_autoprobe_required" in numeric_gate, "Numeric gate uses the historical root form and requires base autoprobe.")
    check("web_local_candidate_and_base_probe", all(token in web_verifier for token in ["LOCAL_CURRENT_WORKTREE_PRE_ACCEPTANCE", "_candidate_bases", "CURRENT_WORKTREE_CANDIDATE_PAYLOAD", "EXACT_TARGET_EXAMPLES_HTTP", "site_measured_candidate_rows", "example_file_path_guard_verified", "_safe_slot_json_name"]) and "candidate HTTP SHA256 mismatch" not in web_verifier, "Web verifier separates local current-candidate identity from actual HTTP site evidence and probes safe bases.")
    ...
    check("request_runtime_blobs", request.get("required_carrier_blob_sha") == EXPECTED["carrier"] and request.get("required_entrypoint_blob_sha") == EXPECTED["entrypoint"] and request.get("required_official_numeric_gate_blob_sha") == EXPECTED["numeric_gate"] and request.get("required_terrain50_resolver_blob_sha") == EXPECTED["terrain_resolver"] and request.get("required_terrain50_crosschecker_blob_sha") == EXPECTED["terrain_crosschecker"] and request.get("required_terrain50_wrapper_blob_sha") == EXPECTED["terrain_wrapper"] and request.get("required_web_verifier_blob_sha") == EXPECTED["web_verifier"] and request.get("required_operator_recovery_blob_sha") == EXPECTED["operator"], "Restart request pins same revision13 runtime chain.")
    check("request_accuracy_web_contract", request.get("terrain50_accuracy_screening_required") is True and float(request.get("terrain50_conservative_two_rmse_sum_m", 0)) == 8.3 and request.get("terrain50_two_rmse_screening_is_confidence_interval") is False and request.get("web_current_candidate_sha256_required") is True and request.get("web_current_candidate_sha256_transport") == "LOCAL_CURRENT_WORKTREE_PRE_ACCEPTANCE" and request.get("web_http_candidate_sha256_required") is False and request.get("web_base_autoprobe_required") is True and request.get("web_operation_file_path_guard_required") is True and request.get("web_example_file_path_guard_required") is True and request.get("web_site_measured_target_rows_required") == TARGET_ROWS, "Restart request matches accuracy and corrected web-integrity safeguards.")

    passed = sum(1 for item in checks if item["passed"])
    payload = {"schema_version":10,"slot_id":"height_difference_2","task_id":TASK_ID,"attempt_id":ATTEMPT_ID,"task_version":TASK_VERSION,"pickup_request_revision":PICKUP_REVISION,"status":"PASS" if passed == len(checks) else "FAIL","passed":passed,"total":len(checks),"checks":checks,"actual_blob_sha":actual,"candidate_transport_contract":"LOCAL_CURRENT_WORKTREE_PRE_ACCEPTANCE_PLUS_HTTP_SITE_MANIFEST_EXAMPLES","operator_recovery_executed":False,"runner_restart_observed":False,"product_rows_promoted":0,"static_contract_only":True,"final_ready":False,"fake_data":False,"db_write":False,"migration":False,"production_deploy":False}
