...
Heartbeat evidence is read from the exact already-validated Git HEAD snapshot,
not from a possibly changing working-tree heartbeat file. The slot heartbeat
supplies stale_after_seconds; the global stable runner must be live, fresh, on
the canonical branch and not busy with another detected task. A receipt is
persisted before 044 and finalized only after 044 passes.
"""
