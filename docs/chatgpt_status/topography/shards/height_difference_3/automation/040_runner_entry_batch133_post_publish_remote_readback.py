#!/usr/bin/env python3
"""Post-publish readback with control-plane, manifest and remote-readback byte seals."""
...
RECEIPT_REL="docs/chatgpt_status/topography/shards/height_difference_3/runner_outputs/047_batch141_coordinator_rewire_receipt/coordinator_runtime_rewire_receipt.json"; VALIDATION_REL="docs/chatgpt_status/topography/shards/height_difference_3/runner_outputs/047_batch141_coordinator_rewire_receipt/coordinator_runtime_rewire_receipt_validation.json"; ACCEPTANCE_REL="docs/chatgpt_status/topography/shards/height_difference_3/runner_outputs/029_batch131_strict12_acceptance/batch131_strict12_local_acceptance.json"; MANIFEST_REL="docs/chatgpt_status/topography/shards/height_difference_3/runner_outputs/031_batch132_remote_readback/batch132_publish_manifest.json"; REMOTE_REL="docs/chatgpt_status/topography/shards/height_difference_3/runner_outputs/031_batch132_remote_readback/batch132_origin_remote_readback.json"
...
def main()->int:
    ...
    if int(h.get("schema_version") or 0)<8 or h.get("status")!="PUBLISH_PENDING_SERIAL_PUBLISHER_REQUIRED" or [int(v) for v in h.get("expected_rows") or []]!=EXPECTED_ROWS:raise ValueError("pre-publish handoff seal/schema mismatch")
    if h.get("runtime_identity_match_passed") is not True or h.get("fresh_host_heartbeat_passed") is not True or h.get("runtime_preflight_freshness_and_head_binding_passed") is not True or h.get("control_plane_receipt_validation_seal_passed") is not True or h.get("data_plane_acceptance_manifest_seal_passed") is not True:raise ValueError("pre-publish runtime/control/data-plane gates missing")
    receipt=repo/RECEIPT_REL; validation=repo/VALIDATION_REL; acceptance=repo/ACCEPTANCE_REL; manifest=repo/MANIFEST_REL
    for p in (receipt,validation,acceptance,manifest):
        if not p.is_file():raise FileNotFoundError(p)
    expected_receipt=str(h.get("coordinator_receipt_sha256") or ""); expected_validation=str(h.get("coordinator_receipt_validation_sha256") or ""); expected_acceptance=str(h.get("strict_local_acceptance_sha256") or ""); expected_manifest=str(h.get("publish_manifest_sha256") or "")
    if any(len(x)!=64 for x in (expected_receipt,expected_validation,expected_acceptance,expected_manifest)):raise ValueError("handoff byte seal missing")
    before={"receipt":sha(receipt),"validation":sha(validation),"acceptance":sha(acceptance),"manifest":sha(manifest)}; expected={"receipt":expected_receipt,"validation":expected_validation,"acceptance":expected_acceptance,"manifest":expected_manifest}
    if before!=expected:raise RuntimeError(f"CONTROL_OR_DATA_PLANE_SEAL_CHANGED_BEFORE_REMOTE_READBACK:{before}:{expected}")
    val=load(validation); m=load(manifest)
    ...
    if m.get("task_id")!=TASK_ID or m.get("continuation_key")!=CONTINUATION or [int(v) for v in m.get("expected_rows") or []]!=EXPECTED_ROWS or len(m.get("files") or [])!=7:raise ValueError("handoff/manifest identity mismatch")
    pre=str(h.get("pre_publish_origin_head") or "").strip().lower()
    if len(pre)!=40 or h.get("pre_publish_origin_fetch_performed") is not True or str(m.get("pre_publish_origin_head") or "").strip().lower()!=pre:raise ValueError("pre-publish origin proof missing")
    ...
    after={"receipt":sha(receipt),"validation":sha(validation),"acceptance":sha(acceptance),"manifest":sha(manifest)}
    if after!=expected:raise RuntimeError(f"CONTROL_OR_DATA_PLANE_SEAL_CHANGED_DURING_REMOTE_READBACK:{after}:{expected}")
    remote_path=repo/REMOTE_REL
    if not remote_path.is_file():raise FileNotFoundError(remote_path)
    remote_sha=sha(remote_path); remote=load(remote_path)
    ...
    if sha(remote_path)!=remote_sha or {"receipt":sha(receipt),"validation":sha(validation),"acceptance":sha(acceptance),"manifest":sha(manifest)}!=expected:raise RuntimeError("REMOTE_READBACK_OR_SEALED_INPUT_CHANGED_BEFORE_FINAL_ACCEPTANCE")
    out=repo/"docs/chatgpt_status/topography/shards/height_difference_3/runner_outputs/033_batch133_coordinator_handoff/batch133_post_publish_remote_acceptance.json"; payload={"schema_version":6,"slot_id":"height_difference_3","task_id":TASK_ID,"continuation_key":CONTINUATION,"status":"REMOTE_HISTORY_CONTROL_AND_DATA_PLANE_SEAL_ACCEPTED_12_ROWS","runtime_python_executable":str(Path(sys.executable).resolve()),"runtime_powershell_executable":ps,"runtime_git_executable":gx,"runtime_identity_match_passed":True,"fresh_host_heartbeat_passed":True,"runtime_preflight_freshness_and_head_binding_passed":True,"control_plane_receipt_validation_seal_passed":True,"data_plane_acceptance_manifest_seal_passed":True,"coordinator_receipt_sha256":expected_receipt,"coordinator_receipt_validation_sha256":expected_validation,"strict_local_acceptance_sha256":expected_acceptance,"publish_manifest_sha256":expected_manifest,"remote_readback_sha256":remote_sha,"coordinator_receipt_binding_key_sha256":h.get("coordinator_receipt_binding_key_sha256"),"coordinator_action_id":h.get("coordinator_action_id"),"coordinator_receipt_nonce":h.get("coordinator_receipt_nonce"),"pre_publish_origin_head":pre,"remote_head":remote.get("remote_head"),"history_mode":history,"first_full_blob_materialization_commit":material,"materialization_commit_delta_gate_mode":delta,"materialization_commit_changes_all_manifest_paths":remote.get("materialization_commit_changes_all_manifest_paths"),"publisher_commit_candidate":candidate,"remote_history_binding_passed":True,"remote_history_and_commit_delta_binding_passed":True,"expected_rows":EXPECTED_ROWS,"verified_count":12,"remote_file_count":7,"all_remote_blobs_match":True,"numeric_publish_acceptance_for_12_rows":True,"sealed_inputs_unchanged_before_and_after_remote_readback":True,"remote_readback_byte_identity_preserved_through_acceptance":True,"child_direct_push_performed":False,"numeric_values_changed":0,"new_task_created":False,"new_runner_created":False,"parallel_runner_used":False,"overall_product_final_ready":False,"final_ready":False,"fake_data":False,"remote_readback_stage":rr}; write(out,payload);print(json.dumps({"ok":True,"status":payload["status"],"pre_publish_origin_head":pre,"materialization_commit":material,"manifest_sha256":expected_manifest,"remote_readback_sha256":remote_sha,"output":str(out)}));return 0
