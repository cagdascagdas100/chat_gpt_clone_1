#!/usr/bin/env python3
"""Fail-closed same-task coordinator wiring validator for height_difference_3 Batch138."""
...
def main()->int:
    ...
    ck("schema5",int(req.get("schema_version") or 0)>=5); ck("identity",req.get("task_id")==TASK_ID and req.get("attempt_id")==ATTEMPT and req.get("continuation_key")==CONT)
    ...
    reads=list(task.get("read_paths") or []); outs=list(task.get("expected_outputs") or []); ck("task_47_reads",len(reads)==NREAD,len(reads)); ck("task_19_outputs",len(outs)==NOUT,len(outs)); ck("resume076_readable",R076 in reads); ck("env_identity_is_generated_output",ENVOUT in outs and ENVOUT not in reads)
    ...
    paths=[REQUEST,TASK,QUEUE,RUN039,POST040,READ038,V041,B042,H043,E044,S036,S033,S032,R076]; rb={}; rows=[]
    for rel in paths:
    ...
    vc=req.get("validator_chain") or {}; ic=req.get("runtime_identity_chain") or {}; ov=req.get("coordinator_runtime_override") or {}
    pins=[("043",vc.get("exact_branch_head_gate_expected_blob_sha"),H043),("044",vc.get("runtime_environment_gate_expected_blob_sha"),E044),("042",vc.get("fresh_origin_bootstrap_expected_blob_sha"),B042),("041",vc.get("same_task_validator_expected_blob_sha"),V041),("039",ov.get("runtime_script_expected_blob_sha"),RUN039),("040",ov.get("post_publish_script_expected_blob_sha"),POST040),("038",ic.get("remote_readback_038_expected_blob_sha"),READ038),("036",ic.get("strict036_expected_blob_sha"),S036),("033",ic.get("strict033_expected_blob_sha"),S033),("032",ic.get("strict032_expected_blob_sha"),S032),("076",ic.get("resume_076_expected_blob_sha"),R076)]
    ...
    os=str(own.get("state") or ""); oid=own.get("owner_page_session_id"); ck("owner_safe",os=="UNCLAIMED" or (os=="CLAIMED" and oid=="chatgpt-height-difference-3-batch138-20260726"),{"state":os,"owner":oid})
    out=repo/"docs/chatgpt_status/topography/shards/height_difference_3/runner_outputs/035_batch134_coordinator_wiring_qa/coordinator_wiring_request_validation.json"; payload={"schema_version":6,"slot_id":"height_difference_3","task_id":TASK_ID,"continuation_key":CONT,"status":"ALREADY_ALIGNED" if queue.get("script_path")==RUN039 else "SAFE_FOR_COORDINATOR_RUNTIME_REWIRE_AFTER_ALL_PREFLIGHT_GATES","checks_passed":len(checks),"checks_total":len(checks),"checks":checks,"fresh_remote_head":remote,"critical_blob_parity":rows,"expected_read_path_count":NREAD,"expected_output_count":NOUT,"runtime_executable_identity_chain_pinned":True,"runtime_environment_record_is_generated_expected_output":True,"fresh_host_heartbeat_still_required":True,"coordinator_action_performed":False,"legacy_queue_mutated":False,"new_task_created":False,"new_runner_created":False,"numeric_values_written":0,"expected_rows":EXPECTED_ROWS,"final_ready":False,"fake_data":False}; out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+"\n",encoding="utf-8"); print(json.dumps({"ok":True,"checks":len(checks),"output":str(out)})); return 0
