#!/usr/bin/env python3
"""Same-task Strict12 entrypoint with fresh TTL rechecks and atomic handoff."""
...
def main() -> int:
    ...
    if local_head_result["exit_code"] != 0 or len(local_head) != 40 or local_head != str(environment.get("canonical_head") or "").lower():
        raise ValueError("runtime local HEAD drift")
    task_blob_result = run([git_executable, "-C", str(repo), "rev-parse", f"HEAD:{TASK_REL}"], repo)
    task_blob = task_blob_result["stdout"].strip().lower()
    if task_blob_result["exit_code"] != 0 or len(task_blob) != 40 or task_blob != str(environment.get("canonical_current_task_blob_sha") or "").lower():
    ...
    head_result = run([git_executable, "-C", str(repo), "rev-parse", f"refs/remotes/origin/{BRANCH}"], repo)
    pre_publish_origin_head = head_result["stdout"].strip().lower()
    if head_result["exit_code"] != 0 or len(pre_publish_origin_head) != 40:
        raise RuntimeError("cannot resolve pre-publish origin HEAD")
    ...
    payload = {
        ...
        "numeric_final_acceptance": "PENDING_SERIAL_PUBLISH_REMOTE_HISTORY_CONTROL_PLANE_AND_DATA_PLANE_SEAL_READBACK",
        ...
        "fake_data": False,
        "stages": {
            "coordinator_rewire_receipt_validation": validation_result,
    ...
    print(json.dumps({
        ...
        "pre_publish_origin_head": pre_publish_origin_head,
        "receipt_sha256": receipt_sha_validated,
        "validation_sha256": validation_sha_validated,
