#!/usr/bin/env python3
"""Generate evidence-bound coordinator receipt for the Batch143 sealed wrapper chain."""
...
def main()->int:
    ...
    if a.effective_runtime_script!=EFFECTIVE_RUN or a.effective_post_publish_script!=EFFECTIVE_POST: raise ValueError("sealed effective entrypoint mismatch")
    ...
    rb=blob(g,r,"HEAD",REQUEST_REL); tb=blob(g,r,"HEAD",TASK_REL); qb=blob(g,r,"HEAD",QUEUE_REL)
    th=hits(git(g,r,"grep","-l","-F",TASK_ID,"HEAD","--",QUEUE_ROOT,check=False)); ah=hits(git(g,r,"grep","-l","-F",ATTEMPT_ID,"HEAD","--",QUEUE_ROOT,check=False)); ih=hits(git(g,r,"grep","-l","-F",IDEMPOTENCY,"HEAD","--",QUEUE_ROOT,check=False)); combined=sorted(set(th)&set(ah)&set(ih))
    ...
    for k,rel in PREFLIGHT_OUTPUTS.items():
        ...
        ph[k]=sha(p)
    nonce=secrets.token_hex(16); created=datetime.now(timezone.utc).isoformat().replace("+00:00","Z"); key=bind([TASK_ID,ATTEMPT_ID,IDEMPOTENCY,CONTINUATION,rb,tb,qb,local,*(ph[k] for k in ("043","045","044","042","041")),EFFECTIVE_RUN,EFFECTIVE_POST,UNDER_RUN,UNDER_POST,action,nonce])
    payload={"schema_version":3,"slot_id":"height_difference_3","status":"COORDINATOR_RUNTIME_REWIRE_RECEIPT","task_id":TASK_ID,"attempt_id":ATTEMPT_ID,"idempotency_key":IDEMPOTENCY,"continuation_key":CONTINUATION,"receipt_created_at_utc":created,"coordinator_action_id":action,"receipt_nonce":nonce,"local_head":local,"fresh_origin_head":remote,"fresh_origin_fetch_refspec":spec,"request_blob_sha":rb,"current_task_blob_sha":tb,"source_queue_blob_sha":qb,"queue_census_basis":["task_id","attempt_id","idempotency_key"],"matching_queue_record_count":1,"matching_queue_records":combined,"preflight_output_sha256":ph,"binding_key_sha256":key,"runtime_override_applied":True,"effective_runtime_script":EFFECTIVE_RUN,"effective_post_publish_script":EFFECTIVE_POST,"underlying_runtime_script":UNDER_RUN,"underlying_post_publish_script":UNDER_POST,"sealed_wrapper_chain_required":True,"coordinator_only":True,"new_queue_record_created":False,"new_task_created":False,"new_runner_created":False,"parallel_runner_used":False,"queue_mutated_by_slot":False,"numeric_values_written":0,"final_ready":False,"fake_data":False}
    out=r/RECEIPT_REL; out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+"\n",encoding="utf-8"); print(json.dumps({"ok":True,"status":payload["status"],"action_id":action,"queue_records":1,"head":local,"receipt_nonce":nonce,"binding_key":key,"effective_runtime_script":EFFECTIVE_RUN,"effective_post_publish_script":EFFECTIVE_POST,"output":str(out)})); return 0
