...
MAX_DOWNLOAD_BYTES = 1500000000
MAX_EXTRACTED_BYTES = 2000000000
...
def _normal_authority(value: Any) -> str:
    text = re.sub('[^a-z0-9]+', ' ', str(value or '').casefold()).strip()
    return ' '.join(text.split())

def _authority_key(value: Any) -> str:
    text = _normal_authority(value)
    prefixes = ('the ', 'city of ', 'london borough of ', 'royal borough of ', 'metropolitan borough of ')
    changed = True
    while changed:
        changed = False
        for prefix in prefixes:
            if text.startswith(prefix):
                ...
                changed = True
                break
    suffixes = (' metropolitan borough council', ' london borough council', ' borough council', ' district council', ' city council', ' county council', ' council')
    changed = True
    while changed:
        changed = False
...
def _slug(value: str) -> str:
    return re.sub('[^a-z0-9]+', '-', value.casefold()).strip('-') or 'authority'
...
def _stream_download(session: requests.Session, url: str, output: Path, timeout: int) -> dict[str, Any]:
    ...
    fd, temp_name = tempfile.mkstemp(prefix=f'.{output.name}_', suffix='.download.tmp', dir=output.parent)
    os.close(fd)
    temp = Path(temp_name)
    total = 0
    resolved_url = ''
    ...
    try:
        ...
        temp.replace(output)
        return {'resolved_url': resolved_url, 'resolved_host': urlparse(resolved_url).hostname, 'content_type': content_type, 'size_bytes': total, 'trusted_redirect': True, 'atomic_materialization': True}
    except Exception:
        temp.unlink(missing_ok=True)
        raise

def _safe_extract_gml(archive: Path, output_dir: Path) -> list[Path]:
    """Extract all accepted vectors transactionally, then publish as one tree."""
    output_dir.parent.mkdir(parents=True, exist_ok=True)
    if output_dir.exists() and (not output_dir.is_dir()):
        raise ValueError(f'HMLR extraction target exists and is not a directory: {output_dir}')
    stage_dir = Path(tempfile.mkdtemp(prefix=f'.{output_dir.name}_', suffix='.extract.stage', dir=output_dir.parent))
    backup_dir = Path(tempfile.mkdtemp(prefix=f'.{output_dir.name}_', suffix='.extract.backup', dir=output_dir.parent))
    backup_dir.rmdir()
    ...
    try:
        with zipfile.ZipFile(archive) as zf:
            for info in zf.infolist():
                ...
                if info.is_dir() or suffix not in {'.gml', '.xml'}:
                    continue
                if info.file_size <= 0:
                    raise ValueError(f'HMLR archive member has invalid size: {info.filename}')
                ...
                if folded in seen_targets:
                    raise ValueError(f'HMLR archive contains duplicate flattened vector name: {name}')
                ...
                with zf.open(info) as source, target.open('wb') as destination:
                    while (chunk := source.read(1024 * 1024)):
                        ...
                        if written > info.file_size:
                            raise ValueError(f'HMLR member expanded beyond declared size: {info.filename}')
                        destination.write(chunk)
                    destination.flush()
                    os.fsync(destination.fileno())
                if written != info.file_size:
                    raise ValueError(f'HMLR member size mismatch after extraction: {info.filename} expected={info.file_size} actual={written}')
        ...
        if backup_dir.exists():
            ...
            previous_moved = False
        return [output_dir / name for name in staged_names]
    finally:
        if stage_dir.exists():
            shutil.rmtree(stage_dir)
        if backup_dir.exists():
            if previous_moved and (not output_dir.exists()):
                backup_dir.replace(output_dir)
            else:
                shutil.rmtree(backup_dir)
        if not published and output_dir.exists() and output_dir.is_dir():
            pass
    ...
    for authority in authorities:
        ...
        for context, href, anchor_text in parser_html.links:
            ...
            if target_key in {context_key, href_key}:
                matches.append({'context': context, 'anchor_text': anchor_text, 'url': resolved, 'target_authority_key': target_key, 'context_authority_key': context_key, 'href_authority_key': href_key, 'match_method': 'EXACT_NORMALIZED_AUTHORITY_CONTEXT_OR_HREF'})
        ...
        for vector in vectors:
            info = {'path': str(vector), 'size_bytes': vector.stat().st_size, 'sha256': _sha256(vector), 'atomic_materialization': True}
            vector_info.append(info)
            vector_paths.append(str(vector))
        records.append({'authority': authority, 'normalized_authority_key': target_key, 'authority_match_method': match['match_method'], 'download_link': match, 'resolved_url': meta['resolved_url'], 'resolved_host': meta['resolved_host'], 'trusted_redirect': meta['trusted_redirect'], 'content_type': meta['content_type'], 'container_type': container_type, 'vectors': vector_info})
    status = 'READY' if len(records) == len(authorities) and (not blocked) else 'BLOCKED_HMLR_SOURCE_PREPARATION'
    payload = {'schema_version': 4, 'slot_id': 'height_difference_3', 'status': status, 'download_page': args.download_page, 'download_page_resolved_url': page_response.url, 'download_page_sha256': hashlib.sha256(page_body).hexdigest(), 'official_entry_host': OFFICIAL_HOST, 'trusted_download_hosts': sorted(TRUSTED_DOWNLOAD_HOSTS), 'candidate_count': len(candidates), 'authority_count': len(authorities), 'prepared_authority_count': len(records), 'records': records, 'blocked': blocked, 'vector_paths': vector_paths, 'authority_match_policy': 'EXACT_NORMALIZED_AUTHORITY_CONTEXT_OR_HREF_ONLY', 'nearest_or_fuzzy_authority_match_used': False, 'atomic_download_materialization': True, 'atomic_vector_materialization': True, 'archive_tree_transactional_publish': True, 'manifest_atomic_materialization': True, 'duplicate_flattened_vector_names_forbidden': True, 'partial_canonical_source_files_forbidden': True, 'measurement_values_written': 0, 'final_ready': False, 'fake_data': False, 'db_write': False, 'migration': False, 'production_deploy': False}
    output = args.output_dir / 'hmlr_source_manifest.json'
    _write(output, payload)
    print(json.dumps({'ok': status == 'READY', 'status': status, 'manifest': str(output), 'atomic_source_materialization': True}))
