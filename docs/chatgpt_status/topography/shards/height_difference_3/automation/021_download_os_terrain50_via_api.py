    ...
    try:
        with request(url, timeout, api_key) as response, temp.open("wb") as handle:
            ...
            if not str(final_url).startswith("https://"):
                raise ValueError(f"OS download resolved to non-HTTPS URL: {final_url}")
...
def _cache_provenance_ok(previous: dict[str, Any], archive: Path, archive_sha256: str) -> tuple[bool, list[str]]:
    reasons = []
...
def main() -> int:
    ...
    if candidate_cache and precomputed_archive_sha:
        cache_reused, cache_reuse_reasons = _cache_provenance_ok(previous_manifest, archive, precomputed_archive_sha)

    if cache_reused:
        selected = {
            "cache_reused": True,
            "max_cache_age_hours": args.max_cache_age_hours,
            "prior_selected_download_metadata": previous_manifest.get("selected_download_metadata"),
        }
        ...
        with request(catalog_url(args.api_key), args.timeout, args.api_key) as response:
            ...
            if not str(final_catalog_url).startswith("https://api.os.uk/"):
                raise ValueError(f"OS catalog resolved off official API host: {final_catalog_url}")
    ...
    if cache_reused and archive_hash.lower() != str(previous_manifest.get("archive_sha256") or "").lower():
    ...
    manifest = {
        ...
        "cache_reuse_rejection_reasons": [] if cache_reused else cache_reuse_reasons,
