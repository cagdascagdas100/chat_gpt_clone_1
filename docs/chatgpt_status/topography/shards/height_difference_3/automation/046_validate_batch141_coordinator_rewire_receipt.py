...
The legacy queue record predates continuation_key, so duplicate census is based on the
stable task_id + attempt_id + idempotency_key triple. Continuation is still required on
the canonical current-task, request and coordinator receipt. Queue state is read-only.
"""
...
def main()->int:
    ...
    receipt=load_json(receipt_path); task=load_json(repo/TASK_REL); request=load_json(repo/REQUEST_REL); queue=load_json(repo/QUEUE_REL); checks=[]
    ...
    request_blob=git_blob(git,repo,"HEAD",REQUEST_REL); task_blob=git_blob(git,repo,"HEAD",TASK_REL); queue_blob=git_blob(git,repo,"HEAD",QUEUE_REL); require("request_blob_binding",receipt.get("request_blob_sha")==request_blob,{"receipt":receipt.get("request_blob_sha"),"actual":request_blob}); require("task_blob_binding",receipt.get("current_task_blob_sha")==task_blob,{"receipt":receipt.get("current_task_blob_sha"),"actual":task_blob}); require("queue_blob_binding",receipt.get("source_queue_blob_sha")==queue_blob,{"receipt":receipt.get("source_queue_blob_sha"),"actual":queue_blob})
    task_hits=normalize_grep_hits(run_git(git,repo,"grep","-l","-F",TASK_ID,"HEAD","--",QUEUE_ROOT,check=False)); attempt_hits=normalize_grep_hits(run_git(git,repo,"grep","-l","-F",ATTEMPT_ID,"HEAD","--",QUEUE_ROOT,check=False)); idem_hits=normalize_grep_hits(run_git(git,repo,"grep","-l","-F",IDEMPOTENCY,"HEAD","--",QUEUE_ROOT,check=False)); combined_hits=sorted(set(task_hits).intersection(attempt_hits).intersection(idem_hits)); require("single_queue_record_for_stable_identity",combined_hits==[QUEUE_REL],{"task_hits":task_hits,"attempt_hits":attempt_hits,"idempotency_hits":idem_hits,"intersection":combined_hits}); require("receipt_duplicate_census",receipt.get("queue_census_basis")==["task_id","attempt_id","idempotency_key"] and int(receipt.get("matching_queue_record_count") or -1)==1 and receipt.get("matching_queue_records")==[QUEUE_REL],{"basis":receipt.get("queue_census_basis"),"count":receipt.get("matching_queue_record_count"),"records":receipt.get("matching_queue_records")})
    ...
    for key,rel in PREFLIGHT_OUTPUTS.items():
        ...
        actual=sha256_file(path); actual_hashes[key]=actual; require(f"preflight_{key}_sha256",receipt_hashes.get(key)==actual,{"receipt":receipt_hashes.get(key),"actual":actual})
    expected_binding=binding_key([TASK_ID,ATTEMPT_ID,IDEMPOTENCY,CONTINUATION,request_blob,task_blob,queue_blob,local_head,*(actual_hashes[k] for k in ("043","045","044","042","041")),RUN039,POST040]); require("receipt_binding_key",receipt.get("binding_key_sha256")==expected_binding,{"receipt":receipt.get("binding_key_sha256"),"expected":expected_binding})
    output=repo/VALIDATION_REL; result={"schema_version":2,"slot_id":"height_difference_3","task_id":TASK_ID,"attempt_id":ATTEMPT_ID,"idempotency_key":IDEMPOTENCY,"continuation_key":CONTINUATION,"status":"COORDINATOR_REWIRE_RECEIPT_VALIDATED","validated_at_utc":now.isoformat().replace("+00:00","Z"),"receipt_age_seconds":age,"receipt_ttl_seconds":RECEIPT_TTL_SECONDS,"fresh_origin_fetch_refspec":fetch_spec,"local_head":local_head,"fresh_origin_head":remote_head,"request_blob_sha":request_blob,"current_task_blob_sha":task_blob,"source_queue_blob_sha":queue_blob,"queue_census_basis":["task_id","attempt_id","idempotency_key"],"matching_queue_record_count":1,"matching_queue_records":combined_hits,"preflight_output_sha256":actual_hashes,"binding_key_sha256":expected_binding,"effective_runtime_script":RUN039,"effective_post_publish_script":POST040,"runtime_override_applied":True,"checks_passed":len(checks),"checks_total":len(checks),"checks":checks,"new_task_created":False,"new_runner_created":False,"parallel_runner_used":False,"queue_mutated_by_slot":False,"numeric_values_written":0,"final_ready":False,"fake_data":False}; output.parent.mkdir(parents=True,exist_ok=True); output.write_text(json.dumps(result,ensure_ascii=False,indent=2)+"\n",encoding="utf-8"); print(json.dumps({"ok":True,"status":result["status"],"queue_records":1,"census_basis":result["queue_census_basis"],"head":local_head,"binding_key":expected_binding,"output":str(output)})); return 0
