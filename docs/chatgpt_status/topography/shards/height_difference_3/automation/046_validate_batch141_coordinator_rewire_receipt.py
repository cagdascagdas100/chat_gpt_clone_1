#!/usr/bin/env python3
"""Validate the bound coordinator receipt for the directly sealed 039/040 chain."""
...
def main()->int:
    ...
    action=str(receipt.get("coordinator_action_id") or "").strip();nonce=str(receipt.get("receipt_nonce") or "").strip();need("action_id",len(action)>=8);need("nonce",len(nonce)==32);need("runtime_override",receipt.get("runtime_override_applied") is True);need("direct_entrypoints",receipt.get("effective_runtime_script")==RUN039 and receipt.get("effective_post_publish_script")==POST040 and receipt.get("direct_entrypoint_control_plane_seal_required") is True)
    ...
    saved=receipt.get("preflight_output_sha256") or {};actual={}
    ...
    key=bind([TASK_ID,ATTEMPT_ID,IDEMPOTENCY,CONTINUATION,rb,tb,qb,local,*(actual[k] for k in ("043","045","044","042","041")),RUN039,POST040,action,nonce]);need("binding",receipt.get("binding_key_sha256")==key)
    out=r/VALIDATION_REL;result={"schema_version":4,"slot_id":"height_difference_3","task_id":TASK_ID,"attempt_id":ATTEMPT_ID,"idempotency_key":IDEMPOTENCY,"continuation_key":CONTINUATION,"status":"COORDINATOR_REWIRE_RECEIPT_VALIDATED","validated_at_utc":now.isoformat().replace("+00:00","Z"),"receipt_age_seconds":age,"receipt_ttl_seconds":RECEIPT_TTL_SECONDS,"receipt_sha256":receipt_sha,"receipt_nonce":nonce,"coordinator_action_id":action,"local_head":local,"fresh_origin_head":remote,"request_blob_sha":rb,"current_task_blob_sha":tb,"source_queue_blob_sha":qb,"matching_queue_record_count":1,"matching_queue_records":combined,"preflight_output_sha256":actual,"binding_key_sha256":key,"effective_runtime_script":RUN039,"effective_post_publish_script":POST040,"direct_entrypoint_control_plane_seal_required":True,"runtime_override_applied":True,"checks_passed":len(checks),"checks_total":len(checks),"checks":checks,"new_task_created":False,"new_runner_created":False,"parallel_runner_used":False,"queue_mutated_by_slot":False,"numeric_values_written":0,"final_ready":False,"fake_data":False};out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(result,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");print(json.dumps({"ok":True,"status":result["status"],"receipt_sha256":receipt_sha,"binding_key":key,"output":str(out)}));return 0
