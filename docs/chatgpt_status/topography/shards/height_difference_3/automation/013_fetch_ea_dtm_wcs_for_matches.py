...
def _load_matches(path: Path) -> list[dict[str, Any]]:
    ...
    if len(matched) != len(values):
        raise ValueError(
            "all candidates must have a unique HMLR match before EA WCS download"
        )
    return matched
...
def _request_xml(
    ...
    if not _official_ea(response.url):
        raise ValueError(
            f"EA WCS XML request redirected off official host: {response.url}"
        )
    ...
    try:
        ...
        raise ValueError(
            f"WCS did not return XML: {response.headers.get('content-type')}"
        ) from exc
...
def _axis_labels(root: ET.Element) -> tuple[str, str]:
    ...
    if not candidates:
        raise ValueError(
            "WCS DescribeCoverage exposed no axisLabels; refusing inferred E/N order"
        )
    if len(candidates) > 1:
        normalized = {(a.casefold(), b.casefold()) for a, b in candidates}
        if len(normalized) > 1:
            raise ValueError(
                f"WCS DescribeCoverage exposed ambiguous axisLabels: {candidates}"
            )
    axis_x, axis_y = candidates[0]
    if (
        axis_x.casefold() not in {"e", "x", "easting"}
        or axis_y.casefold() not in {"n", "y", "northing"}
    ):
        raise ValueError(
            f"unexpected EA WCS axis labels for EPSG:27700: {(axis_x, axis_y)}"
        )
...
def _stream_get(
    ...
    try:
        with session.get(
            ...
            if not _official_ea(response.url):
                raise ValueError(
                    "EA WCS coverage request redirected off official host: "
                    f"{response.url}"
                )
            ...
            with output.open("wb") as handle:
                for chunk in response.iter_content(1024 * 1024):
                    ...
                    if total > MAX_RESPONSE_BYTES:
                        raise ValueError(
                            "EA WCS response exceeds safety size limit"
                        )
                    handle.write(chunk)
                handle.flush()
        ...
        if head.startswith(b"<") and (
            b"exception" in head or b"html" in head
        ):
            raise ValueError(
                "EA WCS returned XML/HTML exception instead of GeoTIFF"
            )
        return {
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    capabilities, caps_body, caps_url = _request_xml(
        session,
        args.wcs_base,
        [
            ("service", "WCS"),
    ...
    description, desc_body, desc_url = _request_xml(
        session,
        args.wcs_base,
        [
            ("service", "WCS"),
    ...
    for row in matches:
        ...
        if geometry.is_empty or geometry.geom_type not in {
            ...
            raise ValueError(
                f"row {row.get('row_no')} has invalid matched geometry"
            )
        ...
        output = (
            args.output_dir
            / "ea_dtm"
            / f"row_{int(row['row_no'])}_ea_dtm_1m.tif"
        )
        output.parent.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            with rasterio.open(temp_output) as dataset:
                if dataset.crs is None:
                    raise ValueError(
                        f"EA raster for row {row.get('row_no')} has no CRS"
                    )
                crs = CRS.from_user_input(dataset.crs)
                if crs != TARGET_CRS:
                    raise ValueError(
                        f"EA raster for row {row.get('row_no')} is {crs}, "
                        "expected EPSG:27700"
                    )
                ...
                if not box(*dataset.bounds).intersects(geometry):
                    raise ValueError(
                        "EA raster does not intersect matched parcel geometry"
                    )
                resolution = [
                    abs(float(dataset.res[0])),
                    abs(float(dataset.res[1])),
                ]
                if any(
                    ...
                    raise ValueError(
                        f"EA raster has invalid resolution: {resolution}"
                    )
                if max(resolution) > 1.1:
                    raise ValueError(
                        "EA raster resolution is coarser than 1.1m: "
                        f"{resolution}"
                    )
                record = {
    ...
    manifest = {
        ...
        "atomic_raster_materialization": True,
        "partial_canonical_rasters_forbidden": True,
