#!/usr/bin/env python3
"""Validate split future_growth_2 Batch 008 without network calls."""
...
def main()->int:
    ap=argparse.ArgumentParser(); ap.add_argument("index_path",type=Path); a=ap.parse_args()
    d=json.loads(a.index_path.read_text(encoding="utf-8"))
    rows=[json.loads((a.index_path.parent/p["path"]).read_text(encoding="utf-8"))["row"] for p in d["parts"]]
    ...
    checks={
      ...
      "layer_inventory":sum(len(r["layers"]) for r in rows)==31,
      "local_sources":sum(len(r["sources"]) for r in rows)==21,
      "freshness_rows":sum(len(r["freshness"]) for r in rows)==15,
      ...
      "future_dates_labelled":all(f["result"]=="FUTURE_TIMETABLE" for r in rows for f in r["freshness"] if f["code"] in {"SCOPING_START","TARGET_ADOPTION"}),
    }
