...
def main() -> int:
    ...
    expected_ids=["FG2-W16-001","FG2-W16-005","FG2-W16-006"]
    check("actual_wave_three_flagged",lambda: (_ for _ in ()).throw(AssertionError(flagged)) if flagged["flagged_count"]!=3 else None)
    check("actual_wave_exact_ids",lambda: (_ for _ in ()).throw(AssertionError(flagged)) if flagged["flagged_candidates"] != expected_ids else None)
    base=deepcopy(payload["candidates"][0])
    ...
    c=deepcopy(base); c["hectares"]=3.99; c["eligibility"]="eligible_ok"; c["capacity_anomaly_evidence"]=""; check("below_area_threshold_not_flagged",lambda:run(c))
    ...
    out={"schema_version":2,"slot_id":"future_growth_2","executed":True,"test_type":"large_site_low_capacity_fail_closed_guard","checks_passed":sum(x["passed"] for x in checks),"checks_total":len(checks),"all_passed":all(x["passed"] for x in checks),"checks":checks,"actual_wave_flagged_count":flagged["flagged_count"],"threshold_hectares":mod.MIN_LARGE_SITE_HECTARES,"canonical_parcel_matches":0,"future_growth_scores_produced":0,"actual_business_data_rows_written":0,"final_ready":False,"fake_data":False,"db_write":False,"migration":False,"production_deploy":False}
