...
$innerBlob='eeb0e3c0f63407a78b26a3d3a81f0a1f66516bba'
...
function StopTreeGeneration([object]$Expected,[bool]$RequireCanonical){
  $id=[int]$Expected.ProcessId;$expectedKey=IdentityKey -Process $Expected;$current=@(CurrentCimProcess -Id $id)
  if($current.Count-eq0){return[ordered]@{process_id=$id;expected_identity_key=$expectedKey;current_identity_key=$null;generation_match=$true;canonical_match=(-not$RequireCanonical);kill_attempted=$false;kill_exit_code=0;already_exited=$true;expected_generation_remaining=$false}}
  ...
  if(-not($same-and$canonical)){return[ordered]@{process_id=$id;expected_identity_key=$expectedKey;current_identity_key=$currentKey;generation_match=$same;canonical_match=$canonical;kill_attempted=$false;kill_exit_code=0;already_exited=(-not$same);expected_generation_remaining=$same}}
  $killExit=-1;$killAttempted=$false;$tk=Get-Command taskkill.exe -ErrorAction SilentlyContinue
  if($tk){$killAttempted=$true;& $tk.Source /PID $id /T /F|Out-Null;$killExit=$LASTEXITCODE}else{try{$killAttempted=$true;Stop-Process -Id $id -Force -ErrorAction Stop;$killExit=0}catch{$killExit=1}}
  ...
  return[ordered]@{process_id=$id;expected_identity_key=$expectedKey;current_identity_key=$currentKey;generation_match=$true;canonical_match=$canonical;kill_attempted=$killAttempted;kill_exit_code=$killExit;already_exited=$false;expected_generation_remaining=$remaining}
}
function WriteReceipt([string]$Status,[int]$ExitCode,[bool]$TimedOut,[string]$ResolvedInner,[object]$Owner,[string]$RemoteHead,[bool]$KillAttempted,[int]$KillExit,[string]$Detail){$path=Join-Path $repoRoot $outputRel;$dir=Split-Path -Parent $path;if(-not(Test-Path -LiteralPath $dir)){New-Item -ItemType Directory -Force -Path $dir|Out-Null};$o=[ordered]@{schema_version=13;slot_id=$slotId;task_id=$taskId;attempt_id=$attemptId;status=$Status;checked_at=[DateTimeOffset]::UtcNow.ToString('o');timeout_seconds=$TimeoutSeconds;timed_out=$TimedOut;inner_exit_code=$ExitCode;inner_path=$ResolvedInner;inner_expected_blob=$innerBlob;temporary_inner_allowed=$true;remote_head=$RemoteHead;ownership_rechecked=$true;ownership_snapshot=$Owner;canonical_f_process_identity_required=$true;foreign_runner_process_fail_closed=$true;direct_heartbeat_requires_repo_root_branch_and_process_start=$true;lock_fallback_requires_pid_repo_root_instance_heartbeat_start_real_process_start_freshness_scope_branch=$true;kill_requires_immediate_pid_creation_date_and_command_recheck=$true;process_tree_kill_attempted=$KillAttempted;process_tree_kill_exit_code=$KillExit;root_stop_evidence=$rootStopEvidence;baseline_canonical_processes=@($baselineCanonical|ForEach-Object{Evidence -Process $_});tracked_attempt_processes=@($trackedAttemptProcesses|ForEach-Object{Evidence -Process $_});tracked_generation_cleanup=@($trackedGenerationCleanup);remaining_tracked_attempt_processes=@($remainingTrackedAttemptProcesses|ForEach-Object{Evidence -Process $_});timeout_spawned_canonical_processes=@($timeoutSpawnedCanonical|ForEach-Object{Evidence -Process $_});spawned_canonical_cleanup=@($spawnedCanonicalCleanup);remaining_timeout_spawned_canonical_processes=@($remainingTimeoutSpawnedCanonical|ForEach-Object{Evidence -Process $_});timeout_tracking_uses_pid_and_creation_date=$true;descendant_snapshot_rejects_processes_older_than_wrapper=$true;remaining_descendant_generations_receive_safe_cleanup=$true;timeout_cleanup_reenumerates_exact_canonical_f_processes=$true;preexisting_process_generations_preserved=$true;same_attempt=$true;new_runner_created=$false;parallel_runner_started=$false;detail=$Detail;final_ready=$false;fake_data=$false};$tmp="$path.tmp.$PID";[IO.File]::WriteAllText($tmp,(($o|ConvertTo-Json -Depth 16)+"`n"),[Text.UTF8Encoding]::new($false));Move-Item -LiteralPath $tmp -Destination $path -Force}
...
if(-not$p.WaitForExit($TimeoutSeconds*1000)){
  ...
  if($wrapperGeneration.Count-eq1){$rootStopEvidence=StopTreeGeneration -Expected $wrapperGeneration[0] -RequireCanonical $false}else{$rootStopEvidence=[ordered]@{process_id=[int]$p.Id;generation_match=$false;kill_attempted=$false;already_exited=$true;detail='wrapper generation unavailable before timeout'}}
  ...
  $trackedCleanup=@();foreach($proc in $remainingTrackedAttemptProcesses){$trackedCleanup+=StopTreeGeneration -Expected $proc -RequireCanonical $false};$trackedGenerationCleanup=@($trackedCleanup);if($remainingTrackedAttemptProcesses.Count-gt0){Start-Sleep -Seconds 1}
  ...
  $cleanup=@();foreach($proc in $timeoutSpawnedCanonical){$cleanup+=StopTreeGeneration -Expected $proc -RequireCanonical $true};$spawnedCanonicalCleanup=@($cleanup);if($timeoutSpawnedCanonical.Count-gt0){Start-Sleep -Seconds 1}
  ...
  $allGone=($remainingTrackedAttemptProcesses.Count-eq0-and$remainingTimeoutSpawnedCanonical.Count-eq0);$killExit=if($null-ne$rootStopEvidence.kill_exit_code){[int]$rootStopEvidence.kill_exit_code}else{0};$status=if($allGone){'BLOCKED_RETRY5_RECOVERY_TIMEOUT_ALL_ATTEMPT_GENERATIONS_TERMINATED'}else{'BLOCKED_RETRY5_RECOVERY_TIMEOUT_PROCESS_GENERATION_REMAINS'};WriteReceipt $status 124 $true $inner $owner $remoteHead $true $killExit "wrapper_pid=$($p.Id);tracked=$($trackedAttemptProcesses.Count);remaining_tracked=$($remainingTrackedAttemptProcesses.Count);remaining_spawned_canonical=$($remainingTimeoutSpawnedCanonical.Count)";if($allGone){exit 124}else{exit 125}
}
$exit=$p.ExitCode;$status=if($exit-eq0){'TIMEOUT_GUARDED_FFSAFE_RETRY5_RECOVERY_COMPLETED'}else{'BLOCKED_INNER_FFSAFE_RETRY5_RECOVERY_FAILED'};WriteReceipt $status $exit $false $inner $owner $remoteHead $false -1 "wrapper_pid=$($p.Id)";exit $exit
