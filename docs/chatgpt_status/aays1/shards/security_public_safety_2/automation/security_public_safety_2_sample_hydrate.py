...
def validate_samples() -> dict[str, Any]:
            ...
            row.update(
                {
                    ...
                    "official_api_one_mile_supporting_count": (
                        len(crimes) if isinstance(crimes, list) else None
                    ),
                    "official_api_semantics": (
                        "ANONYMISED_APPROXIMATE_ONE_MILE_SUPPORTING_EVIDENCE;"
                        "NOT_EXACT_PARCEL_OR_LSOA_COUNT"
                    ),
                    "official_api_error": live.get("error"),
                    "candidate_status": (
                        "CANONICAL_AND_OFFICIAL_API_VERIFIED_IOD25_V2_JOIN_PENDING"
                        if api_pass
                        else "CANONICAL_FOUND_OFFICIAL_API_FAILED"
                    ),
    ...
    return {
        ...
        "parcel_partition": {
            "start": PARCEL_START,
            "end": PARCEL_END,
            "count": PARCEL_END - PARCEL_START + 1,
        },
        "target_ids": SAMPLE_IDS,
        "source_file": str(source_path) if source_path else None,
        "source_file_sha256": source_sha,
        "official_api_latest": {
            key: value for key, value in latest.items() if key != "json"
        }
        | {"month": latest_month},
        "rows": rows,
        "sample_count": len(rows),
        "canonical_sample_count": sum(
            row.get("candidate_status") != "CANONICAL_FEATURE_NOT_FOUND"
            for row in rows
        ),
        ...
        "next_gate": (
            "JOIN_UPDATED_IOD25_V2_CRIME_DOMAIN_AT_LSOA_LEVEL;"
            "THEN_HYDRATE_300_AND_RUN_HTTP_HASH_DOM_CONSOLE_BROWSER_ACCEPTANCE"
        ),
        "fake_data": False,
        "db_write": False,
        "migration": False,
...
def render_html(payload: dict[str, Any]) -> str:
    source_rows = "".join(
        ...
        f"<td>{escape(str(row.get('official_api_semantics') or 'not_available'))}</td>"
        "</tr>"
...
<title>Security / Public Safety — Slot 2 Progress</title>
<style>
body{{font-family:Segoe UI,Arial,sans-serif;margin:22px;line-height:1.4}}
table{{border-collapse:collapse;width:100%;margin:14px 0}}
th,td{{border:1px solid #bbb;padding:7px;text-align:left;vertical-align:top}}
code{{background:#eee;padding:2px 4px}}
</style></head>
<body data-slot-id="{SLOT_ID}" data-final-ready="false">
<h1>Security / Public Safety — Slot 2</h1>
<p><strong>Parsel aralığı:</strong> {PARCEL_START}–{PARCEL_END}</p>
<p><strong>İşlem ilerlemesi:</strong> {completed}/{len(operations)} — %{percent}</p>
<p><strong>Semantik:</strong> <code>AREA_LEVEL_PROXY</code>; parsel ölçümü değildir.</p>
<h2>İşlem akışı — satır satır</h2>
<table><thead><tr><th>#</th><th>Durum</th><th>İşlem</th><th>Detay</th></tr></thead><tbody>{operation_rows}</tbody></table>
<h2>Örnek adaylar</h2>
<table><thead><tr><th>#</th><th>Parsel</th><th>Durum</th><th>Doğruluk /4</th><th>LSOA</th><th>HTTP</th><th>1 mil destek sayısı</th><th>Resmî API</th><th>Semantik</th></tr></thead><tbody>{source_rows}</tbody></table>
<p><code>actual_business_rows_written=0</code>; <code>final_ready=false</code>; <code>fake_data=false</code>.</p>
</body></html>
"""
