...
function StopCanonicalGeneration([object]$Expected){
  ...
  if($current.Count-eq0){return[pscustomobject]@{process_id=$id;expected_identity_key=$expectedKey;current_identity_key=$null;expected_spec=$expectedSpec;current_spec=$null;generation_match=$true;canonical_match=$true;kill_attempted=$false;kill_exit_code=0;already_exited=$true;expected_generation_remaining=$false}}
  $currentKey=ProcessIdentityKey -Process $current[0];$currentSpec=MatchedSpecName $current[0];$sameGeneration=($currentKey-eq$expectedKey);$canonicalMatch=(-not[string]::IsNullOrWhiteSpace($expectedSpec)-and$currentSpec-eq$expectedSpec)
  if(-not($sameGeneration-and$canonicalMatch)){return[pscustomobject]@{process_id=$id;expected_identity_key=$expectedKey;current_identity_key=$currentKey;expected_spec=$expectedSpec;current_spec=$currentSpec;generation_match=$sameGeneration;canonical_match=$canonicalMatch;kill_attempted=$false;kill_exit_code=0;already_exited=(-not$sameGeneration);expected_generation_remaining=$sameGeneration}}
  $killExit=-1;$killAttempted=$false;try{$killAttempted=$true;Stop-Process -Id $id -Force -ErrorAction Stop;$killExit=0}catch{$killExit=1}
