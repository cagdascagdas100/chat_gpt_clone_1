...
function StopCanonicalTree([int]$id){$tk=Get-Command taskkill.exe -ErrorAction SilentlyContinue;if($tk){& $tk.Source /PID $id /T /F|Out-Null;return$LASTEXITCODE};try{Stop-Process -Id $id -Force -ErrorAction Stop;return 0}catch{return 1}}
...
if($daemons.Count-eq1){
  ...
  if($hb.valid-and$null-ne$hb.age_minutes-and$hb.age_minutes-gt$StaleMinutes){
    ...
    $remain=@(Canonical @())
    if($killExit-ne0-or$remain.Count-gt0){Result 'BLOCKED_STALE_CANONICAL_PROCESS_TREE_STOP_NOT_CONFIRMED' $true $false $false $false $hb ("kill_exit=$killExit remaining="+(($remain|ForEach-Object{$_.ProcessId})-join','));exit 4}
    ...
    $signalWritten=SignalIfAbsent;Result 'STALE_CANONICAL_DAEMON_RESTARTED_SINGLE_INSTANCE_FRESH_HEARTBEAT' $true $true $signalWritten $true $fresh.heartbeat "launcher_pid=$($p.Id)";exit 0
  }
...
if($canonical.Count-eq1){$fresh=WaitFreshDaemon 30;if(@($fresh.daemons).Count-eq1-and$fresh.heartbeat-and$fresh.heartbeat.valid){$signalWritten=SignalIfAbsent;Result 'CANONICAL_DAEMON_APPEARED_FRESH_HEARTBEAT_NO_SECOND_PROCESS' $false $false $signalWritten $false $fresh.heartbeat 'Existing launcher completed daemon startup.';exit 0};Result 'CANONICAL_TRANSIENT_PROCESS_ACTIVE_NO_SECOND_PROCESS' $false $false $false $false $fresh.heartbeat 'Existing exact canonical transient process preserved; no second process started.';exit 0}
