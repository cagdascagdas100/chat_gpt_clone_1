...
function HeartbeatInfo([int]$daemonPid){
  $h=ReadJson $heartbeat;$l=ReadJson $lock;$age=$null;$pidMatch=$false;$timeOk=$false;$heartbeatRootPresent=$false;$heartbeatRootMatch=$false;$lockPidMatch=$false;$lockRootMatch=$false;$rootSource='none'
  if($h){$hp=0;if($h.supervisor_pid){$hp=[int]$h.supervisor_pid}elseif($h.daemon_pid){$hp=[int]$h.daemon_pid}elseif($h.pid){$hp=[int]$h.pid};$pidMatch=($hp-eq$daemonPid);$heartbeatRootPresent=(-not[string]::IsNullOrWhiteSpace([string]$h.repo_root));if($heartbeatRootPresent){$heartbeatRootMatch=((Norm([string]$h.repo_root))-eq(Norm $repoRoot))};if($h.heartbeat_at){try{$age=[math]::Round(([DateTimeOffset]::UtcNow-[DateTimeOffset]::Parse([string]$h.heartbeat_at)).TotalMinutes,2);$timeOk=$true}catch{}}}
  if($l){$lp=0;if($l.supervisor_pid){$lp=[int]$l.supervisor_pid}elseif($l.pid){$lp=[int]$l.pid};$lockPidMatch=($lp-eq$daemonPid);$lockRootMatch=((Norm([string]$l.repo_root))-eq(Norm $repoRoot))}
  if($heartbeatRootMatch){$rootSource='heartbeat'}elseif($lockPidMatch-and$lockRootMatch){$rootSource='lock'}
  $rootMatch=$heartbeatRootMatch-or($lockPidMatch-and$lockRootMatch)
  return[pscustomobject]@{valid=($pidMatch-and$rootMatch-and$timeOk);pid_match=$pidMatch;repo_root_match=$rootMatch;repo_root_source=$rootSource;heartbeat_repo_root_present=$heartbeatRootPresent;heartbeat_repo_root_match=$heartbeatRootMatch;lock_pid_match=$lockPidMatch;lock_repo_root_match=$lockRootMatch;age_minutes=$age;raw=$h;raw_lock=$l}
}
function Result([string]$status,[bool]$attempted,[bool]$started,[bool]$signalWritten,[bool]$staleRestart,[object]$hb,[string]$detail){$canonical=@(Canonical @());$daemons=@(Canonical @('daemon'));$foreign=@(ForeignRunnerProcesses);$o=[ordered]@{schema_version=9;slot_id=$slotId;task_id=$taskId;attempt_id=$attemptId;status=$status;checked_at=[DateTimeOffset]::UtcNow.ToString('o');stale_minutes_threshold=$StaleMinutes;start_attempted=$attempted;canonical_runner_started=$started;stale_verified_runner_restarted=$staleRestart;canonical_process_ids=@($canonical|ForEach-Object{[int]$_.ProcessId});canonical_daemon_ids=@($daemons|ForEach-Object{[int]$_.ProcessId});foreign_runner_process_ids=@($foreign|ForEach-Object{[int]$_.ProcessId});foreign_runner_command_lines=@($foreign|ForEach-Object{[string]$_.CommandLine});canonical_root_identity_required=$true;heartbeat_identity=$hb;heartbeat_repo_root_optional_with_exact_lock_fallback=$true;lock_pid_and_repo_root_fallback_required_when_heartbeat_root_absent=$true;queue_refresh_signal_written=$signalWritten;existing_shared_signal_preserved=(-not$signalWritten-and(Test-Path -LiteralPath $signal));exact_target_rows=@($targets);nearest_row_fallback_allowed=$false;existing_single_runner_architecture_reused=$true;new_runner_architecture_created=$false;parallel_runner_started=$false;transient_without_fresh_daemon_is_failure=$true;process_exit_before_kill_is_clean_stop=$true;task_claimed=$false;detail=$detail;final_ready=$false;fake_data=$false;db_write=$false;migration=$false;production_deploy=$false};Atomic $output (($o|ConvertTo-Json -Depth 14)+"`n")}
