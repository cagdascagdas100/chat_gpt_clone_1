...
function StopCanonicalGeneration([object]$Expected){
  $id=[int]$Expected.ProcessId;$expectedKey=ProcessIdentityKey -Process $Expected;$current=@(CurrentCimProcess -Id $id)
  if($current.Count-eq0){return[pscustomobject]@{process_id=$id;expected_identity_key=$expectedKey;current_identity_key=$null;generation_match=$true;canonical_daemon_match=$true;kill_attempted=$false;kill_exit_code=0;already_exited=$true;expected_generation_remaining=$false}}
  $daemonSpec=@($specs|Where-Object{[string]$_.name-eq'daemon'}|Select-Object -First 1)
  $currentKey=ProcessIdentityKey -Process $current[0];$sameGeneration=($currentKey-eq$expectedKey);$daemonMatch=$false;if($daemonSpec.Count-eq1){$daemonMatch=MatchSpec $current[0] $daemonSpec[0]}
  if(-not($sameGeneration-and$daemonMatch)){return[pscustomobject]@{process_id=$id;expected_identity_key=$expectedKey;current_identity_key=$currentKey;generation_match=$false;canonical_daemon_match=$daemonMatch;kill_attempted=$false;kill_exit_code=0;already_exited=$true;expected_generation_remaining=$false}}
...
if($daemons.Count-eq1){
  ...
  if($hb.valid-and$null-ne$hb.age_minutes-and$hb.age_minutes-gt$StaleMinutes){
    ...
    $lastStopEvidence=StopCanonicalGeneration -Expected $daemonSnapshot;Start-Sleep -Seconds 3
    $remain=@(Canonical @())
    if($remain.Count-gt0){Result 'BLOCKED_STALE_CANONICAL_PROCESS_TREE_STOP_NOT_CONFIRMED' $true $false $false $false $hb ("kill_exit=$($lastStopEvidence.kill_exit_code) generation_match=$($lastStopEvidence.generation_match) remaining="+(($remain|ForEach-Object{$_.ProcessId})-join','));exit 4}
    ...
    $signalWritten=SignalIfAbsent;Result 'STALE_CANONICAL_DAEMON_RESTARTED_SINGLE_INSTANCE_FRESH_HEARTBEAT' $true $true $signalWritten $true $fresh.heartbeat "launcher_pid=$($p.Id);kill_exit=$($lastStopEvidence.kill_exit_code);generation_match=$($lastStopEvidence.generation_match)";exit 0
  }
