...
function Atomic([string]$Path,[string]$Text){$dir=Split-Path -Parent $Path;if(-not(Test-Path -LiteralPath $dir)){New-Item -ItemType Directory -Force -Path $dir|Out-Null};$tmp="$Path.tmp.$PID";[IO.File]::WriteAllText($tmp,$Text,[Text.UTF8Encoding]::new($false));Move-Item -LiteralPath $tmp -Destination $Path -Force}
function ReadJson([string]$Path){try{if(Test-Path -LiteralPath $Path -PathType Leaf){return(Get-Content -LiteralPath $Path -Raw -Encoding UTF8|ConvertFrom-Json)}}catch{};return $null}
...
function MatchSpec([object]$Process,[object]$Spec){$cmd=Norm -Value ([string]$Process.CommandLine);$token=Norm -Value ([string]$Spec.token);$root=Norm -Value ([string]$Spec.root);return($cmd.Contains($token)-and$cmd.Contains($root))}
...
function RunnerLike([object]$Process){$cmd=Norm -Value ([string]$Process.CommandLine);foreach($spec in $specs){if($cmd.Contains((Norm -Value ([string]$spec.token)))){return$true}};return$false}
function Canonical([string[]]$Names){$out=@();foreach($proc in @(AllProcs)){foreach($spec in $specs){$nameOk=($Names.Count-eq0-or$Names-contains[string]$spec.name);if($nameOk-and(MatchSpec -Process $proc -Spec $spec)){$out+=$proc;break}}};return@($out|Group-Object ProcessId|ForEach-Object{$_.Group[0]})}
function ForeignRunnerProcesses{$out=@();foreach($proc in @(AllProcs)){if(-not(RunnerLike -Process $proc)){continue};if([string]::IsNullOrWhiteSpace((MatchedSpecName -Process $proc))){$out+=$proc}};return@($out|Group-Object ProcessId|ForEach-Object{$_.Group[0]})}
...
function InspectAndRepairScanLock{
  ...
  $raw=$null;if($lockFile){$raw=ReadJson -Path $lockFile}
  $pidValue=0;if($raw-and$raw.pid){$pidValue=[int]$raw.pid}
  $current=@();if($pidValue-gt0){$current=@(CurrentCimProcess -Id $pidValue)}
  $currentSpec='';if($current.Count-eq1){$currentSpec=MatchedSpecName -Process $current[0]}
  ...
  if($raw){
    ...
    $rootPresent=(-not[string]::IsNullOrWhiteSpace([string]$raw.repo_root))
    if($rootPresent){$actualRoot=Norm -Value ([string]$raw.repo_root);$expectedRoot=Norm -Value $repoRoot;$repoMatch=($actualRoot-eq$expectedRoot)}
  }
  ...
  if($exists-and-not$valid-and$scanWorkers.Count-eq0){Remove-Item -LiteralPath $scanLockPath -Force -Recurse -ErrorAction Stop;$removed=$true;$status='stale_lock_removed_no_exact_scan_worker'}
  elseif($exists-and-not$valid-and$scanWorkers.Count-gt0){$status='invalid_lock_preserved_exact_scan_worker_present'}
...
function HeartbeatInfo([int]$DaemonPid){
  ...
  if($h){$hp=0;if($h.supervisor_pid){$hp=[int]$h.supervisor_pid}elseif($h.daemon_pid){$hp=[int]$h.daemon_pid}elseif($h.pid){$hp=[int]$h.pid};$pidMatch=($hp-eq$DaemonPid);if(-not[string]::IsNullOrWhiteSpace([string]$h.repo_root){$heartbeatRootMatch=((Norm -Value ([string]$h.repo_root))-eq(Norm -Value $repoRoot))};$heartbeatBranchMatch=([string]$h.branch-eq$branch);$startValue=$null;if($h.supervisor_started_at){$startValue=[string]$h.supervisor_started_at;$heartbeatStartField='supervisor_started_at'}elseif($h.started_at){$startValue=[string]$h.started_at;$heartbeatStartField='started_at'};if($startValue-and$processStart){try{$hs=[DateTimeOffset]::Parse($startValue);$heartbeatProcessStartMatch=([math]::Abs(($hs-$processStart).TotalSeconds)-le2)}catch{}};if($h.heartbeat_at){try{$age=[math]::Round(([DateTimeOffset]::UtcNow-[DateTimeOffset]::Parse([string]$h.heartbeat_at)).TotalMinutes,2);$timeOk=$true}catch{}}}
  if($l){$lp=0;if($l.supervisor_pid){$lp=[int]$l.supervisor_pid}elseif($l.pid){$lp=[int]$l.pid};$lockPidMatch=($lp-eq$DaemonPid);$lockRootMatch=((Norm -Value ([string]$l.repo_root))-eq(Norm -Value $repoRoot));$lockBranchMatch=([string]$l.branch-eq$branch);$lockScopeMatch=(-not$l.lock_scope-or[string]$l.lock_scope-eq'single_shared_runner_daemon');if($h-and-not[string]::IsNullOrWhiteSpace([string]$h.instance_id)-and-not[string]::IsNullOrWhiteSpace([string]$l.instance_id)){$instanceMatch=([string]$h.instance_id-eq[string]$l.instance_id)};if($h-and$h.supervisor_started_at-and$l.process_start_time){try{$hs=[DateTimeOffset]::Parse([string]$h.supervisor_started_at);$ls=[DateTimeOffset]::Parse([string]$l.process_start_time);$startMatch=([math]::Abs(($hs-$ls).TotalSeconds)-le2)}catch{}};if($l.process_start_time-and$processStart){try{$ls=[DateTimeOffset]::Parse([string]$l.process_start_time);$lockRealProcessStartMatch=([math]::Abs(($ls-$processStart).TotalSeconds)-le2)}catch{}};if($h-and$h.heartbeat_at-and$l.updated_at){try{$ht=[DateTimeOffset]::Parse([string]$h.heartbeat_at);$lu=[DateTimeOffset]::Parse([string]$l.updated_at);$delta=[math]::Round([math]::Abs(($ht-$lu).TotalSeconds),3);$lockHeartbeatFresh=($delta-le60)}catch{}}}
...
function StopCanonicalGeneration([object]$Expected){
  ...
  if($current.Count-eq0){return[pscustomobject]@{process_id=$id;expected_identity_key=$expectedKey;generation_match=$true;canonical_match=$true;bound_process_start_match=$true;kill_attempted=$false;kill_exit_code=0;already_exited=$true;expected_generation_remaining=$false}}
  ...
  $killExit=-1;$attempted=$false;try{$attempted=$true;Stop-Process -InputObject $bound -Force -ErrorAction Stop;$killExit=0}catch{$killExit=1};Start-Sleep -Milliseconds 250
  $after=@(CurrentCimProcess -Id $id);$remaining=$false;if($after.Count-eq1){$remaining=((IdentityKey -Process $after[0])-eq$expectedKey)}
  return[pscustomobject]@{process_id=$id;expected_identity_key=$expectedKey;generation_match=$true;canonical_match=$true;bound_process_start_match=$true;kill_attempted=$attempted;kill_exit_code=$killExit;already_exited=$false;expected_generation_remaining=$remaining}
}
function Result([string]$Status,[bool]$Attempted,[bool]$Started,[bool]$SignalWritten,[bool]$StaleRestart,[object]$Heartbeat,[string]$Detail){$canonical=@(Canonical -Names @());$daemons=@(Canonical -Names @('daemon'));$foreign=@(ForeignRunnerProcesses);$payload=[ordered]@{schema_version=17;slot_id=$slotId;task_id=$taskId;attempt_id=$attemptId;status=$Status;checked_at=[DateTimeOffset]::UtcNow.ToString('o');stale_minutes_threshold=$StaleMinutes;start_attempted=$Attempted;canonical_runner_started=$Started;stale_verified_runner_restarted=$StaleRestart;canonical_process_ids=@($canonical|ForEach-Object{[int]$_.ProcessId});canonical_daemon_ids=@($daemons|ForEach-Object{[int]$_.ProcessId});foreign_runner_process_ids=@($foreign|ForEach-Object{[int]$_.ProcessId});heartbeat_identity=$Heartbeat;scan_lock_generation_validation=$true;scan_lock_parser_explicit=$true;stale_scan_lock_removed_only_without_exact_scan_worker=$true;scan_lock_evidence=$script:scanLockEvidence;termination_uses_bound_process_object=$true;termination_uses_individual_verified_process_generations=$true;taskkill_tree_used=$false;stale_stop_evidence=@($script:lastStopEvidence);queue_refresh_signal_written=$SignalWritten;exact_target_rows=@($targets);existing_single_runner_architecture_reused=$true;new_runner_architecture_created=$false;parallel_runner_started=$false;detail=$Detail;final_ready=$false;fake_data=$false};Atomic -Path $output -Text (($payload|ConvertTo-Json -Depth 16)+"`n")}
