...
function InspectAndRepairScanLock{
  ...
  $raw=$null;if($lockFile){$raw=ReadJson $lockFile}
  $pidValue=0;if($raw-and$raw.pid){$pidValue=[int]$raw.pid}
  $current=@();if($pidValue-gt0){$current=@(CurrentCimProcess -Id $pidValue)}
  $currentSpec='';if($current.Count-eq1){$currentSpec=MatchedSpecName $current[0]}
  ...
  if($raw){
    ...
    if(-not[string]::IsNullOrWhiteSpace([string]$raw.repo_root){$actualRoot=Norm ([string]$raw.repo_root);$expectedRoot=Norm $repoRoot;$repoMatch=($actualRoot-eq$expectedRoot)}
  }
  ...
  $scanWorkers=@(Canonical @('scan_worker'))
  $removed=$false
