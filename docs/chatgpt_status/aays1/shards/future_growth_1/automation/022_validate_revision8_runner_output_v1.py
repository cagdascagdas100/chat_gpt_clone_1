#!/usr/bin/env python3
"""Fail-closed validator for the future_growth_1 revision-8 shared-runner output."""
...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    checks: dict[str, bool] = {
        ...
        "bug_fix_marker_exact": payload.get("revision7_bug_fixed") == BUG_FIX_MARKER,
        ...
        "geometry_slot_exact": isinstance(geometry, dict) and geometry.get("slot_id") == SLOT_ID,
        "geometry_completed": isinstance(geometry, dict) and geometry.get("state") == "COMPLETED_SOURCE_GEOMETRY_WAVE",
        "geometry_acceptance_present_all_true": isinstance(geometry, dict) and isinstance(geometry.get("acceptance"), dict) and bool(geometry["acceptance"]) and all(value is True for value in geometry["acceptance"].values()),
        "geometry_truth_flags_false": isinstance(geometry, dict) and geometry.get("fake_data") is False and geometry.get("db_write") is False and geometry.get("migration") is False and geometry.get("production_deploy") is False,
        ...
        "query_evidence_rows_19": isinstance(query_evidence, dict) and isinstance(query_evidence.get("rows"), list) and len(query_evidence["rows"]) == 19,
        "query_evidence_zero_promotion_and_scores": isinstance(query_evidence, dict) and query_evidence.get("promotion_eligible_rows") == 0 and query_evidence.get("scores_emitted") == 0,
        ...
        "source_sha_values_valid": isinstance(source_sha, dict) and all(isinstance(value, str) and SHA256_RE.fullmatch(value) for value in source_sha.values()),
