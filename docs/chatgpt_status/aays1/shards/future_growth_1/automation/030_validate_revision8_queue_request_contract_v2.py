#!/usr/bin/env python3
"""Revision-8 queue/request validator v4, retained on the v2 runtime path."""
...
mod.REQUIRED_ACCEPTANCE.update({
  ...
  "runtime_wrapper_selftest_expected":"12/12 PASS",
  "runtime_wrapper_static_checks_expected":25,
  ...
  "runtime_evidence_bundle_selftest_expected":"13/13 PASS",
  "runtime_evidence_bundle_checks_expected":64,
})
mod.REQUIRED_QUEUE_PATH_KEYS.update({"predecessor_dependency_validator_path","predecessor_dependency_selftest_path","runtime_evidence_bundle_validator_path","runtime_evidence_bundle_selftest_path"})
...
def validate(queue,readiness):
    ...
    result["checks"]["single_shared_runner_only"]=(result["checks"].get("single_shared_runner_only") is True and queue.get("sequential_after_task_id")==PREDECESSOR_TASK_ID and queue.get("predecessor_status_path")==PREDECESSOR_STATUS_PATH)
    ...
    result.update(schema_version=4,validation_kind="REVISION8_QUEUE_REQUEST_DEPENDENCY_AND_TEN_OUTPUT_FAIL_CLOSED",result="PASS" if not failed else "FAIL",checks_passed=sum(result["checks"].values()),checks_total=len(result["checks"]),failed_checks=failed,runner_output_validator_selftest_expected="18/18 PASS",expected_output_count=10,predecessor_task_id=PREDECESSOR_TASK_ID,predecessor_status_path=PREDECESSOR_STATUS_PATH)
    return result
...
def main()->int:
    ...
    except Exception as exc: result={"schema_version":4,"slot_id":SLOT_ID,"validation_kind":"REVISION8_QUEUE_REQUEST_DEPENDENCY_AND_TEN_OUTPUT_FAIL_CLOSED","result":"FAIL","checks_passed":0,"checks_total":1,"checks":{"json_load":False},"failed_checks":[f"json_load:{type(exc).__name__}:{exc}"],"runner_execution_claimed":False,"business_progress_claimed":False,"final_ready":False,"fake_data":False,"db_write":False,"migration":False,"production_deploy":False}
