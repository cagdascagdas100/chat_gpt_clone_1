#!/usr/bin/env python3
"""Offline fixtures for revision-8 guarded runtime-with-bundle validator."""
...
def main():
    ...
    cases=[
      case("exact_guarded_bundle_wrapper",exact,"PASS"),
      ...
      case("missing_bundle_validator",exact.replace("036_validate_revision8_runtime_evidence_bundle_v1.py","removed.py"),"FAIL"),
      case("missing_bundle_selftest",exact.replace("037_selftest_revision8_runtime_evidence_bundle_v1.py","removed.py"),"FAIL"),
      ...
      case("bundle_before_output",exact.replace("\n    bundle_validation_run=run","\n    bundle_process=run"),"FAIL"),
      case("wrong_bundle_check_count",exact.replace('bundle_validation.get("checks_passed")!=64','bundle_validation.get("checks_passed")!=63'),"FAIL"),
      case("business_claim",exact+'\n# business_progress_claimed":True\n',"FAIL"),
    ]
    passed=sum(c["pass"] for c in cases)
    out={"schema_version":3,"slot_id":"future_growth_1","selftest_kind":"REVISION8_GUARDED_RUNTIME_WITH_BUNDLE","result":f"{passed}/{len(cases)} PASS","passed":passed,"total":len(cases),"cases":cases,"runner_execution_claimed":False,"business_progress_claimed":False,"final_ready":False}
