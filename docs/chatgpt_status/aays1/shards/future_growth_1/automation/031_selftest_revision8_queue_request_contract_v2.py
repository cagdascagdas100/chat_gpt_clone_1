#!/usr/bin/env python3
"""Offline fixtures for revision-8 queue/request dependency and nine-output contract."""
...
spec=importlib.util.spec_from_file_location("validator_v3",TARGET)
...
def main():
    cases=[
      run_case("exact_dependency_nine_output_contract",lambda q,r:None,"PASS"),
      run_case("legacy_six_outputs",lambda q,r:q.__setitem__("expected_outputs",q["expected_outputs"][:6]),"FAIL"),
      run_case("missing_dependency_acceptance",lambda q,r:q["acceptance_contract"].pop("predecessor_dependency_checks_expected"),"FAIL"),
      run_case("wrong_predecessor_task",lambda q,r:q.__setitem__("sequential_after_task_id","wrong"),"FAIL"),
      run_case("unapproved_cross_slot_token",lambda q,r:q.__setitem__("note","future_growth_2"),"FAIL"),
      run_case("wrong_revision",lambda q,r:q.__setitem__("contract_revision",7),"FAIL"),
      run_case("unpinned_dependency_blob",lambda q,r:q.__setitem__("predecessor_dependency_validator_blob_sha","bad"),"FAIL"),
    ...
    out={"schema_version":3,"slot_id":"future_growth_1","selftest_kind":"REVISION8_QUEUE_REQUEST_DEPENDENCY_NINE_OUTPUT","result":f"{passed}/{len(cases)} PASS","passed":passed,"total":len(cases),"cases":cases,"runner_execution_claimed":False,"business_progress_claimed":False,"final_ready":False}
