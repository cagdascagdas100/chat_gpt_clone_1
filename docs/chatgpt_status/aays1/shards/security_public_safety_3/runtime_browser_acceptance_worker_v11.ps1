...
$selfRelative = 'docs/chatgpt_status/aays1/shards/security_public_safety_3/runtime_browser_acceptance_worker_v11.ps1'
$innerRelative = 'docs/chatgpt_status/aays1/shards/security_public_safety_3/runtime_browser_acceptance_worker_v10.ps1'
...
function Write-FallbackEvidence([string]$Reason,[Nullable[int]]$InnerExitCode) {
    ...
    $status = [ordered]@{
        ...
        worker_contract_version='v11_repository_branch_full_chain_integrity_fail_closed'
        inner_worker_contract_version='v10_queue_bound_staged_unstaged_integrity_fail_closed'
    ...
    $operation = [ordered]@{
        ...
        operations=@([ordered]@{
            ...
            stage='v11_repository_branch_full_chain_integrity_fail_closed'; status='blocked'; accuracy_score_4=4; confidence_score=0
            worker_contract_version='v11_repository_branch_full_chain_integrity_fail_closed'; inner_exit_code=$InnerExitCode
...
$trackedChecks = @(
    ...
    @{Relative='docs/chatgpt_status/aays1/shards/security_public_safety_3/runtime_browser_acceptance_worker_v10.ps1';Expected='2756c4c8b78e895e3dce8fa5174d714ae56941dc'},
...
$queueValid = (
    ...
    [string]$queue.worker_contract_version -eq 'v11_repository_branch_full_chain_integrity_fail_closed' -and
...
if (@($parseErrors).Count -gt 0) {
    Write-FallbackEvidence ('INNER_V10_PARSE_FAILED:' + ((@($parseErrors | ForEach-Object {$_.Message})) -join ' | ')) $null; exit 2
}
$enginePath=$null
...
} catch { Write-FallbackEvidence ('INNER_V10_LAUNCH_EXCEPTION:' + $_.Exception.Message) $innerExitCode; exit 2 }
...
try {
    ...
    if (-not [bool]$status.acceptance_pass) { Write-FallbackEvidence ('INNER_V10_BLOCKED:' + ((@($status.blockers)) -join '|')) $innerExitCode; exit 1 }
    ...
    $passValid=(
        [string]$status.worker_contract_version -eq 'v10_queue_bound_staged_unstaged_integrity_fail_closed' -and
        [bool]$status.queue_worker_binding_pass -and [bool]$status.staged_and_unstaged_integrity_pass -and
        ...
        [bool]$operation.queue_worker_binding_pass -and [bool]$operation.staged_and_unstaged_integrity_pass -and
        [bool]$operation.tracked_worker_integrity_pass -and [bool]$operation.canonical_content_binding_pass -and $innerExitCode -eq 0
    )
    if (-not $passValid) { Write-FallbackEvidence 'INNER_V10_PASS_CONTRACT_INVALID' $innerExitCode; exit 2 }
    Set-Property $status 'worker_contract_version' 'v11_repository_branch_full_chain_integrity_fail_closed'
    Set-Property $status 'inner_worker_contract_version' 'v10_queue_bound_staged_unstaged_integrity_fail_closed'
    ...
    Set-Property $status 'full_chain_staged_unstaged_integrity_pass' $true
    ...
    Write-JsonNoBom $statusPath $status
    Set-Property $operation 'worker_contract_version' 'v11_repository_branch_full_chain_integrity_fail_closed'
    ...
    Set-Property $operation 'full_chain_staged_unstaged_integrity_pass' $true
    ...
    Set-Property $operation 'result' 'RUNTIME_BROWSER_ACCEPTANCE_VERIFIED_V11_REPOSITORY_BRANCH_FULL_CHAIN_INTEGRITY'
    Write-JsonNoBom $operationPath $operationDoc
    exit 0
} catch {
    Write-FallbackEvidence ('INNER_V10_SCHEMA_VALIDATION_EXCEPTION:' + $_.Exception.Message) $innerExitCode
    exit 2
}
