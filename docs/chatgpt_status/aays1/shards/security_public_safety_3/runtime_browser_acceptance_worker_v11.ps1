...
function Write-FallbackEvidence([string]$Reason,[Nullable[int]]$InnerExitCode) {
    ...
    $status = [ordered]@{
        ...
        slot_id=$slotId; task_id=$taskId; continuation_key=$continuationKey
        status='RUNTIME_BROWSER_ACCEPTANCE_BLOCKED'; acceptance_pass=$false
        worker_contract_version='v11_full_chain_staged_unstaged_canonical_integrity_fail_closed'
        ...
        single_runner_only=$true; new_runner=$false; parallel_runner=$false
        data_deleted=$false; force_push_used=$false; reset_hard_used=$false; fake_data=$false
    ...
    $operation = [ordered]@{
        ...
        operations=@([ordered]@{
            ...
            stage='v11_full_chain_staged_unstaged_canonical_integrity_fail_closed'
            status='blocked'; accuracy_score_4=4; confidence_score=0
            worker_contract_version='v11_full_chain_staged_unstaged_canonical_integrity_fail_closed'
            inner_exit_code=$InnerExitCode; result=$Reason; evidence_path=$statusRelative; needs_manual_review=$true
        })
    }
...
$queueValid = (
    ...
    [string]$queue.worker_contract_version -eq 'v11_full_chain_staged_unstaged_canonical_integrity_fail_closed' -and
    [string]$queue.worker_blob_sha -eq $selfBlob -and [bool]$queue.same_task_preserved -and
    [bool]$queue.same_continuation_key_preserved -and -not [bool]$queue.new_task_created -and
    -not [bool]$queue.new_runner_created -and -not [bool]$queue.parallel_runner_created
)
...
if (@($parseErrors).Count -gt 0) {
    Write-FallbackEvidence ('INNER_V9_PARSE_FAILED:' + ((@($parseErrors | ForEach-Object {$_.Message})) -join ' | ')) $null
    exit 2
}
...
try {
    ...
    Write-FallbackEvidence ('INNER_V9_LAUNCH_EXCEPTION:' + $_.Exception.Message) $innerExitCode; exit 2
}
...
try {
    ...
    if (-not [bool]$status.acceptance_pass) {
        Write-FallbackEvidence ('INNER_V9_BLOCKED:' + ((@($status.blockers)) -join '|')) $innerExitCode
        exit 1
    }
    ...
    $passValid=(
        ...
        [bool]$operation.tracked_worker_integrity_pass -and [bool]$operation.canonical_content_binding_pass -and
        $innerExitCode -eq 0
    )
    if (-not $passValid) { Write-FallbackEvidence 'INNER_V9_PASS_CONTRACT_INVALID' $innerExitCode; exit 2 }
    Set-Property $status 'worker_contract_version' 'v11_full_chain_staged_unstaged_canonical_integrity_fail_closed'
    ...
    Write-JsonNoBom $statusPath $status
    Set-Property $operation 'worker_contract_version' 'v11_full_chain_staged_unstaged_canonical_integrity_fail_closed'
    ...
    Set-Property $operation 'result' 'RUNTIME_BROWSER_ACCEPTANCE_VERIFIED_V11_FULL_CHAIN_CANONICAL_INDEX_INTEGRITY'
