(async function () {
  ...
  function resolvePlannedAssetsApiUrl(pathTemplate = "/planned-assets/search") {
    const template = String(pathTemplate || "").trim();
    if (!template) return `${landIntelligenceApiBaseUrl}/planned-assets/search`;
  ...
  function buildPlannedAssetPopupHtml(props) {
    const delivery = Number(props.delivery_probability_score);
    const delay = Number(props.delay_risk_score);
    const distance = Number(props.distance_m);
    return `
      <div class="future-growth-hover-card">
        <div><strong>Planned:</strong> ${escapeHtml(props.title || "-")}</div>
        <div><strong>Tip:</strong> ${escapeHtml(props.asset_type || "-")}</div>
        <div><strong>Durum:</strong> ${escapeHtml(props.status || "-")}</div>
        <div><strong>Delivery:</strong> ${Number.isFinite(delivery) ? `${formatNumber(delivery, 1)}%` : "-"}</div>
        <div><strong>Delay Risk:</strong> ${Number.isFinite(delay) ? `${formatNumber(delay, 1)}%` : "-"}</div>
        <div><strong>Mesafe:</strong> ${Number.isFinite(distance) ? `${formatNumber(distance, 0)} m` : "-"}</div>
        <div><strong>Kaynak:</strong> ${escapeHtml(props.source_name || "-")}</div>
  ...
  async function loadPlannedAssetsLayer(force = false) {
    ...
    const searchUrl = new URL(resolvePlannedAssetsApiUrl("/planned-assets/search"));
    searchUrl.searchParams.set("limit", "2500");
    searchUrl.searchParams.set("offset", "0");
    const searchResponse = await fetchJsonWithTimeout(searchUrl.toString(), {
      label: "Planned assets layer",
      fallback: [],
      backoffGroup: "planned-assets-layer",
    });
    if (requestId !== plannedAssetsRequestId) return;

    let featureCollection = normalizePlannedAssetsFeatureCollection(searchResponse.data, bboxObj);
    let dataMode = searchResponse.ok ? "planned_assets" : "error";
    let errorMessage = searchResponse.ok ? null : searchResponse.error;

    if ((featureCollection.features || []).length === 0) {
      const fgUrl = new URL(resolveFutureGrowthApiUrl(futureGrowthLayerConfig.api.layer));
      fgUrl.searchParams.set("bbox", bbox);
      fgUrl.searchParams.set("zoom", zoom.toFixed(2));
      fgUrl.searchParams.set("limit", "2000");
      const fgResponse = await fetchJsonWithTimeout(fgUrl.toString(), {
        label: "Future Growth fallback for planned assets",
        fallback: createEmptyFeatureCollection,
        backoffGroup: "future-growth-layer",
      });
      if (requestId !== plannedAssetsRequestId) return;
      const fallbackCollection = normalizeFutureGrowthAsPlannedFeatures(fgResponse.data, bboxObj);
      if ((fallbackCollection.features || []).length > 0) {
        featureCollection = fallbackCollection;
        dataMode = "future_growth_fallback";
        errorMessage = null;
      }
    }
  ...
  function setPlannedAssetsLayerVisibility(nextVisible, options = {}) {
    ...
    if (!options.silent) {
      showStatus(`Planlanan yapi katmani ${currentPlannedAssetsVisible ? "acildi" : "kapatildi"}.`);
    }
  }
  ...
  async function setSecurityOverlayVisibility(nextVisible, options = {}) {
    if (!window.AAYS_SECURITY) {
      const loaded = await ensureRuntimeScriptLoaded("AAYS_SECURITY", "./security_overlay.js?v=20260501_021247");
      if (!loaded) {
  ...
  function setMapViewMode(nextMode, forceRefresh = true) {
    ...
    if (currentMapViewMode === "map_only" || currentMapViewMode === "land_use") {
      ...
      if (isEmissionsLayerVisible()) {
        setEmissionsLayerVisibility(false, { silent: true });
      }
      if (currentSecurityVisible) {
  ...
  function isEmissionsLayerVisible() {
    return currentScenarioScoreMode !== "none";
  }

  function setEmissionsLayerVisibility(nextVisible, options = {}) {
    const visible = Boolean(nextVisible);
    if (visible) {
      if (!isParcelViewMode()) {
        setMapViewMode("parcel", false);
      }
      if (currentScenarioScoreMode === "none") {
        currentScenarioScoreMode = String(options.mode || "apartment");
      }
    } else {
      currentScenarioScoreMode = "none";
    }
    ...
    updateMapModeControlState();
    if (!options.silent) {
      showStatus(visible ? "Gaz emisyonu katmani acildi." : "Gaz emisyonu katmani kapatildi.");
    }
  }

  function toggleEmissionsLayer() {
    setEmissionsLayerVisibility(!isEmissionsLayerVisible());
  }
  ...
  function runWorthMenuAction(actionId) {
    ...
    const actionMap = {
      ...
      compare: () => {
        setEmissionsLayerVisibility(!isEmissionsLayerVisible(), { mode: "apartment" });
      },
      topography: () => {
