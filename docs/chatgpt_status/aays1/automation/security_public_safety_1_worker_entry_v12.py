...
ATTEMPT_ID = "security-public-safety-1-20260720-012"
SCRIPT_NAME = "security_public_safety_1_worker_entry_v12.py"
PROGRESS_HTML = ROOT / "england_map_web" / "data" / "aays_21_slots" / "security_public_safety_1" / "progress_v12.html"
    ...
    if PROGRESS_JSON.is_file():
        ...
        for event in list(progress.get("events") or []):
            if event.get("step") == "DETERMINISTIC_PREFLIGHT":
                event["detail"] = (
                    "110 candidate rows, 16 unique endpoints, required files, Python syntax, "
                    "queue, legacy bridge and wrapper safety contracts validated without network calls; "
                    "recursion-safe v11 preflight retained"
                )
