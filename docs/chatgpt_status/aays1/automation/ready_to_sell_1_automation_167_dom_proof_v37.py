...
BASE = HERE / "ready_to_sell_1_automation_167_dom_proof_v36.py"
spec = importlib.util.spec_from_file_location("rts1_v36", BASE)
...
v3.SOURCE_FILES = [DATA_ROOT / "official_source_candidates_20260720.json"] + [
    batch("official_source_candidates", n) for n in range(2, 37)
]
v3.CANDIDATE_FILES = [DATA_ROOT / "verified_candidate_examples_20260720.json"] + [
    batch("verified_candidate_examples", n) for n in range(2, 37)
]
v3.EXPECTED_TOTAL_SOURCES = 389
v3.EXPECTED_TOTAL_CANDIDATES = 176
v3.EXPECTED_EXACT_INSPIRE = 2
v3.EXPECTED_INTERNET_REVERIFIED = 152
v3.EXPECTED_COMPLETED_OPERATIONS = 189
v3.EXPECTED_TOTAL_OPERATIONS = 190
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = text.replace("Aggregate DOM Proof V36", "Aggregate DOM Proof V37")
    text = text.replace("RERUN_AUTOMATION_167_V36", "RERUN_AUTOMATION_167_V37")
    text += (
        "\nV37 aggregate contract: 36 candidate/source batches, 188 raw candidate records, "
        "12 canonical-listing-URL duplicates excluded, 176 unique candidates, "
        "152 internet-reverified rows, 389 verified sources, 3 official planning evidence rows, "
        "189/190 operations and zero unverified parcel values. Favicon-only noise remains ignored; "
...
def main() -> int:
    ...
    report["automation_version"] = "V37"
    aggregate = report.get("aggregate_batch_state", {})
    progress = report.get("progress_page_acceptance", {})
    report["aggregate_contract"] = {
        "candidate_batches": 36,
        "verified_source_batches": 36,
        "raw_candidate_rows": 188,
        "duplicate_candidate_rows_excluded": 12,
        "candidate_rows": 176,
        "internet_reverified_rows": 152,
        "verified_source_rows": 389,
        "official_planning_evidence_rows": 3,
        "strengthened_candidate_rows": 126,
        "completed_operations": 189,
        "total_operations": 190,
        "unverified_parcel_value_rows": 0,
    }
    report["v37_contract_observed"] = {
    ...
    if isinstance(next_step, str):
        report["next_step"] = next_step.replace("AUTOMATION_167_V36", "AUTOMATION_167_V37")
