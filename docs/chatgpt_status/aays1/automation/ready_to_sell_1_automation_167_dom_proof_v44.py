...
BASE = HERE / "ready_to_sell_1_automation_167_dom_proof_v43.py"
spec = importlib.util.spec_from_file_location("rts1_v43", BASE)
...
v3.SOURCE_FILES = [DATA_ROOT / "official_source_candidates_20260720.json"] + [
    batch("official_source_candidates", n) for n in range(2, 44)
]
v3.CANDIDATE_FILES = [DATA_ROOT / "verified_candidate_examples_20260720.json"] + [
    batch("verified_candidate_examples", n) for n in range(2, 44)
]
v3.EXPECTED_TOTAL_SOURCES = 707
v3.EXPECTED_TOTAL_CANDIDATES = 280
v3.EXPECTED_EXACT_INSPIRE = 2
v3.EXPECTED_INTERNET_REVERIFIED = 256
v3.EXPECTED_COMPLETED_OPERATIONS = 231
v3.EXPECTED_TOTAL_OPERATIONS = 232
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = text.replace("Aggregate DOM Proof V43", "Aggregate DOM Proof V44")
    text = text.replace("RERUN_AUTOMATION_167_V43", "RERUN_AUTOMATION_167_V44")
    text += (
        "\nV44 aggregate contract: 43 candidate/source batches, 292 raw candidate records, "
        "12 canonical-listing-URL duplicates excluded, 280 unique candidates, "
        "256 internet-reverified rows, 707 verified sources, 3 official planning evidence rows, "
        "231/232 operations and zero unverified parcel values. Favicon-only noise remains ignored; "
...
def main() -> int:
    ...
    report["automation_version"] = "V44"
    aggregate = report.get("aggregate_batch_state", {})
    progress = report.get("progress_page_acceptance", {})
    report["aggregate_contract"] = {
        "candidate_batches": 43,
        "verified_source_batches": 43,
        "raw_candidate_rows": 292,
        "duplicate_candidate_rows_excluded": 12,
        "candidate_rows": 280,
        "internet_reverified_rows": 256,
        "verified_source_rows": 707,
        "official_planning_evidence_rows": 3,
        "strengthened_candidate_rows": 230,
        "completed_operations": 231,
        "total_operations": 232,
        "unverified_parcel_value_rows": 0,
    }
    report["v44_contract_observed"] = {
    ...
    if isinstance(next_step, str):
        report["next_step"] = next_step.replace("AUTOMATION_167_V43", "AUTOMATION_167_V44")
