...
BASE = HERE / "ready_to_sell_1_automation_167_dom_proof_v45.py"
spec = importlib.util.spec_from_file_location("rts1_v45", BASE)
...
v3.SOURCE_FILES = [DATA_ROOT / "official_source_candidates_20260720.json"] + [
    batch("official_source_candidates", n) for n in range(2, 46)
]
v3.CANDIDATE_FILES = [DATA_ROOT / "verified_candidate_examples_20260720.json"] + [
    batch("verified_candidate_examples", n) for n in range(2, 46)
]
v3.EXPECTED_TOTAL_SOURCES = 845
v3.EXPECTED_TOTAL_CANDIDATES = 326
v3.EXPECTED_EXACT_INSPIRE = 2
v3.EXPECTED_INTERNET_REVERIFIED = 302
v3.EXPECTED_COMPLETED_OPERATIONS = 243
v3.EXPECTED_TOTAL_OPERATIONS = 244
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = text.replace("Aggregate DOM Proof V45", "Aggregate DOM Proof V46")
    text = text.replace("RERUN_AUTOMATION_167_V45", "RERUN_AUTOMATION_167_V46")
    text += (
        "\nV46 aggregate contract: 45 candidate/source batches, 338 raw candidate records, "
        "12 canonical-listing-URL duplicates excluded, 326 unique candidates, "
        "302 internet-reverified rows, 845 verified sources, 3 official planning evidence rows, "
        "243/244 operations and zero unverified parcel values. Favicon-only noise remains ignored; "
...
def main() -> int:
    ...
    report["automation_version"] = "V46"
    aggregate = report.get("aggregate_batch_state", {})
    progress = report.get("progress_page_acceptance", {})
    report["aggregate_contract"] = {
        "candidate_batches": 45,
        "verified_source_batches": 45,
        "raw_candidate_rows": 338,
        "duplicate_candidate_rows_excluded": 12,
        "candidate_rows": 326,
        "internet_reverified_rows": 302,
        "verified_source_rows": 845,
        "official_planning_evidence_rows": 3,
        "strengthened_candidate_rows": 276,
        "completed_operations": 243,
        "total_operations": 244,
        "unverified_parcel_value_rows": 0,
    }
    report["v46_contract_observed"] = {
    ...
    if isinstance(next_step, str):
        report["next_step"] = next_step.replace("AUTOMATION_167_V45", "AUTOMATION_167_V46")
