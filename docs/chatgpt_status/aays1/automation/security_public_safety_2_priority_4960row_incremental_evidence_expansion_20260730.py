...
RECOVERY = ROOT / "docs/chatgpt_status/aays1/automation/security_public_safety_2_wave55_ons_boundary_recovery.py"

if not SOURCE.is_file():
    raise SystemExit(f"SOURCE_SCRIPT_MISSING: {SOURCE}")
if not RECOVERY.is_file():
    raise SystemExit(f"RECOVERY_SCRIPT_MISSING: {RECOVERY}")
...
recovery_namespace = {"__name__": "__main__", "__file__": str(RECOVERY), "__package__": None}
exec(compile(RECOVERY.read_text(encoding="utf-8"), str(RECOVERY), "exec"), recovery_namespace, recovery_namespace)
