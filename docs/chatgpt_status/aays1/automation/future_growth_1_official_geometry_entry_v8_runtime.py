...
def blocked(status:str,detail:Any)->int:
    write_json(WEB_ACCEPTANCE,{"schema_version":2,"slot_id":"future_growth_1","state":"BLOCKED","status":status,"detail":detail,"runner_execution_claimed":False,"business_progress_claimed":False,"actual_business_data_rows_written":0,"final_ready":False,"fake_data":False,"db_write":False,"migration":False,"production_deploy":False})
    return 2

def main()->int:
    required=[CORE,QUEUE_VALIDATOR,QUEUE_SELFTEST,DEPENDENCY_VALIDATOR,DEPENDENCY_SELFTEST,BUNDLE_VALIDATOR,BUNDLE_SELFTEST,QUEUE_MANIFEST,PREDECESSOR_STATUS,OUTPUT_VALIDATOR]
    ...
    bundle_selftest_run=run([sys.executable,str(BUNDLE_SELFTEST)])
    try: bundle_selftest=json.loads(bundle_selftest_run.stdout)
    except Exception: bundle_selftest={}
    if bundle_selftest_run.returncode!=0 or bundle_selftest.get("result")!="13/13 PASS":
        return blocked("BLOCKED_RUNTIME_EVIDENCE_BUNDLE_SELFTEST",bundle_selftest)
    ...
    bundle_validation_run=run([sys.executable,str(BUNDLE_VALIDATOR),str(REPO),str(QUEUE_MANIFEST),"--output",str(BUNDLE_OUTPUT)])
    bundle_validation=read_json(BUNDLE_OUTPUT) if BUNDLE_OUTPUT.is_file() else {}
    if bundle_validation_run.returncode!=0 or bundle_validation.get("result")!="PASS" or bundle_validation.get("checks_passed")!=64 or bundle_validation.get("checks_total")!=64:
        return blocked("BLOCKED_RUNTIME_EVIDENCE_BUNDLE_ACCEPTANCE",bundle_validation)
    write_json(WEB_ACCEPTANCE,{"schema_version":2,"slot_id":"future_growth_1","state":"COMPLETED_RUNTIME_ACCEPTANCE","status":"COMPLETED_DEPENDENCY_CORE_58_OUTPUT_AND_64_BUNDLE_GATES","dependency_validation":dependency,"runner_output_validation":output_validation,"runtime_evidence_bundle_validation":bundle_validation,"runner_execution_claimed":True,"business_progress_claimed":False,"actual_business_data_rows_written":0,"final_ready":False,"fake_data":False,"db_write":False,"migration":False,"production_deploy":False})
    return 0
