...
text = text.replace(old_owner, "github-actions-security-public-safety-2-wave92")
text = text.replace("wave32", "wave92")
if "590" not in text:
    raise SystemExit("EXPECTED_590_TARGET_FRAGMENTS_MISSING")
text = text.replace("590", "20560")
text = text.replace("__WAVE71_QUEUE__", "0067_security_public_safety_2_priority_20560row_incremental_evidence_expansion_20260730.v3.task.json")
text = text.replace("__WAVE71_CONTINUATION__", "bf2a19bc6e2fb5f8bf351bea31c340b142ca6735cf144d2e326809b4ecae3f1b")

required_replacements = [
    ("range(30762, 31352)", "range(30762, 51322)"),
    ("parcel_31351", "parcel_51321"),
    ("MIN_QUALITY = 561", "MIN_QUALITY = 19532"),
    ('"progress_delta_percentage_points": 11.86', '"progress_delta_percentage_points": 2.19'),
    ('"accepted_base_candidate_rows": 520', '"accepted_base_candidate_rows": 20110'),
    ('"incremental_rows_target": 70', '"incremental_rows_target": 450'),
    ('"incremental_rows_completed": 70', '"incremental_rows_completed": 450'),
    ("security_public_safety_2_priority_20560row_incremental_evidence_expansion_20260729", "security_public_safety_2_priority_20560row_incremental_evidence_expansion_20260730"),
    ("priority_20560row_browser_acceptance_wave92_receipt_20260729.json", "priority_20560row_browser_acceptance_wave92_receipt_20260730.json"),
    ("priority_20560row_targeted_retry_wave92_diagnostic_20260729.json", "priority_20560row_targeted_retry_wave92_diagnostic_20260730.json"),
]
...
old_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'20560 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()"""
new_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'20560 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()
            dom_deadline = time.time() + 180
            while dom_rows < 20560 and time.time() < dom_deadline:
