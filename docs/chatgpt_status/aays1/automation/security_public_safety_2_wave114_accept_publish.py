...
text = text.replace(old_owner, "github-actions-security-public-safety-2-wave114")
text = text.replace("wave32", "wave114")
if "590" not in text:
    raise SystemExit("EXPECTED_590_TARGET_FRAGMENTS_MISSING")
text = text.replace("590", "30460")
text = text.replace("__WAVE71_QUEUE__", "0089_security_public_safety_2_priority_30460row_incremental_evidence_expansion_20260731.v3.task.json")
text = text.replace("__WAVE71_CONTINUATION__", "56e3e62bf3872c903c92273cf1ee0c2aa6d5e182adf9c68f3822f9629f539183")

required_replacements = [
    ("range(30762, 31352)", "range(30762, 61222)"),
    ("parcel_31351", "parcel_61221"),
    ("MIN_QUALITY = 561", "MIN_QUALITY = 28937"),
    ('"progress_delta_percentage_points": 11.86', '"progress_delta_percentage_points": 1.48'),
    ('"accepted_base_candidate_rows": 520', '"accepted_base_candidate_rows": 30010'),
    ('"incremental_rows_target": 70', '"incremental_rows_target": 450'),
    ('"incremental_rows_completed": 70', '"incremental_rows_completed": 450'),
    ("security_public_safety_2_priority_30460row_incremental_evidence_expansion_20260729", "security_public_safety_2_priority_30460row_incremental_evidence_expansion_20260731"),
    ("priority_30460row_browser_acceptance_wave114_receipt_20260729.json", "priority_30460row_browser_acceptance_wave114_receipt_20260731.json"),
    ("priority_30460row_targeted_retry_wave114_diagnostic_20260729.json", "priority_30460row_targeted_retry_wave114_diagnostic_20260731.json"),
]
...
old_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'30460 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()"""
new_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'30460 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()
            dom_deadline = time.time() + 180
            while dom_rows < 30460 and time.time() < dom_deadline:
