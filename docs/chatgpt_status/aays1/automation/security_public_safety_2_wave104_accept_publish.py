...
text = text.replace(old_owner, "github-actions-security-public-safety-2-wave104")
text = text.replace("wave32", "wave104")
if "590" not in text:
    raise SystemExit("EXPECTED_590_TARGET_FRAGMENTS_MISSING")
text = text.replace("590", "25960")
text = text.replace("__WAVE71_QUEUE__", "0079_security_public_safety_2_priority_25960row_incremental_evidence_expansion_20260731.v3.task.json")
text = text.replace("__WAVE71_CONTINUATION__", "e744fbe6afb47dc3318636cbe2d4c07affe481c41149cf58f7cba680b5773b9a")

required_replacements = [
    ("range(30762, 31352)", "range(30762, 56722)"),
    ("parcel_31351", "parcel_56721"),
    ("MIN_QUALITY = 561", "MIN_QUALITY = 24662"),
    ('"progress_delta_percentage_points": 11.86', '"progress_delta_percentage_points": 1.73'),
    ('"accepted_base_candidate_rows": 520', '"accepted_base_candidate_rows": 25510'),
    ('"incremental_rows_target": 70', '"incremental_rows_target": 450'),
    ('"incremental_rows_completed": 70', '"incremental_rows_completed": 450'),
    ("security_public_safety_2_priority_25960row_incremental_evidence_expansion_20260729", "security_public_safety_2_priority_25960row_incremental_evidence_expansion_20260731"),
    ("priority_25960row_browser_acceptance_wave104_receipt_20260729.json", "priority_25960row_browser_acceptance_wave104_receipt_20260731.json"),
    ("priority_25960row_targeted_retry_wave104_diagnostic_20260729.json", "priority_25960row_targeted_retry_wave104_diagnostic_20260731.json"),
]
...
old_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'25960 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()"""
new_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'25960 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()
            dom_deadline = time.time() + 180
            while dom_rows < 25960 and time.time() < dom_deadline:
