...
BASE = HERE / "ready_to_sell_1_automation_167_dom_proof_v42.py"
spec = importlib.util.spec_from_file_location("rts1_v42", BASE)
...
v3.SOURCE_FILES = [DATA_ROOT / "official_source_candidates_20260720.json"] + [
    batch("official_source_candidates", n) for n in range(2, 43)
]
v3.CANDIDATE_FILES = [DATA_ROOT / "verified_candidate_examples_20260720.json"] + [
    batch("verified_candidate_examples", n) for n in range(2, 43)
]
v3.EXPECTED_TOTAL_SOURCES = 647
v3.EXPECTED_TOTAL_CANDIDATES = 260
v3.EXPECTED_EXACT_INSPIRE = 2
v3.EXPECTED_INTERNET_REVERIFIED = 236
v3.EXPECTED_COMPLETED_OPERATIONS = 225
v3.EXPECTED_TOTAL_OPERATIONS = 226
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = text.replace("Aggregate DOM Proof V42", "Aggregate DOM Proof V43")
    text = text.replace("RERUN_AUTOMATION_167_V42", "RERUN_AUTOMATION_167_V43")
    text += (
        "\nV43 aggregate contract: 42 candidate/source batches, 272 raw candidate records, "
        "12 canonical-listing-URL duplicates excluded, 260 unique candidates, "
        "236 internet-reverified rows, 647 verified sources, 3 official planning evidence rows, "
        "225/226 operations and zero unverified parcel values. Favicon-only noise remains ignored; "
...
def main() -> int:
    ...
    report["automation_version"] = "V43"
    aggregate = report.get("aggregate_batch_state", {})
    progress = report.get("progress_page_acceptance", {})
    report["aggregate_contract"] = {
        "candidate_batches": 42,
        "verified_source_batches": 42,
        "raw_candidate_rows": 272,
        "duplicate_candidate_rows_excluded": 12,
        "candidate_rows": 260,
        "internet_reverified_rows": 236,
        "verified_source_rows": 647,
        "official_planning_evidence_rows": 3,
        "strengthened_candidate_rows": 210,
        "completed_operations": 225,
        "total_operations": 226,
        "unverified_parcel_value_rows": 0,
    }
    report["v43_contract_observed"] = {
    ...
    if isinstance(next_step, str):
        report["next_step"] = next_step.replace("AUTOMATION_167_V42", "AUTOMATION_167_V43")
