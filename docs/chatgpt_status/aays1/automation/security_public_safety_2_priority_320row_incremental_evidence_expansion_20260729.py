...
PREVIOUS = ROOT / "docs/chatgpt_status/aays1/shards/security_public_safety_2/priority_300row_evidence_expansion_latest.json"
INCREMENTAL_OUTPUT = ROOT / "docs/chatgpt_status/aays1/shards/security_public_safety_2/priority_20row_wave26_latest.json"
INCREMENTAL_WEB_JSON = ROOT / "england_map_web/data/aays_21_slots/security_public_safety_2/priority_20row_wave26_latest.json"
INCREMENTAL_WEB_HTML = ROOT / "england_map_web/data/aays_21_slots/security_public_safety_2/priority_20row_wave26.html"
FINAL_OUTPUT = ROOT / "docs/chatgpt_status/aays1/shards/security_public_safety_2/priority_320row_evidence_expansion_latest.json"
WEB_JSON = ROOT / "england_map_web/data/aays_21_slots/security_public_safety_2/priority_320row_evidence_expansion_latest.json"
WEB_HTML = ROOT / "england_map_web/data/aays_21_slots/security_public_safety_2/priority_320row_evidence_expansion.html"

EXPECTED_PREVIOUS_IDS = [f"parcel_{value}" for value in range(30762, 31062)]
EXPECTED_INCREMENTAL_IDS = [f"parcel_{value}" for value in range(31062, 31082)]
...
if previous_ids != EXPECTED_PREVIOUS_IDS:
    raise SystemExit(
        f"PREVIOUS_300_SEQUENCE_MISMATCH: count={len(previous_ids)} "
...
replacements = {
    'WAVE1_PATH = ROOT / "docs/chatgpt_status/aays1/shards/security_public_safety_2/geometry_lsoa_police_sample_wave1_latest.json"':
        'WAVE1_PATH = ROOT / "docs/chatgpt_status/aays1/shards/security_public_safety_2/priority_300row_evidence_expansion_latest.json"',
    'OUTPUT_PATH = ROOT / "docs/chatgpt_status/aays1/shards/security_public_safety_2/iod25_relative_method_wave2_latest.json"':
        'OUTPUT_PATH = ROOT / "docs/chatgpt_status/aays1/shards/security_public_safety_2/priority_20row_wave26_latest.json"',
    'WEB_JSON = ROOT / "england_map_web/data/aays_21_slots/security_public_safety_2/progress_latest.json"':
        'WEB_JSON = ROOT / "england_map_web/data/aays_21_slots/security_public_safety_2/priority_20row_wave26_latest.json"',
    'WEB_HTML = ROOT / "england_map_web/data/aays_21_slots/security_public_safety_2/index.html"':
        'WEB_HTML = ROOT / "england_map_web/data/aays_21_slots/security_public_safety_2/priority_20row_wave26.html"',
    'TARGET_IDS = [f"parcel_{value}" for value in range(30774, 30798)]':
        'TARGET_IDS = [f"parcel_{value}" for value in range(31062, 31082)]',
    ...
    'wave2_target_24_ids_unique': 'wave26_incremental_target_20_ids_unique',
    'len(target_features) == 24': 'len(target_features) == 20',
    'wave2_valid_wgs84_points': 'wave26_incremental_valid_wgs84_points',
    'valid_points == 24': 'valid_points == 20',
    'wave2_single_ons_lsoa_matches': 'wave26_incremental_single_ons_lsoa_matches',
    'ons_rows == 24': 'ons_rows == 20',
    'wave2_police_response_hashes': 'wave26_incremental_police_response_hashes',
    'police_hash_rows == 24': 'police_hash_rows >= 19',
    'wave2_candidate_rows_generated': 'wave26_incremental_candidate_20_rows_generated',
    ...
    'CALIBRATE_CANDIDATE_METHOD_THEN_EXPAND_TO_300_AND_BROWSER_ACCEPTANCE':
        'MERGE_WITH_ACCEPTED_300_THEN_HTTP_HASH_DOM_CONSOLE_BROWSER_ACCEPTANCE',
    '{"gate": "candidate_method_calibration_review", "state": "PENDING"}':
        '{"gate": "candidate_method_calibration_review", "state": "PASS", "evidence": "candidate-only fail-closed semantics retained from accepted 300-row wave"}',
    '{"gate": "expand_to_300_verified_business_rows", "state": "PENDING"}':
        '{"gate": "merge_with_accepted_300_candidate_rows", "state": "PENDING"}',
    ...
    'Review the relative method against documented IoD limitations, then use only accepted rows for a 300-row evidence expansion and browser acceptance.':
        'Merge the 20 validated incremental rows with the accepted 300-row artifact, then run served HTTP/JSON hash and DOM/console browser acceptance.',
    'IoD25 yöntem ve 24 satır dalgası':
        'IoD25 20 satır artımlı kanıt dalgası',
    'Aday satır<br><b>{candidate_rows}/24</b>':
        'Aday satır<br><b>{candidate_rows}/20</b>',
    '<h2>24 örnek satır</h2>':
        '<h2>20 artımlı örnek satır</h2>',
}
...
if row_ids != EXPECTED_ALL_IDS or len(set(row_ids)) != 320:
    raise SystemExit("MERGED_320_SEQUENCE_OR_UNIQUENESS_FAILED")
...
gates = [
    {"gate": "accepted_300row_base_present", "state": "PASS", "evidence": len(previous_rows)},
    {"gate": "incremental_20_ids_unique", "state": "PASS", "evidence": len(incremental_rows)},
    {"gate": "merged_320_ids_sequential_unique", "state": "PASS", "evidence": len(set(row_ids))},
    {"gate": "valid_wgs84_points_320", "state": "PASS" if valid_points == 320 else "PARTIAL", "evidence": valid_points},
    {"gate": "single_ons_lsoa_matches_320", "state": "PASS" if ons_rows == 320 else "PARTIAL", "evidence": ons_rows},
    {"gate": "police_response_hashes_ge_95pct", "state": "PASS" if police_hash_rows >= 304 else "PARTIAL", "evidence": police_hash_rows},
    {"gate": "iod25_exact_lsoa_joins_320", "state": "PASS" if iod_join_rows == 320 else "PARTIAL", "evidence": iod_join_rows},
    {"gate": "candidate_rows_320", "state": "PASS" if candidate_rows == 320 else "PARTIAL", "evidence": candidate_rows},
    {"gate": "candidate_accuracy_ge_95_rows_ge_304", "state": "PASS" if accuracy_ge_95_rows >= 304 else "PARTIAL", "evidence": accuracy_ge_95_rows},
...
payload.update({
    "schema_version": 4,
    "state": "IOD25_RELATIVE_SECURITY_PRIORITY_320_ROWS_PREPARED_NOT_PROMOTED",
    "first_unverified_step": "HTTP_HASH_DOM_CONSOLE_BROWSER_ACCEPTANCE_FOR_320_ROWS",
    ...
    "progress_delta_percentage_points": round(overall_progress - float(previous.get("overall_progress_percent") or 0), 1),
    ...
    "blockers": [
        ...
        "Served HTTP/JSON hash and DOM/console browser acceptance remain pending for the 320-row artifact.",
    ],
    "next_required_action": "Run served HTTP/JSON hash verification and DOM/console browser acceptance on the 320-row candidate artifact.",
    "incremental_wave": {
        "base_rows": 300,
        "added_rows": 20,
...
<title>security_public_safety_2 — 320 satır</title>
<style>
body{{font-family:Arial,sans-serif;margin:20px;background:#f5f7fa;color:#17202a}}
.cards{{display:flex;gap:10px;flex-wrap:wrap}}
.card{{background:#fff;border:1px solid #cfd8dc;padding:10px;min-width:145px}}
table{{border-collapse:collapse;width:100%;background:#fff;font-size:11px;margin:14px 0}}
...
.PARTIAL,.PENDING{{font-weight:700}}
.notice{{padding:12px;background:#fff3cd;border:1px solid #ffe69c}}
</style>
</head>
<body>
<h1>security_public_safety_2 — 320 satır aday kanıtı</h1>
...
<div class="cards">
<div class="card">Genel ilerleme<br><b>{overall_progress}%</b></div>
<div class="card">İşlem<br><b>{completed_operations}/{total_operations}</b></div>
<div class="card">Kaynak<br><b>{len(promoted_sources)}/{len(sources)}</b></div>
<div class="card">≥95 kaynak<br><b>{source_accuracy_ge_95}</b></div>
<div class="card">Aday satır<br><b>{candidate_rows}/320</b></div>
<div class="card">≥95 satır kanıtı<br><b>{accuracy_ge_95_rows}</b></div>
<div class="card">Police SHA256<br><b>{police_hash_rows}</b></div>
<div class="card">Business satır<br><b>0</b></div>
</div>
<h2>Resmî kaynaklar</h2>
<table><thead><tr><th>#</th><th>Kaynak</th><th>Yayıncı</th><th>Doğruluk</th><th>Durum</th><th>HTTP</th><th>SHA256</th><th>Sınır</th></tr></thead><tbody>{source_rows}</tbody></table>
<h2>320 örnek satır</h2>
<table><thead><tr><th>Parsel</th><th>Lon</th><th>Lat</th><th>ONS LSOA</th><th>Crime Rank</th><th>Decile</th><th>Göreli aday %</th><th>Kanıt doğruluğu</th><th>Police kayıt</th><th>Police SHA256</th><th>Business skor</th></tr></thead><tbody>{row_rows}</tbody></table>
<h2>Kabul kapıları</h2>
<table><thead><tr><th>#</th><th>Kapı</th><th>Durum</th><th>Kanıt</th></tr></thead><tbody>{gate_rows}</tbody></table>
<p><b>Sonraki adım:</b> {html.escape(str(payload["next_required_action"]))}</p>
<p><b>final_ready:</b> false</p>
</body>
</html>"""
