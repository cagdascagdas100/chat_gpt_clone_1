...
text = text.replace(old_owner, "github-actions-security-public-safety-2-wave110")
text = text.replace("wave32", "wave110")
if "590" not in text:
    raise SystemExit("EXPECTED_590_TARGET_FRAGMENTS_MISSING")
text = text.replace("590", "28660")
text = text.replace("__WAVE71_QUEUE__", "0085_security_public_safety_2_priority_28660row_incremental_evidence_expansion_20260731.v3.task.json")
text = text.replace("__WAVE71_CONTINUATION__", "758e067a1160863ad6abd679e9870f108f7ebbbdedd011858c7064963f22910d")

required_replacements = [
    ("range(30762, 31352)", "range(30762, 59422)"),
    ("parcel_31351", "parcel_59421"),
    ("MIN_QUALITY = 561", "MIN_QUALITY = 27227"),
    ('"progress_delta_percentage_points": 11.86', '"progress_delta_percentage_points": 1.57'),
    ('"accepted_base_candidate_rows": 520', '"accepted_base_candidate_rows": 28210'),
    ('"incremental_rows_target": 70', '"incremental_rows_target": 450'),
    ('"incremental_rows_completed": 70', '"incremental_rows_completed": 450'),
    ("security_public_safety_2_priority_28660row_incremental_evidence_expansion_20260729", "security_public_safety_2_priority_28660row_incremental_evidence_expansion_20260731"),
    ("priority_28660row_browser_acceptance_wave110_receipt_20260729.json", "priority_28660row_browser_acceptance_wave110_receipt_20260731.json"),
    ("priority_28660row_targeted_retry_wave110_diagnostic_20260729.json", "priority_28660row_targeted_retry_wave110_diagnostic_20260731.json"),
]
...
old_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'28660 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()"""
new_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'28660 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()
            dom_deadline = time.time() + 180
            while dom_rows < 28660 and time.time() < dom_deadline:
