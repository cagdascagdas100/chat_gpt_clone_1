...
function Invoke-BatchJob {
  ...
  return [pscustomobject]@{
    ...
    state=$state; exit_code=$exitCode; output_status=$outputStatus
...
$jobs = @(
  [pscustomobject]@{ name='pre_site_sync'; script='docs/chatgpt_status/aays1/automation/155_aays1_ready_to_sell_post_promotion_site_verify_20260713.ps1'; expected='docs/chatgpt_status/aays1/status/155_aays1_ready_to_sell_second_wave_dispatch_latest.json'; required='SECOND_WAVE_SITE_VISIBILITY_VERIFIED' },
  [pscustomobject]@{ name='source_scan_300x60_a'; script='docs/chatgpt_status/aays1/automation/152_aays1_bulk_live_source_verify_300x60_20260711.ps1'; expected='docs/chatgpt_status/aays1/status/152_aays1_bulk_live_source_verify_300x60_latest.json'; required='' },
  [pscustomobject]@{ name='evidence_40_a'; script='docs/chatgpt_status/aays1/automation/153_aays1_bulk_photo_polygon_evidence_40_20260711.ps1'; expected='docs/chatgpt_status/aays1/status/153_aays1_bulk_photo_polygon_evidence_40_latest.json'; required='' },
  [pscustomobject]@{ name='source_scan_300x60_b'; script='docs/chatgpt_status/aays1/automation/152_aays1_bulk_live_source_verify_300x60_20260711.ps1'; expected='docs/chatgpt_status/aays1/status/152_aays1_bulk_live_source_verify_300x60_latest.json'; required='' },
  [pscustomobject]@{ name='evidence_40_b'; script='docs/chatgpt_status/aays1/automation/153_aays1_bulk_photo_polygon_evidence_40_20260711.ps1'; expected='docs/chatgpt_status/aays1/status/153_aays1_bulk_photo_polygon_evidence_40_latest.json'; required='' },
  [pscustomobject]@{ name='post_site_sync'; script='docs/chatgpt_status/aays1/automation/155_aays1_ready_to_sell_post_promotion_site_verify_20260713.ps1'; expected='docs/chatgpt_status/aays1/status/155_aays1_ready_to_sell_second_wave_dispatch_latest.json'; required='SECOND_WAVE_SITE_VISIBILITY_VERIFIED' }
)

foreach ($job in $jobs) {
  $r = Invoke-BatchJob -Name $job.name -ScriptRelative $job.script -ExpectedRelative $job.expected -RequiredStatus $job.required
  $results.Add($r)
...
$statusName = if ($failedJobs -eq 0 -and $siteVerified) { 'DOUBLE_WAVE_AND_SITE_VISIBILITY_VERIFIED' } elseif ($afterAll.live -gt $beforeAll.live -or $afterAll.ready -gt $beforeAll.ready) { 'DOUBLE_WAVE_REAL_PROGRESS_SITE_OR_CHILD_PARTIAL' } else { 'DOUBLE_WAVE_BLOCKED_OR_NO_NEW_PROGRESS' }
...
$status = [ordered]@{
  ...
  planned_candidates=600; planned_max_new_sources=120; planned_evidence_operations=80
...
$lines.Add('# AAYS1 ReadyToSell Double Wave and Site Sync')
$lines.Add('')
$lines.Add('- Status: ' + $statusName)
$lines.Add(('- Jobs with output: {0} / {1}; failed={2}' -f $status.jobs_completed_with_output,$status.jobs_total,$failedJobs))
...
foreach ($r in $results) { $lines.Add(('- {0}: {1}; source_delta={2}; evidence_delta={3}; exit={4}' -f $r.name,$r.state,$r.source_delta,$r.evidence_ready_delta,$r.exit_code)) }
