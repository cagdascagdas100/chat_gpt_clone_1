...
def script_suffix(record: object) -> tuple[str | None, str | None, str | None]:
    if not isinstance(record, dict):
        return None, None, None

    for key in ("script_suffix", "suffix"):
        explicit = valid_suffix_token(record.get(key))
        if explicit in SCRIPT_SUFFIX_TO_FAMILY:
            return explicit, SCRIPT_SUFFIX_TO_FAMILY[explicit], key

    for key in ("suffixes", "suffix_chain", "suffix_tokens"):
        suffixes = normalize_suffixes(record.get(key))
        if suffixes and suffixes[-1] in SCRIPT_SUFFIX_TO_FAMILY:
            suffix = suffixes[-1]
            return suffix, SCRIPT_SUFFIX_TO_FAMILY[suffix], key
    ...
    if name:
        ...
        if suffixes and suffixes[-1] in SCRIPT_SUFFIX_TO_FAMILY:
            suffix = suffixes[-1]
            return suffix, SCRIPT_SUFFIX_TO_FAMILY[suffix], f"derived_from_{provenance}"

    return None, None, None

def assess(prior: dict, source: dict, accessed_at: str) -> dict:
    if prior.get("slot_id") != "gas_emissions_2" or prior.get("wave") != 382:
        raise ValueError("PRIOR_WAVE382_SLOT_MISMATCH")
    ...
    observations = [script_suffix(record) for record in records]
    observations = [
    ...
    suffix_frequencies = [
        {
            ...
            "script_family": SCRIPT_SUFFIX_TO_FAMILY[suffix],
            "record_count": count,
        }
    ...
    if not suffix_counts:
        blockers.append("TAR_MEMBER_SCRIPT_SUFFIXES_NOT_AVAILABLE")
    blockers.extend(
        [
            "TAR_MEMBER_SCRIPT_SUFFIX_FREQUENCY_ALONE_DOES_NOT_PROVE_FILE_CONTENT",
            "TAR_MEMBER_SCRIPT_SUFFIX_FREQUENCY_ALONE_DOES_NOT_PROVE_OVERTURE_BUILDING_FEATURES",
    ...
    excerpt = (
        f"prior_wave382_sha256={prior_sha};source_wave368_sha256={source_sha};"
        f"source_path_prefix_records={len(records)};records_with_script_suffixes={len(observations)};"
        f"unique_script_suffixes={len(suffix_frequencies)};business_rows=0;parcel_rows=0"
    )
    runtime_evidence = {
        ...
        "record_scope": "Only explicit suffix metadata or PurePath.suffixes-derived terminal script suffixes from bounded Wave368 metadata were counted; no archive member body was read.",
        "relevant_record_ids_or_excerpt": excerpt,
        "supports_fields": [
            "script_suffix_frequencies",
            "script_family_frequencies",
            "records_with_script_suffixes",
            "unique_script_suffix_count",
            "script_suffix_provenance_counts",
            "no_member_body_read",
        ],
    ...
    return {
        ...
        "records_with_script_suffixes": len(observations),
        "unique_script_suffix_count": len(suffix_frequencies),
        "script_suffix_frequencies": suffix_frequencies,
        "script_family_frequencies": [
            {"script_family": key, "record_count": value}
        ...
        "script_suffix_allowlist": [
            {"suffix": suffix, "script_family": family}
            for suffix, family in sorted(SCRIPT_SUFFIX_TO_FAMILY.items())
        ],
        ...
        "decision": "GHCR_BOTTLE_LAYER_TAR_MEMBER_SCRIPT_SUFFIX_FREQUENCIES_ASSESSED",
        "state": "NO_DATA_CONTINUE",
        "blocker": ";".join(blockers),
        "first_unverified_step": "ASSESS_GHCR_BOTTLE_LAYER_TAR_MEMBER_EXECUTABLE_SUFFIX_FREQUENCIES_OR_NO_DATA_CONTINUE",
...
def self_test() -> None:
    prior = {"slot_id": "gas_emissions_2", "wave": 382, "state": "NO_DATA_CONTINUE"}
    source = {
        "slot_id": "gas_emissions_2",
        "wave": 368,
        "tar_member_path_prefix_records": [
            ...
            {"suffix_tokens": [".ps1"]},
            {"basename": "run.sh"},
            {"path_parts": ["x", "tool.py"]},
            {"member_name": "x/task.ps1"},
            {"normalized_path": "x/start.sh"},
            {"path": r"x\script.py"},
    ...
    output = assess(prior, source, "2026-08-04T04:08:00Z")
    assert output["records_with_script_suffixes"] == 10
    assert output["unique_script_suffix_count"] == 3
    assert output["script_suffix_frequencies"][0]["record_count"] >= 3
    assert output["business_rows_produced"] == output["parcel_rows_bound"] == 0
    assert all(item["script_suffix"] not in {".json", ".txt"} for item in output["script_suffix_frequencies"])
    empty = dict(source)
    empty["tar_member_path_prefix_records"] = []
    zero = assess(prior, empty, "2026-08-04T04:08:00Z")
    assert zero["script_suffix_frequencies"] == []
    assert "WAVE368_TAR_MEMBER_PATH_PREFIX_COUNT_ZERO" in zero["blocker"]
    assert "TAR_MEMBER_SCRIPT_SUFFIXES_NOT_AVAILABLE" in zero["blocker"]
