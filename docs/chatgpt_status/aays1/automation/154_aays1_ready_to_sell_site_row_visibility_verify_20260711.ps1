...
function Remove-JsonBom([string]$text) {
  if ($null -eq $text) { return $text }
  $clean = $text.TrimStart([char]0xFEFF)
  $mojibakeBom = ([string][char]0x00EF) + ([string][char]0x00BB) + ([string][char]0x00BF)
  if ($clean.StartsWith($mojibakeBom)) { $clean = $clean.Substring($mojibakeBom.Length) }
  return $clean
}
...
# Use ASCII structural markers so Windows PowerShell source decoding cannot create a false negative.
$htmlContractOk = $html -and
  $html.Contains('id="newOnly"') -and
  $html.Contains('NOT_PROCESSED') -and
  $html.Contains('badge new') -and
  $html.Contains('new_this_run=true') -and
  $html.Contains('status_json_path') -and
  $html.Contains('report_md_path') -and
  $html.Contains('downloaded_photo_paths') -and
  $html.Contains('polygon_render_path')
...
try {
  ...
  $pageOk = $pageStatus -eq 200 -and $pageContent.Contains('newOnly') -and $pageContent.Contains('NOT_PROCESSED') -and $pageContent.Contains('downloaded_photo_paths') -and $pageContent.Contains('polygon_render_path')
...
try {
  ...
  $remoteText = Remove-JsonBom ([string]$r.Content)
  $remoteData = ($remoteText | ConvertFrom-Json)
  ...
  $jsonOk = $jsonStatus -eq 200 -and $remoteResultCount -gt 0
...
$status = [ordered]@{
  task_id = $taskId
  page_key = 'aays1'
  status = if ($uniqueBlockers.Count -eq 0 -and $healthOk -and $pageOk -and $jsonOk -and $htmlContractOk -and $servedMatchesLocal) { 'SITE_ROW_VISIBILITY_VERIFIED' } else { 'SITE_ROW_VISIBILITY_PARTIAL_OR_BLOCKED' }
  ...
  blockers = $uniqueBlockers; generated_at = [DateTimeOffset]::UtcNow.ToString('o')
