...
BASE = HERE / "ready_to_sell_1_automation_167_dom_proof_v41.py"
spec = importlib.util.spec_from_file_location("rts1_v41", BASE)
...
v3.SOURCE_FILES = [DATA_ROOT / "official_source_candidates_20260720.json"] + [
    batch("official_source_candidates", n) for n in range(2, 42)
]
v3.CANDIDATE_FILES = [DATA_ROOT / "verified_candidate_examples_20260720.json"] + [
    batch("verified_candidate_examples", n) for n in range(2, 42)
]
v3.EXPECTED_TOTAL_SOURCES = 593
v3.EXPECTED_TOTAL_CANDIDATES = 242
v3.EXPECTED_EXACT_INSPIRE = 2
v3.EXPECTED_INTERNET_REVERIFIED = 218
v3.EXPECTED_COMPLETED_OPERATIONS = 219
v3.EXPECTED_TOTAL_OPERATIONS = 220
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = text.replace("Aggregate DOM Proof V41", "Aggregate DOM Proof V42")
    text = text.replace("RERUN_AUTOMATION_167_V41", "RERUN_AUTOMATION_167_V42")
    text += (
        "\nV42 aggregate contract: 41 candidate/source batches, 254 raw candidate records, "
        "12 canonical-listing-URL duplicates excluded, 242 unique candidates, "
        "218 internet-reverified rows, 593 verified sources, 3 official planning evidence rows, "
        "219/220 operations and zero unverified parcel values. Favicon-only noise remains ignored; "
...
def main() -> int:
    ...
    report["automation_version"] = "V42"
    aggregate = report.get("aggregate_batch_state", {})
    progress = report.get("progress_page_acceptance", {})
    report["aggregate_contract"] = {
        "candidate_batches": 41,
        "verified_source_batches": 41,
        "raw_candidate_rows": 254,
        "duplicate_candidate_rows_excluded": 12,
        "candidate_rows": 242,
        "internet_reverified_rows": 218,
        "verified_source_rows": 593,
        "official_planning_evidence_rows": 3,
        "strengthened_candidate_rows": 192,
        "completed_operations": 219,
        "total_operations": 220,
        "unverified_parcel_value_rows": 0,
    }
    report["v42_contract_observed"] = {
    ...
    if isinstance(next_step, str):
        report["next_step"] = next_step.replace("AUTOMATION_167_V41", "AUTOMATION_167_V42")
