...
BASE = HERE / "ready_to_sell_1_automation_167_dom_proof_v33.py"
spec = importlib.util.spec_from_file_location("rts1_v33", BASE)
...
v3.SOURCE_FILES = [DATA_ROOT / "official_source_candidates_20260720.json"] + [
    batch("official_source_candidates", n) for n in range(2, 34)
]
v3.CANDIDATE_FILES = [DATA_ROOT / "verified_candidate_examples_20260720.json"] + [
    batch("verified_candidate_examples", n) for n in range(2, 34)
]
v3.EXPECTED_TOTAL_SOURCES = 285
v3.EXPECTED_TOTAL_CANDIDATES = 146
v3.EXPECTED_EXACT_INSPIRE = 2
v3.EXPECTED_INTERNET_REVERIFIED = 122
v3.EXPECTED_COMPLETED_OPERATIONS = 171
v3.EXPECTED_TOTAL_OPERATIONS = 172
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = text.replace("Aggregate DOM Proof V33", "Aggregate DOM Proof V34")
    text = text.replace("RERUN_AUTOMATION_167_V33", "RERUN_AUTOMATION_167_V34")
    text += (
        "\nV34 aggregate contract: 33 candidate/source batches, 158 raw candidate records, "
        "12 canonical-listing-URL duplicates excluded, 146 unique candidates, "
        "122 internet-reverified rows, 285 verified sources, 3 official planning evidence rows, "
        "171/172 operations and zero unverified parcel values. Favicon-only noise remains ignored; "
...
def main() -> int:
    ...
    report["automation_version"] = "V34"
    aggregate = report.get("aggregate_batch_state", {})
    progress = report.get("progress_page_acceptance", {})
    report["aggregate_contract"] = {
        "candidate_batches": 33,
        "verified_source_batches": 33,
        "raw_candidate_rows": 158,
        "duplicate_candidate_rows_excluded": 12,
        "candidate_rows": 146,
        "internet_reverified_rows": 122,
        "verified_source_rows": 285,
        "official_planning_evidence_rows": 3,
        "strengthened_candidate_rows": 96,
        "completed_operations": 171,
        "total_operations": 172,
        "unverified_parcel_value_rows": 0,
    }
    report["v34_contract_observed"] = {
    ...
    if isinstance(next_step, str):
        report["next_step"] = next_step.replace("AUTOMATION_167_V33", "AUTOMATION_167_V34")
