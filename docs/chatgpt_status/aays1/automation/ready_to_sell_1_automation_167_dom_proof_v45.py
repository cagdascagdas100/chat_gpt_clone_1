...
BASE = HERE / "ready_to_sell_1_automation_167_dom_proof_v44.py"
spec = importlib.util.spec_from_file_location("rts1_v44", BASE)
...
v3.SOURCE_FILES = [DATA_ROOT / "official_source_candidates_20260720.json"] + [
    batch("official_source_candidates", n) for n in range(2, 45)
]
v3.CANDIDATE_FILES = [DATA_ROOT / "verified_candidate_examples_20260720.json"] + [
    batch("verified_candidate_examples", n) for n in range(2, 45)
]
v3.EXPECTED_TOTAL_SOURCES = 773
v3.EXPECTED_TOTAL_CANDIDATES = 302
v3.EXPECTED_EXACT_INSPIRE = 2
v3.EXPECTED_INTERNET_REVERIFIED = 278
v3.EXPECTED_COMPLETED_OPERATIONS = 237
v3.EXPECTED_TOTAL_OPERATIONS = 238
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = text.replace("Aggregate DOM Proof V44", "Aggregate DOM Proof V45")
    text = text.replace("RERUN_AUTOMATION_167_V44", "RERUN_AUTOMATION_167_V45")
    text += (
        "\nV45 aggregate contract: 44 candidate/source batches, 314 raw candidate records, "
        "12 canonical-listing-URL duplicates excluded, 302 unique candidates, "
        "278 internet-reverified rows, 773 verified sources, 3 official planning evidence rows, "
        "237/238 operations and zero unverified parcel values. Favicon-only noise remains ignored; "
...
def main() -> int:
    ...
    report["automation_version"] = "V45"
    aggregate = report.get("aggregate_batch_state", {})
    progress = report.get("progress_page_acceptance", {})
    report["aggregate_contract"] = {
        "candidate_batches": 44,
        "verified_source_batches": 44,
        "raw_candidate_rows": 314,
        "duplicate_candidate_rows_excluded": 12,
        "candidate_rows": 302,
        "internet_reverified_rows": 278,
        "verified_source_rows": 773,
        "official_planning_evidence_rows": 3,
        "strengthened_candidate_rows": 252,
        "completed_operations": 237,
        "total_operations": 238,
        "unverified_parcel_value_rows": 0,
    }
    report["v45_contract_observed"] = {
    ...
    if isinstance(next_step, str):
        report["next_step"] = next_step.replace("AUTOMATION_167_V44", "AUTOMATION_167_V45")
