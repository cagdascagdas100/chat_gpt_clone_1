...
text = text.replace(old_owner, "github-actions-security-public-safety-2-wave84")
text = text.replace("wave32", "wave84")
if "590" not in text:
    raise SystemExit("EXPECTED_590_TARGET_FRAGMENTS_MISSING")
text = text.replace("590", "16960")
text = text.replace("__WAVE71_QUEUE__", "0059_security_public_safety_2_priority_16960row_incremental_evidence_expansion_20260730.v3.task.json")
text = text.replace("__WAVE71_CONTINUATION__", "9a4b6de0bd303868ead980127702e351f234324d603f30023f216ec8a14bd1e0")

required_replacements = [
    ("range(30762, 31352)", "range(30762, 47722)"),
    ("parcel_31351", "parcel_47721"),
    ("MIN_QUALITY = 561", "MIN_QUALITY = 16112"),
    ('"progress_delta_percentage_points": 11.86', '"progress_delta_percentage_points": 2.65'),
    ('"accepted_base_candidate_rows": 520', '"accepted_base_candidate_rows": 16510'),
    ('"incremental_rows_target": 70', '"incremental_rows_target": 450'),
    ('"incremental_rows_completed": 70', '"incremental_rows_completed": 450'),
    ("security_public_safety_2_priority_16960row_incremental_evidence_expansion_20260729", "security_public_safety_2_priority_16960row_incremental_evidence_expansion_20260730"),
    ("priority_16960row_browser_acceptance_wave84_receipt_20260729.json", "priority_16960row_browser_acceptance_wave84_receipt_20260730.json"),
    ("priority_16960row_targeted_retry_wave84_diagnostic_20260729.json", "priority_16960row_targeted_retry_wave84_diagnostic_20260730.json"),
]
...
old_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'16960 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()"""
new_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'16960 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()
            dom_deadline = time.time() + 180
            while dom_rows < 16960 and time.time() < dom_deadline:
