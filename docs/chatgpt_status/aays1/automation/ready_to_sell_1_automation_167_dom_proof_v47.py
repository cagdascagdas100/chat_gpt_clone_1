...
BASE = HERE / "ready_to_sell_1_automation_167_dom_proof_v46.py"
spec = importlib.util.spec_from_file_location("rts1_v46", BASE)
...
v3.SOURCE_FILES = [DATA_ROOT / "official_source_candidates_20260720.json"] + [
    batch("official_source_candidates", n) for n in range(2, 47)
]
v3.CANDIDATE_FILES = [DATA_ROOT / "verified_candidate_examples_20260720.json"] + [
    batch("verified_candidate_examples", n) for n in range(2, 47)
]
v3.EXPECTED_TOTAL_SOURCES = 923
v3.EXPECTED_TOTAL_CANDIDATES = 352
v3.EXPECTED_EXACT_INSPIRE = 2
v3.EXPECTED_INTERNET_REVERIFIED = 328
v3.EXPECTED_COMPLETED_OPERATIONS = 249
v3.EXPECTED_TOTAL_OPERATIONS = 250
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = text.replace("Aggregate DOM Proof V46", "Aggregate DOM Proof V47")
    text = text.replace("RERUN_AUTOMATION_167_V46", "RERUN_AUTOMATION_167_V47")
    text += (
        "\nV47 aggregate contract: 46 candidate/source batches, 364 raw candidate records, "
        "12 canonical-listing-URL duplicates excluded, 352 unique candidates, "
        "328 internet-reverified rows, 923 verified sources, 3 official planning evidence rows, "
        "249/250 operations and zero unverified parcel values. Favicon-only noise remains ignored; "
...
def main() -> int:
    ...
    report["automation_version"] = "V47"
    aggregate = report.get("aggregate_batch_state", {})
    progress = report.get("progress_page_acceptance", {})
    report["aggregate_contract"] = {
        "candidate_batches": 46,
        "verified_source_batches": 46,
        "raw_candidate_rows": 364,
        "duplicate_candidate_rows_excluded": 12,
        "candidate_rows": 352,
        "internet_reverified_rows": 328,
        "verified_source_rows": 923,
        "official_planning_evidence_rows": 3,
        "strengthened_candidate_rows": 302,
        "completed_operations": 249,
        "total_operations": 250,
        "unverified_parcel_value_rows": 0,
    }
    report["v47_contract_observed"] = {
    ...
    if isinstance(next_step, str):
        report["next_step"] = next_step.replace("AUTOMATION_167_V46", "AUTOMATION_167_V47")
