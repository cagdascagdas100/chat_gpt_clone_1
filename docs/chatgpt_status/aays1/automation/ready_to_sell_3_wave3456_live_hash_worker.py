...
WAVES = (
    ...
    "england_map_web/data/aays_21_slots/ready_to_sell_3/research_preload_wave_6_20260720.json",
)
...
def fetch_document(url: str, markers: list[str]) -> dict[str, Any]:
    ...
    if "pdf" in content_type.casefold():
        normalized = raw.decode("latin-1", errors="replace").casefold()
    else:
...
def requires_marketing_revalidation(candidate: dict[str, Any]) -> bool:
    value = str(candidate.get("marketing_status") or "").casefold()
    tokens = (
...
def process_candidate(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    planning = None
    planning_url = candidate.get("official_planning_url") or candidate.get("planning_register_url")
    if planning_url:
    ...
    marketing_revalidation = requires_marketing_revalidation(candidate)
    if marketing_revalidation:
        score = min(score, 89)
    ...
    if marketing_revalidation:
        blockers.append("CURRENT_MARKETING_STATUS_REVALIDATION_REQUIRED")
    ...
    return {
        **candidate,
        ...
        "planning_cross_check_url": planning_url,
        ...
        "marketing_status_revalidation_required": marketing_revalidation,
...
def main() -> int:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3, thread_name_prefix="ready-to-sell-wave3456") as pool:
    ...
    revalidation_count = sum(1 for row in rows if row["marketing_status_revalidation_required"])
    ...
    payload = {
        ...
        "marketing_status_revalidation_required_count": revalidation_count,
    ...
    status_output = root / "docs/chatgpt_status/aays1/shards/ready_to_sell_3/status/wave3456_live_hash_latest.json"
    report_output = root / "docs/chatgpt_status/aays1/shards/ready_to_sell_3/reports/wave3456_live_hash_latest.md"
    ...
    if revalidation_count:
        blockers.append(f"MARKETING_STATUS_REVALIDATION_REQUIRED:{revalidation_count}")
    ...
    report_output.write_text(
        "\n".join([
            "# ReadyToSell 3 — Wave 3/4/5/6 live source evidence",
            "",
            ...
            f"- Marketing status revalidation required: {revalidation_count}",
            "- Promoted rows: 0",
            "- Parcel matches: 0",
            "- Geometry matches: 0",
