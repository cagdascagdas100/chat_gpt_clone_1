...
text = text.replace(old_owner, "github-actions-security-public-safety-2-wave98")
text = text.replace("wave32", "wave98")
if "590" not in text:
    raise SystemExit("EXPECTED_590_TARGET_FRAGMENTS_MISSING")
text = text.replace("590", "23260")
text = text.replace("__WAVE71_QUEUE__", "0073_security_public_safety_2_priority_23260row_incremental_evidence_expansion_20260731.v3.task.json")
text = text.replace("__WAVE71_CONTINUATION__", "36f1b43ca5fd4ff3e2e79e5d3d960a8c479f5cbd20d4db374d9be4305030f1d3")

required_replacements = [
    ("range(30762, 31352)", "range(30762, 54022)"),
    ("parcel_31351", "parcel_54021"),
    ("MIN_QUALITY = 561", "MIN_QUALITY = 22097"),
    ('"progress_delta_percentage_points": 11.86', '"progress_delta_percentage_points": 1.93'),
    ('"accepted_base_candidate_rows": 520', '"accepted_base_candidate_rows": 22810'),
    ('"incremental_rows_target": 70', '"incremental_rows_target": 450'),
    ('"incremental_rows_completed": 70', '"incremental_rows_completed": 450'),
    ("security_public_safety_2_priority_23260row_incremental_evidence_expansion_20260729", "security_public_safety_2_priority_23260row_incremental_evidence_expansion_20260731"),
    ("priority_23260row_browser_acceptance_wave98_receipt_20260729.json", "priority_23260row_browser_acceptance_wave98_receipt_20260731.json"),
    ("priority_23260row_targeted_retry_wave98_diagnostic_20260729.json", "priority_23260row_targeted_retry_wave98_diagnostic_20260731.json"),
]
...
old_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'23260 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()"""
new_dom = """            locator = page.locator("xpath=//h2[contains(normalize-space(.),'23260 örnek satır')]/following-sibling::table[1]/tbody/tr")
            dom_rows = locator.count()
            dom_deadline = time.time() + 180
            while dom_rows < 23260 and time.time() < dom_deadline:
