...
$taskId = '202_aays1_parcel_label_36row_source_classification_publish_20260713'
$expectedCount = 36
...
$inputRels = @(
  'docs/chatgpt_status/aays1/inputs/189_distance_property_types_source_classification_research_hold_20260713.json',
  'docs/chatgpt_status/aays1/inputs/190_distance_property_types_source_classification_research_hold_20260713.json',
  'docs/chatgpt_status/aays1/inputs/191_distance_property_types_source_classification_research_hold_20260713.json',
  'docs/chatgpt_status/aays1/inputs/192_distance_property_types_source_classification_research_hold_20260713.json',
  'docs/chatgpt_status/aays1/inputs/193_distance_property_types_source_classification_research_hold_20260713.json',
  'docs/chatgpt_status/aays1/inputs/194_distance_property_types_source_classification_research_hold_20260713.json'
)
$queueRel = 'docs/chatgpt_status/aays1/queue/202_aays1_parcel_label_36row_source_classification_publish_20260713.task.json'
$evidenceRel = 'docs/chatgpt_status/aays1/evidence/202_parcel_label_36row_source_classification_publish_evidence_20260713.json'
$reportRel = 'docs/chatgpt_status/aays1/reports/202_parcel_label_36row_source_classification_publish_report_20260713.md'
$outputRel = 'docs/chatgpt_status/aays1/runner_outputs/202_aays1_parcel_label_36row_source_classification_publish_20260713_output.json'
$proofRel = 'docs/chatgpt_status/aays1/runner_outputs/202_aays1_parcel_label_36row_source_classification_publish_20260713_browser_http_proof.json'
$taskStatusRel = 'docs/chatgpt_status/aays1/status/202_aays1_parcel_label_36row_source_classification_publish_20260713_status.json'
...
function Parse-JsonText([string]$text) {
  ...
  if ($clean.Length -ge 3 -and [int]$clean[0] -eq 239 -and [int]$clean[1] -eq 187 -and [int]$clean[2] -eq 191) {
...
try {
  ...
  if (@($features).Count -ne $expectedCount) { throw ('expected ' + $expectedCount + ' research features, found ' + @($features).Count) }
  ...
  foreach ($feature in $features) {
    ...
    Set-Value $row 'change_reason' 'task_202_combined_research_batches_189_190_191_192_193_194'
  ...
  if ($updatedCount -ne $expectedCount) { throw ('expected ' + $expectedCount + ' existing rows updated, found ' + $updatedCount) }
  ...
  Set-Value $matrix 'latest_batch_id' '202_source_classification_publish'
  ...
  Save-Json $evidenceRel ([pscustomobject]@{
    task_id=$taskId; generated_at=$now; requested_row_count=$expectedCount; updated_row_count=$updatedCount;
    ...
    }); geometry_policy='No exact parcel or building geometry is created. All thirty-six updated rows remain NOT_BOUND.';
  ...
  Save-Json $changesRel ([pscustomobject]@{
    task_id=$taskId; operation_id='202_source_classification_publish_20260713'; updated_at=$now; new_row_count=0;
  ...
  Save-Json $statusRel ([pscustomobject]@{
    ...
    latest_task_id=$taskId; latest_batch_id='202'; latest_operation_row_count=$updatedCount; new_row_count=0;
  ...
  Save-Json $manifestRel ([pscustomobject]@{
    task_id=$taskId; updated_at=$now; batches_seen=@('189','190','191','192','193','194','202'); latest_enrichment_inputs=@($inputRels);
  ...
  Save-Json $indexRel ([pscustomobject]@{
    task_id=$taskId; generated_at=$now; unique_parcel_count=@($matrix.rows).Count;
    local_present_artifact_count=$presentCount; missing_artifact_count=$missingCount; rows=@($indexRows);
    final_ready=$false; fake_data=$false
  })
  ...
  @(
    '# Task 202 — 36-row source/classification publish',
    '',
    ('- Updated existing rows: ' + $updatedCount),
    '- New rows: 0',
    ('- Average accuracy: ' + $averageAccuracy + '/4'),
    ('- Manual review rows: ' + $manualReviewCount),
    ('- Conflict rows: ' + $conflictCount),
    '- Exact geometry created: 0',
    '- All updated rows remain geometry_status=NOT_BOUND',
    '- final_ready: false'
  ...
  $served = Parse-JsonText ([string]$dataResponse.Content)
  ...
  $httpMatch = ($pageResponse.StatusCode -eq 200 -and $dataResponse.StatusCode -eq 200 -and @($served.rows).Count -eq @($matrix.rows).Count -and $servedTaskRows.Count -eq $expectedCount -and $missingIds.Count -eq 0)
  ...
  Save-Json $proofRel ([pscustomobject]@{
    ...
    source_row_count=@($matrix.rows).Count; served_row_count=@($served.rows).Count; expected_updated_row_count=$expectedCount;
  ...
  $state = if ($httpMatch -and $copyMatch) { 'COMPLETED_36ROW_SOURCE_CLASSIFICATION_VISIBLE_NOT_FINAL' } else { 'BLOCKED_36ROW_RUNTIME_VISIBILITY_MISMATCH' }
