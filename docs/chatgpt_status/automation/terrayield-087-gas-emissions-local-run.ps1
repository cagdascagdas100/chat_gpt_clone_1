...
if ($dataGenerated -and (Test-Path $appJs)) {
  try {
    ...
    if ($app -notmatch "parcel_emissions_scores.geojson") {
      Add-Content -Encoding utf8 -Path $appJs -Value "`n// AAYS_GAS_EMISSIONS_PROXY_INTEGRATION: air.png emissions layer uses ./data/parcel_emissions_scores.geojson; source_type=air_quality_proxy; emission_percent high=bad; no DB write.`n"
    }
...
if ($nodeCheckPass) {
  try {
    ...
    if ($page.StatusCode -eq 200 -and $data.Content -match "emission_percent") { $staticSmokePass = $true; $progress = 100; $status = "READY_FOR_CODEX_MINIMAL_PATCH"; $openUrl = "http://127.0.0.1:$port/"; $serverPid = $server.Id; Start-Process $openUrl } else { $status = "STATIC_SMOKE_FAILED"; Stop-Process -Id $server.Id -Force }
    Pop-Location
  } catch { $status = "STATIC_SMOKE_EXCEPTION"; $errors += $_.Exception.Message; try { Pop-Location } catch {} }
}
if ($status -eq "RUNNING") { $status = if ($progress -eq 100) { "READY_FOR_CODEX_MINIMAL_PATCH" } else { "PARTIAL_OR_BLOCKED" } }
