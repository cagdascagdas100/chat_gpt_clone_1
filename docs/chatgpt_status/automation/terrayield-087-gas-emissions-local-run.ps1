...
$scope = "gas_emissions_air_icon_london_only"
$frontend = Join-Path -Path $repo -ChildPath "england_map_web"
$appJs = Join-Path -Path $frontend -ChildPath "app.js"
$inputGeojson = Join-Path -Path $frontend -ChildPath "data\parcel_air_quality_scores.geojson"
$outRoot = "E:\AAYS_DATA\emissions_air_quality"
$outDir = Join-Path -Path $outRoot -ChildPath "outputs"
$manifestDir = Join-Path -Path $outRoot -ChildPath "manifests"
$reportDir = Join-Path -Path $repo -ChildPath "docs\chatgpt_status\reports"
$runnerOutDir = Join-Path -Path $repo -ChildPath "docs\chatgpt_status\runner_outputs"
$heartbeatDir = Join-Path -Path $repo -ChildPath "docs\chatgpt_status\heartbeat"
$statusDir = Join-Path -Path $repo -ChildPath "docs\chatgpt_status\status"
$dataDir = Join-Path -Path $frontend -ChildPath "data"

New-Item -ItemType Directory -Force -Path $outDir,$manifestDir,$reportDir,$runnerOutDir,$heartbeatDir,$statusDir,$dataDir | Out-Null

$reportTxt = Join-Path -Path $reportDir -ChildPath "$taskId.txt"
$reportJson = Join-Path -Path $reportDir -ChildPath "$taskId.json"
$runnerJson = Join-Path -Path $runnerOutDir -ChildPath "$taskId.json"
$latestJson = Join-Path -Path $runnerOutDir -ChildPath "latest_output.json"
$heartbeat = Join-Path -Path $heartbeatDir -ChildPath "$taskId.txt"
$statusTxt = Join-Path -Path $statusDir -ChildPath "$taskId.txt"
$statusJson = Join-Path -Path $statusDir -ChildPath "$taskId.json"
$nodeCheckLog = Join-Path -Path $reportDir -ChildPath "terrayield-087-node-check.txt"
$serverOut = Join-Path -Path $reportDir -ChildPath "terrayield-087-http-server-out.txt"
$serverErr = Join-Path -Path $reportDir -ChildPath "terrayield-087-http-server-err.txt"
...
"@
  $obj = @{
    task_id=$taskId; scope=$scope; status=$status; overall_progress_percent=$progress; feature_count=$featureCount;
    data_generated=$dataGenerated; app_patched=$appPatched; node_check_pass=$nodeCheckPass; static_smoke_pass=$staticSmokePass;
    local_app_url=$openUrl; local_server_pid=$serverPid; db_write=$false; migration=$false; production_deploy=$false; fake_data=$false;
    input_geojson=$inputGeojson; runtime_geojson="england_map_web/data/parcel_emissions_scores.geojson"; runtime_csv="england_map_web/data/parcel_emissions_scores.csv";
    errors=$errors; manual_stdout_required=$false; next_action=$nextActionValue
  }
...
if (-not (Test-Path -Path $inputGeojson)) {
  $status = "BLOCKED_MISSING_LOCAL_INPUT"
  $errors += "Missing input_proxy_geojson=$inputGeojson"
  Write-TaskReports
} else {
  try {
    $geo = Get-Content -Path $inputGeojson -Raw | ConvertFrom-Json
    ...
    $geoJsonOutLocal = Join-Path -Path $outDir -ChildPath "parcel_emissions_scores.geojson"
    $csvOutLocal = Join-Path -Path $outDir -ChildPath "parcel_emissions_scores.csv"
    $manifestLocal = Join-Path -Path $outDir -ChildPath "parcel_emissions_score_manifest.json"
    $geoJsonRuntime = Join-Path -Path $dataDir -ChildPath "parcel_emissions_scores.geojson"
    $csvRuntime = Join-Path -Path $dataDir -ChildPath "parcel_emissions_scores.csv"
    $manifestRuntime = Join-Path -Path $dataDir -ChildPath "parcel_emissions_score_manifest.json"
    $sourceRegistry = Join-Path -Path $manifestDir -ChildPath "source_registry.csv"
    $evidenceManifest = Join-Path -Path $manifestDir -ChildPath "evidence_manifest.jsonl"
    ...
    Copy-Item -Path $geoJsonOutLocal -Destination $geoJsonRuntime -Force
    Copy-Item -Path $csvOutLocal -Destination $csvRuntime -Force
    Copy-Item -Path $manifestLocal -Destination $manifestRuntime -Force
    "source_id,source_type,notes`nsrc_existing_parcel_air_quality_scores,air_quality_proxy,Local proxy input; not official CO2e/GHG" | Set-Content -Path $sourceRegistry -Encoding utf8
    @{evidence_id="ev_existing_air_quality_proxy_001";source_id="src_existing_parcel_air_quality_scores";collected_at=$generatedAt;verdict="usable_as_proxy"} | ConvertTo-Json -Compress | Set-Content -Path $evidenceManifest -Encoding utf8
...
if ($dataGenerated -and (Test-Path -Path $appJs)) {
  try {
    $app = Get-Content -Path $appJs -Raw
    $marker = "AAYS_GAS_EMISSIONS_PROXY_INTEGRATION_V3"
    if ($app -notmatch [regex]::Escape($marker)) {
      $integrationBlock = @'

  // AAYS_GAS_EMISSIONS_PROXY_INTEGRATION_V3
  ...
  function gasEmissionsColorExpression() {
    return ["case",["!",["has","emission_percent"]],"#9e9e9e",["<=",["to-number",["get","emission_percent"]],20],"#2e7d32",["<=",["to-number",["get","emission_percent"]],40],"#8bc34a",["<=",["to-number",["get","emission_percent"]],60],"#fdd835",["<=",["to-number",["get","emission_percent"]],80],"#fb8c00","#c62828"];
  }
  ...
  function ensureGasEmissionsLegend() {
    ...
    if (!el) {
      ...
      el.innerHTML = ["<strong>Gaz Emisyonu (proxy)</strong>","<div><span style='background:#2e7d32'></span>0-20 Cok Dusuk</div>","<div><span style='background:#8bc34a'></span>21-40 Dusuk</div>","<div><span style='background:#fdd835'></span>41-60 Orta</div>","<div><span style='background:#fb8c00'></span>61-80 Yuksek</div>","<div><span style='background:#c62828'></span>81-100 Cok Yuksek</div>","<div><span style='background:#9e9e9e'></span>Veri Yok</div>","<small>Kaynak: air_quality_proxy; resmi CO2e/GHG degildir.</small>"].join("");
  ...
  async function ensureGasEmissionsLayer() {
    ...
    gasEmissionsLoadPromise = (async () => {
      ...
      if (!map.getSource(GAS_EMISSIONS_SOURCE_ID)) map.addSource(GAS_EMISSIONS_SOURCE_ID, { type: "geojson", data });
      else map.getSource(GAS_EMISSIONS_SOURCE_ID).setData(data);
      if (!map.getLayer(GAS_EMISSIONS_LAYER_ID)) {
        map.addLayer({ id: GAS_EMISSIONS_LAYER_ID, type: "circle", source: GAS_EMISSIONS_SOURCE_ID, paint: { "circle-radius": ["interpolate", ["linear"], ["zoom"], 8, 3, 14, 8], "circle-color": gasEmissionsColorExpression(), "circle-opacity": 0.78, "circle-stroke-color": "#263238", "circle-stroke-width": 0.7 }, layout: { visibility: gasEmissionsVisible ? "visible" : "none" } });
        map.on("click", GAS_EMISSIONS_LAYER_ID, (event) => {
          const feature = event.features && event.features[0]; if (!feature) return;
          const p = feature.properties || {};
          const html = [`<strong>Gaz Emisyonu</strong>`,`<div>Skor: <b>${p.emission_percent ?? "Veri Yok"}</b></div>`,`<div>Seviye: <b>${p.emission_level_label_tr || p.emission_level || "Veri Yok"}</b></div>`,`<div>Guven: <b>${p.confidence_percent ?? "-"}</b></div>`,`<div>Kaynak: <b>${p.source_type || "air_quality_proxy"}</b></div>`,`<div>Guncelleme: <b>${p.calculated_at || "-"}</b></div>`,`<small>Proxy veridir; resmi CO2e/GHG envanteri degildir.</small>`].join("");
          new maplibregl.Popup({ closeButton: true, closeOnClick: true }).setLngLat(event.lngLat).setHTML(html).addTo(map);
        });
  ...
  async function setGasEmissionsVisible(visible) {
    ...
    if (map.getLayer(GAS_EMISSIONS_LAYER_ID)) map.setLayoutProperty(GAS_EMISSIONS_LAYER_ID, "visibility", gasEmissionsVisible ? "visible" : "none");
...
'@
      $closurePattern = "(?s)\r?\n\}\)\(\);\s*$"
      if ([regex]::IsMatch($app, $closurePattern)) { $app = [regex]::Replace($app, $closurePattern, "`n$integrationBlock`n})();`n") }
      else { $app += "`n$integrationBlock`n" }
      Set-Content -Path $appJs -Encoding utf8 -Value $app
    }
    $verifiedApp = Get-Content -Path $appJs -Raw
    if ($verifiedApp -match [regex]::Escape($marker) -and $verifiedApp -match "parcel_emissions_scores.geojson" -and $verifiedApp -match "AAYS_GAS_EMISSIONS_LAYER") { $appPatched = $true; $progress = 75 }
    else { $status = "APP_PATCH_VERIFICATION_FAILED"; $errors += "Gas emissions UI marker/layer not found after patch" }
  } catch { $status = "APP_PATCH_FAILED"; $errors += $_.Exception.Message }
}

if ($appPatched) {
  try {
    node --check "england_map_web\app.js" *> $nodeCheckLog
...
if ($nodeCheckPass) {
  ...
  try {
    ...
    $server = Start-Process -FilePath "python" -ArgumentList "-m http.server $port --bind 127.0.0.1" -RedirectStandardOutput $serverOut -RedirectStandardError $serverErr -PassThru -NoNewWindow
    ...
    if ($page.StatusCode -eq 200 -and $data.Content -match "emission_percent") { $staticSmokePass = $true; $progress = 100; $status = "FINAL_READY"; $openUrl = "http://127.0.0.1:$port/"; $serverPid = $server.Id }
    else { $status = "STATIC_SMOKE_FAILED" }
  } catch { $status = "STATIC_SMOKE_EXCEPTION"; $errors += $_.Exception.Message }
  finally {
    try { Pop-Location } catch {}
    if ($server -and $progress -ne 100) { try { Stop-Process -Id $server.Id -Force } catch {} }
  }
}
...
try {
  ...
  git commit -m "feat: run London gas emissions air icon integration" | Out-Null 2>&1
