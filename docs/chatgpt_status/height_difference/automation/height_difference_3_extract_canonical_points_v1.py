...
def normalise_target(feature: dict) -> dict:
    ...
    return {
        ...
        "lsoa_code": properties.get("security_lsoa_code"),
        "lsoa_name": properties.get("security_lsoa_name"),
        "borough_code": properties.get("security_borough"),
    ...
    *,
    ...
    while True:
        ...
        if not chunk:
            break
        sha256.update(chunk)
        metrics["source_bytes_streamed"] = int(metrics["source_bytes_streamed"]) + len(chunk)
...
def main() -> int:
    repo_root = Path(os.environ.get("AAYS_REPO_ROOT", r"F:\chatgpt\chat_gpt_clone_1_main")).resolve()
    generated_at = utc_now()
    errors: list[str] = []

    try:
        object_type = git_text(repo_root, "cat-file", "-t", BLOB_SHA)
        object_size = int(git_text(repo_root, "cat-file", "-s", BLOB_SHA))
        path_blob_sha = git_text(repo_root, "rev-parse", f"{SOURCE_BRANCH}:{SOURCE_PATH}")
    except Exception as exc:
        object_type = None
        object_size = None
        path_blob_sha = None
        errors.append(f"CANONICAL_GIT_OBJECT_CHECK_FAILED: {exc}")
    ...
    if not errors and object_type == "blob" and path_blob_sha == BLOB_SHA:
        ...
        found, metrics = stream_targets(process.stdout, digest)
        stderr = process.stderr.read().decode("utf-8", errors="replace") if process.stderr else ""
        return_code = process.wait()
        source_sha256 = digest.hexdigest()
        if return_code != 0:
            errors.append(f"GIT_CAT_FILE_STREAM_FAILED_{return_code}: {stderr.strip()}")
        if metrics.get("error"):
            errors.append(str(metrics["error"]))
    else:
        if object_type != "blob":
            errors.append(f"CANONICAL_OBJECT_TYPE_MISMATCH: {object_type}")
        if path_blob_sha != BLOB_SHA:
            errors.append(f"CANONICAL_PATH_BLOB_MISMATCH: {path_blob_sha}")
    ...
    common = {
        ...
        "state": "CANONICAL_POINT_EXTRACTION_PASS" if passed else "CANONICAL_POINT_EXTRACTION_BLOCKED",
        "source": {
            ...
            "source_size_bytes": object_size,
            "stream_sha256": source_sha256,
            "expected_feature_count": EXPECTED_FEATURE_COUNT,
        },
        "target_ids": TARGET_IDS,
        "canonical_point_rows": ordered_rows if passed else [],
        "canonical_point_row_count": len(ordered_rows) if passed else 0,
        ...
        "errors": errors,
        "output_semantics": "CANONICAL_POINTS_ONLY_NO_BOUNDARY_NO_ELEVATION",
    ...
    website = {
        **common,
        "website_row_kind": "CANONICAL_POINT_PREPARATION_ROWS",
        "rows_visible": len(ordered_rows) if passed else 0,
    }

    write_json_atomic(repo_root / OUTPUT_REL, common)
    write_json_atomic(repo_root / RECONCILIATION_REL, reconciliation)
    write_json_atomic(repo_root / WEBSITE_REL, website)
