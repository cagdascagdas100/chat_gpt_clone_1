...
$Out=Join-Path $Reports "final_render_ui_visibility_proof_F_drive_$Now.txt"
...
"MODE=FINAL_RENDER_UI_VISIBILITY_PROOF_F_DRIVE" | Add-Content $Out
...
if($StaticPass -and $Browser -and $HtmlHits.Count -gt 0){
  $Entry=$HtmlHits[0].Path
  $Rel=$Entry.Substring($Web.Length).TrimStart('\') -replace '\\','/'
  $Port=8797
  ...
  if($Py){
    ...
    $Dom=& $Browser --headless --disable-gpu --dump-dom $Url 2>&1
    $DomText=($Dom | Out-String)
    $RenderPass=($DomText -match 'showTopographyOverlay')
    "RENDER_DOM_HAS_SHOW_TOPOGRAPHY=$RenderPass" | Add-Content $Out
    Stop-Process -Id $Server.Id -Force -ErrorAction SilentlyContinue
  }else{
    "RENDER_BLOCKER=python_http_server_not_found" | Add-Content $Out
  }
}
if($StaticPass -and $RenderPass){
...
git -C $W commit -m "Add F drive final render UI visibility proof $Now"
git -C $W pull --rebase --autostash origin $B
git -C $W push origin HEAD:$B
Write-Host 'F drive final render proof done. Check GitHub report.'
