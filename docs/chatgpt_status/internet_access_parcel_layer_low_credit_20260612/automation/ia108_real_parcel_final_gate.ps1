...
$result = [ordered]@{
  task_id=$taskId; page_key=$pageKey; status=$status; completion_percent=$percent; final_ready=$finalReady;
  manual_stdout_required=$false; db_write=$false; migration=$false; production_deploy=$false; fake_data=$false;
  heavy_root=$heavyRoot; source_root=$sourceRoot; source_geojson=$scoreGeoJson; source_csv=$scoreCsv; source_breakdown=$breakdownCsv;
  feature_count=$featureCount; null_geometry_count=$nullGeometryCount; polygon_feature_count=$polygonCount; blockers=$blockers;
  ready_outputs=@{ scores_geojson=$readyGeoJson; scores_csv=$readyCsv; factor_breakdown_csv=$readyBreakdown; detail_json=$detailJson };
  expected_next_action= if ($finalReady) { 'Codex can integrate ready artifacts' } else { 'Run real geometry join task; fake geometry refused' };
