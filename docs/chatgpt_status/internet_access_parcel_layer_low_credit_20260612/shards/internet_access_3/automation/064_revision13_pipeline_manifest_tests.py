...
def main():
 args();src=Path(__file__).with_name("063_full_pipeline_revision13_entry.py").read_text();t=[]
 def ck(n,c):t.append((n,bool(c)))
 ck("CHILD_REV12","057_full_pipeline_revision12_entry.py" in src);ck("TEST_PACKS",all(x in src for x in ["060_os_release_consistency_acceptance_tests.py","062_repo_blob_integrity_matrix_tests.py"]));ck("WORKERS",all(x in src for x in ["059_os_release_consistency_acceptance.py","061_repo_blob_integrity_matrix.py"]));ck("STEPS_50",'effective_pipeline_steps":50' in src);ck("TESTS_304",'contract_tests_target":304' in src);ck("CHECKS_40",'official_source_checks_target":40' in src);ck("SAMPLE_384",'sample_size_target":384' in src);ck("EVIDENCE_40",'target_evidence_rows":40' in src);ck("PRE_HYDRATION","pipeline_passed_pre_hydration" in src and 'full_release_bytes_hydrated":False' in src);ck("SAFETY_FLAGS",all(x in src for x in ["final_ready","fake_data","db_write","migration","production_deploy"]))
