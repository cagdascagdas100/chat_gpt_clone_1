...
def validate_documents(index_html:str,d:dict[str,dict[str,Any]])->dict[str,Any]:
    ...
    require(rep.get('automatic_acceptance') is False and int(rep.get('required_receipts',-1))==2,'reproducibility');available=int(rep.get('real_validated_receipts_available',0));require(available in (0,2),'receipt count')
    if rep.get('exact_reproducibility_pass') is True:require(available==2 and rep.get('status')=='PASS_EXACT_RUNTIME_REPRODUCIBILITY','exact reproducibility mismatch')
    require(lock.get('status') in ('PREPARED_WAITING_EXISTING_RUNNER','PASS_EXECUTION_LOCK_REVIEW_ONLY') and int(lock.get('locked_blob_count',-1))>=14,'lock');require(lock.get('auto_claim') is False and lock.get('queue_submission') is False and lock.get('create_new_runner') is False,'lock mutation')
    if lock.get('exact_execution_lock_pass') is True:require(lock.get('status')=='PASS_EXECUTION_LOCK_REVIEW_ONLY','lock pass mismatch')
    ...
    require(elig.get('auto_claim') is False and elig.get('queue_submission') is False and elig.get('create_new_runner') is False,'eligibility');require(poly.get('polygon_popup_acceptance') in (False,True),'polygon');require(runner.get('single_shared_runner_only') is True and runner.get('create_new_runner') is False and runner.get('queue_submission') is False,'runner');require('execution_lock_latest.json' in str(runner.get('execution_lock_command','')),'lock command');require('runner_blocker_diagnostics_latest.json' in str(runner.get('blocker_diagnostics_command','')),'blocker command')
