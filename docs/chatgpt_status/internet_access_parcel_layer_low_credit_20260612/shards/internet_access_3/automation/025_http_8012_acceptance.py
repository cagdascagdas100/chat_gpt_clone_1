...
REQUIRED_DOM_IDS=('id="metrics"','id="operations"','id="runtime"','id="runtimeResults"','id="runtimeSamples"','id="reproducibility"','id="directZip"','id="targeted"','id="polygon"','id="eligibility"','id="blobs"','id="publish"','id="runner"','id="sources"','id="examples"')
JSON_ENDPOINTS=("progress_latest.json","operations_latest.json","runtime_bundle_latest.json","runtime_results_latest.json","runtime_reproducibility_latest.json","runner_eligibility_latest.json","polygon_popup_acceptance_latest.json","runner_task_latest.json","source_candidates_latest.json","examples_latest.json","remote_publish_latest.json")
...
def fetch_bytes(url:str,*,timeout:float,retries:int,max_bytes:int=8_000_000)->tuple[bytes,str]:
    ...
    for attempt in range(1,retries+1):
        try:
            req=urllib.request.Request(url,headers={"User-Agent":"internet_access_3-http-acceptance/2"})
...
def validate_documents(index_html:str,documents:dict[str,dict[str,Any]])->dict[str,Any]:
    ...
    p=documents["progress_latest.json"];o=documents["operations_latest.json"];rb=documents["runtime_bundle_latest.json"];rr=documents["runtime_results_latest.json"];rep=documents["runtime_reproducibility_latest.json"];elig=documents["runner_eligibility_latest.json"];poly=documents["polygon_popup_acceptance_latest.json"];runner=documents["runner_task_latest.json"];sources=documents["source_candidates_latest.json"];examples=documents["examples_latest.json"];publish=documents["remote_publish_latest.json"]
    ...
    ec=int(examples.get("example_count",-1));require(ec>=19,"expanded examples not visible");require(len(examples.get("examples") or [])==ec,"example count mismatch");require(int(examples.get("verified_product_example_rows",-1))==0,"aggregate examples must not be product rows")
    ...
    return {"operation_rows":len(rows),"runtime_preparation_gates":len(gates),"runtime_rows_visible":real_rows,"reproducibility_status":rep.get("status"),"source_candidates_visible":cc,"promoted_sources_visible":pc,"examples_visible":ec}
