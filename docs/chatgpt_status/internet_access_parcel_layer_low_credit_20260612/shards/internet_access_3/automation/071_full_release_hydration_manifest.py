...
import argparse,hashlib,json,os,re,tempfile,time,urllib.request,zipfile
...
def inspect(p):
 ...
 if prefix.startswith(b"PK") and zipfile.is_zipfile(p):
  with zipfile.ZipFile(p) as z:
   bad=z.testzip();m=[x for x in z.infolist() if not x.is_dir()]
   return {"media_type":"application/zip","zip_integrity_passed":bad is None,"zip_bad_member":bad,"zip_member_count":len(m),"zip_uncompressed_bytes":sum(x.file_size for x in m),"zip_csv_member_count":sum(x.filename.lower().endswith(".csv") for x in m)}
 return {"media_type":"text/csv-or-text" if not prefix.startswith((b"\x00",b"\x1f\x8b")) else "application/octet-stream","zip_integrity_passed":None,"zip_bad_member":None,"zip_member_count":0,"zip_uncompressed_bytes":0,"zip_csv_member_count":0}
...
def validate(p,s):
 ...
 n=p.stat().st_size;es=int(s.get("expected_size") or 0);em=s.get("expected_md5");md5=digest(p,"md5");info=inspect(p);size_ok=es<=0 or n==es;md5_ok=em is None or md5.lower()==str(em).lower();zip_req=str(s.get("package_id") or "").startswith("os_");zip_ok=(info["media_type"]=="application/zip" and info["zip_integrity_passed"] is True and info["zip_csv_member_count"]>0) if zip_req else (info["media_type"]!="application/zip" or info["zip_integrity_passed"] is True);bl=[]
 ...
 return {"valid":not bl,"reason":"VALID" if not bl else ",".join(bl),"bytes":n,"actual_md5":md5,"actual_sha256":digest(p,"sha256"),"size_verified":size_ok,"md5_verified":md5_ok,**info}
...
def transfer(s,part,chunk,timeout):
 cur=part.stat().st_size if part.exists() else 0;h={"User-Agent":"TerraYield-AAYS-internet-access-3/19","Accept":"*/*"}
...
def main():
 ...
 ok=len(done)==4 and not bl;now=__import__("datetime").datetime.now(__import__("datetime").timezone.utc).isoformat();z={"schema_version":2,"task_id":"aays1-internet-access-3-full-release-hydration-20260722","slot_id":SLOT,"state":"runtime_validation_passed" if ok else "blocked","updated_at":now,"release_contract_sha256":hashlib.sha256(json.dumps(reg.get("release_contract"),sort_keys=True,separators=(",",":")).encode()).hexdigest(),"packages_expected":4,"packages_hydrated":len(done),"packages":done,"download_bytes_hydrated":sum(int(x.get("bytes_hydrated") or 0) for x in done),"quarantined_files":sum(len(x.get("quarantined") or []) for x in done),"cache_repairs":sum(bool(x.get("cache_repaired")) for x in done),"source_checks_executed":10,"validation":{"passed":ok,"blockers":bl},"parcel_relations_promoted":0,"confidence_uplifts":0,"actual_business_data_rows_written":0,"output_semantics":"FULL_OFFICIAL_RELEASE_BYTE_HYDRATION_CHECKSUM_ZIP_INTEGRITY_AND_CACHE_REPAIR_MANIFEST_ONLY","final_ready":False,"fake_data":False,"db_write":False,"migration":False,"production_deploy":False};write(r/o.runner_output,z);write(r/o.web_output,z);print(json.dumps(z,ensure_ascii=False,indent=2));return 0 if ok else 2
