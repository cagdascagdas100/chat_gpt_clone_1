...
def main():
 ...
 with tempfile.TemporaryDirectory() as td:
  ...
  ignored=d/"heartbeat.json";ignored.write_bytes(b"1234567");ok("directory_ignore_excludes_heartbeat",w.path_state(d,[ignored])["bytes"]==5)
  ...
  ok("heartbeat_failure_blocks_and_cleans_child",hb_fail["state"]=="blocked" and hb_fail["timeout_kind"]=="heartbeat_write_error" and isinstance(hb_fail["termination"],dict))
  ok("heartbeat_failure_recorded",len(hb_fail["heartbeat_write_errors"])>=1)
 ...
 e=22;z={"schema_version":2,"suite":"runtime_watchdog_supervisor","tests_expected":e,"tests_passed":len(c),"tests_failed":e-len(c),"checks":c,"final_ready":False,"fake_data":False,"db_write":False,"migration":False,"production_deploy":False};print(json.dumps(z,indent=2));return 0 if len(c)==e else 2
