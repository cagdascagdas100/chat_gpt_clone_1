...
SOURCE_PDF = (
    ...
    "about-this-data---fixed-broadband-coverage-and-full-fibre-take-up-2026.pdf"
)
FALLBACK_ZIP_URL = (
    "https://www.ofcom.org.uk/siteassets/resources/documents/research-and-data/"
    "multi-sector/infrastructure-research/connected-nations-spring-2026/"
...
EXPECTED_R2_POSTCODE_ROWS = 1_741_096
MAX_DOWNLOAD_BYTES = 90_000_000
EXPECTED_PATTERN = re.compile(
    r"(?:^|/)202601_fixed_postcode_coverage_r2_([A-Z0-9]+)\.csv$", re.I
)
CORE_ALIASES = {
...
USER_AGENT = "AAYS-internet-access-2/1.0 (+official Ofcom data validation)"
...
def discover_zip_url() -> tuple[str, str]:
    try:
        ...
        matches = re.findall(
            r'href=["\']([^"\']*202601_fixed_broadband_coverage_and_full_fibre_take-up-r1\.zip[^"\']*)',
            page,
            re.I,
        )
        if matches:
...
def inspect_all_r2_files(path: Path) -> dict[str, Any]:
    with zipfile.ZipFile(path, "r") as archive:
        ...
        for index, member in enumerate(r2_members, start=1):
            ...
            prefix_match = EXPECTED_PATTERN.search(member)
            rows.append(
                {
                    "row": index,
                    "file": member,
                    "postcode_area": prefix_match.group(1).upper() if prefix_match else None,
                    "data_rows": row_count,
        ...
        count_ok = len(r2_members) == EXPECTED_R2_POSTCODE_FILE_COUNT
        rows_ok = total_rows == EXPECTED_R2_POSTCODE_ROWS
        return {
            "member_count": len(members),
            "r2_postcode_file_count": len(r2_members),
            "expected_r2_postcode_file_count": EXPECTED_R2_POSTCODE_FILE_COUNT,
            "r2_postcode_file_count_ok": count_ok,
            "total_r2_postcode_rows": total_rows,
            "expected_r2_postcode_rows": EXPECTED_R2_POSTCODE_ROWS,
            "total_r2_postcode_rows_ok": rows_ok,
            "all_r2_files_nonempty_and_core_columns_present": all_ok,
            "zip_crc_ok": True,
            "file_rows": rows,
            "accepted": count_ok and rows_ok and all_ok,
        }
...
<title>internet_access_2 — Ofcom r2 doğrulama</title>
<style>
...
<h1>internet_access_2 — Ofcom r2 doğrulama</h1>
...
<h2>121 r2 dosyası — satır satır</h2><table><thead><tr><th>#</th><th>Dosya</th><th>Alan</th><th>Veri satırı</th><th>Sütun durumu</th><th>Durum</th></tr></thead><tbody id="rows"></tbody></table>
<script>
const d={data_json}; const e=v=>String(v??'').replace(/[&<>"']/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}}[c]));
const cards=[['Durum',d.state],['İlerleme',d.progress_percent+'%'],['İşlem',d.completed_operations+'/'+d.total_operations],['r2 dosya',d.r2_postcode_file_count+'/'+d.expected_r2_postcode_file_count],['r2 satır',d.total_r2_postcode_rows+'/'+d.expected_r2_postcode_rows],['Doğrulanmış aday',d.official_coverage_verified_candidates],['Aday doğruluk',d.accuracy_written],['final_ready',d.final_ready]];
...
document.getElementById('rows').innerHTML=(d.file_rows||[]).map(x=>`<tr><td>${{e(x.row)}}</td><td><code>${{e(x.file)}}</code></td><td>${{e(x.postcode_area)}}</td><td>${{e(x.data_rows)}}</td><td>${{e((x.missing_core_columns||[]).length?'Eksik: '+x.missing_core_columns.join(', '):'Tam')}}</td><td class="${{x.status==='PASS'?'ok':'bad'}}">${{e(x.status)}}</td></tr>`).join('');
</script></body></html>
"""
...
def main() -> int:
    ...
    source_checks = [
        {
            ...
            "evidence": "Official publication and ZIP download page.",
        },
        {
            "row": 2,
            "source": "Ofcom fixed broadband data notes v2",
            "url": SOURCE_PDF,
            "status": "PASS_OFFICIAL_METHOD",
            "evidence": "7 July 2026 correction; 121 all-premises postcode files renamed r2.",
        },
    ]
    ...
    try:
        ...
        if destination.is_file() and destination.stat().st_size >= 1_000_000 and zipfile.is_zipfile(destination):
            ...
            payload["download"] = {"state": "DOWNLOADED", **download_atomic(zip_url, destination)}
        inspection = inspect_all_r2_files(destination)
        payload.update(inspection)
        if inspection["accepted"]:
            payload.update(
                {
                    "state": "OFCom_R2_ZIP_SCHEMA_ACCEPTED",
                    "completed_operations": 3,
                    "progress_percent": 100.0,
                    "percent_change": 33.33,
                    "blocker": None,
                    "next_step": "RUN_STRICT_EXACT_R2_JOIN_FOR_24_VERIFIED_POSTCODE_SAMPLES",
                }
            )
            exit_code = 0
        else:
            payload.update(
                {
                    "state": "OFCom_R2_ZIP_SCHEMA_REJECTED",
                    "blocker": "OFFICIAL_OFCom_R2_ZIP_SCHEMA_OR_ROW_COUNT_NOT_ACCEPTED",
                    "next_step": "REVIEW_R2_FILE_COUNT_ROW_COUNT_AND_CORE_COLUMNS",
                }
            )
    except Exception as exc:
        payload.update(
            {
                "state": "OFCom_R2_ZIP_PROVISION_FAILED",
                "blocker": f"{type(exc).__name__}:{exc}",
                "next_step": "ONE_BOUNDED_RETRY_THEN_MANUAL_ACTION_IF_SAME_FAILURE",
    ...
    report = (
        "# internet_access_2 — Ofcom r2 provision ve tam arşiv doğrulaması\n\n"
        f"- State: {payload['state']}\n"
        f"- Progress: {payload['completed_operations']}/{payload['total_operations']} "
        f"({payload['progress_percent']}%)\n"
        f"- r2 postcode files: {payload.get('r2_postcode_file_count', 0)}/"
        f"{EXPECTED_R2_POSTCODE_FILE_COUNT}\n"
        f"- r2 postcode data rows: {payload.get('total_r2_postcode_rows', 0)}/"
        f"{EXPECTED_R2_POSTCODE_ROWS}\n"
