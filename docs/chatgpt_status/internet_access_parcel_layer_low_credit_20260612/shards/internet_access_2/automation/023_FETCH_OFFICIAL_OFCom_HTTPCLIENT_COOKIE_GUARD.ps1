...
foreach ($Pair in @(@("source", $SourceUrl), @("landing", $LandingUrl))) {
    ...
    $Value = [string]$Pair[1]
    $Uri = $null
    if (-not [System.Uri]::TryCreate($Value, [System.UriKind]::Absolute, [ref]$Uri)) { throw "OFFICIAL_${Name}_URL_INVALID:$Value" }
    if ($Uri.Scheme -ne "https" -or $Uri.Host -ne $AllowedHost) { throw "OFFICIAL_${Name}_URL_SCOPE_VIOLATION:$Value" }
...
function Test-ZipEnvelope {
    ...
    try {
        ...
        $Entries = @($Zip.Entries)
        if ($Entries.Count -lt 1) { throw "ARCHIVE_HAS_NO_ENTRIES" }
        $R2Count = @($Entries | Where-Object { $_.FullName -match '(?i)(^|[/\\_-])r2([/\\_.-]|$)' }).Count
        if ($R2Count -lt 1) { throw "ARCHIVE_HAS_NO_R2_NAMED_ENTRY" }
        return [ordered]@{
            bytes = [int64]$Item.Length
            entry_count = [int]$Entries.Count
            r2_named_entry_count = [int]$R2Count
...
function Invoke-HttpClientCookieDownload {
    param([Parameter(Mandatory=$true)][string]$Destination,[Parameter(Mandatory=$true)][System.Collections.Generic.List[string]]$Errors)
    $Handler = $null; $Client = $null; $LandingResponse = $null; $DownloadRequest = $null; $DownloadResponse = $null; $FileStream = $null
    try {
        ...
        if (-not $LandingResponse.IsSuccessStatusCode) { throw "HTTPCLIENT_LANDING_STATUS:$([int]$LandingResponse.StatusCode)" }
        $LandingStatus = [int]$LandingResponse.StatusCode
        $LandingFinalUri = $LandingResponse.RequestMessage.RequestUri
        if (-not $LandingFinalUri -or $LandingFinalUri.Scheme -ne "https" -or $LandingFinalUri.Host -ne $AllowedHost) { throw "HTTPCLIENT_LANDING_REDIRECT_SCOPE_VIOLATION:$LandingFinalUri" }
        $LandingResponse.Dispose(); $LandingResponse = $null
        ...
        if (-not $DownloadResponse.IsSuccessStatusCode) { throw "HTTPCLIENT_DOWNLOAD_STATUS:$([int]$DownloadResponse.StatusCode)" }
        $DownloadFinalUri = $DownloadResponse.RequestMessage.RequestUri
        if (-not $DownloadFinalUri -or $DownloadFinalUri.Scheme -ne "https" -or $DownloadFinalUri.Host -ne $AllowedHost) { throw "HTTPCLIENT_DOWNLOAD_REDIRECT_SCOPE_VIOLATION:$DownloadFinalUri" }

        $ContentType = [string]$DownloadResponse.Content.Headers.ContentType.MediaType
        if ([string]::IsNullOrWhiteSpace($ContentType)) { throw "HTTPCLIENT_DOWNLOAD_CONTENT_TYPE_MISSING" }
        $ContentTypeNormalized = $ContentType.ToLowerInvariant()
        if ($ContentTypeNormalized -notin $AllowedContentTypes) { throw "HTTPCLIENT_DOWNLOAD_CONTENT_TYPE_REJECTED:$ContentType" }
        ...
        if ($DownloadResponse.Content.Headers.ContentDisposition) { $DispositionName = [string]$DownloadResponse.Content.Headers.ContentDisposition.FileName }
