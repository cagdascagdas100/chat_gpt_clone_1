...
TASK_VERSION = "5.2.8-lineage-reconciliation-quorum"
ATTEMPT_ID = "security-public-safety-3-20260721-017"
SUPERSEDES_ATTEMPT_ID = "security-public-safety-3-20260721-016"
...
def normalise_runtime_row(row: dict[str, Any]) -> None:
    ...
    row["force_lookup_gate"] = bool(force.get("acceptance_passed"))
    row["territorial_crime_coverage_gate"] = bool(
        coverage.get("territorial_crime_coverage_available")
    )
    row["territorial_outcomes_coverage_gate"] = bool(
    ...
    return {
        ...
        **{name: bool(row.get(name)) for name in MEMORY_GATES + FOUR_GATES},
        ...
        "territorial_crime_coverage_gate": bool(
            row.get("territorial_crime_coverage_gate")
        ),
        "territorial_outcomes_coverage_gate": bool(
            row.get("territorial_outcomes_coverage_gate")
        ),
    }
...
def enrich(repo_root: Path, prior_return_code: int) -> int:
    ...
    if "output" in payloads and "website" in payloads:
        ...
        if output_fp != website_fp or output_counts != website_counts:
            errors.append("OUTPUT_WEBSITE_FINGERPRINT_OR_COUNT_MISMATCH")
        else:
    ...
    common = {
        ...
        "strict_gate_version": "exact-blob-stream-mmap-row-bound-lineage-reconciliation-quorum-official-four-gate-v7",
        "fake_data": False,
        "final_ready": False,
    }
    ...
    print(f"ATTEMPT_LINEAGE_PASSED={str(quorum_passed).lower()}")
