...
TARGET_IDS = [f"parcel_{value}" for value in range(61523, 61535)]
...
def http_bytes(url: str, timeout: int = 120) -> tuple[int, bytes]:
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "AAYS-security-public-safety-slot3/3.0",
...
def http_json(url: str, timeout: int = 120) -> tuple[int, bytes, object]:
...
def request_area_evidence(lat: float, lng: float, month: str) -> dict:
...
def main() -> int:
    ...
    try:
        ...
        police_latest.update({"http_status": status, "month": str(payload.get("date", ""))[:7], "response_sha256": hashlib.sha256(body).hexdigest()})
    except Exception as exc:
    ...
    for line, parcel_id in enumerate(TARGET_IDS, start=1):
        ...
        if not feature:
            rows.append({"line": line, "parcel_id": parcel_id, "candidate_status": "CANONICAL_FEATURE_NOT_FOUND", "accuracy_score_4": 0, "needs_manual_review": True, "security_score_percent": None})
            continue
        ...
        if coordinates and police_latest.get("month"):
            lng, lat = coordinates
            area_evidence = request_area_evidence(float(lat), float(lng), str(police_latest["month"]))
            row["area_evidence"] = area_evidence
            api_pass = (
                area_evidence.get("crime_http_status") == 200
                and area_evidence.get("crime_response_sha256")
                and area_evidence.get("outcomes_http_status") == 200
                and area_evidence.get("outcomes_response_sha256")
            )
            row["candidate_status"] = "CANONICAL_AND_OFFICIAL_APIS_VERIFIED" if api_pass else "CANONICAL_FOUND_API_FAILED"
            row["accuracy_score_4"] = 3 if api_pass else 2
        ...
        if api_pass and iod_row and existing_score is not None and coordinates:
            row.update({"candidate_status": "CANONICAL_APIS_IOD25_V2_VERIFIED", "accuracy_score_4": 4, "needs_manual_review": False})
        rows.append(row)
    ...
    sample_path = OUT_ROOT / "security_public_safety_3_sample_candidates_v3_latest.json"
