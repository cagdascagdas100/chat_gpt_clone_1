...
Write-Kv $MasterReport 'BRANCH_ACTUAL' (& git branch --show-current 2>$null)
Write-Kv $MasterReport 'DB_WRITE' 'false'
Write-Kv $MasterReport 'MIGRATION' 'false'
Write-Kv $MasterReport 'DEPLOY' 'false'
Write-Kv $MasterReport 'FAKE_DATA_CREATED' 'false'
$staticJob = Start-Job -Name static_audit -ScriptBlock {
  param($RepoRoot,$AuditReport)
  Set-Location $RepoRoot
  $app='england_map_web\app.js'; $route='terrayield_land_intelligence\app\api\routes\topography_lookup_v2.py'; $main='terrayield_land_intelligence\app\main.py'; $overlay='england_map_web\config\topography.overlay.json'
  ...
  $routeTokens=@('direct_terrarium_dem_lookup','Terrarium DEM local tiles','center_elevation_m','elevation_above_sea_level_m','region_average_elevation_m','elevation_difference_from_region_average_m','source_dataset','source_resolution_m','confidence_level','matching_method','calculation_explanation')
...
$healthJob = Start-Job -Name endpoint_health -ScriptBlock {
  ...
  try { if(Test-Path 'terrayield_land_intelligence\start_open_only_8010.ps1'){ & powershell -NoProfile -ExecutionPolicy Bypass -File 'terrayield_land_intelligence\start_open_only_8010.ps1' *> $null; Start-Sleep -Seconds 8 } } catch { Add-Content -Encoding UTF8 $HealthReport "START_ERROR=$($_.Exception.Message)" }
  ...
  foreach($u in $urls){ try { $r=Invoke-WebRequest -UseBasicParsing -TimeoutSec 20 $u; Add-Content -Encoding UTF8 $HealthReport "URL_OK=$u|true|$($r.StatusCode)"; if($u -like '*lookup*'){ $body=$r.Content; Add-Content -Encoding UTF8 $HealthReport "LOOKUP_BODY_PREFIX=$($body.Substring(0,[Math]::Min(1200,$body.Length)))" } } catch { Add-Content -Encoding UTF8 $HealthReport "URL_OK=$u|false|$($_.Exception.Message)" } }
  try { $t=Invoke-WebRequest -UseBasicParsing -TimeoutSec 20 -Method Head 'http://127.0.0.1:8010/topography/tiles/13/4102/2721.png'; Add-Content -Encoding UTF8 $HealthReport "TILE_HEAD_OK=true|$($t.StatusCode)" } catch { Add-Content -Encoding UTF8 $HealthReport "TILE_HEAD_OK=false|$($_.Exception.Message)" }
