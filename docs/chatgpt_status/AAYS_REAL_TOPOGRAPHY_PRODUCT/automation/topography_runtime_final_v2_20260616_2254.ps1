...
$app='england_map_web\app.js'; $route='terrayield_land_intelligence\app\api\routes\topography_lookup_v2.py'; $main='terrayield_land_intelligence\app\main.py'; $overlay='england_map_web\config\topography.overlay.json'; $startScript='terrayield_land_intelligence\start_open_only_8010.ps1'
...
$svcOk=$false; $lookupOk=$false; $tileOk=$false; $directOk=$false; $startExists=(Test-Path $startScript); $startAttempted=$false
...
KV $Report 'NODE_CHECK_OK' $nodeOk; KV $Report 'PY_COMPILE_OK' $pyOk; KV $Report 'FRONTEND_CONTRACT_OK' $appOk; KV $Report 'BACKEND_DIRECT_DEM_CONTRACT_OK' $routeOk; KV $Report 'TILE_CONFIG_OK' $tileCfg; KV $Report 'START_SCRIPT_EXISTS' $startExists; KV $Report 'START_ATTEMPTED' $startAttempted; KV $Report 'PROGRAM_OPENED' $svcOk; KV $Report 'LOOKUP_ENDPOINT_OK' $lookupOk; KV $Report 'TILE_ENDPOINT_OK' $tileOk; KV $Report 'DIRECT_DEM_LOOKUP_OK' $directOk; KV $Report 'SOURCE_COVERAGE' $coverage
