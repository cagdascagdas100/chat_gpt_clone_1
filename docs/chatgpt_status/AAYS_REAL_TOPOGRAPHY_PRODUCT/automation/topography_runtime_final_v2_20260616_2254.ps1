...
foreach($f in @($app,$route,$main,$overlay)){ Add-Content -Encoding UTF8 $Audit ("FILE_EXISTS=$f|"+(Test-Path $f)) }
...
$londonSource=(Test-Path 'D:\topografik_map\london\terrarium_tiles')
$svcOk=$false; $lookupOk=$false; $tileOk=$false; $directOk=$false
try{ $wc=New-Object System.Net.WebClient; $wc.DownloadString('http'+'://127.0.0.1:8010/england_map_web/') | Out-Null; $svcOk=$true }catch{}
