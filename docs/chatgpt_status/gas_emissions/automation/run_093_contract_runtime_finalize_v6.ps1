...
try{
 ...
 $used=$null;foreach($c in $candidates){if(NE $c){$used=$c;break}}
 ...
 P $data 'metadata' ([pscustomobject]@{task_id=$TaskId;page_key=$PageKey;feature_count=$fc;polygon_count=$pc;point_count=$ptc;geometry_status=$geom;source_used=$used;final_contract_version='093v6';fake_data=$false;db_write=$false;migration=$false;production_deploy=$false})
 ...
 if($a -notmatch 'AAYS_GAS_EMISSIONS_POPUP_BINDING_V093'){
  ...
  function gasEmissionsContractEscape(value) {
    return String(value ?? '').replace(/[&<>"']/g, (m) => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  }
  function buildGasEmissionsContractPanelHtml(props = {}) {
    const p = props || {};
    const g = (v, fallback = 'not provided') => gasEmissionsContractEscape(v ?? fallback);
    return '<div class="gas-emissions-contract" data-gas-emissions-bound="true">'
      + '<div><strong>Gas emissions score:</strong> ' + g(p.emission_percent ?? p.score_percent ?? p.score) + '</div>'
      + '<div><strong>Class / level:</strong> ' + g(p.emission_class ?? p.class ?? p.level) + '</div>'
      + '<div><strong>Color category:</strong> ' + g(p.color_category) + '</div>'
      + '<div><strong>Source / evidence:</strong> ' + g(p.source_evidence ?? p.source_file ?? p.source_type) + '</div>'
      + '<div><strong>Source date:</strong> ' + g(p.source_date) + '</div>'
      + '<div><strong>Confidence / accuracy:</strong> ' + g(p.confidence_scale ?? p.confidence_percent ?? p.confidence) + '</div>'
      + '<div><strong>Matching method:</strong> ' + g(p.matching_method) + '</div>'
      + '<div><strong>Calculation explanation:</strong> ' + g(p.calculation_explanation) + '</div>'
      + '<div><strong>Geometry status:</strong> ' + g(p.geometry_status ?? p.geometry_degraded_status ?? 'degraded_point_proxy') + '</div>'
      + '</div>';
  }
...
'@
  ...
  elseif($a.Contains('  function setSanitizedHtml(element, html) {')){$a=$a.Replace('  function setSanitizedHtml(element, html) {',$helper+'  function setSanitizedHtml(element, html) {')}
  elseif($a.Contains('  function emitLayerRuntimeEvent(layer, status, detail = "") {')){$a=$a.Replace('  function emitLayerRuntimeEvent(layer, status, detail = "") {',$helper+'  function emitLayerRuntimeEvent(layer, status, detail = "") {')}
  elseif($a.Contains('})();')){$a=$a.Replace('})();',$helper+'})();')}
  else{throw 'app.js safe insertion anchor missing'}
 }
 ...
 $dataOk=($fc -gt 0 -and $miss -eq 0);$frontOk=($missingTokens.Count -eq 0);$final=($dataOk -and $frontOk);$pct=93;if($dataOk){$pct=96};if($final){$pct=100};$st='DATA_CONTRACT_PENDING';if($dataOk){$st='FRONTEND_CONTRACT_PENDING'};if($final){$st='FINAL_READY'}
 $sum=[ordered]@{task_id=$TaskId;page_key=$PageKey;status=$st;completion_percent=$pct;final_ready=$final;feature_count=$fc;polygon_count=$pc;point_count=$ptc;geometry_status=$geom;data_contract=$(if($dataOk){'PASS'}else{'FAIL'});frontend_contract=$(if($frontOk){'PASS'}else{'FAIL'});missing_app_tokens=$missingTokens;missing_contract_count=$miss;source_used=$used;output=$Geo;runtime_mode='STATIC_GEOJSON_LAYER_NO_DB_WRITE';frontend_binding='safe_anchor_v6'}
 $sum|ConvertTo-Json -Depth 20|Set-Content -Encoding UTF8 $JsonOut
 $rows += 'status='+$st,'completion_percent='+$pct,'final_ready='+($final.ToString().ToLowerInvariant()),'feature_count='+$fc,'polygon_count='+$pc,'point_count='+$ptc,'geometry_status='+$geom,'data_contract='+$sum.data_contract,'frontend_contract='+$sum.frontend_contract,'expected_output='+$Geo,'json_output='+$JsonOut,'frontend_binding=safe_anchor_v6'
 ...
 git add $Geo $App $Report $StatusFile $JsonOut 2>$null;git commit -m 'terrayield 093 gas emissions v6 final contract outputs' 2>$null;git push origin $Branch 2>$null;exit 0
}catch{$rows+='status=FAILED';$rows+='completion_percent=88';$rows+='final_ready=false';$rows+='error='+$_.Exception.Message;W $Report $rows;W $StatusFile @('status=FAILED','task_id='+$TaskId,'completion_percent=88','final_ready=false','error='+$_.Exception.Message,'report='+$Report);git add $Report $StatusFile 2>$null;git commit -m 'terrayield 093 gas emissions v6 failure report' 2>$null;git push origin $Branch 2>$null;exit 0}
