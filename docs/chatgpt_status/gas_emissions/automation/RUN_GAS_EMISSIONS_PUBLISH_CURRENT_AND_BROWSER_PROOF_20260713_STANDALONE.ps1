...
$status | Add-Member -NotePropertyName status -NotePropertyValue (if($browserPassed){"OFFICIAL_VISIBLE_SAMPLE_ROWS_EXPANDED_$ExpectedRows"}else{"OFFICIAL_VISIBLE_SAMPLE_ROWS_EXPANDED_${ExpectedRows}_BROWSER_BLOCKED"}) -Force
...
$payload = [ordered]@{
  ...
  status=if($browserPassed){'PASS_STANDALONE_LIVE_PUBLISH_AND_BROWSER_PROOF'}else{'FAIL_STANDALONE_BROWSER_PROOF'}
