...
if (-not $repoRoot -or [string]$env:AAYS_PAGE_KEY -ne 'gas_emissions') {
  throw 'GAS_EMISSIONS_ASYNC_DOM_FIX_V2_WRONG_CONTEXT'
}
if ([string]$env:AAYS_TARGET_BRANCH -ne 'codex/aays-single-runner-v5-20260706') {
  throw 'GAS_EMISSIONS_ASYNC_DOM_FIX_V2_WRONG_BRANCH'
}
...
if (-not $patched.Contains($oldBlock)) {
  throw 'GAS_EMISSIONS_ASYNC_DOM_FIX_V2_SOURCE_BLOCK_NOT_FOUND'
}
$patched = $patched.Replace($oldBlock, $newBlock)

if ($patched -notmatch '--no-proxy-server') {
  throw 'GAS_EMISSIONS_ASYNC_DOM_FIX_V2_PROXY_PATCH_MISSING'
}
if ($patched -notmatch 'execute_async_script') {
  throw 'GAS_EMISSIONS_ASYNC_DOM_FIX_V2_ASYNC_PATCH_MISSING'
}
if ($patched -match 'WebDriverWait\(driver, 90\)') {
  throw 'GAS_EMISSIONS_ASYNC_DOM_FIX_V2_OLD_BLOCK_REMAINS'
}

$tmp = Join-Path ([System.IO.Path]::GetTempPath()) ('gas_async_dom_fix_v2_' + [Guid]::NewGuid().ToString('N') + '.ps1')
try {
  ...
  if ($exitCode -ne 0) {
    throw "GAS_EMISSIONS_ASYNC_DOM_FIX_V2_CHILD_FAILED: exit=$exitCode"
  }
} finally {
