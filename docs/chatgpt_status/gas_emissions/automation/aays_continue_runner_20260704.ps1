...
$SourceQueueDir = Join-Path $SourceGasRoot "queue"
EnsureDir $SourceStatusDir; EnsureDir $SourceReportDir; EnsureDir $SourceQueueDir
...
try {
  if (!(Test-Path -LiteralPath $CleanRepo)) {
    Say "Clean evidence repo not found; cloning $RepoUrl" Yellow
  ...
  try {
    ...
    if (!$preStatus) {
      $rebase = RunGit @("rebase", "origin/$Branch")
      $state.steps += [ordered]@{ step="pre_rebase"; code=$rebase.code; output=$rebase.output }
      if ($rebase.code -ne 0) { git rebase --abort | Out-Null 2>&1; throw "pre rebase failed: $($rebase.output)" }
    } else {
      $state.steps += [ordered]@{ step="pre_rebase"; code=0; output="skipped because worktree already had local changes" }
    }
    ...
    if (Test-Path -LiteralPath $bridge) {
      ...
      $state.steps += [ordered]@{ step="bridge"; code=2; output="missing bridge script: $bridge" }
    }
    ...
    if ($LASTEXITCODE -ne 0) {
      ...
      $state.steps += [ordered]@{ step="commit_state"; code=0; output="no staged diff" }
    }

    $fetch2 = RunGit @("fetch", "origin", $Branch)
    $state.steps += [ordered]@{ step="post_fetch"; code=$fetch2.code; output=$fetch2.output }
    if ($fetch2.code -ne 0) { throw "post fetch failed: $($fetch2.output)" }

    $rebase2 = RunGit @("rebase", "origin/$Branch")
    $state.steps += [ordered]@{ step="post_rebase"; code=$rebase2.code; output=$rebase2.output }
    ...
    $state.steps += [ordered]@{ step="post_push"; code=$push.code; output=$push.output }
