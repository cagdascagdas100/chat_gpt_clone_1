...
$rows=@('page_key='+$PageKey,'task_id='+$TaskId,'branch='+$Branch,'automation_path=docs/chatgpt_status/gas_emissions/automation/run_093_contract_runtime_finalize_v4.ps1','runner_mode=single_shared_runner','manual_stdout_required=false','fake_data=false','db_write=false','migration=false','production_deploy=false','started_at='+((Get-Date).ToString('o')))
try{
 ...
 P $data 'metadata' ([pscustomobject]@{task_id=$TaskId;page_key=$PageKey;feature_count=$fc;polygon_count=$pc;point_count=$ptc;geometry_status=$geom;source_used=$used;final_contract_version='093v4';fake_data=$false;db_write=$false;migration=$false;production_deploy=$false})
...
'@
 ...
 $dataOk=($fc -gt 0 -and $miss -eq 0);$frontOk=($missingTokens.Count -eq 0);$final=($dataOk -and $frontOk);$pct=90;if($dataOk){$pct=96};if($final){$pct=100};$st='DATA_CONTRACT_PENDING';if($dataOk){$st='FRONTEND_CONTRACT_PENDING'};if($final){$st='FINAL_READY'}
 ...
 git add $Geo $App $Report $StatusFile $JsonOut 2>$null;git commit -m 'terrayield 093 gas emissions v4 final contract outputs' 2>$null;git push origin $Branch 2>$null;exit 0
}catch{$rows+='status=FAILED';$rows+='completion_percent=86';$rows+='final_ready=false';$rows+='error='+$_.Exception.Message;W $Report $rows;W $StatusFile @('status=FAILED','task_id='+$TaskId,'completion_percent=86','final_ready=false','error='+$_.Exception.Message,'report='+$Report);git add $Report $StatusFile 2>$null;git commit -m 'terrayield 093 gas emissions v4 failure report' 2>$null;git push origin $Branch 2>$null;exit 0}
