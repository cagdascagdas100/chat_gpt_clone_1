(async function () {
  ...
  async function fetchJsonWithTimeout(url, options = {}) {
    ...
    if (effectiveDedupeKey && apiInFlightRequests.has(effectiveDedupeKey)) {
      try {
        ...
        return {
          ok: false,
          data: fallbackData,
          error: `${label}: ayni istek tekrar denenecek`,
        };
      }
    }
    ...
    if (effectiveDedupeKey) {
      apiInFlightRequests.set(effectiveDedupeKey, promise);
    }
    try {
  ...
  const PLANNED_ASSETS_SOURCE_ID = "planned-assets-overlay-source";
  const PLANNED_ASSETS_FILL_LAYER_ID = "planned-assets-overlay-fill";
  const PLANNED_ASSETS_LINE_LAYER_ID = "planned-assets-overlay-line";
  const PLANNED_ASSETS_POINT_LAYER_ID = "planned-assets-overlay-point";
  ...
  const EMISSIONS_CONTROL_MODE = "__gas_emissions_toggle__";
  const GAS_EMISSIONS_SOURCE_ID = "aays-gas-emissions-source";
  const GAS_EMISSIONS_FILL_LAYER_ID = "gas-emissions-fill";
  const GAS_EMISSIONS_LINE_LAYER_ID = "gas-emissions-line";
  const GAS_EMISSIONS_POINT_LAYER_ID = "gas-emissions-point";
  ...
  const GAS_EMISSIONS_FOCUS_CENTER = [-0.4562494108596019, 51.33689063164303];
  const GAS_EMISSIONS_SOURCE_BOUNDS = [[-0.5138973635381069, 51.28031801283589], [0.1505887, 51.672434]];
  const INTERNET_CONTROL_MODE = "__internet_access_toggle__";
  const TOPOGRAPHY_LOOKUP_BASE_URL = "http://127.0.0.1:8765/lookup?parcel_id=";
  const MAP_MODE_VISIBILITY_STORAGE_KEY = "aays.mapMode.visibleModes.v2";
  ...
  function renderScenarioCards(parcel, scenarios) {
    if (!Array.isArray(scenarios) || !scenarios.length) {
      return '<div class="workspace-note">Gercek maliyet verisi henuz hazir degil. Demo senaryo gosterimi kapali.</div>';
    }
    return scenarios
      .map(
        (scenario) => `
  ...
  function renderParcelCompareAction(parcel) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma ï¿½??Â°ÅŸlemi</h3>
        <div class="workspace-row workspace-action-row">
          <span>${compared ? "Bu parsel karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesinde." : "Bu parsel henÃ¼z karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesinde deï¿½??Å¸il."}</span>
          <button
            class="workspace-button"
            type="button"
            data-compare-action="${compared ? "remove" : "add"}"
            data-compare-key="${encodeURIComponent(getParcelCompareKey(parcel))}"
          >${compared ? "- Listeden Ã‡ï¿½??Â±kar" : "+ Listeye Ekle"}</button>
        </div>
      </div>
    `;
  }
  ...
  function renderComparedParcelsList(includeRemoveButton = true) {
    if (!comparedParcels.length) {
      return '<div class="workspace-empty">Haritadan bir parcel seÃ§. AÃ§ï¿½??Â±lan + butonuyla karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesine ekleyebiliriz.</div>';
    }
                  ...
                  <div class="compare-parcel-details">
                    ...
                    ${
                      item.bestScenario
                        ? `
                          <div class="workspace-section">
                            <h3 class="workspace-section-title">En ï¿½??Â°yi Senaryo</h3>
                    ...
                    ${
                      includeRemoveButton
                        ? `<button class="workspace-button" type="button" data-remove-compare="${encodeURIComponent(item.key)}">Listeden Ã‡ï¿½??Â±kar</button>`
                        : ""
                    }
                  </div>
                `
          ...
          .map(
            (entry) => `
              <div class="compare-selection-card">
                ...
                ${
                  entry.selectionId
                    ? `<button class="workspace-button" type="button" data-remove-compare-selection="${entry.selectionId}">Senaryoyu Kaldï¿½??Â±r</button>`
                    : `<button class="workspace-button" type="button" data-remove-compare="${encodeURIComponent(entry.key)}">Parseli Kaldï¿½??Â±r</button>`
                }
              </div>
            `
          )
  ...
  function renderSelectedParcelsTable(compareEntries) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">SeÃ§ilen Parseller</h3>
  ...
  function renderCompareSignalSummaryTable(compareEntries) {
      ...
      .map((entry) => {
        ...
        if (state.loading) {
          return `
            <tr>
              <td>${entry.ref}</td>
              <td colspan="7">Kaynak sinyalleri yÃ¼kleniyor...</td>
            </tr>
          `;
        }
        if (state.error) {
        ...
        if (!state.hasFetched || !state.signals) {
          return `
            <tr>
              <td>${entry.ref}</td>
              <td colspan="7">Bu parsel iÃ§in source sinyali henÃ¼z hazï¿½??Â±r deï¿½??Å¸il.</td>
            </tr>
          `;
        }
        return `
          <tr>
            ...
            <td>${filteredView.officialStatus || (filteredView.filterNotice ? "Filtre dï¿½??Â±ÅŸï¿½??Â±" : "-")}</td>
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Parsel Bazlï¿½??Â± Kaynak Sinyalleri</h3>
  ...
  function renderCompareSignalDetails(compareEntries) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Kaynak Linkleri ve Warning Detaylarï¿½??Â±</h3>
        ${getLandFilterSummaryHtml()}
        <div class="workspace-stack">
          ${parcelEntries
            .map((entry) => {
                ...
                return `
                  <details class="workspace-section workspace-details">
                    ...
                    <div class="workspace-details-body">
                      <div class="workspace-note">${state.loading ? "Kaynak sinyalleri yÃ¼kleniyor..." : "Bu parsel iÃ§in detay henÃ¼z hazï¿½??Â±r deï¿½??Å¸il."}</div>
                    </div>
                  </details>
                `;
              }
                ...
                <details class="workspace-section workspace-details">
                  ...
                  <div class="workspace-details-body">
                    <div class="workspace-stack">
                      ...
                      ${sourceRows || '<div class="workspace-note">Aktif filtreyle eÅŸleÅŸen kaynak linki bulunamadï¿½??Â±.</div>'}
                      ${warningRows || '<div class="workspace-note">Bu parsel iÃ§in ek warning yok.</div>'}
                      <div class="workspace-row"><span>Price Paid History</span><strong>${(state.history || []).length} kayï¿½??Â±t</strong></div>
                    </div>
                  </div>
                </details>
              `;
  ...
  function renderParcelFeaturesAndDistances(compareEntries) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Parsel Ã–zellikleri ve Uzaklï¿½??Â±klar</h3>
        <div class="workspace-stack">
          ${parcelEntries
            .map(
              (entry) => `
                <details class="workspace-section workspace-details">
                  ...
                  <div class="workspace-details-body">
                    <div class="metric-grid">
                      ...
                      <div class="metric-card"><div class="metric-card-label">Eï¿½??Å¸im</div><div class="metric-card-value">%${formatNumber(entry.slopePct, 1)}</div></div>
                      <div class="metric-card"><div class="metric-card-label">Arsa Ã§evresi</div><div class="metric-card-value">${formatDistanceMeters(entry.perimeterM)}</div></div>
                      <div class="metric-card"><div class="metric-card-label">BoÅŸ arazi</div><div class="metric-card-value">%${formatNumber(entry.emptyLandSignal * 100, 0)}</div></div>
                    </div>
  ...
  function renderScenarioDetailsByBuildingType(compareEntries) {
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Senaryo Detaylarï¿½??Â± (Yapï¿½??Â± TÃ¼rÃ¼ne GÃ¶re)</h3>
        <div class="workspace-stack">
          ${compareEntries
            .map((entry) => {
                    ...
                    <div class="settings-table-wrap">
                      <table class="settings-table">
                        <thead>
                          <tr>
                            <th>Yapï¿½??Â± TÃ¼rÃ¼</th>
                            <th>Birim Sayï¿½??Â±sï¿½??Â±</th>
                            <th>Birim Alanï¿½??Â± (mÃ‚Â²)</th>
                            <th>Kullanï¿½??Â±m (mÃ‚Â²)</th>
                            <th>Birim Maliyet</th>
                            <th>Birim Satï¿½??Â±ÅŸ Deï¿½??Å¸eri</th>
                            <th>Toplam Deï¿½??Å¸er</th>
                          </tr>
                        </thead>
                        <tbody>${rows}</tbody>
                      </table>
  ...
  function renderScenarioComparisonCharts(compareEntries) {
    return `
      ${renderScenarioComparisonChart(compareEntries, "profit", "KÃƒÂ¢r (TRY)", (value) => formatNumber(value, 0), "SeÃ§ili senaryolarï¿½??Â±n kÃƒÂ¢r karÅŸï¿½??Â±laÅŸtï¿½??Â±rmasï¿½??Â±")}
      ${renderScenarioComparisonChart(compareEntries, "roiPct", "ROI", (value) => formatNumber(value, 4), "SeÃ§ili senaryolarï¿½??Â±n ROI karÅŸï¿½??Â±laÅŸtï¿½??Â±rmasï¿½??Â±")}
      ${renderScenarioComparisonChart(compareEntries, "irrPct", "IRR (Annual)", (value) => formatNumber(value, 4), "SeÃ§ili senaryolarï¿½??Â±n yï¿½??Â±llï¿½??Â±k IRR karÅŸï¿½??Â±laÅŸtï¿½??Â±rmasï¿½??Â±")}
    `;
  }
  ...
  function renderParcelAnalysisSummaryTable(entries) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma Listesindeki Parseller</h3>
        ${getLandFilterSummaryHtml()}
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
        ...
        return `
          <div class="workspace-section">
            ...
            <div class="metric-grid">
              ...
              <div class="metric-card"><div class="metric-card-label">Aktif BÃ¶lge</div><div class="metric-card-value">${entry.regionLabel}</div></div>
              <div class="metric-card"><div class="metric-card-label">Alan</div><div class="metric-card-value">${formatNumber(entry.areaM2, 0)} mÃ‚Â²</div></div>
            ...
            <div class="workspace-section">
              <h3 class="workspace-section-title">Parsel Ã–zellikleri</h3>
              <div class="workspace-stack">${detailRows || '<div class="workspace-note">Uygun parcel Ã¶zelliï¿½??Å¸i bulunamadï¿½??Â±.</div>'}</div>
            </div>
  ...
  function renderNearbyUnits(nearbyUnits) {
    return nearbyUnits.length
      ? nearbyUnits
          ...
          .join("")
      : '<div class="workspace-note">Aktif bÃ¶lgede uygun POI/birim detayï¿½??Â± bulunamadï¿½??Â±.</div>';
  }

  function renderUnitsOverviewSection(primaryUnits, nearbyUnits) {
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">POI / Birim Uzaklï¿½??Â±klarï¿½??Â±</h3>
  ...
  const LAND_SOURCE_LABELS = {
    ...
    market_listing_adapter: "Lisanslï¿½??Â± Market Feed",
  ...
  function getSalePriceTruth(record, sources = []) {
    ...
    if (!record || !hasSalePrice(record)) {
      return {
        label: "Fiyat yok",
        detail: "Kaynakta ilan fiyatï¿½??Â± bulunamadï¿½??Â±.",
        tone: "neutral",
        isReal: null,
      };
    }
    ...
    if (tier === "licensed") {
      return {
        label: "GerÃ§ek ilan fiyatï¿½??Â±",
        detail: providerName || "Portal listing kaynaï¿½??Å¸ï¿½??Â±",
        tone: "listing",
        isReal: true,
      };
    }
  ...
  function buildSupabaseSourceDetails(records) {
    return (records || []).map((record) => ({
      ...
      detail: {
        ...
        external_label: record.source_url ? "Satï¿½??Â±ÅŸ ilanï¿½??Â±na git" : null,
  ...
  function renderParcelSaleRecords(records) {
    const rows = (records || [])
      ...
      .join("");
    if (!rows) {
      return '<div class="workspace-note">Bu parcel iÃ§in filtrelenmiÅŸ satï¿½??Â±ÅŸ kaydï¿½??Â± yok.</div>';
    }
    return `<table class="data-table"><thead><tr><th>Satï¿½??Â±ÅŸ Kaydï¿½??Â±</th><th>Kaynak</th><th>Durum</th><th>Ilan Alani</th><th>Fiyat</th><th>Fiyat TÃ¼rÃ¼</th><th>Planning</th><th>Link</th><th>Updated</th></tr></thead><tbody>${rows}</tbody></table>`;
  }
  ...
  function buildFilteredLandIntelligenceView(state) {
    ...
    const filterNotice = hasActiveLandFilters() && !hasMatches ? "Aktif filtrelerle eÅŸleÅŸen kaynak sinyali yok." : null;
    return {
  ...
  function renderScenarioScoreCards(parcelDetail, parcelLike) {
    ...
    if (!entries.length) {
      return '<div class="workspace-note">HenÃ¼z scenario scoring hesaplanmadi.</div>';
    }
    return `
      <div class="scenario-score-grid">
        ${entries
  ...
  function renderFacilityContextSection(parcelLike, parcelDetail) {
    ...
    if (!context) {
      return `
        <div class="workspace-section">
          <h3 class="workspace-section-title">Yakï¿½??Â±n Cevre Profili</h3>
          <div class="workspace-note">Buildings & facilities context henÃ¼z hesaplanmadi.</div>
        </div>
      `;
    }
    const nearestRows = [
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">YakÄ±n Cevre Profili</h3>
        <div class="metric-grid">
  ...
  function getPlannedAssetCenterCoordinates(asset) {
    const lat = Number(asset?.centroid_lat ?? asset?.centroidLat);
    const lon = Number(asset?.centroid_lon ?? asset?.centroidLon);
    if (Number.isFinite(lat) && Number.isFinite(lon)) {
      return { lat, lon };
    }
    const geometryCenter = geometryCenterLngLat(asset?.geometry);
    if (geometryCenter && Number.isFinite(geometryCenter.lat) && Number.isFinite(geometryCenter.lng)) {
      return { lat: geometryCenter.lat, lon: geometryCenter.lng };
    }
    return null;
  }
  ...
  function renderFutureDevelopmentSection(parcelDetail, futureGrowthScore, plannedAssets) {
    ...
    const rankedAssets = assets
      ...
      .slice(0, 6);
    const nearestNavigableAsset = rankedAssets
      .map((item) => ({ item, center: getPlannedAssetCenterCoordinates(item) }))
      .filter((entry) => entry.center)
      .sort((left, right) => {
        const leftDistance = Number.isFinite(Number(left.item?.distance_m)) ? Number(left.item.distance_m) : Number.POSITIVE_INFINITY;
        const rightDistance = Number.isFinite(Number(right.item?.distance_m)) ? Number(right.item.distance_m) : Number.POSITIVE_INFINITY;
        return leftDistance - rightDistance;
      })[0] || null;
    const nearestPlannedActionHtml = nearestNavigableAsset
      ? `
        <div class="workspace-row workspace-action-row">
          <button
            class="workspace-button"
            type="button"
            data-nearest-planned-go="1"
            data-nearest-planned-lat="${nearestNavigableAsset.center.lat}"
            data-nearest-planned-lon="${nearestNavigableAsset.center.lon}"
            data-nearest-planned-title="${escapeHtml(nearestNavigableAsset.item?.title || "Planned Asset")}"
          >En YakÄ±n Planlanan YapÄ±ya Git</button>
          <span>${Number.isFinite(Number(nearestNavigableAsset.item?.distance_m)) ? formatDistanceMeters(nearestNavigableAsset.item.distance_m) : "-"}</span>
        </div>
      `
      : '<div class="workspace-note">YakÄ±n planlanan yapÄ± iÃ§in koordinat bilgisi yok.</div>';
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Future Development & Infrastructure</h3>
        <div class="workspace-note">Bu bolum yatirim tavsiyesi degil; resmi kaynaklara dayali evidence-backed forward-looking sinyal ozetidir.</div>
        <div class="metric-grid">
        ...
        <details class="workspace-section workspace-details" open>
          <summary class="workspace-details-summary">YakÄ±ndaki Planlanan Gelismeler</summary>
          <div class="workspace-details-body">
            ${nearestPlannedActionHtml}
            ${
              rankedAssets.length
                ? `<table class="data-table"><thead><tr><th>Proje</th><th>Tip</th><th>Durum</th><th>Mesafe</th><th>Delivery</th><th>Value Impact</th><th>Source Confidence</th><th>Timeline</th><th>Evidence</th><th>Kaynak</th></tr></thead><tbody>${rankedAssets
                    ...
                    `)
                    .join("")}</tbody></table>`
                : '<div class="workspace-note">YakÄ±nda planlanan proje kaydi bulunamadi.</div>'
            }
          </div>
        </details>
        <details class="workspace-section workspace-details" open>
  ...
  function renderLandIntelligenceSection(parcelLike) {
    ...
    const historyNoDataDetails = `
      <details class="workspace-section workspace-details">
        ...
        <div class="workspace-details-body">
          <div class="workspace-note">Gecmis SatÄ±ÅŸ: Veri yok.</div>
        </div>
      </details>
    `;
    const historyLoadingDetails = `
      <details class="workspace-section workspace-details">
        <summary class="workspace-details-summary">Price Paid History</summary>
        <div class="workspace-details-body">
          <div class="workspace-note">Gecmis SatÄ±ÅŸ: Yukleniyor...</div>
        </div>
      </details>
    `;
    ...
    const actionableSaleLink = actionableSaleRecord ? resolveSaleRecordLink(actionableSaleRecord, filteredView.filteredSources) : { url: null, label: "Satï¿½??Â±ÅŸ adresine git" };
    ...
    return `
      <div class="workspace-section">
        ...
        <div class="metric-grid">
          <div class="metric-card"><div class="metric-card-label">Supabase Kaydï¿½??Â±</div><div class="metric-card-value">${filteredView.managedSources.length ? 'Var' : 'Yok'}</div></div>
        ...
        <details class="workspace-section workspace-details" open>
          <summary class="workspace-details-summary">Parcel ï¿½??Â°Ã§ine Yazï¿½??Â±lan Satï¿½??Â±ÅŸ Kayï¿½??Â±tlarï¿½??Â±</summary>
          <div class="workspace-details-body">
            ${renderParcelSaleRecords(filteredSaleRecords)}
          </div>
        </details>
        <details class="workspace-section workspace-details" open>
          <summary class="workspace-details-summary">FiltrelenmiÅŸ Kaynak Ã–zeti</summary>
          <div class="workspace-details-body workspace-stack">${summaryRows || '<div class="workspace-note">Aktif filtreyle eÅŸleÅŸen kaynak Ã¶zeti yok.</div>'}</div>
        </details>
        <details class="workspace-section workspace-details" open>
          <summary class="workspace-details-summary">FiltrelenmiÅŸ Source Links</summary>
          <div class="workspace-details-body workspace-stack">${sourceLinks || '<div class="workspace-note">Aktif filtreyle eÅŸleÅŸen kaynak linki yok.</div>'}</div>
        </details>
        <details class="workspace-section workspace-details">
          <summary class="workspace-details-summary">Price Paid History</summary>
          <div class="workspace-details-body">
            ${historyRows ? `<table class="data-table"><thead><tr><th>Sale Date</th><th>Year</th><th>Price Paid</th><th>Price / m2</th><th>Postcode</th><th>Location</th><th>Yapi Sinifi</th><th>Yapi Tipi</th><th>Tenure</th><th>Area</th></tr></thead><tbody>${historyRows}</tbody></table>` : '<div class="workspace-note">Gecmis SatÄ±ÅŸ: Veri yok.</div>'}
          </div>
        </details>
      </div>
    `;
  ...
  function renderSalesOnlyWorkspace(parcelLike) {
    if (!landIntelligenceApiBaseUrl) {
      return `
        <div class="workspace-section">
          <h2 class="workspace-page-title">Gecmis SatÄ±ÅŸ Modu</h2>
          <div class="workspace-note">API base URL tanimli degil. SatÄ±ÅŸ bilgisi yuklenemedi.</div>
        </div>
      `;
    }
    ...
    return `
      <div class="workspace-section">
        ...
        <div class="metric-grid">
          ...
          <div class="metric-card"><div class="metric-card-label">Son SatÄ±ÅŸ Tarihi</div><div class="metric-card-value">${latest?.sale_date || "-"}</div></div>
          <div class="metric-card"><div class="metric-card-label">Son SatÄ±ÅŸ Fiyati</div><div class="metric-card-value">${latest?.price_paid ? formatSalePrice(Number(latest.price_paid)) : "-"}</div></div>
        ...
        <details class="workspace-section workspace-details" open>
          <summary class="workspace-details-summary">SatÄ±ÅŸ Kayitlari</summary>
          <div class="workspace-details-body">
            ${historyRows
  ...
  function renderParcelDetailsAccordion(parcelLike) {
    return `
      <details class="workspace-section workspace-details">
        <summary class="workspace-details-summary">Parsel Ayrï¿½??Â±ntï¿½??Â±larï¿½??Â±</summary>
        <div class="workspace-details-body">
          <div class="metric-grid">
            ...
            <div class="metric-card"><div class="metric-card-label">Eï¿½??Å¸im</div><div class="metric-card-value">${formatNumber(parcelLike.slopePct, 1)}%</div></div>
            <div class="metric-card"><div class="metric-card-label">Arsa Ã§evresi</div><div class="metric-card-value">${formatDistanceMeters(parcelLike.perimeterM)}</div></div>
            <div class="metric-card"><div class="metric-card-label">BoÅŸ arazi</div><div class="metric-card-value">%${formatNumber(parcelLike.emptyLandSignal * 100, 0)}</div></div>
            <div class="metric-card"><div class="metric-card-label">Aktif BÃ¶lge</div><div class="metric-card-value">${parcelLike.regionLabel}</div></div>
          </div>
        </div>
      </details>
    `;
  ...
  const PARCEL_ANALYSIS_FACTORS = [
    ...
    { factor: "Metro (UlaÅŸï¿½??Â±m)", unitKey: "metro" },
    { factor: "OtobÃ¼s Duraï¿½??Å¸ï¿½??Â±", unitKey: "otobus" },
    { factor: "AVM", unitKey: "avm" },
    { factor: "Deniz ï¿½??Â°skelesi / Su Kenarï¿½??Â±", unitKey: "iskele" },
    { factor: "Nehir (Su Kaynaklarï¿½??Â±)", unitKey: "nehir" },
    { factor: "ï¿½??Â°mar Planï¿½??Â± & Hukuki Durum", unitKey: null },
    { factor: "Topografya & Zemin Ã–zellikleri", unitKey: null },
  ...
  function renderParcelAnalysisDistanceTable(parcel, factorRows) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Alternatif Yapï¿½??Â± TÃ¼rleri (KarÅŸï¿½??Â±laÅŸtï¿½??Â±rmalï¿½??Â±)</h3>
        <div class="workspace-note">Parsel ${parcel.ref}</div>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
              <tr>
                <th>FaktÃ¶r / Yapï¿½??Â± TÃ¼rÃ¼</th>
                <th>Mesafe</th>
                <th>Skor (1-10)</th>
              </tr>
            </thead>
  ...
  function renderBuildingTypeCoefficientTable() {
    ...
    return `
      <div class="workspace-section">
        <div class="workspace-note">Kurulum ekranï¿½??Â±ndaki katsayï¿½??Â± tablosuna gÃ¶re yapï¿½??Â± tÃ¼rÃ¼ faktÃ¶r daï¿½??Å¸ï¿½??Â±lï¿½??Â±mï¿½??Â±</div>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
              <tr>
  ...
  function renderGeometricAverageDistributionTable(parcel, factorRows, options = {}) {
    ...
    return `
      <div class="workspace-section">
        ${showTitle ? '<h3 class="workspace-section-title">Yapï¿½??Â± TÃ¼rÃ¼ FaktÃ¶r Daï¿½??Å¸ï¿½??Â±lï¿½??Â±mï¿½??Â± (Geometrik Ortalama)</h3>' : ""}
        <div class="workspace-note">Parsel ${parcel.ref}</div>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
              <tr>
                ...
                ${header}
                <th>Toplam Deï¿½??Å¸er</th>
                <th>Toplam Maliyet Deï¿½??Å¸eri</th>
              </tr>
            </thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
        ${footerNote ? '<div class="analysis-footnote">MVP Notu: Bu analiz, mesafe skorlarï¿½??Â± ile kurulum katsayï¿½??Â±larï¿½??Â±nï¿½??Â± geometrik ortalama mantï¿½??Â±ï¿½??Å¸ï¿½??Â±nda birleÅŸtirir.</div>' : ""}
      </div>
    `;
  }
  ...
  function renderRecommendationGeometricTables(parcel, insights) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Yapï¿½??Â± TÃ¼rÃ¼ FaktÃ¶r Daï¿½??Å¸ï¿½??Â±lï¿½??Â±mï¿½??Â± (Geometrik Ortalama)</h3>
        ${tableEntries
          .map(
            (entry) => `
          ...
          .join("")}
        <div class="analysis-footnote">MVP notu: Bu oneriler real-only endpoint verisine dayanir. Veri yoksa no-data durumu korunur; otomatik demo/fake uretim yapilmaz.</div>
      </div>
    `;
  }
  ...
  function calculateScenarioCapacity(parcel) {
    ...
    return planningSettings.capacityMethod === "Satï¿½??Â±labilir Alan"
  ...
  function buildScenarioSelectionRows(parcel, allowedTypes, minUtilization) {
    if (AAYS_REAL_DATA_ONLY) {
      const allowed = new Set((allowedTypes || []).map((item) => String(item || "").trim().toLowerCase()));
      return generateScenarioModels(parcel)
        .filter((scenario) => !allowed.size || allowed.has(String(scenario?.name || "").trim().toLowerCase()))
        .filter((scenario) => Number(scenario?.utilization || 0) >= minUtilization)
        .sort((a, b) => Number(b?.profit || 0) - Number(a?.profit || 0) || Number(b?.roiPct || 0) - Number(a?.roiPct || 0));
    }
  ...
  function buildScenarioSelectionEntryFromRealModel(sourceEntry, scenario, selectionId = null) {
    if (!sourceEntry || !scenario) return null;
    const parcelVariant = getPlanningParcelVariant(sourceEntry);
    const factorRows = buildParcelAnalysisFactorRowsFromUnits(sourceEntry.primaryUnits || [], sourceEntry.nearbyUnits || []);
    const adjustedSaleMap = computeAdjustedSaleValueMap(factorRows);
    const planningTypeKey = String(scenario?.planningTypeKey || getScenarioPlanningTypeKey(scenario?.name || "")).trim().toLowerCase();
    const typeConfig = scenario?.typeConfig || getBuildingTypeConfigByKey(planningTypeKey);
    const usedM2 = Number(scenario?.usedM2 || scenario?.sellableAreaM2 || 0);
    const capacityM2 = Number(scenario?.capacityM2 || parcelVariant?.areaM2 || usedM2 || 1);
    const normalizedScenario = {
      name: scenario?.name || typeConfig?.scenarioName || "Scenario",
      shortName: typeConfig?.label || scenario?.name || "Scenario",
      buildCost: Number(scenario?.buildCost || 0),
      revenue: Number(scenario?.revenue || 0),
      otherCosts: Number(scenario?.otherCosts || 0),
      totalCost: Number(scenario?.totalCost || 0),
      profit: Number(scenario?.profit || 0),
      roiPct: Number(scenario?.roiPct || 0),
      irrPct: Number(scenario?.irrPct || 0),
      usedM2,
      capacityM2: capacityM2 > 0 ? capacityM2 : 1,
      utilization: Number(scenario?.utilization || (capacityM2 > 0 ? usedM2 / capacityM2 : 0)),
      units: Number(scenario?.units || 1),
      typeConfig,
      confidenceScore: Number(scenario?.confidenceScore || 0),
      confidenceBand: String(scenario?.confidenceBand || "-"),
      components: [
        {
          typeKey: typeConfig?.key || planningTypeKey || "karma",
          label: typeConfig?.label || scenario?.name || "Scenario",
          surfaceArea: usedM2,
          floors: 1,
          usedM2,
          equivalentUnits: Number(scenario?.units || 1),
          typeConfig: typeConfig || getBuildingTypeConfigByKey("karma"),
          adjustedUnitSaleValue: Number(scenario?.salePricePerM2 || typeConfig?.unitSaleValue || 0),
          buildCost: Number(scenario?.buildCost || 0),
          revenue: Number(scenario?.revenue || 0),
        },
      ],
  ...
  function addScenarioSelectionToCompare(sourceEntry, draftEntries, selectedAutoScenario = null) {
    const entry = AAYS_REAL_DATA_ONLY
      ? buildScenarioSelectionEntryFromRealModel(sourceEntry, selectedAutoScenario)
      : buildScenarioSelectionEntryFromDraft(sourceEntry, draftEntries);
  ...
  function renderScenarioAutoTable(rows) {
    if (!rows.length) {
      return '<div class="workspace-empty">Bu ayarlarla Ã¼retilebilen senaryo bulunamadï¿½??Â±.</div>';
    }
    const header = `
      <div class="scenario-table-row scenario-table-head">
        <span>Senaryo</span>
        <span>Kullanï¿½??Â±m</span>
        <span>Doluluk</span>
  ...
  function renderScenarioSelectionSummaryCards(selectionEntry) {
    ...
    const warning = selectionEntry.overCapacity
      ? `<div class="settings-warning">Toplam kullanim ${formatNumber(selectionEntry.scenario.usedM2, 0)} mÃ‚Â², parcel kapasitesi olan ${formatNumber(
          ...
          0
        )} mÃ‚Â² degerini asiyor. KarÅŸÄ±laÅŸtÄ±rmaya eklemeden once kullanim dusurulmeli.</div>`
      : "";
    return `
      ${warning}
  ...
  function renderPlanningSettingsPanel(candidates) {
    ...
    const parcelRows = candidates.length
      ? candidates
          ...
          .join("")
      : '<tr><td colspan="4">HenÃ¼z seÃ§ilebilir parcel bulunamadï¿½??Â±.</td></tr>';
    ...
    return `
      <div class="workspace-section settings-hero">
        <div class="workspace-note settings-warning">Uygulama, aÅŸaï¿½??Å¸ï¿½??Â±daki varsayï¿½??Â±mlar ile Ã§alï¿½??Â±ÅŸï¿½??Â±r. ï¿½??Â°stersen buradan deï¿½??Å¸iÅŸtirip kaydedebilirsin.</div>
        <h3 class="workspace-section-title settings-title">Uygulama aÃ§ï¿½??Â±lmadan Ã¶nce temel varsayï¿½??Â±mlarï¿½??Â± girin ve onaylayï¿½??Â±n.</h3>
        <div class="workspace-note">Devam ederseniz, sistem varsayï¿½??Â±lan deï¿½??Å¸erlerle baÅŸlar. ï¿½??Â°sterseniz burada deï¿½??Å¸iÅŸtirebilirsiniz.</div>
        <div class="settings-grid">
          <div>
            <label class="field-label" for="planningUsableRatio">Kullanï¿½??Â±labilir arsa Ã§arpanï¿½??Â±</label>
            <input id="planningUsableRatio" type="number" min="0.1" max="1" step="0.05" value="${planningSettings.usableRatio}" />
          </div>
          <div>
            <label class="field-label" for="planningCapacityMethod">Kapasite hesabï¿½??Â± yÃ¶ntemi</label>
            <select id="planningCapacityMethod">
              <option value="Taban Alanï¿½??Â±" ${planningSettings.capacityMethod === "Taban Alanï¿½??Â±" ? "selected" : ""}>Taban Alanï¿½??Â±</option>
              <option value="Satï¿½??Â±labilir Alan" ${planningSettings.capacityMethod === "Satï¿½??Â±labilir Alan" ? "selected" : ""}>Satï¿½??Â±labilir Alan</option>
            </select>
          </div>
          <div>
          ...
          <div>
            <label class="field-label" for="planningProjectMonths">Proje sÃ¼resi (ay) Ã¢â‚¬â€ IRR iÃ§in</label>
            <input id="planningProjectMonths" type="number" min="6" max="60" step="1" value="${planningSettings.projectMonths}" />
          </div>
        </div>
      </div>
      <div class="workspace-section">
        <h3 class="workspace-section-title">Parsel deï¿½??Å¸erleri (alan ve arsa maliyeti)</h3>
        <div class="workspace-note">Web sÃ¼rÃ¼mÃ¼nde sadece eriÅŸilebilir seÃ§ili parselleri dÃ¼zenliyoruz.</div>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
              <tr><th>Parsel ID</th><th>BÃ¶lge</th><th>Arsa alanï¿½??Â± (mÃ‚Â²)</th><th>Tahmini arsa maliyeti (TRY)</th></tr>
            </thead>
      ...
      <div class="workspace-section">
        <h3 class="workspace-section-title">Yapï¿½??Â± tÃ¼rleri (birim alan / maliyet / satï¿½??Â±ÅŸ deï¿½??Å¸eri)</h3>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
              <tr><th>key</th><th>Yapï¿½??Â± tÃ¼rÃ¼</th><th>Birim alanï¿½??Â± (mÃ‚Â²)</th><th>Min (mÃ‚Â²)</th><th>Max (mÃ‚Â²)</th><th>Birim maliyet (TRY)</th><th>Birim satï¿½??Â±ÅŸ deï¿½??Å¸eri (TRY)</th></tr>
            </thead>
      ...
      <div class="workspace-section">
        <h3 class="workspace-section-title">Mesafe ve Yapï¿½??Â± TÃ¼rÃ¼ Deï¿½??Å¸er Analiz Tablosu</h3>
        <div class="workspace-note">AÅŸaï¿½??Å¸ï¿½??Â±daki katsayï¿½??Â±larï¿½??Â± deï¿½??Å¸iÅŸtirmezseniz, sistem varsayï¿½??Â±lan deï¿½??Å¸erlerle devam eder.</div>
        <div class="settings-table-wrap">
          <table class="settings-table">
      ...
      <div class="workspace-section">
        <h3 class="workspace-section-title">Ortalama katsayï¿½??Â±lar (bilgilendirme)</h3>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead><tr><th></th><th>Yapï¿½??Â± TÃ¼rÃ¼</th><th>Ortalama Katsayï¿½??Â±</th></tr></thead>
            <tbody>${averageTableRows}</tbody>
          </table>
        </div>
        <div class="settings-actions">
          <button class="workspace-button" type="button" data-settings-reset="true">Varsayï¿½??Â±lanlara DÃ¶n</button>
  ...
  function renderWorkspace() {
    ...
    const scenarioScreens = ["recommendation", "scenario_select", "compare", "scenario_compare"];
    const shouldLoadScenarioModels = scenarioScreens.includes(activeWorkspaceScreen);
    const scenarios = shouldLoadScenarioModels ? generateScenarioModels(parcel) : [];
    const best = shouldLoadScenarioModels ? bestScenario(parcel) : null;
    const insights = buildParcelInsights(parcel);
    ...
    if (!parcel && !["portfolio", "parcel", "compare", "contact", "scenario_select"].includes(activeWorkspaceScreen)) {
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-empty">Bu paneli doldurmak icin haritadan bir parcel secelim. Sonra ayni ekranda Portfoy, Oneri, Senaryo ve Karşılaştırma akislarini web uygulamada kullanabiliriz.</div>
        </div>
      `);
      return;
    }
    ...
    if (activeWorkspaceScreen === "portfolio") {
        ...
        <div class="workspace-panel">
          ${
            parcel
              ? `
                <div class="metric-grid">
                  <div class="metric-card"><div class="metric-card-label">SeÃ§ili Parcel</div><div class="metric-card-value">${parcel.ref}</div></div>
                  <div class="metric-card"><div class="metric-card-label">Aktif BÃ¶lge</div><div class="metric-card-value">${parcel.regionLabel}</div></div>
              ...
              `
              : `<div class="workspace-note">Detay kartlarini doldurmak icin haritadan bir parcel sec. KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesi ise aÅŸaï¿½??Å¸ï¿½??Â±da gÃ¶rÃ¼nmeye devam eder.</div>`
          }
          <div class="workspace-section">
            <h3 class="workspace-section-title">KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma Listesine Eklenen Parseller</h3>
            <div class="workspace-note">Haritadan parcel seÃ§ince popup ve saï¿½??Å¸ panel Ã¼zerinde + / - aksiyonu gÃ¶rÃ¼nÃ¼r. KarÅŸï¿½??Â±laÅŸtï¿½??Â±rmadaki parseller koyu gÃ¶rÃ¼nÃ¼r.</div>
      ...
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <h2 class="workspace-page-title">Ã–neri SonuÃ§larï¿½??Â± Ã¢â‚¬â€ Parsel Analizi</h2>
            ${
              analysisEntries.length
                ? `<div class="workspace-note">Bu ekran artï¿½??Â±k karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesindeki gÃ¼ncel parselleri birlikte gÃ¶sterir. Listeden Ã§ï¿½??Â±karï¿½??Â±lan parseller burada gÃ¶rÃ¼nmez.</div>`
                : '<div class="workspace-note">Parsel Analizi ekranï¿½??Â±nï¿½??Â± doldurmak iÃ§in Ã¶nce haritadan parselleri karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesine ekleyelim.</div>'
            }
          </div>
          ${
            analysisEntries.length
              ...
              `
              : '<div class="workspace-empty">KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesi boÅŸ. Haritadan bir parsele tï¿½??Â±klayï¿½??Â±p + ile ekledikten sonra bu ekran Ã§oklu karÅŸï¿½??Â±laÅŸtï¿½??Â±rmalï¿½??Â± dolacaktï¿½??Â±r.</div>'
          }
        </div>
      `);
      ...
      const financeRows = best
        ? `
          <div class="workspace-stack">
            ...
            <div class="workspace-row"><span>Satï¿½??Â±ÅŸ Geliri</span><strong>${formatMoney(best.revenue)}</strong></div>
            <div class="workspace-row"><span>Tahmini Arsa Maliyeti</span><strong>${formatMoney(parcel.landCost)}</strong></div>
            <div class="workspace-row"><span>ï¿½??Â°nÅŸaat KÃƒÂ¢rlï¿½??Â±lï¿½??Â±ï¿½??Å¸ï¿½??Â±</span><strong>${formatMoney(best.profit)}</strong></div>
          </div>
        `
        : '<div class="workspace-note">Finansal Ã¶zet Ã¼retilemedi.</div>';
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <h2 class="workspace-page-title">Ã–neri Motoru Ã¢â‚¬â€ Ã–neri Ã–zeti</h2>
            <div class="workspace-stack">
          ...
          <div class="workspace-section">
            <h3 class="workspace-section-title">Ã–nerilen Yapï¿½??Â± TÃ¼rÃ¼: ${best ? best.name : "-"}</h3>
            <div class="workspace-note"><strong>Model GÃ¼ven DÃ¼zeyi:</strong> ${formatNumber(scoreCard.confidence, 2)}</div>
          </div>
          <div class="workspace-section">
            <h3 class="workspace-section-title">Ã‡ok Kriterli Deï¿½??Å¸erlendirme Ã–zeti</h3>
            <div class="workspace-stack">
              <div class="workspace-note">ï¿½??Â°mar uygunluï¿½??Å¸u (stub) ve pazar sinyali birlikte ele alï¿½??Â±nï¿½??Â±r.</div>
              <div class="workspace-note">Konum/eriÅŸim, piyasa ve arsa Ã¶zellikleri ayrï¿½??Â± skorlanï¿½??Â±r.</div>
              <div class="workspace-note">Aï¿½??Å¸ï¿½??Â±rlï¿½??Â±klï¿½??Â± skor ile alternatifler kï¿½??Â±yaslanï¿½??Â±r.</div>
            </div>
          </div>
          <div class="workspace-section">
            <h3 class="workspace-section-title">Ã‡ok Kriterli Skor Kartï¿½??Â±</h3>
            ${renderScoreBar("Konum", scoreCard.location)}
            ${renderScoreBar("Piyasa", scoreCard.market)}
            ${renderScoreBar("Arsa Ã–zellikleri", scoreCard.land)}
            <div class="metric-grid">
          ...
          <div class="workspace-section">
            ...
            <div class="workspace-stack">
              ...
              <div class="workspace-row"><span>Eï¿½??Å¸im</span><strong>%${formatNumber(parcel.slopePct, 1)}</strong></div>
              <div class="workspace-row"><span>Ã‡evre Bilgisi</span><strong>Arsa Ã§evresi ${formatDistanceMeters(parcel.perimeterM)}</strong></div>
              <div class="workspace-row"><span>BoÅŸ arazi sinyali</span><strong>%${formatNumber(parcel.emptyLandSignal * 100, 0)}</strong></div>
            </div>
          </div>
          ...
          ${renderLandIntelligenceSection(parcel)}
          <div class="workspace-section">
            <h3 class="workspace-section-title">Finansal Ã–zet</h3>
            ${financeRows}
          </div>
          <div class="workspace-section">
            <h3 class="workspace-section-title">Diï¿½??Å¸er Senaryolar</h3>
      ...
      const manualSelectionPreview = scenarioSourceEntry
        ...
        : null;
      const realSelectionPreview = (AAYS_REAL_DATA_ONLY && scenarioSourceEntry && selectedScenario)
        ? buildScenarioSelectionEntryFromRealModel(scenarioSourceEntry, selectedScenario, "preview-selection")
        : null;
      const selectionPreview = AAYS_REAL_DATA_ONLY ? realSelectionPreview : manualSelectionPreview;
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <div class="workspace-row workspace-action-row">
              <h2 class="workspace-page-title">Parsel ve Senaryo SeÃ§imi (Parsel Bazlï¿½??Â±)</h2>
              <button class="workspace-button" type="button" data-toggle-scenario-settings="true">${scenarioSelectState.showSettings ? "Ayarlarï¿½??Â± Gizle" : "Varsayï¿½??Â±mlarï¿½??Â± DÃ¼zenle"}</button>
            </div>
          </div>
          ${scenarioSelectState.showSettings ? renderPlanningSettingsPanel(candidates) : ""}
          <div class="workspace-section">
            <h3 class="workspace-section-title">Parsel SeÃ§imi</h3>
            ${
              candidates.length
                ? `
                  <label class="field-label" for="scenarioSelectParcel">Parsel seÃ§ (KarÅŸï¿½??Â±laÅŸtï¿½??Â±rmaya eklenenler)</label>
                ...
                `
                : '<div class="workspace-note">KarÅŸï¿½??Â±laÅŸtï¿½??Â±rmaya eklenmiÅŸ parcel yok. Haritadan bir parcel seÃ§ip + ile ekleyebilir ya da mevcut seÃ§ili parcel Ã¼zerinden ilerleyebilirsin.</div>'
            }
            <label class="field-label" for="scenarioSelectManual">Parsel ID ile seÃ§</label>
            ...
            ${
              selectedCandidate
                ...
                : '<div class="workspace-note">HenÃ¼z kullanï¿½??Â±labilir bir parcel bulunamadï¿½??Â±.</div>'
            }
          </div>
          <div class="workspace-section">
            <h3 class="workspace-section-title">Otomatik Senaryo Ãœretimi</h3>
            <div class="metric-grid">
              <div class="metric-card">
                <div class="metric-card-label">Senaryo sayï¿½??Â±sï¿½??Â± (otomatik)</div>
            ...
            <div class="workspace-section-title">Dahil edilecek yapï¿½??Â± tÃ¼rleri</div>
            ...
            ${
              scenarioParcel
                ? `<div class="workspace-note"><strong>Parsel alanï¿½??Â±:</strong> ${formatNumber(scenarioParcel.areaM2, 0)} mÃ‚Â² Ã‚Â· <strong>Kullanï¿½??Â±labilir kapasite:</strong> ${formatNumber(
            ...
            ${
              bestAuto
                ? `<div class="workspace-note"><strong>En kÃƒÂ¢rlï¿½??Â± otomatik senaryo:</strong> ${bestAuto.name} Ã‚Â· KÃƒÂ¢r: ${formatMoney(bestAuto.profit)} Ã‚Â· ROI: %${formatNumber(
                    bestAuto.roiPct,
                    1
                  )}</div>`
                : ""
          ...
          <div class="workspace-section">
            <h3 class="workspace-section-title">Senaryo SeÃ§imi (Kullanici)</h3>
            ${
              AAYS_REAL_DATA_ONLY
                ? '<div class="workspace-note">Gercek veri modunda manuel demo hesap kapali. Karşılaştırma listesine otomatik cost-preview senaryosu eklenir.</div>'
                : `
          ...
          <div class="workspace-section">
            <h3 class="workspace-section-title">SonuÃ§</h3>
            ${renderScenarioSelectionSummaryCards(selectionPreview)}
          </div>
          ${scenarioParcel ? renderLandIntelligenceSection(scenarioParcel) : ""}
          <div class="workspace-section">
            <h3 class="workspace-section-title">KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma Listesi</h3>
            <div class="workspace-note">En az 2 ve en fazla 10 senaryo karÅŸï¿½??Â±laÅŸtï¿½??Â±rabiliriz.</div>
            <div class="workspace-row workspace-action-row">
              <button
                class="workspace-button"
                type="button"
                data-add-scenario-selection="true"
                ${selectionPreview && !selectionPreview.overCapacity ? "" : "disabled"}
              >SeÃ§imi KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma Listesine Ekle</button>
              <button
                class="workspace-button subtle"
                type="button"
                data-reset-manual-selection="true"
                ${(AAYS_REAL_DATA_ONLY || manualDraftEntries.length) ? "" : "disabled"}
              >SeÃ§imi Temizle</button>
            </div>
            ${
      ...
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <h2 class="workspace-page-title">KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma (ROI/IRR) Ã¢â‚¬â€ SeÃ§ilen Senaryolar</h2>
            ${
              !compareEntries.length
                ? '<div class="workspace-empty">Arsa & Senaryo Secimi ekraninda bir senaryo kurup karsilastirma listesine ekleyelim.</div>'
                : `
                  <div class="workspace-note">Streamlit akï¿½??Â±ÅŸï¿½??Â±ndaki gibi seÃ§ilen parseller, seÃ§ilen senaryolar ve detay tablolarï¿½??Â± aÅŸaï¿½??Å¸ï¿½??Â±da birlikte gÃ¶steriliyor.</div>
                  <div class="workspace-row workspace-action-row">
                    <span>SeÃ§ilen Senaryolar</span>
                    <button class="workspace-button" type="button" data-clear-scenario-selections="true">TÃ¼m listeyi temizle</button>
                  </div>
      ...
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <h2 class="workspace-page-title">Senaryo KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma Ã¢â‚¬â€ SeÃ§ilen Senaryolar</h2>
            ${
              !compareEntries.length
                ? '<div class="workspace-empty">Senaryo karÅŸï¿½??Â±laÅŸtï¿½??Â±rmasï¿½??Â± iÃ§in once birkac kullanici senaryosu ekleyelim.</div>'
                : `
                  <div class="workspace-row workspace-action-row">
                    <span>SeÃ§ilen Senaryolar</span>
                    <button class="workspace-button" type="button" data-clear-scenario-selections="true">TÃ¼m listeyi temizle</button>
      ...
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <h2 class="workspace-page-title">ï¿½??Â°letiÅŸim MenÃ¼sÃ¼ Ã¢â‚¬â€ Arsa Satï¿½??Â±n Alma Kanallarï¿½??Â±</h2>
            ${
              !compareEntries.length
                ? '<div class="workspace-empty">ï¿½??Â°letiÅŸim akï¿½??Â±ÅŸï¿½??Â± iÃ§in Ã¶nce birkaÃ§ parceli karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesine ekleyelim.</div>'
                : `
                  <div class="workspace-note">SeÃ§ilen parseller iÃ§in satï¿½??Â±n alma kanallarï¿½??Â± ve yapï¿½??Â± tÃ¼rÃ¼ne gÃ¶re yÃ¼klenici iletiÅŸimleri aÅŸaï¿½??Å¸ï¿½??Â±da listelenir.</div>
                  <div class="workspace-section">
                    <h3 class="workspace-section-title">KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesindeki parseller</h3>
      ...
      `);
      void hydrateContactContractorTables(compareEntries);
    }
    ...
    workspaceContentEl.querySelectorAll("[data-remove-compare]").forEach((buttonEl) => {
      buttonEl.addEventListener("click", () => {
        ...
        if (result.changed) {
          showStatus(`${result.entry.ref} karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesinden Ã§ï¿½??Â±karï¿½??Â±ldï¿½??Â±.`);
        }
      });
    });
    ...
    workspaceContentEl.querySelectorAll("[data-remove-compare-selection]").forEach((buttonEl) => {
      buttonEl.addEventListener("click", () => {
        ...
        if (result.changed) {
          showStatus(`${result.entry.ref} senaryosu karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesinden Ã§ï¿½??Â±karï¿½??Â±ldï¿½??Â±.`);
        }
      });
    });
    ...
    if (clearCompareEl) {
      clearCompareEl.addEventListener("click", () => {
        ...
        showStatus("KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesi temizlendi.");
      });
    }
    ...
    if (clearScenarioSelectionsEl) {
      clearScenarioSelectionsEl.addEventListener("click", () => {
        ...
        showStatus("Senaryo karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesi temizlendi.");
      });
    }
    ...
    workspaceContentEl.querySelectorAll("[data-compare-action]").forEach((buttonEl) => {
      buttonEl.addEventListener("click", () => {
        ...
        if (result.changed) {
          showStatus(`${result.entry.ref} karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesine eklendi.`);
          refreshActiveParcelPopup();
        }
      });
    });

    workspaceContentEl.querySelectorAll("[data-nearest-planned-go]").forEach((buttonEl) => {
      buttonEl.addEventListener("click", () => {
        if (!map) return;
        const lat = Number(buttonEl.getAttribute("data-nearest-planned-lat"));
        const lon = Number(buttonEl.getAttribute("data-nearest-planned-lon"));
        const title = buttonEl.getAttribute("data-nearest-planned-title") || "Planned Asset";
        if (!(Number.isFinite(lat) && Number.isFinite(lon))) {
          showStatus("YakÄ±n planlanan yapÄ± iÃ§in koordinat bilgisi bulunamadÄ±.", true);
          return;
        }
        map.easeTo({
          center: [lon, lat],
        ...
        showStatus(`Planlanan yapÄ± odaÄŸÄ± aÃ§Ä±ldÄ±: ${title}`);
      });
    });
    ...
    if (settingsConfirmEl) {
      settingsConfirmEl.addEventListener("click", () => {
        ...
        showStatus("Varsayï¿½??Â±mlar kaydedildi, program gÃ¼ncel ayarlarla devam ediyor.");
        renderWorkspace();
      });
    }
    ...
    if (addScenarioSelectionEl) {
      addScenarioSelectionEl.addEventListener("click", () => {
        ...
        const allowedTypes = WORKSPACE_SCENARIOS.filter((name) => scenarioSelectState.allowedTypes.includes(name));
        const autoRows = candidate?.parcel ? buildScenarioSelectionRows(candidate.parcel, allowedTypes, scenarioSelectState.minUtil) : [];
        const selectedAutoScenario =
          autoRows.find((scenario) => scenario.name === scenarioSelectState.selectedScenarioName) ||
          autoRows[0] ||
          null;
        const result = addScenarioSelectionToCompare(sourceEntry, draftEntries, selectedAutoScenario);
        if (result.reason === "capacity") {
          showStatus("Bu senaryo parcel kapasitesini aÅŸï¿½??Â±yor. Eklenmeden Ã¶nce kullanim azaltï¿½??Â±lmalï¿½??Â±.", true);
          return;
        }
        if (result.changed) {
          showStatus(`${result.entry.ref} icin senaryo karsilastirma listesine eklendi.`);
        }
      });
    }
  ...
  let topographyTileRuntimeAvailable = null;
  let topographyTileRuntimeCheckedAt = 0;
  let topographyTileRuntimeLastWarnedAt = 0;
  let currentFutureGrowthVisible = Boolean(futureGrowthLayerConfig.defaultVisible && futureGrowthLayerConfig.enabled);
  let currentPlannedAssetsVisible = false;
  ...
  let parcelFallbackCache = new Map();
  ...
  let plannedAssetsFetchTimer = null;
  let plannedAssetsRequestId = 0;
  let plannedAssetPopup = null;
  ...
  const realScenarioModelCache = new Map();
  const realScenarioModelInFlight = new Map();
  let costBuildingTypeOptionsCache = { ts: 0, items: [] };

  function getTimedCacheData(cacheMap, key) {
    const payload = cacheMap.get(key);
    if (!payload) return null;
    if (!payload || typeof payload !== "object" || !("data" in payload) || !("ts" in payload)) {
      return payload;
    }
    const ageMs = Date.now() - Number(payload.ts || 0);
    if (ageMs > GLOBAL_OVERLAY_CACHE_TTL_MS) {
      cacheMap.delete(key);
      return null;
    }
    return payload.data;
  ...
  function setTimedCacheData(cacheMap, key, data) {
    cacheMap.set(key, {
      ts: Date.now(),
      data,
    });
  ...
  let contactScreenHydrationTicket = 0;
  ...
  let scenarioSelectState = {
    ...
    maxScenarios: 200,
    minUtil: 0.4,
    allowedTypes: ["Konut Apartmanï¿½??Â±", "Site", "Karma Kullanï¿½??Â±m", "Ticari", "Ofis", "Konut", "EndÃ¼striyel", "Prefabrik", "Tarï¿½??Â±msal"],
  ...
  const worthMenuButtons = new Map();
  let mapModeControlInstance = null;
  let worthModeButton = null;
  let worthMenuEl = null;
  let worthMenuOpen = false;
  let topographyModeButton = null;
  let plannedModeButton = null;
  let futureGrowthModeButton = null;
  let historicSalesModeButton = null;
  let emissionsModeButton = null;
  let securityModeButton = null;
  let internetModeButton = null;
  let currentSecurityVisible = false;
  const parcelElevationCache = new Map();
  const PERF_PROFILE_AGGRESSIVE = AAYS_PERF_PROFILE === "aggressive";
  const PERF_PROFILE_BALANCED = AAYS_PERF_PROFILE === "balanced" || (!PERF_PROFILE_AGGRESSIVE && AAYS_PERF_PROFILE !== "conservative");
  const WORKSPACE_SCENARIOS = ["Konut Apartmanï¿½??Â±", "Site", "Karma Kullanï¿½??Â±m", "Ticari", "Ofis", "Konut", "EndÃ¼striyel", "Prefabrik", "Tarï¿½??Â±msal"];
  const LAND_SIGNAL_MIN_ZOOM = Number(config.landSignalMinZoom || 11.0);
  ...
  const LAND_MARKET_FETCH_LIMIT = Number(config.landMarketFetchLimit || (PERF_PROFILE_AGGRESSIVE ? 1200 : 2000));
  const LAND_BROWNFIELD_FETCH_LIMIT = Number(config.landBrownfieldFetchLimit || (PERF_PROFILE_AGGRESSIVE ? 1200 : 2000));
  ...
  const PARCEL_FALLBACK_PAGE_LIMIT = Math.min(5000, Math.max(400, Number(config.parcelFallbackPageLimit || (PERF_PROFILE_AGGRESSIVE ? 900 : 1200))));
  const PARCEL_FALLBACK_MAX_PAGES = Math.min(8, Math.max(1, Number(config.parcelFallbackMaxPages || (PERF_PROFILE_AGGRESSIVE ? 2 : 4))));
  const PARCEL_FALLBACK_MAX_FEATURES = Math.max(800, Number(config.parcelFallbackMaxFeatures || (PERF_PROFILE_AGGRESSIVE ? 1800 : 3200)));
  const PARCEL_FALLBACK_CACHE_TTL_MS = Math.max(4000, Number(config.parcelFallbackCacheTtlMs || (PERF_PROFILE_AGGRESSIVE ? 12000 : 18000)));
  const PARCEL_FALLBACK_BBOX_DECIMALS = Math.min(6, Math.max(2, Number(config.parcelFallbackBboxDecimals || (PERF_PROFILE_AGGRESSIVE ? 4 : 3))));
  const PARCEL_FALLBACK_ZOOM_BUCKET = Math.min(2, Math.max(0.25, Number(config.parcelFallbackZoomBucket || 0.5)));
  ...
  const GLOBAL_SALE_READY_FETCH_LIMIT = Number(config.globalSaleReadyFetchLimit || (PERF_PROFILE_AGGRESSIVE ? 600 : 1000));
  const GLOBAL_HISTORY_FETCH_LIMIT = Number(config.globalHistoryFetchLimit || (PERF_PROFILE_AGGRESSIVE ? 600 : 1000));
  const GLOBAL_OVERVIEW_PAGE_LIMIT = Number(config.globalOverviewPageLimit || (PERF_PROFILE_AGGRESSIVE ? 300 : 500));
  const GLOBAL_OVERVIEW_MAX_PAGES = Number(config.globalOverviewMaxPages || (PERF_PROFILE_AGGRESSIVE ? 2 : 3));
  const GLOBAL_OVERLAY_CACHE_TTL_MS = Number(config.globalOverlayCacheTtlMs || (PERF_PROFILE_AGGRESSIVE ? 20000 : 30000));
  ...
  const MAP_VIEW_MODE_LABELS = {
    ...
    land_use: "Arazi kullanım (renkli)",
  };
  const WORTH_CONTROL_MODE = "__worth_panel_toggle__";
  const MAP_MODE_CONTROL_ITEMS_ALL = [
    {
      mode: "map_only",
      label: "Parselsiz harita",
      iconUrl: "./assets/icons/terrayield_icons/normal_map.png",
    },
    {
      mode: "land_use",
      label: "Topografik Harita",
      iconUrl: "./assets/icons/terrayield_icons/topografic_map.png",
    },
    {
      mode: "parcel",
      label: "Parsellerli gorunum",
      iconUrl: "./assets/icons/terrayield_icons/parcel_menu.png",
    },
    {
    ...
    },
    {
      ...
      iconUrl: "./assets/icons/terrayield_icons/future_growing_prognose.png",
      futureGrowthToggle: true,
    },
    {
      mode: EMISSIONS_CONTROL_MODE,
      label: "Gaz emisyonu katmani",
      iconUrl: "./assets/icons/terrayield_icons/air.png",
      emissionsToggle: true,
    },
    {
      mode: SECURITY_CONTROL_MODE,
      label: "GÃ¼venlik katmanÄ±",
      iconUrl: "./assets/icons/terrayield_icons/security.png",
      securityToggle: true,
    },
    {
      mode: INTERNET_CONTROL_MODE,
      label: "Ä°nternet katmanÄ±",
      iconUrl: "./assets/icons/terrayield_icons/internet.png",
      internetToggle: true,
    },
  ];
  const MAP_MODE_PRIMARY_ITEMS = [
    { mode: "map_only", label: "Normal harita", iconUrl: "./assets/icons/terrayield_icons/normal_map.png", mapMode: "map_only" },
    { mode: "land_use", label: "Topografik harita", iconUrl: "./assets/icons/terrayield_icons/topografic_map.png", mapMode: "land_use" },
    { mode: "parcel", label: "Parseller", iconUrl: "./assets/icons/terrayield_icons/parcel_menu.png", mapMode: "parcel" },
  ...
  const MAP_WORTH_MENU_ITEMS = [
    ...
    { id: "compare", label: "Hava Kirliligi", iconUrl: "./assets/icons/terrayield_icons/air.png" },
    { id: "topography", label: "Yukselti", iconUrl: "./assets/icons/worth-waves.svg" },
    { id: "security", label: "Güvenlik", iconUrl: "./assets/icons/worth-security.svg" },
    { id: "internet", label: "Internet", iconUrl: "./assets/icons/worth-global.svg" },
    { id: "contact", label: "Kontakt", iconUrl: "./assets/icons/worth-logistics.svg" },
    { id: "future_growth", label: "Gelecek Gelisim", iconUrl: "./assets/icons/worth-growth.svg" },
    { id: "legal", label: "Legal", iconUrl: "./assets/icons/worth-legal.svg" },
  ];
  ...
  function loadVisibleMapModeControlModes() {
    try {
      ...
      const requiredModes = getDefaultMapModeControlModes();
      requiredModes.forEach((mode) => {
        if (available.has(mode) && !filtered.includes(mode)) {
          filtered.push(mode);
        }
      });
  ...
  const LAND_USE_CATEGORY_MAPPING = {
    ...
    { id: "portfolio", label: "Portfoy Analizi" },
    { id: "parcel", label: "Parsel Analizi" },
    { id: "recommendation", label: "Oneri Motoru" },
    { id: "scenario_select", label: "Arsa & Senaryo Secimi" },
    { id: "compare", label: "Karşılaştırma (ROI/IRR)" },
    { id: "scenario_compare", label: "Senaryo Karşılaştırma" },
    { id: "contact", label: "İletişim Menüsü" },
  ];

  const DEFAULT_PLANNING_SETTINGS = {
    usableRatio: 0.6,
    capacityMethod: "Taban Alanï¿½??Â±",
    far: 1.5,
    projectMonths: 18,
    buildingTypes: [
      { key: "mustakil", label: "MÃ¼stakil Ev", scenarioName: "Konut", unitArea: 150, minArea: 120, maxArea: 220, unitCost: 2000000, unitSaleValue: 3000000 },
      { key: "apartman", label: "Apartman", scenarioName: "Konut Apartmanï¿½??Â±", unitArea: 400, minArea: 250, maxArea: 650, unitCost: 6500000, unitSaleValue: 9000000 },
      ...
      { key: "karma", label: "Karma", scenarioName: "Karma Kullanï¿½??Â±m", unitArea: 500, minArea: 300, maxArea: 900, unitCost: 7000000, unitSaleValue: 10000000 },
      { key: "endustriyel", label: "EndÃ¼striyel", scenarioName: "EndÃ¼striyel", unitArea: 800, minArea: 500, maxArea: 1400, unitCost: 10000000, unitSaleValue: 14000000 },
      { key: "tarimsal", label: "Tarï¿½??Â±msal", scenarioName: "Tarï¿½??Â±msal", unitArea: 500, minArea: 250, maxArea: 900, unitCost: 3000000, unitSaleValue: 4200000 },
    ],
    factorMatrix: [
      ...
      { factor: "Metro (UlaÅŸï¿½??Â±m)", mustakil: 6, apartman: 9, site: 9, prefab: 5, perakende: 8, ofis: 9, karma: 9, endustriyel: 10, tarimsal: 3 },
      { factor: "OtobÃ¼s Duraï¿½??Å¸ï¿½??Â±", mustakil: 7, apartman: 8, site: 8, prefab: 6, perakende: 9, ofis: 8, karma: 9, endustriyel: 8, tarimsal: 5 },
      { factor: "AVM", mustakil: 8, apartman: 9, site: 10, prefab: 6, perakende: 10, ofis: 9, karma: 8, endustriyel: 4, tarimsal: 4 },
      { factor: "Deniz ï¿½??Â°skelesi / Su Kenarï¿½??Â±", mustakil: 5, apartman: 7, site: 7, prefab: 5, perakende: 6, ofis: 5, karma: 6, endustriyel: 8, tarimsal: 7 },
      { factor: "Nehir (Su Kaynaklarï¿½??Â±)", mustakil: 7, apartman: 8, site: 8, prefab: 6, perakende: 7, ofis: 7, karma: 6, endustriyel: 9, tarimsal: 9 },
      { factor: "ï¿½??Â°mar Planï¿½??Â± & Hukuki Durum", mustakil: 9, apartman: 10, site: 10, prefab: 8, perakende: 8, ofis: 7, karma: 7, endustriyel: 3, tarimsal: 6 },
      { factor: "Topografya & Zemin Ã–zellikleri", mustakil: 7, apartman: 8, site: 7, prefab: 7, perakende: 7, ofis: 8, karma: 7, endustriyel: 9, tarimsal: 8 },
  ...
  function normalizeCombinedSalesLayerFeature(feature) {
    ...
    const inferredExternalEvidence = Boolean(
      props.listing_url
      || props.ask_price
    ...
    const externalEvidenceAvailable = coerceSalesLayerBoolean(props.external_market_evidence_available) || inferredExternalEvidence;
  ...
  async function loadLocalCombinedSalesFallbackFeatures() {
    if (localCombinedSalesFallbackPromise) {
      return localCombinedSalesFallbackPromise;
    }
    localCombinedSalesFallbackPromise = (async () => {
      try {
        const response = await fetch(LOCAL_COMBINED_SALES_FALLBACK_URL);
        if (!response.ok) {
          return [];
        }
        const payload = await response.json();
        const features = Array.isArray(payload?.features) ? payload.features : [];
        return features.map((feature) => normalizeCombinedSalesLayerFeature(feature)).filter(Boolean);
      } catch (_error) {
        return [];
      }
    })();
  ...
  function getTopographyTileRuntimeProbeUrl() {
    const tileUrl = Array.isArray(topographyOverlayConfig.tiles) ? topographyOverlayConfig.tiles[0] : "";
    if (typeof tileUrl !== "string" || !tileUrl) return null;
    if (!/127\.0\.0\.1:8099|localhost:8099/i.test(tileUrl)) return null;
    return tileUrl
      .replace("{z}", "10")
      .replace("{x}", "512")
      .replace("{y}", "340");
  ...
  async function probeTopographyTileRuntime(force = false) {
    const probeUrl = getTopographyTileRuntimeProbeUrl();
    if (!probeUrl) {
      topographyTileRuntimeAvailable = true;
      topographyTileRuntimeCheckedAt = Date.now();
      return true;
    }
    const nowTs = Date.now();
    if (!force && Number.isFinite(topographyTileRuntimeCheckedAt) && (nowTs - topographyTileRuntimeCheckedAt) < 25000 && topographyTileRuntimeAvailable !== null) {
      return Boolean(topographyTileRuntimeAvailable);
    }
    const controller = new AbortController();
    ...
    try {
      ...
      topographyTileRuntimeAvailable = response.ok;
    } catch (_error) {
      topographyTileRuntimeAvailable = false;
    } finally {
      window.clearTimeout(timer);
      topographyTileRuntimeCheckedAt = Date.now();
    }
  ...
  async function toggleTopographyOverlay() {
    const nextVisible = !currentTopographyVisible;
    if (nextVisible) {
      const runtimeAvailable = await probeTopographyTileRuntime(false);
      if (!runtimeAvailable) {
        currentTopographyVisible = false;
        updateTopographyLayerVisibility();
        if (!AAYS_TOPOGRAPHY_TILE_REQUIRED) {
          if ((Date.now() - topographyTileRuntimeLastWarnedAt) > 5000) {
            showStatus("Topografya overlay runtime (8099) su an ulasilamiyor. Yukselti popup verisi devam eder.", true);
            topographyTileRuntimeLastWarnedAt = Date.now();
          }
          updateStats();
          return;
        }
        showStatus("Topografya runtime zorunlu ama su an ulasilamiyor (8099).", true);
        updateStats();
        return;
      }
    }
    setTopographyOverlayVisibility(nextVisible);
  }
  ...
  function resolvePlannedAssetsApiUrl(pathTemplate = "/planned-assets/parcel-layer") {
    const template = String(pathTemplate || "").trim();
    if (!template) return `${landIntelligenceApiBaseUrl}/planned-assets/parcel-layer`;
    if (/^https?:\/\//i.test(template)) return template;
    return `${landIntelligenceApiBaseUrl}${template.startsWith("/") ? template : `/${template}`}`;
  }

  function getPlannedAssetColor(assetType) {
    const key = String(assetType || "").trim().toLowerCase();
    if (["new_station", "rail_upgrade", "metro_tram_project", "road_scheme", "bridge_tunnel", "highway_improvement"].includes(key)) {
      return "#2563eb";
    }
    if (["residential_development", "commercial_development", "mixed_use_development", "major_employment_zone"].includes(key)) {
      return "#ea580c";
    }
    if (["regeneration_area", "brownfield_site", "local_plan_allocation"].includes(key)) {
      return "#7c3aed";
    }
    return "#0f766e";
  }

  function normalizePlannedAssetFeature(item, index = 0) {
    const row = item && typeof item === "object" ? item : {};
    const geometry = row.geometry && typeof row.geometry === "object" ? row.geometry : null;
    const lat = Number(row.centroid_lat);
    const lon = Number(row.centroid_lon);
    let normalizedGeometry = geometry;
    if (!normalizedGeometry && Number.isFinite(lat) && Number.isFinite(lon)) {
      normalizedGeometry = {
        type: "Point",
        coordinates: [lon, lat],
      };
    ...
    const idValue = row.id ?? row.planned_asset_id ?? row.asset_id ?? `planned-${index + 1}`;
    return {
      type: "Feature",
      geometry: normalizedGeometry,
      properties: {
        planned_asset_id: idValue,
        title: row.title || `Planned Asset ${idValue}`,
        asset_type: row.asset_type || "unknown",
        status: row.status || "unknown",
        delivery_probability_score: Number(row.delivery_probability_score),
        delay_risk_score: Number(row.delay_risk_score),
        source_name: row.source_name || "-",
        source_url: row.source_url || null,
        local_authority: row.local_authority || "-",
        planned_start_year: row.planned_start_year ?? null,
        planned_completion_year: row.planned_completion_year ?? null,
        distance_m: Number(row.distance_m),
        evidence_text: row.evidence_text || "",
        color_hex: getPlannedAssetColor(row.asset_type),
        layer_kind: "planned_asset",
      },
    };
  }

  function featureIntersectsBbox(feature, bboxObj) {
    if (!bboxObj) return true;
    const bounds = geometryBounds(feature?.geometry);
    if (!bounds) return false;
    return !(
      bounds.maxLng < bboxObj.west
      || bounds.minLng > bboxObj.east
      || bounds.maxLat < bboxObj.south
      || bounds.minLat > bboxObj.north
    );
  }

  function normalizePlannedAssetsFeatureCollection(items, bboxObj = null) {
    const features = (Array.isArray(items) ? items : [])
      .map((item, index) => normalizePlannedAssetFeature(item, index))
      .filter(Boolean)
      .filter((feature) => featureIntersectsBbox(feature, bboxObj));
    return {
      type: "FeatureCollection",
      features,
    };
  ...
  function normalizePlannedParcelLayerPayload(payload, bboxObj = null) {
    const rawCollection = payload && typeof payload === "object" && payload.type === "FeatureCollection"
      ? payload
      : createEmptyFeatureCollection();
    const features = (Array.isArray(rawCollection.features) ? rawCollection.features : [])
      ...
      .map((feature) => ({
        ...
        properties: {
          ...(feature.properties || {}),
          layer_kind: feature?.properties?.layer_kind || "planned_parcel",
          color_hex: feature?.properties?.color_hex || "#16a34a",
          color_category: feature?.properties?.color_category || "high",
          confidence_accuracy_label: feature?.properties?.confidence_accuracy_label || "Low Accuracy",
        },
      }));
    const lookupStatus = String(
      payload?.lookup_status
      || rawCollection?.lookup_status
      || rawCollection?.metadata?.lookup_status
      || (features.length ? "OK" : "NO_RESULTS")
    ).trim().toUpperCase();
    return {
      collection: {
        type: "FeatureCollection",
        features,
        metadata: rawCollection?.metadata || {},
      },
      lookupStatus,
      dataPresent: payload?.data_present !== false,
    };
  }

  function normalizeFutureGrowthAsPlannedFeatures(payload, bboxObj = null) {
    const features = (Array.isArray(payload?.features) ? payload.features : [])
      .map((feature, index) => {
        const props = feature?.properties || {};
        ...
        return {
          ...
          geometry,
          properties: {
            planned_asset_id: `fg-${Number.isFinite(parcelId) ? parcelId : index + 1}`,
            title: `Future Growth Parcel ${Number.isFinite(parcelId) ? parcelId : "-"}`,
            asset_type: "future_growth_candidate",
            status: "scored",
            delivery_probability_score: Number(props.confidence_score),
            delay_risk_score: Number.isFinite(Number(props.confidence_score)) ? Math.max(0, 100 - Number(props.confidence_score)) : null,
            source_name: "future_growth_layer",
            source_url: null,
            local_authority: props.local_authority_code || "-",
            planned_start_year: null,
            planned_completion_year: null,
            distance_m: null,
            evidence_text: "Future Growth fallback: planned assets table is empty.",
            color_hex: String(props.hex_color || "#f59e0b"),
            fallback_from_future_growth: true,
            parcel_id: Number.isFinite(parcelId) ? parcelId : null,
            layer_kind: "planned_asset_future_growth_fallback",
          },
        };
      })
      .filter(Boolean)
  ...
  function addPlannedAssetsLayers() {
    if (!map) return;
    if (!map.getSource(PLANNED_ASSETS_SOURCE_ID)) {
      map.addSource(PLANNED_ASSETS_SOURCE_ID, {
        type: "geojson",
        data: createEmptyFeatureCollection(),
      });
    }
    if (!map.getLayer(PLANNED_ASSETS_FILL_LAYER_ID)) {
      map.addLayer({
        id: PLANNED_ASSETS_FILL_LAYER_ID,
        type: "fill",
        source: PLANNED_ASSETS_SOURCE_ID,
        filter: ["in", ["geometry-type"], ["literal", ["Polygon", "MultiPolygon"]]],
        layout: {
          visibility: currentPlannedAssetsVisible ? "visible" : "none",
        },
        paint: {
          "fill-color": ["coalesce", ["get", "color_hex"], "#ea580c"],
          "fill-opacity": 0.22,
        },
      });
    }
    if (!map.getLayer(PLANNED_ASSETS_LINE_LAYER_ID)) {
      map.addLayer({
        id: PLANNED_ASSETS_LINE_LAYER_ID,
        type: "line",
        source: PLANNED_ASSETS_SOURCE_ID,
        ...
        paint: {
          "line-color": ["coalesce", ["get", "color_hex"], "#7c2d12"],
          "line-width": [
            "interpolate",
            ["linear"],
            ["zoom"],
    ...
    if (!map.getLayer(PLANNED_ASSETS_POINT_LAYER_ID)) {
      map.addLayer({
        id: PLANNED_ASSETS_POINT_LAYER_ID,
        type: "circle",
        source: PLANNED_ASSETS_SOURCE_ID,
        filter: ["in", ["geometry-type"], ["literal", ["Point", "MultiPoint"]]],
        layout: {
        ...
        paint: {
          "circle-color": ["coalesce", ["get", "color_hex"], "#ea580c"],
          "circle-radius": [
            "interpolate",
            ["linear"],
            ["zoom"],
  ...
  function updatePlannedAssetsLayerVisibility() {
    const visibility = currentPlannedAssetsVisible ? "visible" : "none";
    [PLANNED_ASSETS_FILL_LAYER_ID, PLANNED_ASSETS_LINE_LAYER_ID, PLANNED_ASSETS_POINT_LAYER_ID].forEach((layerId) => {
      if (map?.getLayer(layerId)) {
        map.setLayoutProperty(layerId, "visibility", visibility);
      }
    });
    if (!currentPlannedAssetsVisible && plannedAssetPopup) {
      plannedAssetPopup.remove();
      plannedAssetPopup = null;
    }
    updateMapModeControlState();
  }

  function buildPlannedAssetPopupHtml(props) {
    const plannedBuildings = Array.isArray(props.planned_buildings) ? props.planned_buildings.slice(0, 3) : [];
    const headlineValue = Number(props.planned_building_1_value);
    const headlineProbability = Number(props.planned_building_1_probability);
    const headlineDistance = Number(props.nearest_distance_m);
    const buildingRows = plannedBuildings.length
      ? plannedBuildings.map((item) => `
          <div class="signal-popup-list">
            <div><strong>Proje ${escapeHtml(String(item?.rank || "-"))}:</strong> ${escapeHtml(item?.project_name || "-")}</div>
            <div><strong>Tip:</strong> ${escapeHtml(item?.planned_asset_type || "-")}</div>
            <div><strong>Durum:</strong> ${escapeHtml(item?.planning_status || "-")}</div>
            <div><strong>Value:</strong> ${Number.isFinite(Number(item?.planned_value_score)) ? `${formatNumber(Number(item.planned_value_score), 1)}/100` : "-"}</div>
            <div><strong>Probability:</strong> ${Number.isFinite(Number(item?.planned_probability_percent)) ? `${formatNumber(Number(item.planned_probability_percent), 1)}%` : "-"}</div>
            <div><strong>Completion:</strong> ${escapeHtml(item?.planned_completion_month || "insufficient evidence")}</div>
            <div><strong>Source:</strong> ${escapeHtml(item?.source_name || "-")}</div>
            <div><strong>Source Date:</strong> ${escapeHtml(item?.source_date || "-")}</div>
            <div><strong>Evidence:</strong> ${escapeHtml(item?.evidence || "insufficient evidence")}</div>
          </div>
        `).join("")
      : `<div class="workspace-note">Bu parcel icin planned-building eslesmesi bulunamadi.</div>`;
    return `
      <div class="future-growth-hover-card">
        <div><strong>Nearby Planned Developments</strong></div>
        <div><strong>Parcel:</strong> ${escapeHtml(props.parcel_ref || String(props.parcel_id || "-"))}</div>
        <div><strong>Count:</strong> ${Number.isFinite(Number(props.planned_asset_count)) ? formatNumber(Number(props.planned_asset_count), 0) : "-"}</div>
        <div><strong>Color:</strong> ${escapeHtml(props.color_category || "-")}</div>
        <div><strong>Accuracy:</strong> ${escapeHtml(props.confidence_accuracy_label || "-")}</div>
        <div><strong>Relation:</strong> ${escapeHtml(props.relation_type || "-")}</div>
        <div><strong>Method:</strong> ${escapeHtml(props.matching_method || "-")}</div>
        <div><strong>Top Value:</strong> ${Number.isFinite(headlineValue) ? `${formatNumber(headlineValue, 1)}/100` : "-"}</div>
        <div><strong>Top Probability:</strong> ${Number.isFinite(headlineProbability) ? `${formatNumber(headlineProbability, 1)}%` : "-"}</div>
        <div><strong>Nearest Distance:</strong> ${Number.isFinite(headlineDistance) ? `${formatNumber(headlineDistance, 0)} m` : "-"}</div>
        <div><strong>Explanation:</strong> ${escapeHtml(props.calculation_explanation || "-")}</div>
        ${props.source_url ? `<div><a href="${escapeHtml(props.source_url)}" target="_blank" rel="noopener noreferrer">Kaynak linki</a></div>` : ""}
        <hr />
        ${buildingRows}
      </div>
    `;
  ...
  function openPlannedAssetPopup(feature, lngLat) {
    if (!map || !feature || !lngLat) return;
    if (plannedAssetPopup) {
      plannedAssetPopup.remove();
    }
    plannedAssetPopup = new maplibregl.Popup({ closeButton: true, maxWidth: "340px" })
      .setLngLat(lngLat)
      .setHTML(buildPlannedAssetPopupHtml(feature.properties || {}))
      .addTo(map);
    plannedAssetPopup.on("close", () => {
      plannedAssetPopup = null;
    });
  }

  async function loadPlannedAssetsLayer(force = false) {
    if (!map) return;
    if (!currentPlannedAssetsVisible) {
      setGeoJsonSourceData(PLANNED_ASSETS_SOURCE_ID, createEmptyFeatureCollection());
      plannedAssetsState = {
        ...plannedAssetsState,
        featureCount: 0,
        error: null,
    ...
    const quantizedBbox = quantizeBboxString(bbox, 4);
    if (!force && quantizedBbox === plannedAssetsState.lastBBoxKey) {
      return;
    }

    const requestId = ++plannedAssetsRequestId;
    const layerUrl = new URL(resolvePlannedAssetsApiUrl("/planned-assets/parcel-layer"));
    layerUrl.searchParams.set("bbox", bbox);
    layerUrl.searchParams.set("limit", "2500");
    const layerResponse = await fetchJsonWithTimeout(layerUrl.toString(), {
      label: "Planned parcel layer",
      fallback: createEmptyFeatureCollection,
      backoffGroup: "planned-assets-layer",
    });
    if (requestId !== plannedAssetsRequestId) return;

    const normalizedLayerPayload = normalizePlannedParcelLayerPayload(layerResponse.data, bboxObj);
    const featureCollection = normalizedLayerPayload.collection;
    const featureCount = Array.isArray(featureCollection.features) ? featureCollection.features.length : 0;
    const lookupStatus = normalizedLayerPayload.lookupStatus || (featureCount > 0 ? "OK" : "NO_RESULTS");
    const dataMode = layerResponse.ok
      ? (lookupStatus === "NO_RESULTS" ? "no_results" : "planned_parcels")
      : "error";
    const errorMessage = layerResponse.ok ? null : layerResponse.error;

    setGeoJsonSourceData(PLANNED_ASSETS_SOURCE_ID, featureCollection);
    plannedAssetsState = {
      ...plannedAssetsState,
      featureCount,
      lastLoadedAt: new Date().toISOString(),
      ...
      mode: dataMode,
      lookupStatus,
      dataPresent: Boolean(layerResponse.ok && normalizedLayerPayload.dataPresent),
    };
    if (force) {
      if (!layerResponse.ok) {
        showStatus(`Planlanan yapi katmani acilamadi: ${errorMessage || "backend yanit vermedi"}`, true);
      } else if (lookupStatus === "NO_RESULTS") {
        showStatus("Planlanan yapi katmani acildi, bu gorunumde eslesen parcel yok.", true);
      } else {
        showStatus(`Planlanan yapi katmani acildi (${featureCount} parcel).`);
      }
    }
    updateMapModeControlState();
  }

  function schedulePlannedAssetsRefresh(force = false) {
    window.clearTimeout(plannedAssetsFetchTimer);
    plannedAssetsFetchTimer = window.setTimeout(() => {
      void loadPlannedAssetsLayer(force);
    }, force ? 120 : 300);
  }

  function setPlannedAssetsLayerVisibility(nextVisible, options = {}) {
    currentPlannedAssetsVisible = Boolean(nextVisible);
    addPlannedAssetsLayers();
    updatePlannedAssetsLayerVisibility();
    if (currentPlannedAssetsVisible) {
      schedulePlannedAssetsRefresh(true);
    }
    if (!options.silent) {
      showStatus(currentPlannedAssetsVisible ? "Planlanan yapi katmani yukleniyor..." : "Planlanan yapi katmani kapatildi.", currentPlannedAssetsVisible);
    }
  }

  function togglePlannedAssetsLayer() {
    setPlannedAssetsLayerVisibility(!currentPlannedAssetsVisible);
  }

  async function focusNearestPlannedStructure() {
    if (!map || !landIntelligenceApiBaseUrl) return false;
    const center = map.getCenter();
    const nearbyUrl = new URL(resolvePlannedAssetsApiUrl("/planned-assets/nearby"));
    nearbyUrl.searchParams.set("lat", Number(center.lat).toFixed(6));
    nearbyUrl.searchParams.set("lon", Number(center.lng).toFixed(6));
    nearbyUrl.searchParams.set("radius", "12000");
    nearbyUrl.searchParams.set("limit", "80");
    const nearbyResponse = await fetchJsonWithTimeout(nearbyUrl.toString(), {
      label: "Nearest planned asset",
      fallback: [],
      backoffGroup: "planned-assets-nearby",
    });
    if (nearbyResponse.ok && Array.isArray(nearbyResponse.data) && nearbyResponse.data.length > 0) {
      const selected = nearbyResponse.data[0];
      const feature = normalizePlannedAssetFeature(selected, 0);
      const point = geometryCenterLngLat(feature?.geometry);
      if (feature && point) {
        map.easeTo({
        ...
        window.setTimeout(() => {
          openPlannedAssetPopup(feature, point);
        }, 350);
        showStatus("En yakin planlanan yapiya odaklanildi.");
        return true;
      }
    }

    const fgUrl = new URL(resolveFutureGrowthApiUrl(futureGrowthLayerConfig.api.layer));
    const bbox = mapBoundsToBboxString();
    fgUrl.searchParams.set("bbox", bbox || "-8.7,49.8,2.1,61.1");
    fgUrl.searchParams.set("zoom", Number(map.getZoom()).toFixed(2));
    fgUrl.searchParams.set("limit", "2000");
    const fgResponse = await fetchJsonWithTimeout(fgUrl.toString(), {
      label: "Future Growth fallback nearest planned",
      fallback: createEmptyFeatureCollection,
      backoffGroup: "future-growth-layer",
    });
    const centerPoint = { lng: center.lng, lat: center.lat };
    const nearest = (Array.isArray(fgResponse.data?.features) ? fgResponse.data.features : [])
      .map((feature) => {
        const parcelId = Number(feature?.properties?.parcel_id);
        const point = geometryCenterLngLat(feature?.geometry);
        if (!Number.isFinite(parcelId) || !point) return null;
        return {
          parcelId,
          point,
          distance: haversineDistanceMeters(centerPoint, point),
        };
      })
      .filter(Boolean)
      .sort((a, b) => a.distance - b.distance)[0];

    if (nearest && Number.isFinite(nearest.parcelId)) {
      await focusParcelByIdForContractor(nearest.parcelId);
      showStatus("Planned asset verisi yok; en yakin future-growth parcel odaklandi.", true);
      return true;
    }

    showStatus("Bu gorunumde planlanan yapi verisi bulunamadi.", true);
    return false;
  }

  async function ensureRuntimeScriptLoaded(globalKey, scriptPath) {
    if (window[globalKey]) return true;
    const selector = `script[data-aays-runtime="${globalKey}"]`;
    ...
    return await new Promise((resolve) => {
      ...
      scriptEl.async = true;
      scriptEl.dataset.aaysRuntime = globalKey;
      scriptEl.addEventListener("load", () => resolve(Boolean(window[globalKey])));
      scriptEl.addEventListener("error", () => resolve(false));
  ...
  function ensureInlineSecurityBridge(forceRebind = false) {
    const existingBridge = window.AAYS_SECURITY;
    if (!forceRebind && existingBridge && (typeof existingBridge.activate === "function" || typeof existingBridge.toggle === "function")) {
      return existingBridge;
    }
    const sourceId = "aays-security-parcel-source";
    const fillLayerId = "aays-security-fill";
    const lineLayerId = "aays-security-line";
    const pointLayerId = "aays-security-points";
    const dataUrl = "./data/parcel_security_scores_rechecked_0_120m_spatial.geojson?v=20260622-security-root-data-v2";
    const levelColors = {
      "Cok Dusuk": "#7f1d1d",
      "Dusuk": "#ef4444",
      "Orta": "#f59e0b",
    ...
    const colorExpr = [
      "match",
      ["coalesce", ["get", "safety_level"], ["get", "security_level"]],
      "Cok Dusuk", levelColors["Cok Dusuk"],
      "Dusuk", levelColors["Dusuk"],
      "Orta", levelColors["Orta"],
      "Iyi", levelColors["Iyi"],
      "Cok Iyi", levelColors["Cok Iyi"],
      "#6b7280",
    ];
    let inlinePopup = null;
    let inlineActive = false;

    function securityPopupRow(label, value) {
      const safeValue = value === undefined || value === null || value === "" ? "-" : value;
      return `<div class="aays-security-popup-row"><b>${escapeHtml(label)}:</b> ${escapeHtml(String(safeValue))}</div>`;
    }

    function securitySourceEvidence(props) {
      const items = [];
      if (props.security_lsoa_code) items.push(`LSOA ${props.security_lsoa_code}`);
      if (props.security_borough) items.push(`Borough ${props.security_borough}`);
      if (props.confidence_flags) items.push(`flags=${props.confidence_flags}`);
      if (props.enhanced_method_version) items.push(`enhanced=${props.enhanced_method_version}`);
      if (props.uplift_method_version) items.push(`uplift=${props.uplift_method_version}`);
      if (props.nearest_lsoa_rescue_method_version) items.push(`nearest=${props.nearest_lsoa_rescue_method_version}`);
      return items.join(" | ");
    }

    function securityCalculation(props) {
      const items = [];
      if (props.spatial_match_method) items.push(`match=${props.spatial_match_method}`);
      if (props.weighted_crime_12m !== undefined && props.weighted_crime_12m !== null) items.push(`weighted_crime_12m=${props.weighted_crime_12m}`);
      if (props.weighted_monthly_avg !== undefined && props.weighted_monthly_avg !== null) items.push(`weighted_monthly_avg=${props.weighted_monthly_avg}`);
      if (props.spatial_score !== undefined && props.spatial_score !== null) items.push(`spatial_score=${props.spatial_score}`);
      return items.join(" | ");
    }

    function securitySourceDate(props) {
      return props.uplift_checked_at || props.nearest_lsoa_rescue_checked_at || "-";
    }

    function attachSecurityPopup(layerId) {
      if (!map.getLayer(layerId)) return;
      map.__aaysInlineSecurityPopupBound = map.__aaysInlineSecurityPopupBound || {};
      if (map.__aaysInlineSecurityPopupBound[layerId]) return;
      map.__aaysInlineSecurityPopupBound[layerId] = true;
      map.on("click", layerId, (event) => {
        const feature = event?.features?.[0];
        if (!feature) return;
        const props = feature.properties || {};
        const level = props.safety_level || props.security_level || "-";
        const color = levelColors[level] || "#6b7280";
        const html = [
          '<div class="aays-security-popup">',
          '<div class="aays-security-popup-title">Guvenlik Bilgisi</div>',
          securityPopupRow("Parcel ID", props.security_parcel_id || props.parcel_id),
          securityPopupRow("Security Score", props.safety_score || props.security_score),
          securityPopupRow("Security Level", level),
          securityPopupRow("Color Category", color),
          securityPopupRow("Accuracy Rating", props.confidence_label || props.accuracy_rating),
          securityPopupRow("Confidence Score", props.confidence_score),
          securityPopupRow("Matching Method", props.spatial_match_method || props.matching_method),
          securityPopupRow("Source / Evidence", securitySourceEvidence(props)),
          securityPopupRow("Source Date", securitySourceDate(props)),
          securityPopupRow("Calculation", securityCalculation(props)),
          "</div>",
        ].join("");
        if (inlinePopup) inlinePopup.remove();
        inlinePopup = new maplibregl.Popup({ closeButton: true, closeOnClick: true })
    ...
    function setInlineVisibility(visibility) {
      console.info("[AAYS_SECURITY_DEBUG] setInlineVisibility", {
        visibility,
        hasMap: Boolean(map),
      });
    ...
    function ensureInlineLayers() {
      console.info("[AAYS_SECURITY_DEBUG] ensureInlineLayers:start", {
        hasMap: Boolean(map),
        styleLoaded: Boolean(map && map.isStyleLoaded && map.isStyleLoaded()),
      });
      if (!map || !map.isStyleLoaded || !map.isStyleLoaded()) {
        setTimeout(ensureInlineLayers, 350);
        return;
      }
      if (!map.getSource(sourceId)) {
        console.info("[AAYS_SECURITY_DEBUG] addSource", { sourceId, dataUrl });
        map.addSource(sourceId, {
          type: "geojson",
          data: dataUrl,
          promoteId: "security_parcel_id",
        });
      }
      if (!map.getLayer(fillLayerId)) {
        map.addLayer({
          ...
          paint: {
            ...
            "fill-opacity": ["case", ["==", ["get", "security_match_status"], "MATCHED"], 0.48, 0.18],
          },
      ...
      if (!map.getLayer(pointLayerId)) {
        map.addLayer({
          ...
          paint: {
            ...
            "circle-radius": ["interpolate", ["linear"], ["zoom"], 5, 2, 9, 3.5, 13, 5.5, 16, 7],
            "circle-opacity": ["case", ["==", ["get", "security_match_status"], "MATCHED"], 0.82, 0.28],
            "circle-stroke-color": "#111827",
      ...
      attachSecurityPopup(pointLayerId);
      attachSecurityPopup(fillLayerId);
      if (inlineActive) {
        setInlineVisibility("visible");
      }
    }

    function setSecurityStatusText(text) {
      const statusEl = document.getElementById("securityOverlayStatus");
      if (statusEl) {
        statusEl.textContent = text;
      }
      console.info("[AAYS_SECURITY_DEBUG] statusText", { text, hasStatusEl: Boolean(statusEl) });
    }

    const bridge = {
      activate() {
        console.info("[AAYS_SECURITY_DEBUG] bridge.activate", {
          hasMap: Boolean(map),
          styleLoaded: Boolean(map && map.isStyleLoaded && map.isStyleLoaded()),
        });
        inlineActive = true;
        ensureInlineLayers();
        setInlineVisibility("visible");
        setSecurityStatusText("Guvenlik katmani acik.");
      },
      deactivate() {
        console.info("[AAYS_SECURITY_DEBUG] bridge.deactivate");
        inlineActive = false;
        setInlineVisibility("none");
        if (inlinePopup) inlinePopup.remove();
        setSecurityStatusText("Guvenlik katmani kapali.");
      },
      toggle() {
        if (inlineActive) {
          this.deactivate();
    ...
    window.AAYS_SECURITY = bridge;
    return bridge;
  }

  async function setSecurityOverlayVisibility(nextVisible, options = {}) {
    if (!window.AAYS_SECURITY) {
      void ensureRuntimeScriptLoaded("AAYS_SECURITY", "./security_overlay.js?v=20260622-security-root-data-v2").then((loaded) => {
        if (!loaded) {
          emitLayerRuntimeEvent("security", "runtime_load_failed", "security_overlay.js");
        }
      });
    }
    const bridge = ensureInlineSecurityBridge(true);
    console.info("[AAYS_SECURITY_DEBUG] setSecurityOverlayVisibility", {
    ...
    if (!bridge || (typeof bridge.activate !== "function" && typeof bridge.deactivate !== "function")) {
      if (!options.silent) {
        showThrottledStatus("security_runtime_missing", "Güvenlik katmani yuklu degil. security_overlay.js dosyasini kontrol edin.", true, 6000);
      }
      emitLayerRuntimeEvent("security", "runtime_missing", "bridge_unavailable");
      return;
    }
    ...
    try {
      ...
      if (!options.silent) {
        showStatus(`Güvenlik katmani hatasi: ${error?.message || "bilinmeyen hata"}`, true);
      }
      emitLayerRuntimeEvent("security", "activate_failed", error?.message || "unknown");
      updateMapModeControlState();
      return;
    }
    applySecurityMapControlFlag();
    if (!options.silent) {
      showStatus(`Güvenlik katmani ${currentSecurityVisible ? "acildi" : "kapatildi"}.`);
    }
    emitLayerRuntimeEvent("security", currentSecurityVisible ? "active" : "inactive", "manual_toggle");
    updateMapModeControlState();
  }

  function toggleSecurityOverlay() {
    void setSecurityOverlayVisibility(!currentSecurityVisible);
  }

  function getInternetBridge() {
    const bridge = window.AAYS_INTERNET;
    if (!bridge) return null;
    const hasActivate = typeof bridge.activate === "function";
    const hasDeactivate = typeof bridge.deactivate === "function";
    const hasToggle = typeof bridge.toggle === "function";
    if (!(hasActivate || hasDeactivate || hasToggle)) return null;
    return bridge;
  }

  function isInternetLayerVisible() {
    const bridge = getInternetBridge();
    if (!bridge) return false;
    try {
      if (typeof bridge.isActive === "function") {
        return Boolean(bridge.isActive());
      }
  ...
  async function setInternetLayerVisibility(nextVisible, options = {}) {
    if (!window.AAYS_INTERNET) {
      const loaded = await ensureRuntimeScriptLoaded("AAYS_INTERNET", "./internet_access_overlay.js?v=20260525-internet-layer-v1");
      if (!loaded) {
        emitLayerRuntimeEvent("internet", "runtime_load_failed", "internet_access_overlay.js");
      }
    }
    const bridge = getInternetBridge();
    if (!bridge) {
      if (!options.silent) {
        showThrottledStatus("internet_runtime_missing", "Internet katmani runtime bulunamadi. internet_access_overlay.js yuklenmeli.", true, 6000);
      }
      emitLayerRuntimeEvent("internet", "runtime_missing", "bridge_unavailable");
      updateMapModeControlState();
      return false;
    }

    const targetVisible = Boolean(nextVisible);
    try {
      if (targetVisible) {
        if (typeof bridge.activate === "function") {
          await bridge.activate();
        } else if (typeof bridge.toggle === "function" && !isInternetLayerVisible()) {
          await bridge.toggle();
        }
      } else if (typeof bridge.deactivate === "function") {
        bridge.deactivate();
      ...
      if (!options.silent) {
        showStatus(`Internet katmani hatasi: ${error?.message || "bilinmeyen hata"}`, true);
      }
      emitLayerRuntimeEvent("internet", "activate_failed", error?.message || "unknown");
      updateMapModeControlState();
      return false;
    }

    const active = isInternetLayerVisible();
    if (!options.silent) {
      if (targetVisible && active) {
        const state = typeof bridge.getState === "function" ? bridge.getState() : null;
        const featureCount = Number(state?.featureCount);
        if (Number.isFinite(featureCount) && featureCount <= 0) {
          showStatus("Internet katmani acildi, bu bbox icin veri yok.", true);
        } else {
          const countText = Number.isFinite(featureCount) ? ` (${featureCount} feature)` : "";
          showStatus(`Internet katmani acildi${countText}.`);
        }
        emitLayerRuntimeEvent("internet", "active", Number.isFinite(featureCount) ? `feature_count=${featureCount}` : "");
      } else if (targetVisible && !active) {
        const state = typeof bridge.getState === "function" ? bridge.getState() : null;
        const reason = state?.lastError ? ` (${state.lastError})` : "";
        showStatus(`Internet katmani acilamadi${reason}.`, true);
        emitLayerRuntimeEvent("internet", "inactive_after_activate", state?.lastError || "");
      } else {
        showStatus("Internet katmani kapatildi.");
        emitLayerRuntimeEvent("internet", "inactive", "manual_deactivate");
      }
    }
    updateMapModeControlState();
    return active === targetVisible;
  }

  function toggleInternetLayer() {
    void setInternetLayerVisibility(!isInternetLayerVisible());
  }
  function getParcelElevationFromFeature(feature) {
    const props = feature?.properties || {};
    const valueCandidates = [
      props.center_elevation_m,
      props.elevation_m,
      props.avg_elevation_m,
      props.mean_elevation_m,
      props.dem_elevation_m,
      props.topography_elevation_m,
    ];
  ...
  function getParcelElevationCacheKey(feature) {
    const props = feature?.properties || {};
    return String(
      props.parcel_id
      || props.parcelId
      || props.matched_parcel_id
      || props.PARCEL_ID
      || props.parcel_ref
      || props.inspire_id
      || ""
  ...
  function getFeatureCenterForTopographyLookup(feature) {
    const props = feature?.properties || {};
    const latCandidates = [props.merkez_lat, props.centroid_lat, props.lat, props.latitude];
    const lonCandidates = [props.merkez_lon, props.centroid_lon, props.lon, props.longitude];
    for (let i = 0; i < latCandidates.length; i += 1) {
      const lat = Number(latCandidates[i]);
      const lon = Number(lonCandidates[i]);
      if (Number.isFinite(lat) && Number.isFinite(lon)) {
        return { lat, lon };
      }
    }
  ...
  async function fetchParcelElevationForPopup(feature) {
    const cacheKey = getParcelElevationCacheKey(feature);
    if (!cacheKey) return;
    if (parcelElevationCache.has(cacheKey)) return;
    const center = getFeatureCenterForTopographyLookup(feature);
    let url = `${TOPOGRAPHY_LOOKUP_BASE_URL}${encodeURIComponent(cacheKey)}`;
    if (center) {
      url += `&lat=${encodeURIComponent(center.lat)}&lon=${encodeURIComponent(center.lon)}`;
    }
    try {
  ...
  function getFacilityStatusMessage() {
    ...
    parts.push(`Görünüm: ${getMapViewModeLabel(currentMapViewMode)}`);
  ...
  function setMapViewMode(nextMode, forceRefresh = true) {
    ...
    if (currentMapViewMode === "map_only" || currentMapViewMode === "land_use") {
      worthMenuOpen = false;
      if (currentPlannedAssetsVisible) {
        setPlannedAssetsLayerVisibility(false, { silent: true });
      }
      if (currentFutureGrowthVisible) {
        setFutureGrowthLayerVisibility(false, { silent: true });
      }
      if (isEmissionsLayerVisible()) {
        void setEmissionsLayerVisibility(false, { silent: true });
      }
      if (currentSecurityVisible) {
        void setSecurityOverlayVisibility(false, { silent: true });
      }
      if (isInternetLayerVisible()) {
        void setInternetLayerVisibility(false, { silent: true });
      }
    }
  ...
  function ensureOfficialSalesModeFocus() {
    ...
    if (Number.isFinite(zoom) && zoom < LAND_SIGNAL_MIN_ZOOM) {
      ...
      showStatus(`SatÄ±ÅŸa uygun parcel katmani icin harita ${LAND_SIGNAL_MIN_ZOOM}+ zoom seviyesine getirildi.`, true);
    }
  }
  ...
  function getEmissionsConfidenceLabel(value) {
    const numeric = Number(value);
  ...
  function getEmissionsLevelMeta(value) {
    const numeric = Number(value);
    if (!Number.isFinite(numeric)) {
      return { label: "No Data", color: "#6b7280" };
    }
    if (numeric <= 20) return { label: "Very Low Emission", color: "#16a34a" };
    if (numeric <= 40) return { label: "Low Emission", color: "#84cc16" };
    if (numeric <= 60) return { label: "Medium Emission", color: "#facc15" };
    if (numeric <= 80) return { label: "High Emission", color: "#fb923c" };
    return { label: "Very High Emission", color: "#dc2626" };
  }

  function ensureInlineEmissionsBridge(forceRebind = false) {
    const existingBridge = window.AAYS_GAS_EMISSIONS;
    if (!forceRebind && existingBridge && (typeof existingBridge.activate === "function" || typeof existingBridge.toggle === "function")) {
      return existingBridge;
    }
    ...
    let inlinePopup = null;
    let inlineActive = false;
    const directSourceMode = true;
    let sourceRecords = [];
    let sourceLookupByParcelId = new Map();
    let sourceLookupByParcelRef = new Map();
    let featureCount = 0;
    let lastError = null;
    ...
    let sourceFocusLngLat = null;
    let datasetLoadStatus = "idle";
    let datasetFeatureCount = 0;

    function computeSourceSpatialMetadata(features) {
      let west = Infinity;
      let south = Infinity;
      let east = -Infinity;
      let north = -Infinity;
      ...
      (Array.isArray(features) ? features : []).forEach((feature) => {
        ...
        if (!focus) focus = [lng, lat];
        west = Math.min(west, lng);
        south = Math.min(south, lat);
        east = Math.max(east, lng);
        north = Math.max(north, lat);
      });
      sourceFocusLngLat = focus;
      sourceBounds = Number.isFinite(west) && Number.isFinite(south) && Number.isFinite(east) && Number.isFinite(north)
    ...
    function ensureLegendElement() {
      ...
      legendEl.id = "aays-gas-emissions-legend";
      legendEl.style.position = "absolute";
      legendEl.style.right = "14px";
      legendEl.style.bottom = "18px";
      legendEl.style.zIndex = "6";
      legendEl.style.maxWidth = "220px";
      legendEl.style.padding = "10px 12px";
      legendEl.style.borderRadius = "10px";
      legendEl.style.background = "rgba(17, 24, 39, 0.92)";
      legendEl.style.color = "#f9fafb";
      legendEl.style.boxShadow = "0 8px 24px rgba(15, 23, 42, 0.35)";
    ...
    function renderLegend() {
      ...
      if (!el) return;
      const rows = [
        { label: "0-20% Very Low", color: "#16a34a" },
        { label: "21-40% Low", color: "#84cc16" },
        { label: "41-60% Medium", color: "#facc15" },
        { label: "61-80% High", color: "#fb923c" },
        { label: "81-100% Very High", color: "#dc2626" },
        { label: "No Data", color: "#6b7280" },
      ];
      const rowsHtml = rows.map((row) => `
        <div style="display:flex;align-items:center;gap:8px;margin-top:4px;">
          <span style="display:inline-block;width:12px;height:12px;border-radius:3px;background:${row.color};flex:none;"></span>
          <span>${escapeHtml(row.label)}</span>
        </div>
      `).join("");
      const meta = featureCount > 0
        ? `${featureCount} gorunen feature`
        : (lastError ? `Hata: ${lastError}` : "Bu gorunumde veri yok");
      const modeText = geometryMode === "polygon_join"
        ? "Parsel poligon esleme"
        : (geometryMode === "point_fallback"
          ? "Nokta fallback"
          : (geometryMode === "point_source" ? "GeoJSON point source" : "Katman hazir"));
      el.dataset.featureCount = String(featureCount || 0);
      el.dataset.sourceRecords = String(directSourceMode ? GAS_EMISSIONS_SOURCE_FEATURE_COUNT : (sourceRecords.length || 0));
      el.dataset.geometryMode = String(geometryMode || "none");
      el.dataset.lastError = String(lastError || "");
      el.dataset.datasetLoadStatus = String(datasetLoadStatus || "");
      el.dataset.datasetFeatureCount = String(datasetFeatureCount || 0);
      setSanitizedHtml(el, `
        <div style="font-weight:700;margin-bottom:6px;">Gas Emissions</div>
        <div style="opacity:0.85;margin-bottom:6px;">${escapeHtml(modeText)} | proxy source</div>
        ${rowsHtml}
        <div style="margin-top:8px;opacity:0.8;">${escapeHtml(meta)}</div>
      `);
    }

    function buildLookupIndex(features) {
      sourceLookupByParcelId = new Map();
      sourceLookupByParcelRef = new Map();
      sourceRecords = Array.isArray(features) ? features : [];
      computeSourceSpatialMetadata(sourceRecords);
      sourceRecords.forEach((feature) => {
        const props = feature?.properties || {};
        const parcelId = String(props.parcel_id || "").trim();
        const parcelRef = String(props.parcel_ref || "").trim();
        if (parcelId) sourceLookupByParcelId.set(parcelId, feature);
        if (parcelRef) sourceLookupByParcelRef.set(parcelRef, feature);
      });
    }

    async function ensureDatasetLoaded() {
      if (sourceRecords.length > 0) return true;
      try {
        datasetLoadStatus = "fetching";
        const response = await fetch(GAS_EMISSIONS_DATA_URL, { cache: "no-store" });
        if (!response.ok) {
          datasetLoadStatus = `http_${response.status}`;
          lastError = `http_${response.status}`;
          buildLookupIndex([]);
          datasetFeatureCount = 0;
          return false;
        }
        datasetLoadStatus = "reading_text";
        const rawText = await response.text();
        datasetLoadStatus = "parsing_json";
        const payload = JSON.parse(String(rawText || "").replace(/^\uFEFF/, ""));
        const features = Array.isArray(payload?.features) ? payload.features : [];
        datasetFeatureCount = features.length;
        datasetLoadStatus = features.length > 0 ? "loaded" : "loaded_empty";
        buildLookupIndex(features);
        lastError = null;
        return true;
      } catch (error) {
        datasetLoadStatus = "exception";
        datasetFeatureCount = 0;
        lastError = error?.message || "dataset_load_failed";
        buildLookupIndex([]);
        return false;
      }
    }

    function getLookupMatch(parcelFeature) {
      const props = parcelFeature?.properties || {};
      const parcelId = String(props.parcel_id || props.PARCEL_ID || "").trim();
      const parcelRef = String(props.parcel_ref || props.PARCEL_REF || props.inspire_id || "").trim();
      if (parcelId && sourceLookupByParcelId.has(parcelId)) {
        return { match: sourceLookupByParcelId.get(parcelId), matchingMethod: "parcel_id" };
      }
      if (parcelRef && sourceLookupByParcelRef.has(parcelRef)) {
        return { match: sourceLookupByParcelRef.get(parcelRef), matchingMethod: "parcel_ref" };
      }
      return null;
    }

    function buildEmissionsProps(parcelFeature, rawFeature, matchingMethod, geometrySource) {
      const baseProps = parcelFeature?.properties || {};
      const rawProps = rawFeature?.properties || {};
      const emissionPercent = Number(rawProps.emission_percent ?? rawProps.gas_emission_percent ?? rawProps.pollutionRiskPercent);
      const confidencePercent = Number(rawProps.confidencePercent ?? rawProps.confidence_percent);
      const meta = getEmissionsLevelMeta(emissionPercent);
      const nearestName = rawProps.airQualityNearestName || rawProps.nearest_name || "-";
      const nearestDistanceKm = Number(rawProps.airQualityNearestDistanceKm);
      const matching = matchingMethod || rawProps.matching_method || "point_proxy_no_parcel_polygon";
      const sourceDate = rawProps.last_updated || rawProps.calculated_at || rawProps.source_date || "-";
      const explanation = Number.isFinite(Number(rawProps.pollutionRiskPercent))
        ? "Emission percent uses pollutionRiskPercent (high=bad) from the air-quality proxy dataset."
        : "Emission percent is derived from the air-quality proxy dataset and mapped to a high=bad scale.";
      return {
        ...baseProps,
        parcel_id: baseProps.parcel_id || rawProps.parcel_id || null,
        parcel_ref: baseProps.parcel_ref || rawProps.parcel_ref || null,
        emission_percent: Number.isFinite(emissionPercent) ? emissionPercent : null,
        emission_level: meta.label,
        emission_color_hex: meta.color,
        confidence_percent: Number.isFinite(confidencePercent) ? confidencePercent : null,
        confidence_label: getEmissionsConfidenceLabel(confidencePercent),
        source_type: rawProps.source_type || "air_quality_proxy",
        source_date: sourceDate,
        matching_method: matching,
        nearest_source_name: nearestName,
        nearest_source_distance_km: Number.isFinite(nearestDistanceKm) ? nearestDistanceKm : null,
        gas_emissions_note: rawProps.gas_emissions_note || "Air pollution risk proxy; not official CO2e inventory.",
        calculation_explanation: explanation,
        geometry_source: geometrySource,
        air_quality_percent: Number(rawProps.airQualityPercent),
        pollution_risk_percent: Number(rawProps.pollutionRiskPercent),
      };
    }

    function buildVisiblePolygonFeatures() {
      const bbox = mapBoundsToBboxString();
      const visibleParcels = buildParcelUseFeaturesFromParcelTileSources(bbox, 6000);
      const dedupe = new Set();
      const output = [];
      visibleParcels.forEach((parcelFeature) => {
        const lookup = getLookupMatch(parcelFeature);
        if (!lookup) return;
        const props = parcelFeature?.properties || {};
        const dedupeKey = String(props.parcel_id || props.parcel_ref || props.inspire_id || output.length);
        if (dedupe.has(dedupeKey)) return;
        dedupe.add(dedupeKey);
        output.push({
          type: "Feature",
          geometry: parcelFeature.geometry,
          properties: buildEmissionsProps(parcelFeature, lookup.match, lookup.matchingMethod, "visible_parcel_tile_join"),
        });
      });
      return output;
    }

    function buildFallbackPointFeatures() {
      const bbox = parseBboxString(mapBoundsToBboxString());
      if (!bbox) return [];
      const output = [];
      sourceRecords.forEach((feature) => {
        ...
        output.push({
          ...
          properties: buildEmissionsProps({ properties: {} }, feature, "point_proxy_no_parcel_polygon", "point_fallback"),
        });
      });
      return output;
    }

    async function focusToData() {
      if (!map || typeof map.easeTo !== "function") return false;
      if (!Array.isArray(sourceFocusLngLat) || sourceFocusLngLat.length < 2) return false;
      return new Promise((resolve) => {
        ...
        try {
          if (typeof map.once === "function") {
            map.once("moveend", () => finish(true));
          }
          map.easeTo({
            center: sourceFocusLngLat,
    ...
    function buildPopupHtml(props) {
      const emissionPercent = Number(props.emission_percent);
      const confidencePercent = Number(props.confidence_percent);
      const nearestDistance = Number(props.nearest_source_distance_km);
      return [
        '<div class="aays-security-popup">',
        '<div class="aays-security-popup-title">Gas Emissions</div>',
        popupRow("Parcel ID", props.parcel_id || props.parcel_ref || "-"),
        popupRow("Gas Emission", Number.isFinite(emissionPercent) ? `${formatNumber(emissionPercent, 1)}%` : "-"),
        popupRow("Level", props.emission_level || "-"),
        popupRow("Risk Color", props.emission_color_hex || "-"),
        popupRow("Source Confidence", Number.isFinite(confidencePercent) ? `${formatNumber(confidencePercent, 1)}/100 (${props.confidence_label || "Unknown"})` : "-"),
        popupRow("Source Type", props.source_type || "-"),
        popupRow("Source Evidence", props.nearest_source_name || props.gas_emissions_note || "-"),
        popupRow("Source Distance", Number.isFinite(nearestDistance) ? `${formatNumber(nearestDistance, 2)} km` : "-"),
        popupRow("Source Date", props.source_date || "-"),
        popupRow("Matching Method", props.matching_method || "-"),
        popupRow("Calculation", props.calculation_explanation || "-"),
        popupRow("Geometry Mode", props.geometry_source || "-"),
        "</div>",
      ].join("");
    }
    ...
    function attachPopup(layerId) {
      if (!map.getLayer(layerId)) return;
      map.__aaysGasPopupBound = map.__aaysGasPopupBound || {};
      if (map.__aaysGasPopupBound[layerId]) return;
      map.__aaysGasPopupBound[layerId] = true;
      map.on("click", layerId, (event) => {
        const feature = event?.features?.[0];
        if (!feature) return;
        if (inlinePopup) inlinePopup.remove();
        inlinePopup = new maplibregl.Popup({ closeButton: true, closeOnClick: true, maxWidth: "360px" })
          .setLngLat(event.lngLat)
    ...
    function setInlineVisibility(visibility) {
      [GAS_EMISSIONS_FILL_LAYER_ID, GAS_EMISSIONS_LINE_LAYER_ID, GAS_EMISSIONS_POINT_LAYER_ID].forEach((layerId) => {
        if (map.getLayer(layerId)) {
          map.setLayoutProperty(layerId, "visibility", visibility);
        }
      });
    ...
    function colorExpression() {
      return [
        "step",
        ["coalesce", ["get", "emission_percent"], -1],
        "#6b7280",
        0, "#16a34a",
        21, "#84cc16",
        41, "#facc15",
    ...
    function ensureInlineLayers() {
      if (!map || !map.isStyleLoaded || !map.isStyleLoaded()) {
        setTimeout(ensureInlineLayers, 300);
        return false;
      }
      if (!map.getSource(GAS_EMISSIONS_SOURCE_ID)) {
        map.addSource(GAS_EMISSIONS_SOURCE_ID, {
          type: "geojson",
          data: directSourceMode ? GAS_EMISSIONS_DATA_URL : createEmptyFeatureCollection(),
          generateId: true,
        });
      }
      if (!map.getLayer(GAS_EMISSIONS_FILL_LAYER_ID)) {
        map.addLayer({
      ...
      if (!map.getLayer(GAS_EMISSIONS_LINE_LAYER_ID)) {
        map.addLayer({
      ...
      if (!map.getLayer(GAS_EMISSIONS_POINT_LAYER_ID)) {
        map.addLayer({
          id: GAS_EMISSIONS_POINT_LAYER_ID,
          type: "circle",
          source: GAS_EMISSIONS_SOURCE_ID,
          filter: ["==", ["geometry-type"], "Point"],
          paint: {
            "circle-color": colorExpression(),
            "circle-radius": ["interpolate", ["linear"], ["zoom"], 5, 2.5, 9, 4, 13, 6, 16, 8],
            "circle-opacity": 0.88,
            "circle-stroke-color": "#111827",
      ...
      attachPopup(GAS_EMISSIONS_FILL_LAYER_ID);
      attachPopup(GAS_EMISSIONS_LINE_LAYER_ID);
      attachPopup(GAS_EMISSIONS_POINT_LAYER_ID);
      if (inlineActive) {
        setInlineVisibility("visible");
      }
      return true;
    ...
    async function ensureInlineLayersReady() {
      for (let attempt = 0; attempt < 20; attempt += 1) {
        const ready = ensureInlineLayers();
        if (ready && map?.getSource?.(GAS_EMISSIONS_SOURCE_ID)) {
          return true;
        }
        await new Promise((resolve) => window.setTimeout(resolve, 200));
      }
      return Boolean(map?.getSource?.(GAS_EMISSIONS_SOURCE_ID));
    }

    async function refresh() {
      if (!inlineActive || !map) return;
      ...
      refreshPromise = (async () => {
        if (directSourceMode) {
          ...
          sourceBounds = sourceBounds || GAS_EMISSIONS_SOURCE_BOUNDS;
          sourceFocusLngLat = sourceFocusLngLat || GAS_EMISSIONS_FOCUS_CENTER;
          datasetLoadStatus = "maplibre_direct_source";
          datasetFeatureCount = GAS_EMISSIONS_SOURCE_FEATURE_COUNT;
          featureCount = GAS_EMISSIONS_SOURCE_FEATURE_COUNT;
        ...
        const loaded = await ensureDatasetLoaded();
        const layersReady = await ensureInlineLayersReady();
        if (!layersReady) {
          lastError = "style_or_source_not_ready";
          featureCount = 0;
          geometryMode = "none";
          renderLegend();
          refreshPromise = null;
        ...
        let features = loaded ? buildVisiblePolygonFeatures() : [];
        geometryMode = "polygon_join";
        if (features.length === 0 && loaded) {
          features = buildFallbackPointFeatures();
          geometryMode = features.length > 0 ? "point_fallback" : "none";
        }
        featureCount = features.length;
        const source = map.getSource(GAS_EMISSIONS_SOURCE_ID);
        ...
        featureCount = 0;
        geometryMode = "none";
        const source = map && map.getSource ? map.getSource(GAS_EMISSIONS_SOURCE_ID) : null;
        if (source && typeof source.setData === "function") {
    ...
    if (map && !map.__aaysGasRefreshBound) {
      map.__aaysGasRefreshBound = true;
      map.on("moveend", () => {
        if (inlineActive) {
          void refresh();
        }
    ...
    const bridge = {
      async activate() {
        inlineActive = true;
        if (document.body) document.body.dataset.aaysGasBridgePhase = "activate_start";
        lastError = null;
        if (directSourceMode) {
          await ensureDatasetLoaded();
          if (sourceRecords.length === 0) {
          ...
          if (document.body) document.body.dataset.aaysGasBridgePhase = "activate_done";
          return;
        }
        ensureInlineLayers();
        setInlineVisibility("visible");
        renderLegend();
        setLegendVisible(true);
        if (document.body) document.body.dataset.aaysGasBridgePhase = "activate_before_refresh";
        await refresh();
        if (document.body) document.body.dataset.aaysGasBridgePhase = "activate_done";
      },
      deactivate() {
        inlineActive = false;
        if (document.body) document.body.dataset.aaysGasBridgePhase = "deactivate";
        setInlineVisibility("none");
      ...
      getState() {
        return {
          active: inlineActive,
          featureCount,
          sourceRecords: directSourceMode ? GAS_EMISSIONS_SOURCE_FEATURE_COUNT : sourceRecords.length,
          geometryMode,
          lastError,
    ...
    window.AAYS_GAS_EMISSIONS = bridge;
    return bridge;
  }

  function getEmissionsBridge() {
    const bridge = window.AAYS_GAS_EMISSIONS;
    if (!bridge) return null;
    if (typeof bridge.activate !== "function" && typeof bridge.toggle !== "function") return null;
    return bridge;
  }

  function isEmissionsLayerVisible() {
    const bridge = getEmissionsBridge();
    if (!bridge) return false;
    try {
      if (typeof bridge.isActive === "function") {
        return Boolean(bridge.isActive());
  ...
  async function setEmissionsLayerVisibility(nextVisible, options = {}) {
    const bridge = ensureInlineEmissionsBridge(true);
    if (!bridge) {
      if (!options.silent) {
        showThrottledStatus("gas_emissions_runtime_missing", "Gaz emisyonu runtime bridge bulunamadi.", true, 6000);
      }
      updateMapModeControlState();
      return false;
    }
    if (nextVisible && !isParcelViewMode()) {
      setMapViewMode("parcel", false);
    }
    const targetVisible = Boolean(nextVisible);
    try {
      if (targetVisible) {
        await bridge.activate();
        const firstState = typeof bridge.getState === "function" ? bridge.getState() : null;
        if ((firstState?.geometryMode === "point_source" && typeof bridge.focusToData === "function")
          || ((Number(firstState?.featureCount) || 0) === 0 && (Number(firstState?.sourceRecords) || 0) > 0 && typeof bridge.focusToData === "function")) {
          await bridge.focusToData();
          if (firstState?.geometryMode !== "point_source") {
      ...
      if (!options.silent) {
        showStatus(`Gaz emisyonu katmani hatasi: ${error?.message || "bilinmeyen hata"}`, true);
      }
      emitLayerRuntimeEvent("gas_emissions", "activate_failed", error?.message || "unknown");
      updateMapModeControlState();
      return false;
    }
    const active = isEmissionsLayerVisible();
    if (!options.silent) {
      if (targetVisible && active) {
        const state = typeof bridge.getState === "function" ? bridge.getState() : null;
        const currentFeatureCount = Number(state?.featureCount);
        if (Number.isFinite(currentFeatureCount) && currentFeatureCount > 0) {
          showStatus(`Gaz emisyonu katmani acildi (${currentFeatureCount} feature).`);
        } else {
          const reason = state?.lastError ? ` (${state.lastError})` : "";
          showStatus(`Gaz emisyonu katmani acildi, bu gorunumde veri yok${reason}.`, true);
        }
        emitLayerRuntimeEvent("gas_emissions", "active", Number.isFinite(currentFeatureCount) ? `feature_count=${currentFeatureCount}` : "");
      } else {
        showStatus("Gaz emisyonu katmani kapatildi.");
        emitLayerRuntimeEvent("gas_emissions", "inactive", "manual_deactivate");
      }
    }
    updateMapModeControlState();
    return active === targetVisible;
  ...
  function toggleEmissionsLayer() {
    void setEmissionsLayerVisibility(!isEmissionsLayerVisible());
  }

  let gasEmissionsPopupLookupPromise = null;
  let gasEmissionsPopupLookupByParcelId = new Map();
  let gasEmissionsPopupLookupByParcelRef = new Map();

  function normalizeGasLookupKey(value) {
    return String(value ?? "").trim();
  }

  function getGasEmissionParcelLookupKeysFromFeature(feature) {
    const props = feature?.properties || {};
    const keys = [
      props.parcel_id,
      props.PARCEL_ID,
      ...
      props.parsel_ref,
      props.id,
      getParcelRef(feature),
    ]
      .map((value) => normalizeGasLookupKey(value))
      .filter(Boolean);
    return Array.from(new Set(keys));
  }

  function findGasEmissionsRecordForParcel(feature) {
    const lookupKeys = getGasEmissionParcelLookupKeysFromFeature(feature);
    for (const key of lookupKeys) {
      if (sourceLookupByParcelId?.has?.(key)) {
        const sourceFeature = sourceLookupByParcelId.get(key);
        return sourceFeature?.properties || sourceFeature || null;
      }
      if (sourceLookupByParcelRef?.has?.(key)) {
        const sourceFeature = sourceLookupByParcelRef.get(key);
        return sourceFeature?.properties || sourceFeature || null;
      }
      if (gasEmissionsPopupLookupByParcelId.has(key)) {
        return gasEmissionsPopupLookupByParcelId.get(key);
      }
      if (gasEmissionsPopupLookupByParcelRef.has(key)) {
        return gasEmissionsPopupLookupByParcelRef.get(key);
      }
    }
    return null;
  }
  ...
  async function ensureGasEmissionsPopupLookupLoaded() {
    if (gasEmissionsPopupLookupByParcelId.size > 0 || gasEmissionsPopupLookupByParcelRef.size > 0) {
      return true;
    }
    if (gasEmissionsPopupLookupPromise) return gasEmissionsPopupLookupPromise;
    gasEmissionsPopupLookupPromise = (async () => {
      try {
        const response = await fetch(GAS_EMISSIONS_DATA_URL, { cache: "no-store" });
        ...
        const payload = JSON.parse(String(rawText || "").replace(/^\uFEFF/, ""));
        const features = Array.isArray(payload?.features) ? payload.features : [];
        gasEmissionsPopupLookupByParcelId = new Map();
        gasEmissionsPopupLookupByParcelRef = new Map();
        features.forEach((feature) => {
          const props = feature?.properties || {};
          const parcelId = normalizeGasLookupKey(props.parcel_id);
          const parcelRef = normalizeGasLookupKey(props.parcel_ref);
          if (parcelId) gasEmissionsPopupLookupByParcelId.set(parcelId, props);
          if (parcelRef) gasEmissionsPopupLookupByParcelRef.set(parcelRef, props);
        });
        return true;
      } catch (_error) {
        return false;
      } finally {
        gasEmissionsPopupLookupPromise = null;
      }
    })();
    return gasEmissionsPopupLookupPromise;
  }

  function buildGasEmissionsPopupMetaHtml(feature) {
    if (!isEmissionsLayerVisible()) return "";
    const props = findGasEmissionsRecordForParcel(feature);
    if (!props) {
      return `<div class="parcel-popup-meta" style="margin-top:8px;">Gas Emissions: veri eslesmesi bulunamadi.</div>`;
    }
    const emissionPercent = Number(props.emission_percent ?? props.gas_emission_percent ?? props.pollutionRiskPercent);
    const confidencePercent = Number(props.confidencePercent ?? props.confidence_percent);
    const levelMeta = getEmissionsLevelMeta(emissionPercent);
    const nearestDistanceKm = Number(props.airQualityNearestDistanceKm ?? props.nearest_distance_km);
    return `
      <div class="parcel-popup-meta" style="margin-top:8px;">
        <strong>Gas Emissions</strong><br />
        Emission: ${Number.isFinite(emissionPercent) ? `${formatNumber(emissionPercent, 1)}%` : "-"}<br />
        Level: ${levelMeta.label}<br />
        Risk color: ${levelMeta.color}<br />
        Confidence: ${Number.isFinite(confidencePercent) ? `${formatNumber(confidencePercent, 1)}/100` : "-"} (${getEmissionsConfidenceLabel(confidencePercent)})<br />
        Source: ${props.source_type || "air_quality_proxy"}<br />
        Source date: ${props.last_updated || props.calculated_at || props.source_date || "-"}<br />
        Evidence: ${props.airQualityNearestName || props.gas_emissions_note || "-"}<br />
        ${Number.isFinite(nearestDistanceKm) ? `Source distance: ${formatNumber(nearestDistanceKm, 2)} km<br />` : ""}
        Method: ${props.airQuality?.method || props.matching_method || "air_quality_proxy"}<br />
        Explanation: ${props.gas_emissions_note || "Air pollution risk proxy; not official CO2e inventory."}
      </div>
    `;
  }

  function ensureStreetViewPanelOpen() {
    try {
      const launcher = document.getElementById("aays-rs-launcher");
      if (launcher && launcher.dataset.active !== "1") {
        launcher.click();
      }
    } catch (_error) {
      // no-op
  ...
  function setHistoricSalesLayerFromControl() {
  ...
  function toggleWorthMenu(forceOpen = null) {
    worthMenuOpen = typeof forceOpen === "boolean" ? forceOpen : !worthMenuOpen;
    updateMapModeControlState();
  }

  function runWorthMenuAction(actionId) {
    const action = String(actionId || "").trim();
    if (!action) return;
    const ensureParcelMode = () => {
      if (!isParcelViewMode()) {
        setMapViewMode("parcel", false);
      }
    };
    const actionMap = {
      planned: () => {
        ensureParcelMode();
        setPlannedAssetsLayerVisibility(!currentPlannedAssetsVisible, { silent: false });
        if (currentPlannedAssetsVisible) {
          void focusNearestPlannedStructure();
        }
        activeWorkspaceScreen = "parcel";
        renderWorkspace();
      },
      cost: () => {
        ensureParcelMode();
        activeWorkspaceScreen = "scenario_select";
        renderWorkspace();
        showStatus("Maliyet menusu acildi.");
      },
      compare: () => {
        void setEmissionsLayerVisibility(!isEmissionsLayerVisible());
      },
      topography: () => {
        void toggleTopographyOverlay();
      },
      security: () => {
        toggleSecurityOverlay();
      },
      internet: () => {
        void toggleInternetLayer();
      },
      contact: () => {
        ensureParcelMode();
        activeWorkspaceScreen = "contact";
        renderWorkspace();
        showStatus("Kontakt menusu acildi.");
      },
      future_growth: () => {
        ensureParcelMode();
        setFutureGrowthLayerVisibility(!currentFutureGrowthVisible, { silent: false });
        activeWorkspaceScreen = "scenario_compare";
        renderWorkspace();
        showStatus("Gelecek gelisim paneli acildi.");
      },
      legal: () => {
        showStatus("Legal menusu bir sonraki guncellemede acilacak.", true);
      },
    };
  ...
  function updateMapModeControlState() {
    ...
    if (worthModeButton) {
      worthModeButton.classList.toggle("is-active", worthMenuOpen);
      worthModeButton.title = worthMenuOpen ? "Deger menusunu kapat" : "Deger menusunu ac";
    }
    if (worthMenuEl) {
      worthMenuEl.classList.toggle("is-open", worthMenuOpen);
    }
    ...
    if (plannedModeButton) {
      plannedModeButton.classList.toggle("is-active", currentPlannedAssetsVisible);
      plannedModeButton.classList.remove("is-placeholder");
      plannedModeButton.title = currentPlannedAssetsVisible
        ? "Planlanan yapi katmanini kapat"
        : "Planlanan yapi katmanini ac";
    }
    ...
    if (emissionsModeButton) {
      emissionsModeButton.classList.toggle("is-active", isEmissionsLayerVisible());
      emissionsModeButton.classList.remove("is-placeholder");
      emissionsModeButton.title = isEmissionsLayerVisible()
        ? "Gaz emisyonu katmanini kapat"
        : "Gaz emisyonu katmanini ac";
    }
    if (securityModeButton) {
      ...
      securityModeButton.title = currentSecurityVisible
        ? "GÃ¼venlik katmanÄ±nÄ± kapat"
        : "GÃ¼venlik katmanÄ±nÄ± aÃ§";
    }
    if (internetModeButton) {
      const active = isInternetLayerVisible();
      internetModeButton.classList.toggle("is-active", active);
      internetModeButton.classList.remove("is-placeholder");
      internetModeButton.title = active
        ? "Ä°nternet katmanÄ±nÄ± kapat"
        : "Ä°nternet katmanÄ±nÄ± aÃ§";
    }
  }
  ...
  function createMapModeControl() {
    return {
      onAdd() {
        ...
        primaryRow.className = "map-mode-primary-row";

        MAP_MODE_PRIMARY_ITEMS.forEach((item) => {
          ...
          if (item.mode === WORTH_CONTROL_MODE) {
            worthModeButton = buttonEl;
            buttonEl.addEventListener("click", () => {
              toggleWorthMenu();
            });
          } else {
            ...
            buttonEl.addEventListener("click", () => {
              toggleWorthMenu(false);
              setMapViewMode(item.mapMode || item.mode, true);
              if ((item.mapMode || item.mode) === "map_only") {
                ensureStreetViewPanelOpen();
              }
            });
          }
          primaryRow.appendChild(buttonEl);
        });
        container.appendChild(primaryRow);

        worthMenuEl = document.createElement("div");
        worthMenuEl.className = "map-worth-menu";
        MAP_WORTH_MENU_ITEMS.forEach((item) => {
          const buttonEl = document.createElement("button");
          buttonEl.type = "button";
          buttonEl.className = "map-worth-item";
          buttonEl.title = item.label;
          buttonEl.setAttribute("aria-label", item.label);
          buttonEl.dataset.worthItem = item.id;
          const iconEl = document.createElement("img");
          iconEl.src = item.iconUrl;
          iconEl.alt = item.label;
          ...
          worthMenuButtons.set(item.id, buttonEl);
          buttonEl.addEventListener("click", () => {
            runWorthMenuAction(item.id);
          });
          worthMenuEl.appendChild(buttonEl);
        });
        container.appendChild(worthMenuEl);

        historicSalesModeButton = null;
        topographyModeButton = worthMenuButtons.get("topography") || null;
        plannedModeButton = worthMenuButtons.get("planned") || null;
        futureGrowthModeButton = worthMenuButtons.get("future_growth") || null;
        emissionsModeButton = worthMenuButtons.get("compare") || null;
        securityModeButton = worthMenuButtons.get("security") || null;
        internetModeButton = worthMenuButtons.get("internet") || null;
      ...
      onRemove() {
        ...
        worthMenuButtons.clear();
        worthModeButton = null;
        worthMenuEl = null;
        worthMenuOpen = false;
        topographyModeButton = null;
        plannedModeButton = null;
        futureGrowthModeButton = null;
        historicSalesModeButton = null;
        emissionsModeButton = null;
        securityModeButton = null;
        internetModeButton = null;
      },
    };
  }
  ...
  function updateLandFilterSummary() {
    ...
    if (landOverlayFilters.saleReadyQuickFilter !== "all") {
      parts.push(`hï¿½??Â±zlï¿½??Â± gÃ¶rÃ¼nÃ¼m: ${getSaleReadyQuickFilterLabel(landOverlayFilters.saleReadyQuickFilter)}`);
    }
    if (landOverlayFilters.reviewOnly) {
      parts.push("sadece review gereken kayï¿½??Â±tlar");
    }
    setSanitizedText(landFilterSummaryEl, parts.length
      ? `Aktif filtreler: ${parts.join(" Ã‚Â· ")}`
      : "Harita sinyalleri tÃ¼m confidence seviyeleriyle gÃ¶steriliyor.");
  ...
  function getLandFilterSummaryHtml() {
    ...
    if (landOverlayFilters.reviewOnly) {
      parts.push("Review-only gÃ¶rÃ¼nÃ¼m");
    }
    if (!parts.length) {
      return '<div class="workspace-note">Aktif filtre yok. Harita tÃ¼m uygun sinyalleri gÃ¶steriyor.</div>';
    }
      ...
      ? `
        <div class="global-sale-panel">
          <div class="global-sale-panel-header">
            <div class="nearest-sale-panel-title">Program Genelindeki Satï¿½??Â±ÅŸa Uygun Parcel Dizini</div>
            <div class="nearest-sale-panel-note">
              ${hasActiveLandFilters()
                ? `Aktif filtrelerle ${landOverlayState.globalSaleReadyVisibleCount} parcel gÃ¶rÃ¼nÃ¼yor. Temel dizinde ${landOverlayState.globalSaleReadyBaseCount} parcel var.`
                : `TÃ¼m yÃ¼klÃ¼ satï¿½??Â±ÅŸa uygun parcel seti burada. Uzak zoomda turuncu cluster olarak gÃ¶rÃ¼nÃ¼r; bir kayda tï¿½??Â±klayï¿½??Â±nca harita ilgili parsele uÃ§ar.`}
            </div>
          </div>
          <div class="global-sale-metrics">
            <span class="global-sale-metric"><strong>${landOverlayState.globalSaleReadyVisibleCount}</strong> toplam satï¿½??Â±ÅŸ sinyali</span>
            <span class="global-sale-metric"><strong>${landOverlayState.globalRealPriceCount}</strong> gerÃ§ek fiyatlï¿½??Â± kayï¿½??Â±t</span>
            <span class="global-sale-metric"><strong>${landOverlayState.globalNonDemoVisibleCount}</strong> demo hariÃ§ gÃ¶rÃ¼nen</span>
          </div>
              ...
              .map(
                (group) => `
                  ...
                  >
                    ...
                    <span>${group.realPrice} gerÃ§ek fiyat</span>
                  </button>
                `
              )
              .join("")}
              ...
              .map(
                (item) => `
                  <div class="nearest-sale-card">
                    ...
                    <div class="nearest-sale-link-row">${buildExternalSaleLink(item.saleUrl, item.saleLinkLabel || "Satï¿½??Â±ÅŸ adresine git")}</div>
                  </div>
                `
              )
              .join("") || '<div class="workspace-note">Aktif filtrelerle program genelinde gÃ¶rÃ¼nÃ¼r satï¿½??Â±ÅŸ parceli kalmadï¿½??Â±.</div>'}
          </div>
          <div class="nearest-sale-panel-note">Son global yenileme: ${formatLandOverlayDate(landOverlayState.globalSaleReadyLoadedAt)}</div>
        </div>
      `
      : '<div class="workspace-note">Program genelindeki satï¿½??Â±ÅŸa uygun parcel dizini yÃ¼kleniyor...</div>';
    if (historyOverviewMode) {
      globalSaleReadyBlock = `
        <div class="global-sale-panel">
          <div class="global-sale-panel-header">
            <div class="nearest-sale-panel-title">Program Genelindeki Gecmis SatÄ±ÅŸ Parcel Dizini</div>
            <div class="nearest-sale-panel-note">
    ...
    const currentViewSaleBlock = currentViewSaleItems.length
      ? `
        <div class="nearest-sale-panel">
          <div class="nearest-sale-panel-title">Bu GÃ¶rÃ¼nÃ¼mdeki Satï¿½??Â±ÅŸa Uygun Parceller</div>
          <div class="nearest-sale-panel-note">
            Haritada hareket ettikÃ§e bu liste gÃ¼ncellenir. Su anki gorunumde ${currentViewSaleVisibleCount} parcel var${currentViewSaleVisibleCount > currentViewSaleItems.length ? `; ilk ${currentViewSaleItems.length} kayit listeleniyor.` : "."}
          </div>
      ...
      `
      : `<div class="workspace-note">Bulundugun gorunumde henuz satï¿½??Â±ÅŸa uygun parcel bulunmadi. Haritayi kaydirinca veya biraz yakinlasinca bu mini panel dolacak.</div>`;
  ...
  function buildSaleReadyFeatureSummary(feature, options = {}) {
    ...
    return {
      ...
      saleLinkLabel: actionableLink?.label || "Satï¿½??Â±ÅŸ adresine git",
  ...
  async function fetchGlobalOverviewFeaturesBySignal({ signalKey, signalValue, label, cacheMap, fetchLimit, force = false }) {
    ...
    const highDetailZoom = Number.isFinite(zoom) && zoom >= 11;
    const pageLimit = Math.max(100, Math.min(Math.max(1, Number(fetchLimit || GLOBAL_OVERVIEW_PAGE_LIMIT)), highDetailZoom ? 1200 : 2500));
    const maxPages = Math.max(1, Math.min(Number(GLOBAL_OVERVIEW_MAX_PAGES || 1), highDetailZoom ? 2 : GLOBAL_OVERVIEW_MAX_PAGES));
    const cacheKey = `${signalKey}:${bbox || "all"}:limit=${pageLimit}:pages=${maxPages}`;
    if (!force) {
      const cached = getTimedCacheData(cacheMap, cacheKey);
      if (cached) {
        return cached || [];
      }
    }
    ...
    setTimedCacheData(cacheMap, cacheKey, collected);
    return collected;
  }
  ...
  async function fetchCombinedSalesLayerFeatures(force = false) {
    ...
    const fetchLimit = Math.max(
      400,
      Math.min(
        highDetailZoom ? 1200 : 2200,
        Math.max(
          Number(GLOBAL_SALE_READY_FETCH_LIMIT || 1000),
          Number(GLOBAL_HISTORY_FETCH_LIMIT || 1000),
    ...
    if (!force) {
      const cached = getTimedCacheData(combinedSalesLayerCache, cacheKey);
      if (cached) {
        return cached || [];
      }
    }
    ...
    const fetchPromise = (async () => {
      ...
      if (!payloadResponse.ok) {
        const fallbackFeatures = await loadLocalCombinedSalesFallbackFeatures();
        if (fallbackFeatures.length > 0) {
          showStatus("Canli satis katmani yanit vermedi. Yerel fallback satis katmani yuklendi.", true);
          setTimedCacheData(combinedSalesLayerCache, cacheKey, fallbackFeatures);
      ...
      const features = (Array.isArray(payload?.features) ? payload.features : [])
        ...
        .filter(Boolean);
      setTimedCacheData(combinedSalesLayerCache, cacheKey, features);
      return features;
    })();
    if (!force) {
  ...
  async function fetchGlobalSaleReadyFeatures(force = false) {
    ...
    const zoom = Number(map?.getZoom?.() || 0);
    const highDetailZoom = Number.isFinite(zoom) && zoom >= 11;
    const pageLimit = Math.max(100, Math.min(Math.max(1, Number(GLOBAL_SALE_READY_FETCH_LIMIT || GLOBAL_OVERVIEW_PAGE_LIMIT)), highDetailZoom ? 1200 : 2500));
    const maxPages = Math.max(1, Math.min(Number(GLOBAL_OVERVIEW_MAX_PAGES || 1), highDetailZoom ? 2 : GLOBAL_OVERVIEW_MAX_PAGES));
    const cacheKey = `market_listing_adapter:${bbox || "all"}:limit=${pageLimit}:pages=${maxPages}:status=${landOverlayFilters.listingStatus}`;
    if (!force) {
      const cached = getTimedCacheData(globalSaleReadyCache, cacheKey);
      if (cached) {
        return cached || [];
      }
    }
    ...
    const fetchPromise = (async () => {
      ...
      setTimedCacheData(globalSaleReadyCache, cacheKey, resolvedFeatures);
  ...
  async function fetchGlobalHistoricSaleFeatures(force = false) {
    const fetchLimit = Math.max(100, Math.min(Number(GLOBAL_HISTORY_FETCH_LIMIT || 1000), 2500));
    const bbox = getUnionRegionBboxString() || getLandOverlayBboxString();
    const cacheKey = `verified-history-centroids:${bbox || "all"}:limit=${fetchLimit}`;
    if (!force) {
      const cached = getTimedCacheData(globalHistoricSaleCache, cacheKey);
    ...
    const payloadResponse = await fetchJsonWithTimeout(`${landIntelligenceApiBaseUrl}/map/sales-history/parcels?${params.toString()}`, {
      label: "Verified gecmis satis merkezleri",
      fallback: createEmptyFeatureCollection,
      timeoutMs: LAND_MARKET_TIMEOUT_MS,
      backoffGroup: "land-verified-history-centroids",
      ignoreBackoff: true,
    });
    if (!payloadResponse.ok) {
      throw new Error(payloadResponse.error || "Verified gecmis satis merkezleri yuklenemedi.");
    }
    const payload = payloadResponse.data || createEmptyFeatureCollection();
    const features = (Array.isArray(payload?.features) ? payload.features : [])
      .map((feature) => normalizeCombinedSalesLayerFeature(feature))
      .filter(Boolean);
    setTimedCacheData(globalHistoricSaleCache, cacheKey, features);
    return features;
  }
  ...
  async function syncGlobalSaleReadyOverview(force = false) {
    ...
    try {
      ...
      landOverlayState = {
        ...
        globalSaleReadyLoadedAt: new Date().toISOString(),
        error: error?.message || (currentHistoricSalesVisible ? "Program geneli gecmis satis parcel dizini yuklenemedi." : "Program geneli satï¿½??Â±ÅŸ parcel dizini yÃ¼klenemedi."),
      };
    }
    updateLandSourceStatusPanel();
  }
  ...
  function updateNearestSaleJumpButton() {
    ...
    if (!shouldShow) {
      ...
      setSanitizedText(jumpNearestSaleBtnEl, "En Yakın Satışa Uygun Parsele Git");
      return;
    }
    setSanitizedText(jumpNearestSaleBtnEl, "En Yakın Satışa Uygun Parsele Git");
  }
  ...
  function buildVerifiedSalesHistoryHtml(props) {
    ...
    const rows = records.slice(0, 10).map((r) => {
      const price = r.sale_price_gbp ?? r.price_paid_gbp ?? r.price_paid;
      const area = r.sale_area_m2 ?? r.building_area_m2;
      const pricePerM2 = r.sale_price_per_m2_gbp ?? r.price_per_m2_gbp;
      const accuracy = r.accuracy_scale || r.parcel_match_accuracy || "D_UNKNOWN";
      const sourceUrl = String(r.source_url || "").trim();
      const sourceFile = String(r.source_file || "").trim();
      const evidenceHash = String(r.evidence_hash || "").trim();
      const sourceLink = sourceUrl
        ? `<a href="${escapeHtml(sourceUrl)}" target="_blank" rel="noopener noreferrer">source URL</a>`
        : "source URL unavailable";
      return '<div class="workspace-section">' +
        '<div class="workspace-row"><span>' +
        escapeHtml(String(r.sale_year || "-")) + ' &middot; ' + escapeHtml(String(r.property_type || "-")) +
        '</span><strong>' + escapeHtml(String(accuracy)) + '</strong></div>' +
        '<div class="workspace-row"><span>Price</span><strong>' +
        (price ? ('GBP ' + formatNumber(Number(price), 0)) : '-') + '</strong></div>' +
        '<div class="workspace-row"><span>Building area</span><strong>' +
        (area ? (formatNumber(Number(area), 0) + ' m&sup2;') : 'unknown') + '</strong></div>' +
        '<div class="workspace-row"><span>Price / m&sup2;</span><strong>' +
        (pricePerM2 ? ('GBP ' + formatNumber(Number(pricePerM2), 0)) : 'unknown') + '</strong></div>' +
        '<div class="workspace-row"><span>Evidence</span><strong>' + sourceLink + '</strong></div>' +
        '<div class="workspace-row"><span>Source file</span><strong>' + escapeHtml(sourceFile || "unknown") + '</strong></div>' +
        '<div class="workspace-row"><span>Evidence hash</span><strong>' + escapeHtml(evidenceHash || "unknown") + '</strong></div>' +
        '</div>';
    }).join("");
    ...
    return '<div class="workspace-section verified-sales-history-card">' +
      ...
      '<div class="metric-card"><div class="metric-card-label">Sale area</div><div class="metric-card-value">' + (p.latest_sale_area_m2 ? (formatNumber(Number(p.latest_sale_area_m2), 0) + ' m&sup2;') : 'unknown') + '</div></div>' +
      '<div class="metric-card"><div class="metric-card-label">Price per m&sup2;</div><div class="metric-card-value">' + (p.latest_sale_price_per_m2_gbp ? ('GBP ' + formatNumber(Number(p.latest_sale_price_per_m2_gbp), 0)) : 'unknown') + '</div></div>' +
      ...
      '<div class="metric-card"><div class="metric-card-label">Accuracy</div><div class="metric-card-value">' + escapeHtml(String(p.accuracy_scale || p.reliability_band || "-")) + '</div></div>' +
      '</div><div class="workspace-stack">' + (rows || '<div class="workspace-note">No sale record rows available.</div>') + '</div>' +
      '<div class="workspace-note">Official verified sale history only. Required chain: HMLR PPD &rarr; AddressBase UPRN &rarr; Title_No &rarr; NPS polygon &rarr; parcel.</div>' +
      '</div>';
  }
  ...
  function renderCompactLandIntelligenceCard(parcelRef) {
    ...
    return `
      <div class="workspace-section">
        ...
        <div class="workspace-note">Son kaynak gÃ¼ncellemesi: ${formatLandIntelligenceDate(filteredView.latestUpdatedAt || state.signals.latest_source_updated_at)}</div>
        ${warnings ? `<div class="workspace-stack">${warnings}</div>` : '<div class="workspace-note">Bu parsel icin ekstra warning yok.</div>'}
        <details class="workspace-section workspace-details">
          <summary class="workspace-details-summary">Kaynak Linkleri ve History Ã–zeti</summary>
          <div class="workspace-details-body workspace-stack">
            ${sourceLinks || '<div class="workspace-note">Aktif filtreyle eÅŸleÅŸen kaynak linki bulunamadi.</div>'}
  ...
  function updateCityInfo(city) {
    ...
    const landingText = city.landing_reason
      ...
      : "";
    const qaText = qa
      ? ` QA: ${qa.qa_status}. Ã–nerilen kodlar: ${(qa.recommended_codes || []).join(", ") || "-"}.`
      : "";
  ...
  async function focusCity(city) {
    ...
    if (city.status === "needs_alias_qa") {
      ...
      showStatus(`${city.city} icin en yakin parcel odaï¿½??Å¸ï¿½??Â±na gidildi. Merkez nokta parcel disinda kalabiliyor.`, true);
    } else {
  ...
  function buildExternalSaleLink(url, label = "Satï¿½??Â±ÅŸ adresine git") {
  ...
  function getListingFeatureLinkLabel(feature) {
    ...
    return "Satï¿½??Â±ÅŸ adresine git";
  }

  function resolveSaleRecordLink(record, sources = []) {
    if (!record) {
      return { url: null, label: "Satï¿½??Â±ÅŸ adresine git" };
    }
    if (record.external_url) {
      return { url: record.external_url, label: record.external_label || "Satï¿½??Â±ÅŸ adresine git" };
    }
    ...
    return {
      ...
      label: detail?.external_label || record.external_label || "Satï¿½??Â±ÅŸ adresine git",
    };
  }
  ...
  function resolveLandSourceLink(source) {
    ...
    return {
      ...
      label: detail?.external_label || "Kaynaï¿½??Å¸a git",
    };
  }
  ...
  function getScenarioPlanningTypeKey(scenarioName) {
    const key = String(scenarioName || "").trim().toLowerCase();
    if (key.includes("apartman")) return "apartman";
    if (key.includes("site")) return "site";
    if (key.includes("karma")) return "karma";
    if (key.includes("ticari")) return "perakende";
    if (key.includes("ofis")) return "ofis";
    if (key.includes("end")) return "endustriyel";
    if (key.includes("prefab")) return "prefab";
    if (key.includes("tarim")) return "tarimsal";
    if (key.includes("konut")) return "mustakil";
    return "karma";
  }

  function getScenarioApiBuildingTypeKey(scenarioName) {
    const key = String(scenarioName || "").trim().toLowerCase();
    if (key.includes("apartman")) return "apartment_block";
    if (key.includes("site")) return "residential_estate";
    if (key.includes("karma")) return "mixed_use";
    if (key.includes("ticari")) return "retail";
    if (key.includes("ofis")) return "office";
    if (key.includes("end")) return "industrial_unit";
    if (key.includes("prefab")) return "prefabricated";
    if (key.includes("tarim")) return "agricultural";
    if (key.includes("konut")) return "residential";
    return "mixed_use";
  }

  function getScenarioParcelId(parcel) {
    const props = parcel?.properties || {};
    const candidates = [props.parcel_id, props.parcelId, props.matched_parcel_id, props.PARCEL_ID, props.id];
  ...
  async function loadCostBuildingTypeOptions() {
    const nowTs = Date.now();
    if ((nowTs - Number(costBuildingTypeOptionsCache.ts || 0)) < 60000 && Array.isArray(costBuildingTypeOptionsCache.items) && costBuildingTypeOptionsCache.items.length) {
      return costBuildingTypeOptionsCache.items;
    }
    const response = await fetchJsonWithTimeout(`${landIntelligenceApiBaseUrl}/cost/building-types/options`, {
      label: "Cost building type options",
      fallback: { items: [] },
      backoffGroup: "cost-building-types",
    });
    if (response.ok) {
      const items = Array.isArray(response?.data?.items) ? response.data.items : [];
      costBuildingTypeOptionsCache = { ts: Date.now(), items };
      return items;
    }
    return [];
  }

  async function fetchRealScenarioModels(parcel) {
    if (!parcel || !landIntelligenceApiBaseUrl) return [];
    const parcelId = getScenarioParcelId(parcel);
    if (!(Number.isFinite(parcelId) && parcelId > 0)) {
      return [];
    }
    const cacheKey = `${parcelId}:${Math.round(Number(parcel.areaM2 || 0))}`;
    const cached = realScenarioModelCache.get(cacheKey);
    if (cached && (Date.now() - Number(cached.ts || 0)) < 60000) {
      return Array.isArray(cached.data) ? cached.data : [];
    }
    if (realScenarioModelInFlight.has(cacheKey)) {
      return await realScenarioModelInFlight.get(cacheKey);
    }

    const loader = (async () => {
      const scenarioNames = Array.isArray(WORKSPACE_SCENARIOS) && WORKSPACE_SCENARIOS.length
        ? WORKSPACE_SCENARIOS.slice()
        : ["Konut Apartmani", "Site", "Karma Kullanim", "Ticari", "Ofis", "Konut", "Endustriyel", "Prefabrik", "Tarimsal"];
      const allowedApiBuildingTypes = new Set(
        (await loadCostBuildingTypeOptions())
          .map((item) => String(item?.building_type_key || "").trim().toLowerCase())
          .filter(Boolean)
      );
      const grossArea = Math.max(900, Number(parcel.areaM2 || 0) * 0.65);
      const capacityM2 = Math.max(1, Number(parcel.areaM2 || 0));
      const estimatedSalePerM2 = Number(parcel?.properties?.latest_sale_price_per_m2_gbp || parcel?.properties?.price_per_m2_gbp || parcel?.properties?.latest_history_price_per_m2 || 0);
      const results = [];

      for (const name of scenarioNames) {
        const apiBuildingType = getScenarioApiBuildingTypeKey(name);
        const planningTypeKey = getScenarioPlanningTypeKey(name);
        if (allowedApiBuildingTypes.size && !allowedApiBuildingTypes.has(apiBuildingType)) {
          continue;
        }
        const payload = {
          parcel_id: parcelId,
          ...
          floors: 1,
          gross_internal_area_m2: grossArea,
          output_payload_files: false,
          db_write: false,
          production_deploy: false,
        };
        const response = await fetchJsonWithTimeout(`${landIntelligenceApiBaseUrl}/cost/estimate/preview`, {
          label: `Cost estimate preview ${name}`,
          timeoutMs: Math.max(API_FETCH_TIMEOUT_MS, 9000),
          fallback: null,
          backoffGroup: "cost-estimate-preview",
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
          dedupeKey: `cost-estimate-preview::${parcelId}::${apiBuildingType}::${Math.round(grossArea)}`,
        });
        ...
        const totalCost = Number(breakdown.total_cost);
        const buildCost = Number(breakdown.adjusted_build_cost);
        const unexpected = Number(breakdown.contingency);
        const landCost = Number(parcel.landCost || 0);
        const otherCosts = Math.max(0, totalCost - buildCost - landCost);
        if (!Number.isFinite(totalCost) || totalCost <= 0) {
          continue;
        }
        const revenue = Number.isFinite(estimatedSalePerM2) && estimatedSalePerM2 > 0
          ? estimatedSalePerM2 * grossArea
          : totalCost;
        const profit = revenue - totalCost;
        const roiPct = totalCost > 0 ? (profit / totalCost) * 100 : 0;
        const projectMonths = 24;
        const irrPct = totalCost > 0 ? ((revenue / totalCost) ** (12 / projectMonths) - 1) * 100 : 0;
        const typeConfig = getBuildingTypeConfigByKey(planningTypeKey);
        const utilization = Math.min(1.6, Math.max(0, grossArea / capacityM2));
        const equivalentUnits = Math.max(1, grossArea / Math.max(1, Number(typeConfig?.unitArea || grossArea)));
        results.push({
          name,
          buildingType: apiBuildingType,
          planningTypeKey,
          typeConfig,
          ...
          capacityM2,
          utilization,
          constructionCostPerM2: Number(breakdown.base_rate_gbp_per_gia_m2 || 0),
          salePricePerM2: estimatedSalePerM2,
          sellableAreaM2: grossArea,
          landCost,
          projectMonths,
          unexpectedPct: Number.isFinite(unexpected) && Number.isFinite(buildCost) && buildCost > 0 ? (unexpected / buildCost) : 0,
          buildCost: Number.isFinite(buildCost) ? buildCost : 0,
          unexpected: Number.isFinite(unexpected) ? unexpected : 0,
          otherCosts,
          totalCost,
          revenue,
          profit,
          roiPct,
          irrPct,
          confidenceScore: Number(estimate?.confidence_score || 0),
          confidenceBand: String(estimate?.confidence_band || "-")
        });
      }

      realScenarioModelCache.set(cacheKey, { ts: Date.now(), data: results });
      return results;
    })();

    realScenarioModelInFlight.set(cacheKey, loader);
    try {
      return await loader;
    } finally {
      realScenarioModelInFlight.delete(cacheKey);
    }
  }

  function scheduleRealScenarioModelLoad(parcel) {
    if (!AAYS_REAL_DATA_ONLY || !parcel) return;
    void fetchRealScenarioModels(parcel).then(() => {
      if (["recommendation", "scenario_select", "compare", "scenario_compare"].includes(activeWorkspaceScreen)) {
        renderWorkspace();
      }
    });
  }
  ...
  function generateScenarioModels(parcel) {
    ...
    const cacheKey = `${parcelId}:${Math.round(Number(parcel.areaM2 || 0))}`;
    const cached = realScenarioModelCache.get(cacheKey);
    if (cached && (Date.now() - Number(cached.ts || 0)) < 60000 && Array.isArray(cached.data)) {
      return cached.data;
    }
    scheduleRealScenarioModelLoad(parcel);
    return [];
  }

  function bestScenario(parcel) {
    const scenarios = generateScenarioModels(parcel);
    return scenarios.slice().sort((a, b) => b.irrPct - a.irrPct)[0] || null;
  }

  function resolveSourceExternalUrl(source) {
    ...
    const winner = candidates.find((candidate) => typeof candidate === "string" && /^https?:\/\//i.test(candidate.trim()));
    return winner ? winner.trim() : null;
  }

  function contractorContactsForEntry(entry) {
    const typeKey = String(entry?.scenario?.typeConfig?.key || "karma").trim().toLowerCase();
    const labels = {
      mustakil: "Mustakil",
      apartman: "Apartman",
      site: "Site",
    ...
    return {
      buildingType: labels[typeKey] || "Karma",
      company: "REAL_DATA_ONLY",
      contact: "REAL_DATA_ONLY",
      phone: "REAL_DATA_ONLY",
      email: "REAL_DATA_ONLY",
      note: "Demo kontakt fallback kapali. Kayitlar sadece /api/contractor/parcel/{id}/contacts uzerinden gelir.",
    };
  }

  function renderPurchaseChannelsTable(filteredView) {
    const sources = Array.isArray(filteredView?.filteredSources) ? filteredView.filteredSources.slice(0, 12) : [];
    const rows = sources
      .map((source) => {
        const sourceName = getLandSourceDisplayName(source?.source_name);
        const sourceStatus = getLandSourceStatus(source) || "-";
        const confidence = getLandSourceConfidence(source);
        const sourceUrl = resolveSourceExternalUrl(source);
        const confidenceText = confidence === null ? "-" : `${formatNumber(confidence, 1)}%`;
        const linkText = sourceUrl
          ? `<a href="${escapeHtml(sourceUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(sourceUrl)}</a>`
          : "-";
        const notes = source?.requires_review
          ? "Review gerekli"
          : (source?.match_method ? `match=${escapeHtml(source.match_method)}` : "Aktif");
        return `
          <tr>
    ...
    const safeRows = rows || `
      <tr>
        <td colspan="5">Bu parsel icin dogrulanmis canli kaynak linki bulunamadi.</td>
      </tr>
    `;
    return `
      <div class="settings-table-wrap">
  ...
  function getContactEntryParcelId(entry) {
    const parcelId = getFeatureParcelId(entry?.feature || null);
    if (Number.isFinite(parcelId) && parcelId > 0) {
      return parcelId;
    }
    const fallback = Number(entry?.ref);
  ...
  function getContactEntryStructureType(entry) {
    const direct = String(entry?.scenario?.typeConfig?.key || "").trim().toLowerCase();
    if (direct) return direct;
    const fromComponent = String(entry?.scenario?.components?.[0]?.typeKey || "").trim().toLowerCase();
    if (fromComponent) return fromComponent;
    return "";
  }

  function normalizeContactFlag(value) {
    return String(value || "").trim().toLowerCase() === "true";
  }

  function resolveContactStatus(row) {
    const legalScore = Number(row?.legal_contact_score);
    if (normalizeContactFlag(row?.do_not_contact)) return "DO_NOT_CONTACT";
    if (Number.isFinite(legalScore) && legalScore < 50) return "DO_NOT_CONTACT";
    const raw = String(row?.contact_status || "").trim().toUpperCase();
    return raw || "READY";
  }

  function buildContactEvidenceText(row) {
    const matchMethod = row?.match_method ? String(row.match_method) : "-";
    const matchScore = row?.match_score ? String(row.match_score) : "-";
    const reliability = row?.reliability_score ? String(row.reliability_score) : "-";
    const confidence = row?.data_confidence_score ? String(row.data_confidence_score) : "-";
    const legal = row?.legal_contact_score ? String(row.legal_contact_score) : "-";
    const officeAddress = row?.registered_office_address ? String(row.registered_office_address) : "-";
    const regionLabel = row?.region ? String(row.region) : "-";
    const activityLabel = row?.activity_density_label ? String(row.activity_density_label) : "-";
    const sourceUrl = row?.company_source_url ? String(row.company_source_url) : "";
    const sourceLink = sourceUrl ? `<a href="${escapeHtml(sourceUrl)}" target="_blank" rel="noopener noreferrer">source</a>` : "-";
    return `method=${escapeHtml(matchMethod)} score=${escapeHtml(matchScore)} | R=${escapeHtml(reliability)} C=${escapeHtml(confidence)} L=${escapeHtml(legal)} | bolge=${escapeHtml(regionLabel)} yogunluk=${escapeHtml(activityLabel)} | adres=${escapeHtml(officeAddress)} | ${sourceLink}`;
  }

  function renderContactRowsFromApi(rows, entry, structureType) {
    const fallbackContact = contractorContactsForEntry(entry);
    const buildingTypeLabel = fallbackContact.buildingType || "-";
    const scenarioName = entry?.scenario?.name || "-";
    if (!Array.isArray(rows) || !rows.length) {
      return `
        <tr>
          <td>${escapeHtml(buildingTypeLabel)}</td>
          <td>${escapeHtml(scenarioName)}</td>
          <td colspan="5">Bu parsel/yapi turu icin temas edilebilir emlakci kaydi bulunamadi.</td>
        </tr>
      `;
    }
    return rows.slice(0, 5).map((row) => {
      const status = resolveContactStatus(row);
      const blocked = status === "DO_NOT_CONTACT";
      const phone = blocked ? "SUPPRESSED_DO_NOT_CONTACT" : (row?.phone || row?.contact_phone || row?.telephone || "-");
      const email = blocked ? "SUPPRESSED_DO_NOT_CONTACT" : (row?.email || row?.contact_email || row?.website || "-");
      const company = row?.company_name || row?.contractor_id || "-";
      const note = buildContactEvidenceText(row);
      const typeLabel = row?.structure_type || structureType || buildingTypeLabel;
      return `
        <tr>
  ...
  function renderContractorContactsTableApiAware(entry) {
    const fallbackContact = contractorContactsForEntry(entry);
    const parcelId = getContactEntryParcelId(entry);
    const structureType = getContactEntryStructureType(entry);
    const scenarioName = entry?.scenario?.name || "-";
    return `
      <div class="settings-table-wrap" data-contractor-contact-table data-parcel-id="${parcelId || ""}" data-structure-type="${structureType}" data-scenario-name="${escapeHtml(scenarioName)}">
        <table class="settings-table">
          <thead>
            <tr>
              <th>Yapi Turu</th>
          ...
          <tbody data-contractor-contact-rows>
            <tr data-contractor-contact-loading="true">
              <td>${escapeHtml(fallbackContact.buildingType)}</td>
              <td>${escapeHtml(scenarioName)}</td>
              <td colspan="5">${parcelId ? "Emlakci onerileri yukleniyor..." : "Parsel ID bulunamadi, API sorgusu atlandi."}</td>
            </tr>
          </tbody>
        </table>
        <div class="workspace-note" data-contractor-contact-source-note>${parcelId ? `parcel_id=${parcelId} icin emlakci eslesmesi sorgulaniyor.` : "Parcel ID olmadigi icin API eslesmesi yapilamadi."}</div>
      </div>
    `;
  }

  async function hydrateContactContractorTables(compareEntries) {
    const renderTicket = ++contactScreenHydrationTicket;
    const tables = Array.from(workspaceContentEl.querySelectorAll("[data-contractor-contact-table]"));
    if (!tables.length) return;
    const entryByParcelId = new Map();
    (compareEntries || []).forEach((entry) => {
      const parcelId = getContactEntryParcelId(entry);
      if (Number.isFinite(parcelId) && parcelId > 0 && !entryByParcelId.has(String(parcelId))) {
        entryByParcelId.set(String(parcelId), entry);
      }
    });

    for (const tableWrap of tables) {
      if (renderTicket !== contactScreenHydrationTicket || activeWorkspaceScreen !== "contact") {
        return;
      }
      const tbody = tableWrap.querySelector("[data-contractor-contact-rows]");
      const noteEl = tableWrap.querySelector("[data-contractor-contact-source-note]");
      if (!tbody) continue;
      const parcelId = Number(tableWrap.getAttribute("data-parcel-id"));
      const structureType = String(tableWrap.getAttribute("data-structure-type") || "").trim().toLowerCase();
      const entry = entryByParcelId.get(String(parcelId)) || (compareEntries || []).find((candidate) => String(getContactEntryParcelId(candidate) || "") === String(parcelId));
      if (!Number.isFinite(parcelId) || parcelId <= 0 || !entry) {
        tbody.innerHTML = renderContactRowsFromApi([], entry || {}, structureType);
        if (noteEl) noteEl.textContent = "Parsel ID bulunamadigi icin API sorgusu atlandi.";
        continue;
      }
      ...
      params.set("limit", "8");
      if (structureType) {
        params.set("structure_type", structureType);
      }
      const response = await fetchJsonWithTimeout(
        `/api/contractor/parcel/${parcelId}/contacts?${params.toString()}`,
        {
          label: `Contractor contacts parcel ${parcelId}`,
          backoffGroup: "contractor_contacts",
          fallback: { rows: [] },
        }
      );
      ...
      if (!response.ok) {
        tbody.innerHTML = renderContactRowsFromApi([], entry, structureType);
        if (noteEl) noteEl.textContent = `Emlakci API hatasi: ${response.error || "unknown error"}`;
        continue;
      }

      const payload = response.data || {};
      const rows = Array.isArray(payload.rows) ? payload.rows : [];
      tbody.innerHTML = renderContactRowsFromApi(rows, entry, structureType);
      if (noteEl) {
        noteEl.textContent = `parcel_id=${parcelId} | total=${payload.total_rows || 0} | ready=${payload.ready_rows || 0} | blocked=${payload.blocked_rows || 0} | mode=${payload.integration_mode || "-"} | source=${payload.source_matches_file || "-"}`;
      }
    }
  }
        ...
        (entry) => {
          ...
          const signalSummary = signalState.signals
            ? `
              ...
              ${filteredView.topManagedRecord ? `<div class="workspace-note">YÃ¶netimli kayï¿½??Â±t: <strong>${filteredView.topManagedRecord.parcel_name || filteredView.topManagedRecord.label || "-"}</strong> Ã‚Â· ${filteredView.topManagedRecord.listing_status || filteredView.topManagedRecord.status || "-"}</div>` : ""}
            `
            : `<div class="workspace-note">${signalState.loading ? "Kaynak sinyalleri yÃ¼kleniyor..." : "Kaynak sinyali henÃ¼z hazï¿½??Â±r deï¿½??Å¸il."}</div>`;
          return `
            <div class="workspace-section">
              <h3 class="workspace-section-title">Parsel ${entry.ref}</h3>
              <div class="metric-grid">
                <div class="metric-card"><div class="metric-card-label">Parsel ID</div><div class="metric-card-value">${entry.ref}</div></div>
                <div class="metric-card"><div class="metric-card-label">Arsa alanï¿½??Â± (mÃ‚Â²)</div><div class="metric-card-value">${formatNumber(entry.areaM2, 0)}</div></div>
              ...
              ${signalSummary}
              <div class="workspace-note">Bu parsel aÅŸaï¿½??Å¸ï¿½??Â±daki kanallarda listelenebilir. Her platform farklï¿½??Â± eriÅŸim ve fiyatlandï¿½??Â±rma/finansman akï¿½??Â±ÅŸï¿½??Â±na sahip olabilir.</div>
              <h4 class="workspace-section-title">Satï¿½??Â±n alma kanallarï¿½??Â± ve fiyat Ã¶zetleri</h4>
              ${renderPurchaseChannelsTable(filteredView)}
              <h4 class="workspace-section-title">Parsel grubu kapsamina gore emlakci iletisimleri</h4>
              ${renderContractorContactsTableApiAware(entry)}
            </div>
          `;
        }
      )
  ...
  function formatParcelFeature(feature) {
    ...
    return entries.length ? entries.join("\n") : "SeÃ§ili parcel Ã¶zelliï¿½??Å¸i bulunamadï¿½??Â±.";
  }
  ...
  function formatSalesOnlyFeatureSummary(feature) {
    ...
    const lines = [
      ...
      `Son SatÄ±ÅŸ Tarihi: ${props.latest_history_sale_date || "-"}`,
      `Son SatÄ±ÅŸ Fiyati: ${formatSalePrice(props.latest_history_price_paid)}`,
  ...
  function buildParcelPopupContent(feature, lngLat) {
    ...
    const topSaleLinkData = topSaleRecord ? resolveSaleRecordLink(topSaleRecord, filteredView?.filteredSources || []) : { url: null, label: "Satï¿½??Â±ÅŸ adresine git" };
    ...
    const elevationCacheKey = getParcelElevationCacheKey(feature);
    const elevationFromFeature = getParcelElevationFromFeature(feature);
    const elevationFromCache = elevationCacheKey ? parcelElevationCache.get(elevationCacheKey) : null;
    const parcelElevationMeters = Number.isFinite(Number(elevationFromFeature))
      ? Number(elevationFromFeature)
      : (Number.isFinite(Number(elevationFromCache)) ? Number(elevationFromCache) : null);
    ...
    if (isSalesOnlyModeActive()) {
      ...
      setSanitizedHtml(container, `
        ...
        <div class="signal-popup-list">
          <div><strong>Mod:</strong> Gecmis SatÄ±ÅŸ</div>
          ...
          <div><strong>Son SatÄ±ÅŸ Tarihi:</strong> ${latest?.sale_date || "-"}</div>
          <div><strong>Son SatÄ±ÅŸ Fiyati:</strong> ${latest?.price_paid ? formatSalePrice(Number(latest.price_paid)) : "-"}</div>
          ...
          <div><strong>Denizden yukseklik:</strong> ${parcelElevationText}</div>
        <div class="parcel-popup-row"><span>Bölge ortalamasından fark:</span> <strong>${formatElevationDifferenceFromRegionAverage(parcelElevationCache?.get?.(parcelId) ?? properties?.topography_lookup ?? properties?.elevation_lookup ?? properties)}</strong></div>
        </div>
      `);
      return container;
    }
      ...
      ${
        filteredView
          ? `<div class="parcel-popup-meta" style="margin-top:8px;">
              ...
              ${topSaleRecord ? `En ust kayï¿½??Â±t: ${topSaleRecord.parcel_name || topSaleRecord.label || topSaleRecord.listing_id || "-"} Ã‚Â· ${topSaleRecord.listing_status || topSaleRecord.status || "-"}<br />Fiyat: ${topSalePrice} Ã‚Â· ${topSaleTruth?.label || "Fiyat tipi bilinmiyor"}<br />Kaynak: ${topSaleTruth?.detail || "-"}<br />Link: ${topSaleLinkTruth?.label || "Link tipi bilinmiyor"} Ã‚Â· ${topSaleLinkTruth?.detail || "-"}<br />${buildExternalSaleLink(topSaleLinkData.url, topSaleLinkData.label)}` : "Satï¿½??Â±ÅŸ kaydï¿½??Â±: yok"}
            </div>`
          : ""
      }
      ${buildGasEmissionsPopupMetaHtml(feature)}
    ...
    actionButton.setAttribute("aria-label", compared ? "KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesinden Ã§ï¿½??Â±kar" : "KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesine ekle");
    actionButton.title = compared ? "KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesinden Ã§ï¿½??Â±kar" : "KarÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesine ekle";
    actionButton.addEventListener("click", (event) => {
      ...
      if (result.changed) {
        showStatus(`${parcel.ref} ${currentCompared ? "karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesinden Ã§ï¿½??Â±karï¿½??Â±ldï¿½??Â±" : "karÅŸï¿½??Â±laÅŸtï¿½??Â±rma listesine eklendi"}.`);
      }
  ...
  function openParcelPopup(feature, lngLat) {
    ...
    const elevationCacheKey = getParcelElevationCacheKey(feature);
    const hasElevationInFeature = Number.isFinite(getParcelElevationFromFeature(feature));
    if (!hasElevationInFeature && elevationCacheKey && !parcelElevationCache.has(elevationCacheKey)) {
      void fetchParcelElevationForPopup(feature).then(() => {
        if (!activeParcelPopupState?.feature) return;
        const activeKey = getParcelElevationCacheKey(activeParcelPopupState.feature);
        if (activeKey && activeKey === elevationCacheKey) {
          refreshActiveParcelPopup();
        }
      });
    }
    if (isEmissionsLayerVisible()) {
      void ensureGasEmissionsPopupLookupLoaded().then((loaded) => {
        if (!loaded || !activeParcelPopupState?.feature) return;
        const activeRef = getParcelRef(activeParcelPopupState.feature);
        if (activeRef && activeRef === getParcelRef(feature)) {
          refreshActiveParcelPopup();
        }
      });
    }
  ...
  function openSignalPopup(feature, lngLat, kind) {
    ...
    const actionableSaleLink = actionableSaleRecord
      ...
      : { url: null, label: "Satï¿½??Â±ÅŸ adresine git" };
    ...
    if (kind === "official_sale") {
      title = props.parcel_ref || props.inspire_id || props.parcel_id || "SatÄ±ÅŸa Uygun Parcel";
      rows = [
        ...
        { label: "Fiyat TÃ¼rÃ¼", value: actionableSaleTruth.label },
        { label: "Fiyat Notu", value: actionableSaleTruth.detail },
        { label: "Link TÃ¼rÃ¼", value: actionableSaleLinkTruth.label },
        { label: "Link Notu", value: actionableSaleLinkTruth.detail },
        { label: "SatÄ±ÅŸ adresi", value: buildExternalSaleLink(actionableSaleLink.url, actionableSaleLink.label) },
      ...
      title = props.parcel_ref || props.inspire_id || props.parcel_id || "Gecmis SatÄ±ÅŸ Parcel";
      ...
      rows = [
        ...
        { label: "SatÄ±ÅŸ yili", value: latestHistorySaleYear && latestHistorySaleYear !== "-" ? latestHistorySaleYear : "-" },
      ...
      tags = ["Gecmis SatÄ±ÅŸ", "Price Paid", Number(props.history_transaction_count) > 0 ? "Kayitli" : null].filter(Boolean);
    } else if (kind === "official_sale_footprint") {
      title = props.parcel_name || props.listing_id || "Satï¿½??Â±ÅŸ Alanï¿½??Â±";
      rows = [
        { label: "Satï¿½??Â±ÅŸ alanï¿½??Â±", value: props.parcel_name || props.listing_id || "-" },
        { label: "EÅŸleÅŸen parcel", value: props.matched_parcel_ref || props.matched_inspire_id || "-" },
        { label: "Authority", value: props.local_authority || "-" },
        ...
        { label: "Link TÃ¼rÃ¼", value: actionableSaleLinkTruth.label },
        { label: "Link Notu", value: actionableSaleLinkTruth.detail },
        { label: "Satï¿½??Â±ÅŸ adresi", value: buildExternalSaleLink(actionableSaleLink.url || getListingFeatureLink(feature), actionableSaleLink.label || getListingFeatureLinkLabel(feature)) },
        { label: "Confidence", value: props.confidence_score ? formatNumber(props.confidence_score, 1) : "-" },
      ...
      rows = [
        ...
        { label: "SatÄ±ÅŸ adresi", value: buildExternalSaleLink(props.external_url || props.source_url, props.external_label || "Ilana git") },
        { label: "Confidence", value: props.confidence_score || props.highest_confidence_score ? formatNumber(Number(props.confidence_score ?? props.highest_confidence_score), 1) : "-" },
        { label: "Last Updated", value: formatLandOverlayDate(props.source_updated_at || props.last_updated) },
        { label: "Review", value: props.requires_review ? "Gerekli" : "Yok" },
      ];
  ...
  function clearDynamicSources() {
    [
      ...
      FUTURE_GROWTH_VECTOR_LAYER_ID,
      PLANNED_ASSETS_FILL_LAYER_ID,
      PLANNED_ASSETS_LINE_LAYER_ID,
      PLANNED_ASSETS_POINT_LAYER_ID,
  ...
  function resolveParcelPmtilesUrl(parcelRegion) {
    const localCandidate = String(parcelRegion?.parcelsPmtilesUrl || "").trim();
    const remoteCandidate = String(parcelRegion?.pmtiles_hf_url || "").trim();
    const isLocalRuntime = /^(localhost|127\.0\.0\.1)$/i.test(window.location.hostname || "");
    const hasVerifiedLocalAsset = parcelRegion?.localPmtilesAvailable === true;
    const preferredCandidate = isLocalRuntime
      ? (hasVerifiedLocalAsset ? (localCandidate || remoteCandidate) : (remoteCandidate || null))
      : (remoteCandidate || localCandidate);
    if (!preferredCandidate) return null;
    try {
      return new URL(preferredCandidate, window.location.href);
    } catch (_error) {
      return null;
    }
  }
  ...
  function addParcelLayers(region) {
    config.regions.forEach((parcelRegion) => {
      const pmtilesUrl = resolveParcelPmtilesUrl(parcelRegion);
      if (!pmtilesUrl) {
        console.warn("Parcel PMTiles URL tanimi bulunamadi:", parcelRegion?.slug || "unknown-region");
        return;
      }
    ...
    map.addLayer({
      ...
      filter: [
        ...
        [
          ...
          ["!", ["has", "market_polygon_displayable"]],
        ],
      ],
      layout: {
  ...
  // AAYS_VIEWPORT_PARCEL_REGION_VISIBILITY_HELPER
  // Keeps Great Britain parcel coverage available, but only renders regional PMTiles layers
  // that intersect the current viewport. This avoids loading all GB parcel tiles on pan/zoom.
  function getCurrentViewportBboxForParcelVisibility(expandRatio = 0.35) {
    ...
    const lngPad = Math.max(0.02, Math.abs(east - west) * expandRatio);
    const latPad = Math.max(0.02, Math.abs(north - south) * expandRatio);
  ...
  function getRegionBboxForParcelVisibility(region) {
  ...
  function bboxIntersectsForParcelVisibility(a, b) {
    if (!a || !b) return true;
    return !(
      a.east < b.west
      || a.west > b.east
  ...
  function shouldRenderParcelRegionInCurrentViewport(region) {
    const viewportBbox = getCurrentViewportBboxForParcelVisibility(0.35);
    const regionBbox = getRegionBboxForParcelVisibility(region);

    if (!viewportBbox || !regionBbox) {
      return !activeRegion || activeRegion.slug === region?.slug;
    }

    return bboxIntersectsForParcelVisibility(viewportBbox, regionBbox);
  }

  function hasRenderableParcelPmtilesInViewport() {
    if (!map || !isParcelViewMode() || typeof map.queryRenderedFeatures !== "function") return false;
    return config.regions.some((region) => {
      if (!shouldRenderParcelRegionInCurrentViewport(region)) return false;
      const fillLayerId = getParcelFillLayerId(region);
      if (!map.getLayer(fillLayerId)) return false;
      try {
        const visibility = map.getLayoutProperty(fillLayerId, "visibility");
        if (visibility === "none") return false;
        const rendered = map.queryRenderedFeatures(undefined, { layers: [fillLayerId] }) || [];
        return rendered.length > 0;
      } catch (_error) {
        try {
          const rendered = map.queryRenderedFeatures({ layers: [fillLayerId] }) || [];
          return rendered.length > 0;
        } catch (_innerError) {
          return false;
        }
  ...
  function updateParcelLayerVisibility() {
    ...
    const showFallbackParcels = parcelVisible && !hasRenderableParcelPmtilesInViewport();
    config.regions.forEach((region) => {
      const regionVisible = parcelVisible && shouldRenderParcelRegionInCurrentViewport(region);
      ...
      if (map.getLayer(warmLayerId)) {
        const warmVisible = landUseMode && shouldRenderParcelRegionInCurrentViewport(region);
    ...
    [
      ...
      if (map.getLayer(layerId)) {
        const shouldShow = layerId.startsWith("fallback-") ? showFallbackParcels : parcelVisible;
  ...
  function getParcelFallbackKey(bbox, zoom) {
    const quantizedBbox = quantizeBboxString(bbox, PARCEL_FALLBACK_BBOX_DECIMALS);
    const zoomBucket = Math.round((Number(zoom) || 0) / PARCEL_FALLBACK_ZOOM_BUCKET) * PARCEL_FALLBACK_ZOOM_BUCKET;
    return `${quantizedBbox}|z=${zoomBucket.toFixed(2)}`;
  }

  function getParcelFallbackCachedPayload(key) {
    const cacheEntry = parcelFallbackCache.get(key);
    if (!cacheEntry) return null;
    if ((Date.now() - Number(cacheEntry.cachedAt || 0)) > PARCEL_FALLBACK_CACHE_TTL_MS) {
      parcelFallbackCache.delete(key);
      return null;
    }
    return cacheEntry.payload || null;
  }

  function saveParcelFallbackCache(key, payload) {
    if (parcelFallbackCache.size > 24) {
      const oldestKey = parcelFallbackCache.keys().next().value;
      if (oldestKey) {
        parcelFallbackCache.delete(oldestKey);
      }
    }
    parcelFallbackCache.set(key, {
      cachedAt: Date.now(),
      payload,
    });
  }

  async function fetchParcelFallbackPaged({ bbox, key, requestId }) {
    const features = [];
    let page = 0;
    let offset = 0;
    let fetchError = null;
    ...
    while (page < PARCEL_FALLBACK_MAX_PAGES) {
      const params = new URLSearchParams({
        bbox,
        exclude_demo: "true",
        limit: String(PARCEL_FALLBACK_PAGE_LIMIT),
        offset: String(offset),
      });
      const payloadResponse = await fetchJsonWithTimeout(`${landIntelligenceApiBaseUrl}/map/parcels?${params.toString()}`, {
        label: "Yerel parcel fallback",
        fallback: createEmptyFeatureCollection,
        timeoutMs: Math.max(API_FETCH_TIMEOUT_MS, 11000),
        backoffGroup: "parcel-fallback",
        dedupe: true,
        dedupeKey: `parcel_fallback:${key}:p${page}`,
      });
      if (requestId !== parcelFallbackRequestId) {
        return null;
      }
      if (payloadResponse.error) {
        fetchError = payloadResponse.error;
        break;
      }
      const chunkFeatures = Array.isArray(payloadResponse.data?.features) ? payloadResponse.data.features : [];
      if (!chunkFeatures.length) break;

      features.push(...chunkFeatures);
      if (features.length >= PARCEL_FALLBACK_MAX_FEATURES) {
        features.length = PARCEL_FALLBACK_MAX_FEATURES;
        truncated = true;
        break;
      }
      if (chunkFeatures.length < PARCEL_FALLBACK_PAGE_LIMIT) break;

      page += 1;
      offset += PARCEL_FALLBACK_PAGE_LIMIT;
    }
  ...
  async function syncParcelFallbackParcels(force = false) {
    ...
    if (hasRenderableParcelPmtilesInViewport()) {
      setGeoJsonSourceData("fallback-parcels", createEmptyFeatureCollection());
      parcelFallbackState = {
        ...parcelFallbackState,
        featureCount: 0,
    ...
    const bbox = mapBoundsToBboxString();
    if (!bbox) return;
    const key = getParcelFallbackKey(bbox, map.getZoom());
    if (!force && parcelFallbackState.lastBBoxKey === key) {
      return;
    }

    const cachedPayload = force ? null : getParcelFallbackCachedPayload(key);
    if (cachedPayload) {
      setGeoJsonSourceData("fallback-parcels", cachedPayload);
      const cachedFeatureCount = Array.isArray(cachedPayload?.features) ? cachedPayload.features.length : 0;
      parcelFallbackState = {
        featureCount: cachedFeatureCount,
        lastLoadedAt: new Date().toISOString(),
        lastBBoxKey: key,
        error: null,
      };
    ...
    try {
      const pagedResult = await fetchParcelFallbackPaged({ bbox, key, requestId });
      if (!pagedResult || requestId !== parcelFallbackRequestId) return;

      const payload = pagedResult.payload || createEmptyFeatureCollection();
      setGeoJsonSourceData("fallback-parcels", payload);
      const payloadFeatures = Array.isArray(payload?.features) ? payload.features : [];
      const payloadFeatureCount = payloadFeatures.length;
      if (!pagedResult.error) {
        saveParcelFallbackCache(key, payload);
      }
      parcelFallbackState = {
        featureCount: payloadFeatureCount,
        lastLoadedAt: new Date().toISOString(),
        lastBBoxKey: key,
        error: pagedResult.error || null,
      };
      if (!pagedResult.error && payloadFeatureCount === 0) {
        showThrottledStatus(
          "parcel_fallback_no_data",
          "Bu görünüm için gerçek parsel verisi bulunamadı.",
          true,
          6000
        );
      }
      if (pagedResult.truncated) {
        showThrottledStatus(
          "parcel_fallback_truncated",
          "Parseller yukleniyor: bu gorunumde cok fazla geometri var, kademeli gosterim uygulaniyor.",
          true,
          6000
        );
      }
      ...
      showThrottledStatus(
        "parcel_fallback_fetch_error",
        "Bu görünüm için gerçek parsel verisi şu anda alınamadı.",
        true,
        6500
      );
    }
  ...
  async function refreshLandIntelligenceMapLayers(force = false) {
    ...
    try {
      ...
      const [officialSaleResponse, historicSalesResponse, officialSaleFootprintsResponse, brownfieldResponse] = await Promise.all([
        ...
        shouldFetchHistoric
          ? fetchJsonWithTimeout(`${landIntelligenceApiBaseUrl}/map/sales-history/parcels?${historicSaleParams.toString()}&geometry_mode=polygon`, {
        ...
        shouldFetchFootprints
          ? fetchJsonWithTimeout(`${landIntelligenceApiBaseUrl}/map/listings?${officialSaleFootprintParams.toString()}`, {
              label: "SatÄ±ÅŸ footprint katmani",
              fallback: createEmptyFeatureCollection,
              timeoutMs: LAND_OVERLAY_TIMEOUT_MS,
  ...
  function scheduleLandIntelligenceRefresh(force = false) {
    ...
    }, force ? 180 : (PERF_PROFILE_AGGRESSIVE ? 420 : 640));
  }
  ...
  function scheduleGlobalOverviewRefresh(force = false) {
    ...
    }, force ? 220 : (PERF_PROFILE_AGGRESSIVE ? 520 : 820));
  }
  ...
  function bindParcelInteractions() {
    ...
    [PLANNED_ASSETS_FILL_LAYER_ID, PLANNED_ASSETS_LINE_LAYER_ID, PLANNED_ASSETS_POINT_LAYER_ID].forEach((layerId) => {
      map.on("click", layerId, (event) => {
        const feature = (event.features || [])[0];
        if (!feature) return;
        const centerPoint = geometryCenterLngLat(feature?.geometry) || event.lngLat;
        updateSelectedParcel(feature);
        if (centerPoint) {
          openPlannedAssetPopup(feature, centerPoint);
        }
      });
    });
    ...
    [...config.regions.map((region) => getParcelFillLayerId(region)), "fallback-parcels-fill", "sale-ready-overview-clusters", "sale-ready-overview-point", "official-sale-parcel-fill", "official-sale-parcel-outline-halo", "official-sale-parcel-line", "official-sale-parcel-line-top", "official-sale-footprint-fill", "official-sale-footprint-line", "historic-sale-parcel-fill", "historic-sale-parcel-outline-halo", "historic-sale-parcel-line", "brownfield-polygons-fill", "brownfield-points", FUTURE_GROWTH_FILL_LAYER_ID, FUTURE_GROWTH_POINT_LAYER_ID, PLANNED_ASSETS_FILL_LAYER_ID, PLANNED_ASSETS_LINE_LAYER_ID, PLANNED_ASSETS_POINT_LAYER_ID, "market-listing-polygons-fill", "market-listing-polygons-outline-halo", "market-listing-polygons-line", "market-listing-points", "facilities-polygons-fill", "facilities-polygons-line", "facilities-points", "parcel-use-parcels-fill", "parcel-use-parcels-line", "scenario-score-fill", "scenario-score-line", "poi-points", "poi-points-clusters"].forEach((layerId) => {
      map.on("mouseenter", layerId, () => {
        if (
  ...
  async function switchRegion(region, reason = "auto") {
    ...
    showStatus(`${region.label} yÃ¼kleniyor...`, true);
    if (parcelPopup) {
      parcelPopup.remove();
      parcelPopup = null;
      activeParcelPopupState = null;
  ...
  function bindControls() {
    setSanitizedHtml(regionSelectEl, `<option value="__AUTO__">Auto</option>${config.regions
      ...
      .join("")}`);
    if (citySelectEl) {
      setSanitizedHtml(citySelectEl, `<option value=\"__NONE__\">Şehir seçin</option>${cityFocusPoints
  ...
  window.__aaysActiveMap = map;
  window.__AAYS_MAP__ = map;
  ...
  function tyCard(id) {
    ...
    return `
      <div class="ty-candidate-metrics-card" data-ty-candidate-metrics="${tyEsc(id)}">
        <div class="ty-candidate-metrics-title">Satï¿½??Â±ÅŸ Geometrisi Aday Plan Bilgisi</div>
        <div class="ty-candidate-metrics-badge">Durum: derived / estimated candidate Ã‚Â· GÃ¼ven: ${tyConfidenceLabel(m.confidence)}</div>
        <table class="ty-candidate-metrics-table">
          <tr><td>Alan</td><td>${tyFmtM2(m.area_m2)}</td></tr>
          <tr><td>Hesaplanan poligon alanï¿½??Â±</td><td>${tyFmtM2(m.computed_area_m2)}</td></tr>
          <tr><td>Ã‡evre</td><td>${tyFmtM(m.perimeter_m)}</td></tr>
          <tr><td>Alan kaynaï¿½??Å¸ï¿½??Â±</td><td>${tyEsc(m.area_source)} ${m.area_raw ? "(" + tyEsc(m.area_raw) + ")" : ""}</td></tr>
          <tr><td>YÃ¶ntem</td><td>${tyEsc(m.method)}</td></tr>
          <tr><td>Belge/gÃ¶rsel kanï¿½??Â±tï¿½??Â±</td><td>${tyEsc(m.doc_evidence || "unknown")}</td></tr>
        </table>
        <div class="ty-candidate-metrics-subtitle">Kenar uzunluklarï¿½??Â±</div>
        ...
        <div class="ty-candidate-metrics-note">
          Bu geometri gerÃ§ek/original verified boundary deï¿½??Å¸ildir. Alan, konum, kaynak sinyalleri ve oranlardan tÃ¼retilmiÅŸ satï¿½??Â±ÅŸ geometrisi adayï¿½??Â±dï¿½??Â±r.
        </div>
      </div>
    `;
  }
...
(function () {
  ...
  const TY_RUNTIME_CONFIG =
    (typeof window !== "undefined" && (window.__AAYS_APP_CONFIG__ || window.__AAYS_CONFIG__)) || {};
  const ENABLE_TY_POPUP_AUGMENT = TY_RUNTIME_CONFIG.enablePopupSaleGeometryAugment === true;
  ...
  function tyInjectWorkspaceCard() {
    if (typeof selectedParcelFeature === "undefined" || !selectedParcelFeature) return;
    tyEnsureStyle();
  ...
  if (typeof buildParcelPopupContent === "function") {
  ...
  if (typeof updateSelectedParcel === "function") {
  ...
  if (typeof renderWorkspace === "function") {
    ...
    renderWorkspace = function () {
      ...
      try {
        setTimeout(() => {
          tyInjectWorkspaceCard();
          tySetOverlayFeature(typeof selectedParcelFeature !== "undefined" ? (selectedParcelFeature || null) : null);
        }, 0);
      } catch (err) {
...
/* AAYS_OVERLAY_API_FAIL_CLOSED_PATCH
   Purpose:
   - Prevent repeated slow/500 overlay API calls during map pan/zoom.
   - Do not create fake/demo data.
   - Return empty real-only FeatureCollection for failing map overlay endpoints.
   - Keep cost/contact real-only flows untouched.
*/
(function installAaysOverlayApiFailClosedPatch() {
  const CIRCUIT_TTL_MS = 60000;
  ...
  function isReadOnlyMapOverlayEndpoint(url) {
    if (!url) return false;
    ...
    return (
      ...
      || path === "/map/listings"
      || path === "/map/brownfield"
      || path === "/map/sales-history/combined"
      || path === "/facilities"
    );
  }

  function emptyFeatureCollection(endpoint, reason) {
    return {
      type: "FeatureCollection",
      features: [],
      source: "real_only_empty",
      real_data_only: true,
  ...
  function isCircuitOpen(url) {
    ...
    const until = circuit.get(key) || 0;
    return until > now();
  }

  function openCircuit(url, reason) {
    const key = circuitKey(url);
    circuit.set(key, now() + CIRCUIT_TTL_MS);
    console.warn("AAYS overlay API fail-closed circuit opened:", key, reason);
  }

  const originalFetch = window.fetch.bind(window);

  window.fetch = async function aaysOverlayFailClosedFetch(input, init) {
    const url = normalizeUrl(input && input.url ? input.url : input);

    if (isReadOnlyMapOverlayEndpoint(url) && isCircuitOpen(url)) {
      const payload = emptyFeatureCollection(url.pathname, "circuit_open_after_recent_backend_failure");
    ...
    try {
      ...
      if (isReadOnlyMapOverlayEndpoint(url) && response && response.status >= 500) {
        openCircuit(url, `http_${response.status}`);
        const payload = emptyFeatureCollection(url.pathname, `backend_${response.status}`);
        return new Response(JSON.stringify(payload), {
          status: 200,
          headers: { "Content-Type": "application/json" }
        });
      ...
      if (isReadOnlyMapOverlayEndpoint(url)) {
        openCircuit(url, error?.message || "network_error");
        const payload = emptyFeatureCollection(url.pathname, error?.message || "network_error");
        return new Response(JSON.stringify(payload), {
          status: 200,
          headers: { "Content-Type": "application/json" }
        });
  ...
  window.AAYS_OVERLAY_API_FAIL_CLOSED_PATCH = {
    status: () => ({
      installed: true,
      circuitCount: circuit.size,
      circuitKeys: Array.from(circuit.keys())
    }),
    reset: () => circuit.clear()
  };
})();
...
function normalizeElevationLookupResult(value) {
  ...
  if (typeof value === "number") {
    return { center_elevation_m: Number.isFinite(value) ? value : null, region_average_elevation_m: null, elevation_difference_from_region_average_m: null };
  }
  if (typeof value === "object") {
    const n = (v) => { const x = Number(v); return Number.isFinite(x) ? x : null; };
    return {
      center_elevation_m: n(value.center_elevation_m ?? value.elevation_m ?? value.height_m ?? value.altitude_m),
      region_average_elevation_m: n(value.region_average_elevation_m),
      elevation_difference_from_region_average_m: n(value.elevation_difference_from_region_average_m),
...
/* AAYS_LAYER_RUNTIME_GUARD_V1_START
   Scope: GET-only frontend runtime guard for /map/parcels, /map/listings, /map/sales-history/combined.
   No DB write. No migration. No DDL. No deploy. No fake/demo data.
*/
(function AAYS_LAYER_RUNTIME_GUARD_V1() {
  "use strict";

  if (typeof window === "undefined") return;
  if (window.__AAYS_LAYER_RUNTIME_GUARD_V1__) return;
  if (typeof window.fetch !== "function") return;

  window.__AAYS_LAYER_RUNTIME_GUARD_V1__ = true;

  const nativeFetch = window.fetch.bind(window);
  const cache = new Map();
  const inFlight = new Map();
  const latestAbortByEndpoint = new Map();
  ...
  const ENDPOINTS = [
    ...
    },
    {
      ...
      pattern: /\/map\/sales-history\/combined(?:\?|$)/,
      ttlMs: 60000,
      maxPayloadBytes: 1000000,
      quantizeDecimals: 4
    }
  ...
  function methodOf(input, init) {
    ...
    const fromInput = input && typeof input === "object" && "method" in input ? input.method : undefined;
  ...
  function toUrl(input) {
    try {
      ...
      if (input && typeof input === "object" && "url" in input) return new URL(input.url, window.location.origin);
    } catch (_) {}
    return null;
  }
  ...
  function endpointFor(url) {
    ...
    return ENDPOINTS.find((e) => e.pattern.test(path)) || null;
  }
  ...
  function quantizeUrl(url, decimals) {
    const next = new URL(url.toString());

    const bbox = next.searchParams.get("bbox");
    if (bbox) {
      const parts = bbox.split(",").map((p) => roundValue(p.trim(), decimals));
      if (parts.length === 4 && parts.every(Boolean)) {
        next.searchParams.set("bbox", parts.join(","));
      }
    }

    [
      "min_lon","min_lng","minLng","west","xmin","x_min",
      "min_lat","minLat","south","ymin","y_min",
      "max_lon","max_lng","maxLng","east","xmax","x_max",
      "max_lat","maxLat","north","ymax","y_max"
    ].forEach((key) => {
      if (next.searchParams.has(key)) {
        next.searchParams.set(key, roundValue(next.searchParams.get(key), decimals));
      }
    });

    if (next.searchParams.has("limit")) {
      const limit = Number(next.searchParams.get("limit"));
      if (Number.isFinite(limit) && limit > 200) next.searchParams.set("limit", "200");
    }
  ...
  function cloneInit(init) {
    const out = {};
    if (!init) return out;
    Object.keys(init).forEach((key) => { out[key] = init[key]; });
    return out;
  }

  function callerHasSignal(input, init) {
    if (init && init.signal) return true;
    if (input && typeof input === "object" && "signal" in input && input.signal) return true;
    return false;
  }

  function rebuildInput(input, url) {
    const href = url.toString();
    if (typeof input === "string" || input instanceof URL) return href;
    if (input && typeof input === "object" && "url" in input && typeof Request !== "undefined") {
      try { return new Request(href, input); } catch (_) { return href; }
    }
    return input;
  }

  function keyFor(method, endpoint, url) {
    return method + "|" + endpoint.name + "|" + url.pathname + "?" + url.searchParams.toString();
  }

  function getCache(key) {
    const item = cache.get(key);
    if (!item) return null;
    if (Date.now() - item.createdAt > item.ttlMs) {
      cache.delete(key);
      return null;
    }
    try { return item.response.clone(); } catch (_) {
      cache.delete(key);
      return null;
    }
  }

  function setCache(key, endpoint, response) {
    if (!response || !response.ok) return;

    const lengthHeader = response.headers && response.headers.get ? response.headers.get("content-length") : null;
    const payloadBytes = lengthHeader ? Number(lengthHeader) : NaN;

    if (Number.isFinite(payloadBytes) && payloadBytes > endpoint.maxPayloadBytes) {
      console.warn("[AAYS_LAYER_RUNTIME_GUARD] Large " + endpoint.name + " payload: " + payloadBytes + " bytes. Prefer PMTiles/vector/bbox-first source.");
    }

    try {
      cache.set(key, {
        createdAt: Date.now(),
        ttlMs: endpoint.ttlMs,
  ...
  window.fetch = function guardedFetch(input, init) {
    ...
    const endpoint = endpointFor(originalUrl);
    if (!endpoint) return nativeFetch(input, init);

    const normalizedUrl = quantizeUrl(originalUrl, endpoint.quantizeDecimals);
    const key = keyFor(method, endpoint, normalizedUrl);
    ...
    const nextInit = cloneInit(init);

    if (!callerHasSignal(input, init) && typeof AbortController !== "undefined") {
      const previous = latestAbortByEndpoint.get(endpoint.name);
      if (previous && previous.key !== key && previous.controller) {
        try { previous.controller.abort(); } catch (_) {}
      }

      const controller = new AbortController();
      latestAbortByEndpoint.set(endpoint.name, { key: key, controller: controller });
      nextInit.signal = controller.signal;
    }

    const nextInput = rebuildInput(input, normalizedUrl);
    const startedAt = (window.performance && performance.now) ? performance.now() : Date.now();

    const promise = nativeFetch(nextInput, nextInit).then((response) => {
      const elapsedMs = ((window.performance && performance.now) ? performance.now() : Date.now()) - startedAt;
      if (elapsedMs > 1200) {
        console.warn("[AAYS_LAYER_RUNTIME_GUARD] Slow " + endpoint.name + " fetch: " + Math.round(elapsedMs) + " ms", normalizedUrl.pathname + normalizedUrl.search);
      }
      setCache(key, endpoint, response);
      return response;
    }).catch((error) => {
      if (error && error.name === "AbortError") {
        console.debug("[AAYS_LAYER_RUNTIME_GUARD] Aborted stale " + endpoint.name + " request.");
      }
      throw error;
    }).finally(() => {
      inFlight.delete(key);
      const latest = latestAbortByEndpoint.get(endpoint.name);
      if (latest && latest.key === key) latestAbortByEndpoint.delete(endpoint.name);
    });
  ...
  window.__AAYS_LAYER_RUNTIME_GUARD_STATUS__ = function status() {
    return {
      enabled: true,
  ...
  console.info("[AAYS_LAYER_RUNTIME_GUARD] enabled: cache + de-dupe + bbox quantization + stale abort.");
}());
/* AAYS_LAYER_RUNTIME_GUARD_V1_END */

/* AAYS_LAYER_LAZY_ACTION_GUARD_V1_START
   GET-only frontend lazy marker/diagnostic guard.
   Does not create fake data. Does not perform DB write/deploy/migration.
*/
(function AAYS_LAYER_LAZY_ACTION_GUARD_V1(){
  "use strict";
  if (typeof window === "undefined") return;
  if (window.__AAYS_LAYER_LAZY_ACTION_GUARD_V1__) return;
  window.__AAYS_LAYER_LAZY_ACTION_GUARD_V1__ = true;
  window.__AAYS_LAYER_LAZY_ACTION_STATUS__ = {
    enabled: true,
    rule: "Listings and sales-history must be user-action/layer-toggle loaded, not map-move blocking.",
    endpoints: ["/map/listings", "/map/sales-history/combined"]
  };
...
// Required by air.png gas emissions toggle.
...
window.AAYS_GAS_EMISSIONS_LAYER_CONTRACT = Object.freeze({
  ...
  icon: "air.png",
  mode: "EMISSIONS_CONTROL_MODE",
  sourceId: "aays-gas-emissions-source",
