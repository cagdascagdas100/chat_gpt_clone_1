...
function Invoke-PythonStage([string]$Name, [string]$RelativeScript, [string]$RequiredInput = '') {
  $started = UtcNow
  $script = Join-Path $RepoRoot $RelativeScript
  $log = Join-Path ([IO.Path]::GetTempPath()) ('gas_emissions_1_' + $Name + '_' + [Guid]::NewGuid().ToString('N') + '.log')
  if (-not (Test-Path -LiteralPath $script -PathType Leaf)) {
    return [pscustomobject]@{ name=$Name; status='MISSING_SCRIPT'; exit_code=127; started_at=$started; ended_at=(UtcNow); script=$RelativeScript; blocker='SCRIPT_NOT_FOUND'; log_tail=$null }
  }
  if ($RequiredInput -and -not (Test-Path -LiteralPath (Join-Path $RepoRoot $RequiredInput) -PathType Leaf)) {
    return [pscustomobject]@{ name=$Name; status='SKIPPED_MISSING_INPUT'; exit_code=3; started_at=$started; ended_at=(UtcNow); script=$RelativeScript; blocker=('MISSING_INPUT:' + $RequiredInput); log_tail=$null }
  }
  $python = Resolve-Python
  if ($null -eq $python) {
    return [pscustomobject]@{ name=$Name; status='MISSING_RUNTIME'; exit_code=127; started_at=$started; ended_at=(UtcNow); script=$RelativeScript; blocker='PYTHON_OR_PY_NOT_FOUND'; log_tail=$null }
  }
  Push-Location $RepoRoot
  try {
    $args = @($python.Prefix) + @($script)
    & $python.File @args 2>&1 | Out-File -LiteralPath $log -Encoding utf8
    $code = $LASTEXITCODE
    $state = if ($code -eq 0) { 'PASS' } else { 'BLOCKED_STAGE_REPORTED' }
    return [pscustomobject]@{ name=$Name; status=$state; exit_code=$code; started_at=$started; ended_at=(UtcNow); script=$RelativeScript; blocker=$(if ($code -eq 0) { $null } else { 'NONZERO_EXIT' }); log_tail=(Tail-Text $log) }
  }
  catch {
    return [pscustomobject]@{ name=$Name; status='STAGE_EXCEPTION'; exit_code=126; started_at=$started; ended_at=(UtcNow); script=$RelativeScript; blocker=$_.Exception.Message; log_tail=(Tail-Text $log) }
  }
...
try {
  if (-not (Test-LocalServer $baseUrl)) {
    ...
    if ($null -eq $python) {
      ...
      $serverProcess = Start-Process -FilePath $python.File -ArgumentList $serverArgs -PassThru -WindowStyle Hidden -RedirectStandardOutput $stdout -RedirectStandardError $stderr
      ...
      if (-not (Test-LocalServer $baseUrl)) { $serverError = 'LOCAL_HTTP_SERVER_NOT_READY' }
    }
  }

  $browserStarted = UtcNow
  $browserScript = Join-Path $RepoRoot $paths.browser
  $browserLog = Join-Path ([IO.Path]::GetTempPath()) ('gas_emissions_1_browser_' + [Guid]::NewGuid().ToString('N') + '.log')
  if ($serverError) {
    $stages += [pscustomobject]@{ name='browser_dump_dom'; status='BLOCKED_LOCAL_HTTP_SERVER'; exit_code=3; started_at=$browserStarted; ended_at=(UtcNow); script=$paths.browser; blocker=$serverError; log_tail=$null }
  }
  elseif (-not (Test-Path -LiteralPath $browserScript -PathType Leaf)) {
    $stages += [pscustomobject]@{ name='browser_dump_dom'; status='MISSING_SCRIPT'; exit_code=127; started_at=$browserStarted; ended_at=(UtcNow); script=$paths.browser; blocker='SCRIPT_NOT_FOUND'; log_tail=$null }
  }
  else {
    try {
      Push-Location $RepoRoot
      ...
      $stages += [pscustomobject]@{ name='browser_dump_dom'; status='STAGE_EXCEPTION'; exit_code=126; started_at=$browserStarted; ended_at=(UtcNow); script=$paths.browser; blocker=$_.Exception.Message; log_tail=(Tail-Text $browserLog) }
    }
    finally { Pop-Location }
  }

  # These stages are intentionally not serially blocked by browser acceptance.
  $stages += Invoke-PythonStage 'hmlr_inspire_proximity' $paths.hmlr
  $stages += Invoke-PythonStage 'binary_prtr_pi_parse' $paths.binary
  $stages += Invoke-PythonStage 'classify_prtr_pi_records' $paths.classifier $binaryInput
  $stages += Invoke-PythonStage 'semantic_annual_air_mass_gate' $paths.semantic $classifierInput
}
finally {
...
$payload = [ordered]@{
  ...
  stages = $stages
  sequencing_policy = 'BROWSER_FAILURE_DOES_NOT_BLOCK_INDEPENDENT_HMLR_OR_PRTR_PI_STAGES'
...
# A completed orchestrator returns success even when an individual fail-closed stage is blocked.
# Stage-level JSON remains authoritative and prevents a single dependency from freezing the queue.
exit 0
