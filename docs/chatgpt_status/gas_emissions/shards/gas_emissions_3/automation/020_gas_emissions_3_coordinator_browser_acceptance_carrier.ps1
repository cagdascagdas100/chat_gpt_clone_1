...
try {
    ...
    $steps = @(
        ...
        [ordered]@{step=13;name='CAPTURE_MATRIX_SCREENSHOT';state='PASS';evidence=$screenshot.path},
        [ordered]@{step=14;name='HASH_SCREENSHOT_SHA256';state='PASS';evidence=$screenshot.sha256},
        [ordered]@{step=15;name='CHECK_BROWSER_STDERR';state='PASS';evidence="script_errors=$($scriptErrors.Count)"},
        [ordered]@{step=16;name='WRITE_LOCAL_RESULT';state='PASS';evidence=(Repo-Relative $resultPath)},
        [ordered]@{step=17;name='COORDINATOR_SERIAL_PUBLISH';state='PENDING_COORDINATOR';evidence='No child direct push'},
        [ordered]@{step=18;name='REMOTE_COMMIT_READBACK';state='PENDING_COORDINATOR';evidence='Queue publisher must confirm remote HEAD'},
        [ordered]@{step=19;name='CHATGPT_FINAL_STATE_RECONCILIATION';state='PENDING_REMOTE_PROOF';evidence='Advance summary/checkpoint only after remote result readback'}
    )
    $status = [ordered]@{schema_version=7;slot_id=$slotId;updated_at=$generatedAt;status='COORDINATOR_CANONICAL_4_PAGE_EXACT_DOM_RUNTIME_PASS_AWAITING_SERIAL_REMOTE_PUBLISH_READBACK';runner_version=7;runner_policy='EXISTING_CANONICAL_F_SHARED_RUNNER_ONLY';steps=$steps;browser_path=$browser;git_executable=$gitExe;browser_dom_passed=$true;browser_acceptance_passed=$false;canonical_pages_verified=4;canonical_page_rows_verified=25;canonical_dom_rows_verified=100;canonical_dom_headers_verified=28;screenshot_captured=$true;screenshot_sha256=$screenshot.sha256;coordinator_serial_publish_pending=$true;parcel_binding_gate_passed=$false;final_ready=$false;fake_data=$false;db_write=$false;migration=$false;production_deploy=$false}
    ...
    if (Test-Path -LiteralPath $pickupPath -PathType Leaf) {
        ...
        $pickup.status='CANONICAL_4_PAGE_EXACT_DOM_RUNTIME_PASS_AWAITING_COORDINATOR_SERIAL_REMOTE_PUBLISH_READBACK';$pickup.runtime_checks_passed=1;$pickup.runtime_checks_total=2;$pickup.browser_dom_verified_rows=100;$pickup.browser_acceptance_passed=$false;$pickup.screenshot_captured=$true;$pickup.updated_at=$generatedAt
        Write-Json $pickup $pickupPath
    }
    Write-Output "GAS_EMISSIONS_3_COORDINATOR_RUNTIME_PASS PAGES=4 DOM=100 HEADERS=28 SCREENSHOT_SHA256=$($screenshot.sha256) NEXT=COORDINATOR_SERIAL_PUBLISH_READBACK"
    exit 0
}
catch {
    $errorPayload = [ordered]@{schema_version=5;slot_id=$slotId;generated_at=[DateTime]::UtcNow.ToString('o');status='COORDINATOR_BROWSER_RUNTIME_BLOCKED';error=$_.Exception.Message;browser_dom_passed=$false;browser_acceptance_passed=$false;parcel_binding_gate_passed=$false;measured_parcel_values_produced=0;final_ready=$false;fake_data=$false;db_write=$false;migration=$false;production_deploy=$false}
