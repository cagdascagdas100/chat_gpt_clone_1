...
$precheckUrl = 'http://127.0.0.1:8012/england_map_web/data/aays_18_slots/gas_emissions_3/browser_acceptance_precheck.html?runner=coordinator-v10'
$matrixUrl = 'http://127.0.0.1:8012/england_map_web/data/aays_18_slots/gas_emissions_3/canonical_matrix_100_browser_harness.html?runner=coordinator-v10'
...
function Capture-Screenshot([string]$Browser,[string]$Url) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Screenshot browser exited with code $LASTEXITCODE" }
    ...
    return [ordered]@{status='PASS';url=$Url;canonical_matrix_url=$canonicalMatrixUrl;browser_path=$Browser;exit_code=0;path='docs/chatgpt_status/gas_emissions/shards/gas_emissions_3/acceptance/013_gas_emissions_3_matrix_browser_screenshot_latest.png';byte_count=[int64]$info.Length;sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath $screenshotPath).Hash.ToLowerInvariant();stderr_path='docs/chatgpt_status/gas_emissions/shards/gas_emissions_3/acceptance/020_coordinator_browser_runtime_latest/screenshot_stderr.log';captured_at=[DateTime]::UtcNow.ToString('o');remote_readback_required=$true}
}
...
try {
    ...
    $currentRemoteHead = Get-RemoteHead
    ...
    if ([int]$authoritativeCheckpoint.sequence -lt 22) { throw "Authoritative checkpoint sequence regressed: $($authoritativeCheckpoint.sequence)" }
    ...
    $runtimeCheckpoint | Add-Member -NotePropertyName current_remote_head_observed -NotePropertyValue $currentRemoteHead -Force
    ...
    $steps = @(
        ...
        [ordered]@{step=9;name='DRIVE_CANONICAL_MATRIX_GAS_PAGES_1_TO_4';state='PASS';evidence='pages=4/4'},
        [ordered]@{step=10;name='AGGREGATE_EXACT_100_CANONICAL_DOM_ROWS';state='PASS';evidence="rows=$matrixAcceptanceRows unique=100"},
        [ordered]@{step=11;name='VERIFY_EXACT_28_SERIALIZED_HEADERS';state='PASS';evidence="headers=$matrixAcceptanceHeaders missing=$($missingHeaders.Count)"},
        [ordered]@{step=12;name='VERIFY_UNAMBIGUOUS_DOM_COMPLETION_MARKERS';state='PASS';evidence="complete=$matrixPassMarker pages=$matrixPageEvidence"},
        [ordered]@{step=13;name='CAPTURE_MATRIX_SCREENSHOT_AND_SHA256';state='PASS';evidence=$screenshot.sha256},
        ...
        [ordered]@{step=17;name='REMOTE_COMMIT_READBACK';state='PENDING_COORDINATOR';evidence='Queue publisher must confirm remote readback'},
        [ordered]@{step=18;name='CHATGPT_FINAL_STATE_RECONCILIATION';state='PENDING_REMOTE_PROOF';evidence='Advance only after remote result readback'}
    )
    $status = [ordered]@{schema_version=10;slot_id=$slotId;updated_at=$generatedAt;status='COORDINATOR_V10_FORWARD_BRANCH_DOM_RUNTIME_PASS_AWAITING_SERIAL_REMOTE_PUBLISH_READBACK';runner_version=10;runner_policy='EXISTING_CANONICAL_F_SHARED_RUNNER_ONLY';steps=$steps;git=$gitEvidence;browser_path=$browser;git_executable=$gitExe;browser_dom_passed=$true;browser_acceptance_passed=$false;canonical_pages_verified=4;canonical_page_rows_verified=25;canonical_dom_rows_verified=100;canonical_dom_headers_verified=28;screenshot_captured=$true;screenshot_sha256=$screenshot.sha256;coordinator_serial_publish_pending=$true;parcel_binding_gate_passed=$false;final_ready=$false;fake_data=$false;db_write=$false;migration=$false;production_deploy=$false}
    ...
    if (Test-Path -LiteralPath $pickupPath -PathType Leaf) {
        ...
        $pickup.status='COORDINATOR_V10_FORWARD_BRANCH_DOM_RUNTIME_PASS_AWAITING_SERIAL_REMOTE_PUBLISH_READBACK';$pickup.runtime_checks_passed=1;$pickup.runtime_checks_total=2;$pickup.browser_dom_verified_rows=100;$pickup.browser_acceptance_passed=$false;$pickup.screenshot_captured=$true;$pickup.updated_at=$generatedAt
        Write-Json $pickup $pickupPath
    }
    Write-Output "GAS_EMISSIONS_3_COORDINATOR_V10_RUNTIME_PASS PAGES=4 DOM=100 HEADERS=28 SCREENSHOT_SHA256=$($screenshot.sha256) NEXT=COORDINATOR_SERIAL_PUBLISH_READBACK"
    exit 0
}
catch {
    $errorPayload = [ordered]@{schema_version=6;slot_id=$slotId;generated_at=[DateTime]::UtcNow.ToString('o');status='COORDINATOR_BROWSER_RUNTIME_BLOCKED';error=$_.Exception.Message;browser_dom_passed=$false;browser_acceptance_passed=$false;parcel_binding_gate_passed=$false;measured_parcel_values_produced=0;final_ready=$false;fake_data=$false;db_write=$false;migration=$false;production_deploy=$false}
