...
function RepoRoot($start){ $d=[IO.DirectoryInfo](Resolve-Path $start -ErrorAction SilentlyContinue); while($d){ if(Test-Path (Join-Path $d.FullName ".git")){ return $d.FullName }; $d=$d.Parent }; return (Get-Location).Path }
...
function PushPaths($repo,$paths,$msg){
  ...
  try{ git -C $repo add -- $rel; git -C $repo diff --cached --quiet; if($LASTEXITCODE -ne 0){ git -C $repo commit -m $msg; git -C $repo push origin $Branch } }catch{}
}
...
SetText $ro "# Runner output`nstarted_at: $((Get-Date).ToString('s'))`n"
SetText $latest (@{page_key=$PageKey;state='started';completion_percent=30;FINAL_READY=$false;updated_at=(Get-Date).ToString('s')} | ConvertTo-Json)
...
$web=Join-Path $productRoot "england_map_web"; $app=Join-Path $web "app.js"; $index=Join-Path $web "index.html"; $overlay=Join-Path $web "security_overlay.js"
$carrier="UNDETECTED"
if(Test-Path $app){ if(HasText $app "parcel-use-parcels"){ $carrier="frontend:parcel-use-parcels" } elseif(HasText $app "fallback-parcels"){ $carrier="frontend:fallback-parcels" } elseif(HasText $app "/map/parcels"){ $carrier="api:/map/parcels" } elseif(HasText $app "pmtiles"){ $carrier="frontend:pmtiles_candidate" } elseif(HasText $app "parcels_inspire"){ $carrier="backend:parcels_inspire_candidate" } else { $blockers.Add("parcel_polygon_carrier_not_found") } } else { $blockers.Add("app_js_missing") }
...
$contractComplete=$false; $pointCount="UNKNOWN"; $polyCount="UNKNOWN"; $missing=@()
if($securityLookup -ne "UNDETECTED"){
  try{ $sample=(Get-Content $securityLookup -TotalCount 8000) -join "`n" }catch{ $sample=""; $blockers.Add("security_lookup_read_failed") }
...
$productFiles=@()
if(Test-Path $web){
  SetText $helper "window.AAYSSecurityContract=window.AAYSSecurityContract||{normalizeSecurityContract:function(p){return p||{}},securityContractHtml:function(p){p=p||{};return '<div class=security-contract-output><b>Security score:</b> '+(p.security_score||'n/a')+'<br><b>Security level:</b> '+(p.security_level||'n/a')+'<br><b>Source:</b> '+(p.source_name||'n/a')+'<br><b>Evidence:</b> '+(p.evidence||'n/a')+'<br><b>Matching:</b> '+(p.matching_method||'n/a')+'<br><b>Calculation:</b> '+(p.calculation_explanation||'n/a')+'</div>'},updateRightPanel:function(p){var d=document.getElementById('aays-security-contract-panel')||document.body.appendChild(document.createElement('aside'));d.id='aays-security-contract-panel';d.innerHTML=this.securityContractHtml(p)}};"
  SetText $joiner "window.AAYSSecurityParcelThematic={ready:false,mode:'pending_polygon_carrier_join',note:'aggregate public safety signal, not incident point truth'};"
  $productFiles += $helper; $productFiles += $joiner
}
$helperLoaded=$false; $joinerLoaded=$false
if(Test-Path $index){
  $html=Get-Content $index -Raw
  if($html -notmatch 'security_contract_normalizer\.js'){ $html=$html -replace '</body>',"<script src=\"security_contract_normalizer.js\"></script>`r`n<script src=\"security_parcel_thematic_runtime.js\"></script>`r`n</body>"; SetText $index $html; $productFiles += $index }
  $helperLoaded=($html -match 'security_contract_normalizer\.js'); $joinerLoaded=($html -match 'security_parcel_thematic_runtime\.js')
} else { $blockers.Add("index_html_missing") }
$rightPanel=$helperLoaded; $popup=(($helperLoaded) -or ((Test-Path $overlay) -and (HasText $overlay "security_score")))
try{ $r=Invoke-WebRequest -UseBasicParsing "http://127.0.0.1:8010/map/parcels?bbox=-0.55,51.28,0.35,51.75&limit=5" -TimeoutSec 8; AddLine $smoke "carrier_http: $($r.StatusCode)"; if($r.Content -match 'Polygon|MultiPolygon'){ $polyCount="RUNTIME_POLYGON_PRESENT" } }catch{ AddLine $smoke "carrier_http: failed" }
...
$completion=35; if(Test-Path $productRoot){$completion+=5}; if($carrier -ne "UNDETECTED"){$completion+=10}; if($securityLookup -ne "UNDETECTED"){$completion+=10}; if($contractComplete){$completion+=15}; if($helperLoaded){$completion+=8}; if($joinerLoaded){$completion+=7}; if($popup){$completion+=5}; if($rightPanel){$completion+=5}; if($polyCount -ne "UNKNOWN"){$completion+=5}; if($browserSmokeOk){$completion=100}; if($completion -gt 99 -and !$browserSmokeOk){$completion=99}
