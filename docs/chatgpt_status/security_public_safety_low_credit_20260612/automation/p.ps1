...
$m = Test-Path (Join-Path $a 'data/parcel_security_scores_summary.json')
$x = Test-Path (Join-Path $d 'security_view_probe.png')
$c = "state: page_probe_done`npercent: 99`nfinal: false`nreason: runtime_action_needed`nindex: $i`napp: $j`noverlay: $o`ndata: $g`nsummary: $m`nshot: $x"
Set-Content -Path $s -Value $c -Encoding UTF8
Set-Content -Path (Join-Path $d 'page_probe_latest.txt') -Value $c -Encoding UTF8
Write-Output $c
