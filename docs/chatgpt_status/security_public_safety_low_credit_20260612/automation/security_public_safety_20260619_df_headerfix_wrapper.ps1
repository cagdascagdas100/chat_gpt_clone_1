...
$txt = Get-Content $source -Raw
if ($txt.StartsWith('$ErrorActionPreference') -and $txt -match '(?s)^\$ErrorActionPreference\s*=\s*"Continue"\s*\r?\nparam\(') {
  $txt = $txt -replace '(?s)^\$ErrorActionPreference\s*=\s*"Continue"\s*\r?\nparam\(', 'param('
  $txt = $txt -replace '\)\s*\r?\n\r?\n\$RequiredFields', ")`n`n`$ErrorActionPreference = \"Continue\"`n`n`$RequiredFields"
}
$txt | Out-File -FilePath $temp -Encoding utf8
& $temp
$code = $LASTEXITCODE
