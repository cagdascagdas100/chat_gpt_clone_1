...
$taskId="terrayield-047-security-parcel-polygon-canonical-contract-single-runner"
...
$applyReport=Join-Path $reportRoot "047_single_runner_apply_$stamp.md"
$smokeReport=Join-Path $reportRoot "047_smoke_$stamp.md"
$fieldReport=Join-Path $reportRoot "047_field_contract_$stamp.json"
$blockerReport=Join-Path $reportRoot "047_blockers_$stamp.md"
$runnerOutput=Join-Path $outRoot "047_runner_output_$stamp.log"
...
$py=Join-Path $outRoot "047_contract_probe_$stamp.py"
WriteText $py @'
...
res={"generated_at":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),"web_root":str(web) if web else None,"point_feature_count":0,"polygon_feature_count":0,"canonical_file_exists":False,"contract_fields_complete":False,"missing_fields":required,"blockers":[]}
if not web:
    ...
    if canon.exists():
        ...
        for f in cfeats:
            ...
            if not miss:
                res["contract_fields_complete"]=True; res["missing_fields"]=[]; break
            res["missing_fields"]=miss
...
WriteText $smokeReport "# 047 smoke`ngenerated_at: $((Get-Date).ToString('s'))`nhealth_status: $health`napp_status: $app`nsecurity_js_status: $js`nsecurity_css_status: $css`ncanonical_polygon_geojson_status: $canonStatus`n"
$unique=@($blockers|Select-Object -Unique)
$ready=$false;$percent=72
...
$blockText="# 047 blockers`nfinal_decision: $decision`n";if($unique.Count){$unique|ForEach-Object{$blockText+="- $_`n"}}else{$blockText+="- none`n"}
WriteText $blockerReport $blockText
WriteText $applyReport "# 047 apply report`ngenerated_at: $((Get-Date).ToString('s'))`nstatus: $(if($ready){'READY'}else{'BLOCKED'})`ncompletion_percent: $percent`nfinal_decision: $decision`nworktree_root: $WorktreeRoot`nweb_root: $webRoot`nfield_contract_report: $fieldReport`nsmoke_report: $smokeReport`nblocker_report: $blockerReport`nrunner_output: $runnerOutput`nseparate_runner_required: false`npowershell_required_from_user: false`n"
WriteText $runnerOutput "task_id: $taskId`nscript_path: docs/chatgpt_status/$pageKey/automation/vrun.ps1`nworktree_root: $WorktreeRoot`nweb_root: $webRoot`nfield_contract_report: $fieldReport`nsmoke_report: $smokeReport`nblocker_report: $blockerReport`napply_report: $applyReport`nfinal_decision: $decision`n"
WriteText (Join-Path $statusRoot "latest.json") (@{page_key=$pageKey;task_id=$taskId;cycle="cycle047";current_status=if($ready){"READY"}else{"BLOCKED"};completion_percent=$percent;final_ready=$ready;remaining_blockers=$unique;latest_final_report=$applyReport;latest_smoke_report=$smokeReport;latest_runner_output=$runnerOutput;updated_at=(Get-Date).ToString("o");db_write=$false;ddl=$false;migration=$false;production_deploy=$false;separate_runner_required=$false;next_user_message="devam et"}|ConvertTo-Json -Depth 10)
WriteText (Join-Path $heartRoot "latest.json") (@{page_key=$pageKey;task_id=$taskId;status=if($ready){"READY"}else{"BLOCKED"};completion_percent=$percent;updated_at=(Get-Date).ToString("o");latest_runner_output=$runnerOutput}|ConvertTo-Json -Depth 8)
