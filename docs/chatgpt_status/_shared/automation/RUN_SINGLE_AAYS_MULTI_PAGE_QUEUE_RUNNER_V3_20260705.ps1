...
function To-JsonText([object]$Obj) { $Obj | ConvertTo-Json -Depth 50 }
...
function Add-Blocker([string]$Code) {
  if ($Code -and -not ($script:Summary.blockers -contains $Code)) { $script:Summary.blockers += $Code }
}
...
if (-not $RepoRoot.StartsWith('F:\', [System.StringComparison]::OrdinalIgnoreCase)) {
  throw 'BLOCKED_WRONG_REPO_ROOT: ' + $RepoRoot
}
...
$GitLogPath = Join-Path $LogDir "MULTI_PAGE_git_args_$RunId.log"
...
function Invoke-AaysGit {
  ...
  Add-Content -LiteralPath $GitLogPath -Encoding UTF8 -Value ("[{0}] cwd={1} git {2}" -f (Now-Utc), $Cwd, ($GitArgs -join ' '))
...
function Stage-AllowedOnly([string]$Root, [string[]]$Allowed) {
  ...
  foreach ($p in $changed) {
    Assert-GitOk (Invoke-AaysGit $Root add -- $p) 'ADD_FAILED'
  }
...
function Ensure-TaskWorktree([object]$Task) {
  ...
  $worktree = Join-Path $WorkRoot ("AAYS_${safePage}_${safeTask}")
  $url = 'https://github.com/' + $RepoFullName + '.git'
  if (-not (Test-Path -LiteralPath $worktree)) {
    Ensure-Dir (Split-Path -Parent $worktree)
    Assert-GitOk (Invoke-AaysGit (Split-Path -Parent $worktree) clone --branch $Task.target_branch --single-branch $url $worktree) 'TASK_CLONE_FAILED'
  }
  $dirty = @(Get-GitChangedPaths $worktree)
  if ($dirty.Count -gt 0) {
    throw ('BLOCKED_WORKTREE_DIRTY: ' + ($dirty -join ','))
  }
...
function Browser-Gate {
  ...
  $playwright = $false
  try {
    $r = & node -e "try{require.resolve('playwright');process.exit(0)}catch(e){process.exit(1)}" 2>$null
    $playwright = ($LASTEXITCODE -eq 0)
  ...
  return [pscustomobject]@{
    ...
    npm_exists = [bool]$npm
    edge_or_chrome_exists = [bool]$edge
    playwright_available = $playwright
    site_8010_ok = $site8010
    site_8020_ok = $site8020
    browser_smoke_degraded_ok = ($smoke -and -not $playwright)
    browser_smoke_passed = $smoke
  }
}
...
function Push-Sync([string]$Worktree, [string]$Branch, [string]$CommitMessage) {
  ...
  if ($cached.output) {
    Assert-GitOk (Invoke-AaysGit $Worktree commit -m $CommitMessage) 'COMMIT_FAILED'
  }
...
function Run-Task([object]$Task) {
  ...
  Write-TaskFile $worktree $startedRel ([ordered]@{
    task_id=$taskId; page_key=$page; started_at=Now-Utc; queue_seen=$true; queue_started=$true
    single_runner_lock_acquired=$true; task_runs_in_clean_worktree=$true
  ...
  Write-TaskFile $worktree $Task.queue_rel ([ordered]@{
    task_id=$taskId; page_key=$page; status='running'; target_branch=$Task.target_branch
    script_path=$Task.script_path; allowed_paths=$Task.allowed_paths
    no_fake_final_ready=$true; no_db_write=$true; no_migration=$true; no_production_deploy=$true
  })
  ...
  $oldRoot = $env:AAYS_REPO_ROOT
  $oldTask = $env:AAYS_TASK_ID
  $oldPage = $env:AAYS_PAGE_KEY
  ...
  try {
    ...
    $env:AAYS_REPO_ROOT = $oldRoot
    $env:AAYS_TASK_ID = $oldTask
    $env:AAYS_PAGE_KEY = $oldPage
  }
  ...
  if (Test-Path -LiteralPath $gatePath) {
    try { $gate = Get-Content -LiteralPath $gatePath -Raw | ConvertFrom-Json } catch { Add-Blocker 'GATE_PARSE_FAILED' }
  }
  if ($null -eq $gate) {
    $gate = [pscustomobject]@{
      source_row_gate_passed=$false; ui_token_gate_passed=$false; browser_smoke_passed=$browser.browser_smoke_passed
      post_sync_ok=$false; manual_review_required=$true; fake_data=$false
    }
    Write-TaskFile $worktree $gateRel $gate
  }

  Write-TaskFile $worktree $reportRel ("TASK_ID=$taskId`nPAGE_KEY=$page`nRUNNER_V3=20260705`nnode_exists=$($browser.node_exists)`nnpm_exists=$($browser.npm_exists)`nedge_or_chrome_exists=$($browser.edge_or_chrome_exists)`nplaywright_available=$($browser.playwright_available)`nsite_8010_ok=$($browser.site_8010_ok)`nsite_8020_ok=$($browser.site_8020_ok)`nbrowser_smoke_passed=$($browser.browser_smoke_passed)`nautomation_exit_code=$automationCode`nfake_data=false`n--- output ---`n$automationOutput")
  ...
  Push-Sync $worktree $Task.target_branch "AAYS shared runner output $page $taskId"

  $finalReady = (
    (As-Bool (Get-Prop $gate 'source_row_gate_passed')) -and
    (As-Bool (Get-Prop $gate 'ui_token_gate_passed')) -and
    $browser.browser_smoke_passed -and
    (-not (As-Bool (Get-Prop $gate 'manual_review_required'))) -and
    (-not (As-Bool (Get-Prop $gate 'fake_data'))) -and
    $automationCode -eq 0
  )
  $completed = [ordered]@{
    task_id=$taskId; page_key=$page; completed_at=Now-Utc
    queue_seen=$true; queue_started=$true; single_runner_lock_acquired=$true
    task_runs_in_clean_worktree=$true; allowed_paths_enforced=$true
    runner_output_uploaded=$true; post_sync_ok=$true; PUSH_SYNC_OK=$true; CONTINUE_RUNNER_READY=$true
    browser_environment=$browser; final_ready=$finalReady; fake_data=$false; db_write=$false; migration=$false; production_deploy=$false
    blockers=@($script:Summary.blockers)
  }
  Write-TaskFile $worktree $completedRel $completed
  Write-TaskFile $worktree $mirrorRel $completed
  Write-TaskFile $worktree $Task.queue_rel ([ordered]@{
    task_id=$taskId; page_key=$page; status='done'; runner_completed_at=Now-Utc
    PUSH_SYNC_OK=$true; CONTINUE_RUNNER_READY=$true; final_ready=$finalReady
    no_fake_final_ready=$true; no_db_write=$true; no_migration=$true; no_production_deploy=$true
  })
  ...
  Push-Sync $worktree $Task.target_branch "AAYS shared runner completion $page $taskId"
  $script:Summary.runner_output_uploaded = $true
  $script:Summary.post_sync_ok = $true
  $script:Summary.PUSH_SYNC_OK = $true
  $script:Summary.CONTINUE_RUNNER_READY = $true
  $script:Summary.final_ready = [bool]$finalReady
...
if (Test-Path -LiteralPath $LockPath) {
  ...
  $payload = [ordered]@{
    status = if ($stale) { 'STALE_LOCK_DETECTED' } else { 'RUNNER_LOCK_ACTIVE' }
    stale = $stale; lock_age_minutes = $ageMinutes; checked_at = Now-Utc
    single_runner_lock_acquired = $false; final_ready = $false
    blocker = if ($stale) { 'STALE_LOCK_REPORTED_NOT_DELETED' } else { 'SINGLE_RUNNER_ALREADY_ACTIVE' }
    lock_path = $LockPath
  }
  Write-Utf8 $LatestStatusPath (To-JsonText $payload)
  Write-Utf8 (Join-Path $ReportDir ("MULTI_PAGE_lock_$RunId.json")) (To-JsonText $payload)
  Write-Output (To-JsonText $payload)
  exit 0
}
...
try {
  ...
  foreach ($file in $queueFiles) {
    try { $parsed += Parse-Queue $file } catch { $script:Summary.skipped += [pscustomobject]@{ queue=$file.FullName; reason='QUEUE_PARSE_FAILED'; error=$_.Exception.Message } }
  }
  $script:Summary.queue_seen = ($parsed.Count -gt 0)
  $readyStatuses = @('queued','pending','retry_pending','failed_transient')
  $skipStatuses = @('done','completed','done_on_target_branch','failed_final','blocked_manual','archived','superseded','superseded_by_force_pickup','running')
  $valid = @()
  foreach ($q in $parsed) {
    if (-not $q.valid) {
      $script:Summary.skipped += [pscustomobject]@{ queue=$q.queue_rel; reason='INVALID_QUEUE_CONTRACT'; validation_errors=$q.validation_errors; status=$q.status }
      continue
    }
    if ($skipStatuses -contains $q.status_norm) {
      $script:Summary.skipped += [pscustomobject]@{ queue=$q.queue_rel; reason='SKIP_STATUS'; status=$q.status }
      continue
    }
    if (-not ($readyStatuses -contains $q.status_norm)) {
      $script:Summary.skipped += [pscustomobject]@{ queue=$q.queue_rel; reason='UNKNOWN_STATUS'; status=$q.status }
      continue
    }
    $valid += $q
  }
  $ordered = @($valid | Sort-Object @{Expression='priority';Ascending=$true}, @{Expression='queue_rel';Ascending=$true})
  Write-Utf8 $SelectionDebugPath (To-JsonText ([ordered]@{ checked_at=Now-Utc; ready_count=$ordered.Count; ready=$ordered }))
  Write-Utf8 $SkipDebugPath (To-JsonText ([ordered]@{ checked_at=Now-Utc; skipped=$script:Summary.skipped }))
  if ($ordered.Count -eq 0) { Add-Blocker 'NO_VALID_PENDING_QUEUE_ITEMS' }
  if (-not $ScanOnly -and $MaxTasks -gt 0) {
    foreach ($task in ($ordered | Select-Object -First $MaxTasks)) {
      try {
        $result = Run-Task $task
        $script:Summary.processed += $result
      } catch {
        Add-Blocker 'RUNNER_TASK_FAILED'
        $script:Summary.processed += [pscustomobject]@{ task_id=$task.task_id; page_key=$task.page_key; completed=$false; final_ready=$false; error=$_.Exception.Message }
      }
    }
  }
} catch {
  Add-Blocker 'SHARED_RUNNER_FAILED'
  $script:Summary.error = $_.Exception.Message
} finally {
  $script:Summary.checked_at = Now-Utc
  $script:Summary.CONTINUE_RUNNER_READY = ($script:Summary.single_runner_lock_acquired -and ($script:Summary.queue_seen -or $script:Summary.queue_started))
  if (-not $script:Summary.post_sync_ok) { $script:Summary.final_ready = $false }
  Write-Utf8 $LatestStatusPath (To-JsonText $script:Summary)
  Write-Utf8 (Join-Path $ReportDir ("MULTI_PAGE_runner_output_$RunId.json")) (To-JsonText $script:Summary)
  if (Test-Path -LiteralPath $LockPath) {
    try {
