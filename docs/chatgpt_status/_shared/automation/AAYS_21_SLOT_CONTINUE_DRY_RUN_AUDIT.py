...
def load_coordinator(script: Path):
    spec = importlib.util.spec_from_file_location("aays_21_coordinator", script)
...
def audit(root: Path, coordinator_script: Path) -> dict:
    ...
    legacy_internet = build_task(
        module,
        "internet_access_3",
        module.SLOT_SPECS["internet_access_3"],
    )
    ...
    try:
        ...
        legacy_internet_blocked = (
            "INTERNET_SLOT_REQUIRES_18_OR_21_SLOT_WORKSTREAM" in str(exc)
        )

    legacy_18_internet = build_task(
        module,
        "internet_access_1",
        module.SLOT_SPECS["internet_access_1"],
    )
    legacy_18_internet["task_id"] = "dryrun_legacy_18_internet"
    legacy_18_internet["workstream_id"] = "AAYS_18_SLOT_SAFE_PARALLEL_V1"
    legacy_18_internet["read_paths"] = [
        module.SLOT_SPECS["internet_access_1"]["business_root"],
        "docs/chatgpt_status/_shared/slots_18/internet_access_1",
    ]
    legacy_18_internet_accepted = (
        coordinator.classify_task(legacy_18_internet) == "internet_access_1"
    )

    legacy_future = build_task(
        module,
        "future_growth_1",
        module.SLOT_SPECS["future_growth_1"],
    )
    legacy_future["task_id"] = "dryrun_legacy_future"
    legacy_future["workstream_id"] = "AAYS_18_SLOT_SAFE_PARALLEL_V1"
    try:
        coordinator.classify_task(legacy_future)
        legacy_future_blocked = False
    except Exception as exc:
        legacy_future_blocked = (
            "FUTURE_GROWTH_SLOT_REQUIRES_21_SLOT_WORKSTREAM" in str(exc)
        )
    ...
    web_task["exact_write_paths"] = [
        "england_map_web/data/aays_21_slots/internet_access_1/output.json"
    ]
    try:
    ...
    report = {
        ...
        if (
            ...
            and legacy_18_internet_accepted
            and legacy_future_blocked
        ...
        "legacy_18_internet_accepted": legacy_18_internet_accepted,
        "legacy_future_growth_blocked": legacy_future_blocked,
