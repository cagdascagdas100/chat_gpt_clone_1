...
$runner = Join-Path $automationRoot "RUN_SINGLE_AAYS_MULTI_PAGE_QUEUE_RUNNER.ps1"
...
$lockPayload = [ordered]@{ pid=$PID; runner="RUN_AAYS_STABLE_LEGACY_RUNNER_DAEMON_20260707"; repo_root=$RepoRoot; branch=$MainBranch; started_at=Now-Utc; updated_at=Now-Utc; final_ready=$false }
...
try {
  do {
    $loop++
    $started = Now-Utc
    Write-Json $heartbeatPath ([ordered]@{ daemon_pid=$PID; loop=$loop; heartbeat_at=$started; runner_active=$true; final_ready=$false; fake_data=$false; db_write=$false; migration=$false; production_deploy=$false })
    ...
    $payload = [ordered]@{ checked_at=Now-Utc; daemon_pid=$PID; loop=$loop; status="runner_loop_completed"; runner_exit_code=$code; runner_output_tail=$tail; CONTINUE_RUNNER_READY=$true; final_ready=$false; fake_data=$false; db_write=$false; migration=$false; production_deploy=$false }
    Write-Json $statusPath $payload
