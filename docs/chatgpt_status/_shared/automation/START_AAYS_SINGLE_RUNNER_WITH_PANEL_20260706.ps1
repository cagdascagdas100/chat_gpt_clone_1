...
if (-not $runnerState.active) {
  if ($NoLoop) {
    $args = @("--repo-root",$repoRoot,"--work-root",$WorkRoot,"--mode","scan","--task-id","ready_to_sell_3_automation_167_dom_parcel_geometry_gate_20260801","--","-RepoRoot",$repoRoot,"-RepoFullName",$RepoFullName,"-MainBranch",$MainBranch,"-WorkRoot",$WorkRoot,"-MaxTasks",$MaxTasks,"-StaleMinutes",$StaleMinutes,"-ScanOnly")
    if ($NoPush) { $args += "-NoPush" }
    $out = & python $compatHelper @args 2>&1
    $runnerStatus = "runner_scan_only_completed"
  } else {
    $args = @($compatHelper,"--repo-root",$repoRoot,"--work-root",$WorkRoot,"--mode","daemon","--task-id","ready_to_sell_3_automation_167_dom_parcel_geometry_gate_20260801","--","-IntervalSeconds",$IntervalSeconds,"-HeartbeatSeconds",$HeartbeatSeconds,"-RefreshIntervalSeconds",$RefreshIntervalSeconds,"-MaxTasks",$MaxTasks,"-RepoRoot",$repoRoot,"-RepoFullName",$RepoFullName,"-MainBranch",$MainBranch,"-WorkRoot",$WorkRoot,"-StaleMinutes",$StaleMinutes)
    if ($NoPush) { $args += "-NoPush" }
...
$state = [ordered]@{
  ...
  runner_engine = "stable_legacy_worktree_runner_20260707_slot21_queue_compat_helper"
