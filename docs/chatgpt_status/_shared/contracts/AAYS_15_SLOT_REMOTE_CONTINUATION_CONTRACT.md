...
- Gercek 15-eszamanli ChatGPT akis icin 15 ayri sayfa gerekir; her sayfa omru boyunca tek SLOT_ID kullanir.
- Uzun task timeout ve heartbeat lease varsayilani 3600 saniyedir. Kullanici 20-30 dakikada bir `devam et` yazdiginda canli task duplicate edilmez.
- Queue dosyasi tercihen `*.v3.task.json` olur; eski `*.v2.task.json` uyumlulugu korunur.
