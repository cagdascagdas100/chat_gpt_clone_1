#!/usr/bin/env python3
"""Bounded exact-site scan of the official NAEI UK point-source workbook."""
...
def match_target(rows: list[dict[str, Any]], target: dict[str, Any], source_error: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "matches": matches,
        "decision": "EXACT_SITE_POINT_SOURCE_ROWS_VERIFIED" if matches else "NO_DATA_CONTINUE",
        "error": source_error,
    }


def load_workbook(contract: dict[str, Any], fixture_xlsx: Path | None) -> dict[str, Any]:
    ...
    try:
        if fixture_xlsx:
            raw, status = fixture_xlsx.read_bytes(), 200
        else:
            url = manifest["source_url"]
            parsed = urllib.parse.urlparse(url)
            require(parsed.scheme == "https", "source URL must use HTTPS")
            require(parsed.netloc == "naei.energysecurity.gov.uk", "source host mismatch")
            request = urllib.request.Request(
                url,
                headers={
                    "User-Agent": "AAYS-NAEI-Point-Sources-Gate/1.0",
                    "Accept": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,*/*;q=0.5",
                },
                method="GET",
            )
            with urllib.request.urlopen(request, timeout=int(policy["workbook_timeout_seconds"])) as response:
                status = int(getattr(response, "status", response.getcode()))
                raw = response.read(int(policy["maximum_workbook_bytes"]) + 1)
        require(status == 200, f"unexpected HTTP status {status}")
        require(len(raw) <= int(policy["maximum_workbook_bytes"]), "workbook exceeds byte limit")
        rows, sheet_names = parse_workbook_rows(raw, policy)
        result.update({"workbook_bytes": raw, "workbook_status": status, "rows": rows, "sheet_names": sheet_names})
...
def main() -> int:
    ...
    source = load_workbook(contract, args.fixture_xlsx)
    rows = source["rows"]
    results = [match_target(rows, target, source["workbook_error"]) for target in targets]
    ...
    if matched_targets == target_count:
        state, next_step = "MATCHES_VERIFIED", "VALIDATE_NAEI_POINT_SOURCE_POLLUTANT_COLUMNS_FOR_GAS_EMISSIONS_BINDING"
    elif matched_targets:
        state, next_step = "PARTIAL_MATCH_CONTINUE", "ADVANCE_UNMATCHED_TARGET_AND_VALIDATE_MATCHED_NAEI_POINT_SOURCE_ROWS"
    else:
        state, next_step = "NO_DATA_CONTINUE", "ADVANCE_TO_NEXT_UNVERIFIED_GAS_EMISSIONS_SOURCE_AFTER_NAEI_POINT_SOURCES_NO_DATA"
    raw = source["workbook_bytes"]
    output = {
        "schema_version": 3,
        ...
        "execution_mode": "SYNTHETIC_FIXTURE" if args.fixture_xlsx else "LIVE_NETWORK",
        ...
        "decision": {
            ...
            "all_workbook_sheets_scanned": raw is not None,
