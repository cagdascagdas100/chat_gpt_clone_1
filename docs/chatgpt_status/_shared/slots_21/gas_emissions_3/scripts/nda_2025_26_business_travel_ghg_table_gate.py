#!/usr/bin/env python3
"""Validate and publish the official NDA-only business-travel GHG table rows."""
...
REQUIRED_RECORD_IDS = (
    ...
    "ggc_boundary_and_methodology",
    "nda_only_business_travel_ghg_table_2025_26",
    "scope3_boundary_context",
)
...
def main() -> int:
    ...
    methodology = by_id.get("ggc_boundary_and_methodology", {}).get("proven_fields", {})
    table = by_id.get("nda_only_business_travel_ghg_table_2025_26", {}).get("proven_fields", {})
    scope3 = by_id.get("scope3_boundary_context", {}).get("proven_fields", {})
    ...
    exact_match = (
        ...
        and methodology.get("organisation_scope") == "NDA only"
        and methodology.get("methodology") == "UK Government Greenhouse Gas Reporting Conversion Factors 2025"
        and table.get("organisation_scope") == "NDA only"
        and table.get("period") == "2025/26"
        and table.get("unit") == "tCO2e"
        and table.get("total_business_travel_emissions") == 286.0
        and table.get("domestic_flight_emissions") == 7.0
        and table.get("international_flight_emissions") == 102.0
        and table.get("site_or_group_allocation_provided") is False
        and scope3.get("business_travel_is_scope3_component") is True
    )

    matches: list[dict[str, Any]] = []
    if exact_match:
        rows = (
            ("NDA_ONLY_2025_26_TOTAL_BUSINESS_TRAVEL_GHG", "total_business_travel_GHG_emissions", 286.0),
            ("NDA_ONLY_2025_26_DOMESTIC_FLIGHT_GHG", "domestic_flight_GHG_emissions", 7.0),
            ("NDA_ONLY_2025_26_INTERNATIONAL_FLIGHT_GHG", "international_flight_GHG_emissions", 102.0),
        )
        for row_id, metric_type, value in rows:
            matches.append({
                "row_id": row_id,
                ...
                "source_record_id": "nda_only_business_travel_ghg_table_2025_26",
                "source_line_range": [7641, 7653],
                "unit_conversion_applied": False,
                "value_inferred": False,
            })

    state = "EXACT_NDA_ONLY_BUSINESS_TRAVEL_GHG_TABLE_ROWS_VERIFIED" if matches else "NO_DATA_CONTINUE"
    next_step = (
        "ADVANCE_TO_NEXT_UNVERIFIED_GAS_EMISSIONS_SOURCE_AFTER_NDA_ONLY_2025_26_BUSINESS_TRAVEL_GHG_TABLE_PUBLISHED"
        if matches
        else "ADVANCE_TO_NEXT_UNVERIFIED_GAS_EMISSIONS_SOURCE_AFTER_NDA_ONLY_2025_26_BUSINESS_TRAVEL_NO_DATA"
    )
    blocker = "NONE" if matches else "OFFICIAL_NDA_2025_26_RECORD_SET_DID_NOT_PASS_EXACT_BUSINESS_TRAVEL_GHG_TABLE_GATE"
    ...
    output = {
        ...
        "decision": {
            ...
            "nda_only_boundary_preserved": True,
            "business_travel_total_not_recalculated": True,
            "flight_subcategories_not_added_to_business_travel_total": True,
            "scope3_total_not_recalculated": True,
            "site_or_group_allocation_prohibited": True,
