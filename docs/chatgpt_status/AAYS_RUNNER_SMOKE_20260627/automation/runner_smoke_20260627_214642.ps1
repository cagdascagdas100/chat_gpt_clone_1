...
$RepoOutputPath = 'F:\chatgpt\chat_gpt_clone_1_main\docs\chatgpt_status\AAYS_RUNNER_SMOKE_20260627\runner_outputs\runner_smoke_20260627_214642.output.txt'
$StatusPath = 'F:\chatgpt\chat_gpt_clone_1_main\docs\chatgpt_status\AAYS_RUNNER_SMOKE_20260627\status\runner_smoke_20260627_214642.status.json'
$HeartbeatPath = 'F:\chatgpt\chat_gpt_clone_1_main\docs\chatgpt_status\AAYS_RUNNER_SMOKE_20260627\heartbeat\runner_smoke_20260627_214642.heartbeat.txt'
$DoPush = $true
...
if ($DoPush) {
  try {
    $body = $body.Replace('runner_push=not_requested','runner_push=attempting')
    Set-Content -LiteralPath $RepoResultPath -Value $body -Encoding UTF8
    $status.runner_push = 'attempting'
    ...
    if ($pending) {
      git -C $RepoRoot commit -m "test $PageKey runner smoke proof $TaskId phase1" | Out-Null
      git -C $RepoRoot push origin main | Out-Null

      $status.runner_push = 'proven'
      (Get-Content -LiteralPath $RepoResultPath -Raw).Replace('runner_push=attempting','runner_push=proven').Replace('runner_push=not_requested','runner_push=proven') | Set-Content -LiteralPath $RepoResultPath -Encoding UTF8
      $status | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $StatusPath -Encoding UTF8

      git -C $RepoRoot add -- "docs/chatgpt_status/$PageKey"
      $pendingProof = git -C $RepoRoot status --short -- "docs/chatgpt_status/$PageKey"
      if ($pendingProof) {
        git -C $RepoRoot commit -m "test $PageKey runner push proof $TaskId phase2" | Out-Null
        git -C $RepoRoot push origin main | Out-Null
      }
    } else {
