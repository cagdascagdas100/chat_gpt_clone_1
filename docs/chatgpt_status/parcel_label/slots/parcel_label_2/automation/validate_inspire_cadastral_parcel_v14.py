...
BASE_PATH = Path(__file__).with_name("validate_inspire_cadastral_parcel_v13.py")
spec = importlib.util.spec_from_file_location("parcel_label_2_validator_v13", BASE_PATH)
if spec is None or spec.loader is None:
    raise RuntimeError("PARCEL_LABEL_2_VALIDATOR_V13_IMPORT_FAILED")
previous = importlib.util.module_from_spec(spec)
spec.loader.exec_module(previous)

BACKEND_PATH = Path(__file__).with_name("secure_lxml_backend_v2.py")
backend_spec = importlib.util.spec_from_file_location("parcel_label_2_secure_lxml_backend_v2", BACKEND_PATH)
if backend_spec is None or backend_spec.loader is None:
    raise RuntimeError("PARCEL_LABEL_2_SECURE_LXML_BACKEND_V2_IMPORT_FAILED")
