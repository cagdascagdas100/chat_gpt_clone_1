...
BASE_PATH = Path(__file__).with_name("secure_lxml_backend_v5.py")
spec = importlib.util.spec_from_file_location("parcel_label_2_secure_lxml_backend_v5", BASE_PATH)
if spec is None or spec.loader is None:
    raise RuntimeError("XML_SECURE_LXML_BACKEND_V5_IMPORT_FAILED")
...
class _CumulativeElementNameBoundedStructureTarget(previous._MiscNodeBoundedStructureTarget):
    """Bound cumulative expanded element-name characters from parser callbacks."""

    def __init__(self, limits: dict[str, int]) -> None:
        super().__init__(limits)
        self.total_element_name_chars = 0
        self.max_element_name_chars_seen = 0

    def start(self, tag: str, attrs: dict[str, str]):
        value = str(tag)
        length = len(value)
        self.total_element_name_chars += length
        if self.total_element_name_chars > self.limits["max_total_element_name_chars"]:
            self._fail(
                "XML_TOTAL_ELEMENT_NAME_CHAR_LIMIT_EXCEEDED:"
                f"{self.total_element_name_chars}:{self.limits['max_total_element_name_chars']}"
            )
        self.max_element_name_chars_seen = max(self.max_element_name_chars_seen, length)
        return super().start(value, attrs)

    def close(self):
        output = super().close()
        return output | {
            "xml_structure_total_element_name_chars": self.total_element_name_chars,
            "xml_structure_max_element_name_chars": self.max_element_name_chars_seen,
        }
...
def validate_xml_structure(
    ...
    *,
    ...
    max_total_element_name_chars: int = 64 * 1024 * 1024,
    ...
    limits = {
        ...
        "max_total_element_name_chars": _positive(
            "max_total_element_name_chars", max_total_element_name_chars
        ),
    ...
    target = _CumulativeElementNameBoundedStructureTarget(limits)
    ...
    return backend | lexical | structure | {
        ...
        "xml_structure_element_name_char_limit": limits["max_name_chars"],
        "xml_structure_total_element_name_char_limit": limits["max_total_element_name_chars"],
        ...
        "xml_element_name_cumulative_validation_passed": True,
