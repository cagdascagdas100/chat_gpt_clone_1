...
BASE_PATH = Path(__file__).with_name("bind_inspire_enfield_batch_v15.py")
base_spec = importlib.util.spec_from_file_location("parcel_label_2_v15", BASE_PATH)
if base_spec is None or base_spec.loader is None:
    raise RuntimeError("PARCEL_LABEL_2_V15_IMPORT_FAILED")
previous = importlib.util.module_from_spec(base_spec)
base_spec.loader.exec_module(previous)

TRANSPORT_PATH = Path(__file__).with_name("official_hmlr_transport_v2.py")
transport_spec = importlib.util.spec_from_file_location("parcel_label_2_redirect_policy_transport", TRANSPORT_PATH)
if transport_spec is None or transport_spec.loader is None:
    raise RuntimeError("PARCEL_LABEL_2_REDIRECT_POLICY_TRANSPORT_IMPORT_FAILED")
...
base.TASK_VERSION = "7.5-separate-index-and-binary-redirect-policy-batch"
EXACT_WEB = base.REPO / "england_map_web/data/aays_21_slots/parcel_label_2/progress_wave24_exact_result_latest.json"
...
_MAX_INDEX_BYTES = previous._MAX_INDEX_BYTES
...
_INDEX_OPENER, _INDEX_COOKIE_JAR, _INDEX_REDIRECT_HANDLER = transport.build_trusted_opener(
    _PRIMARY_HOST,
    require_primary=True,
)
_BINARY_OPENER, _BINARY_COOKIE_JAR, _BINARY_REDIRECT_HANDLER = transport.build_trusted_opener(
    _PRIMARY_HOST,
...
def fetch(url: str, timeout: int, attempts: int = 2):
    ...
    transport.validate_official_hmlr_url(
        url,
