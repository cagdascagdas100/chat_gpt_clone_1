...
TASK_ID = "parcel-label-3-natural-england-lnr-coordinate-v1-20260803"
PROBE_BLOB_SHA = "ea8e95593a58ab6cbb9369abc30bc38ce8543ad9"
LAYER_URL = "https://services.arcgis.com/JJzESW51TqeY9uat/arcgis/rest/services/Local_Nature_Reserves_England/FeatureServer/0"
FEATURE_SERVICE_URL = "https://services.arcgis.com/JJzESW51TqeY9uat/arcgis/rest/services/Local_Nature_Reserves_England/FeatureServer"
DATASET_URL = "https://www.data.gov.uk/dataset/acdf4a9e-a115-41fb-bbe9-603c819aa7f7/local-nature-reserves-england1"
GUIDANCE_URL = "https://www.gov.uk/guidance/create-and-manage-local-nature-reserves"
...
MAX_BYTES = 1_048_576
MAX_FEATURES = 5
OUT_FIELDS = ["OBJECTID", "REF_CODE", "NAME", "MEASURE", "LABEL"]
POINTS = {
...
def evidence(
    *,
    ...
    return {
        ...
        "terms_or_license_urls": [DATASET_URL, OGL_URL],
...
def metadata_attempt(timeout: float) -> tuple[list[str], dict[str, Any]]:
    ...
    try:
        ...
        return selected, evidence(
            ...
            record_scope="one bounded official Natural England Local Nature Reserves layer metadata request; maximum 1 MiB",
            supports_fields=["layer name", "polygon geometry type", "published field names", "record limit"],
            excerpt=excerpt,
            http_status=status,
        )
    except Exception as exc:
        error = f"NATURAL_ENGLAND_LNR_METADATA_ERROR:{type(exc).__name__}:{exc}"
...
def point_attempt(point: dict[str, Any], fields: list[str], timeout: float) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    try:
                    ...
                    rows.append(
                        {
                            ...
                            "context_only": True,
                            "designation_context": "Local Nature Reserve",
                            "exact_parcel_binding": False,
                            "property_type_binding": False,
                        }
                    )
        excerpt = json.dumps(
            {"feature_count": len(rows), "field_names": fields[:12], "exceededTransferLimit": parsed.get("exceededTransferLimit") if isinstance(parsed, dict) else None},
            ensure_ascii=False,
            separators=(",", ":"),
        )
        return rows, evidence(
            ...
            record_scope="one bounded exact-point intersection query against official Natural England Local Nature Reserves layer; maximum five features, twelve fields, no geometry and 1 MiB",
            supports_fields=fields[:12],
            excerpt=excerpt,
            http_status=status,
        )
    except Exception as exc:
        error = f"NATURAL_ENGLAND_LNR_POINT_ERROR:{type(exc).__name__}:{exc}"
        return [], evidence(
            parcel_id=point["parcel_id"],
            point=point,
            source_url=url,
...
def build_payload(points: list[dict[str, Any]], timeout: float) -> dict[str, Any]:
    ...
    return {
        ...
        "blocker": {
            "code": None if produced else "NATURAL_ENGLAND_LNR_NO_USABLE_RESPONSE_OR_NO_POINT_MATCH",
            "state": "NONE" if produced else "NO_DATA_CONTINUE",
            "manual_action_required": False,
            "retry_unchanged_route": False,
        },
        "next_unverified_step": "SELECT_NEXT_OFFICIAL_OR_FREE_SOURCE_AFTER_NATURAL_ENGLAND_LNR_COORDINATE",
        "layer_url": LAYER_URL,
...
def validate_only(base: Path) -> None:
    ...
    print("PASS_TARGET_4_NATURAL_ENGLAND_LNR_1_METADATA_PLUS_3_EXACT_POINT_QUERIES_MAX5_EACH_MAX12_FIELDS_MAX1MIB_CONTEXT_ONLY")
...
def main() -> int:
    ...
    atomic_json(base / "docs/chatgpt_status/_shared/slots_21/parcel_label_3/natural_england_lnr_coordinate_result_latest.json", payload)
    atomic_json(base / "england_map_web/data/aays_21_slots/parcel_label_3/natural_england_lnr_coordinate_latest.json", payload)
