...
TASK_ID='parcel-label-3-lambeth-libraries-v1-20260801'
ENDPOINT='https://gis.lambeth.gov.uk/arcgis/rest/services/LambethLibraries/FeatureServer/0/query'
DIR='https://gis.lambeth.gov.uk/arcgis/rest/services'
SERVICE='https://gis.lambeth.gov.uk/arcgis/rest/services/LambethLibraries/FeatureServer'
...
OUT=('docs/chatgpt_status/_shared/slots_21/parcel_label_3/lambeth_libraries_result_latest.json','england_map_web/data/aays_21_slots/parcel_label_3/lambeth_libraries_latest.json')
 ...
 try:
  ...
  for f in fs:
   if isinstance(f,dict) and isinstance(f.get('attributes'),dict) and isinstance(f.get('geometry'),dict):
    cs.append({'parcel_id':p['parcel_id'],'candidate_label':'Library','candidate_only':True,'source_attributes':f['attributes'],'source_geometry':f['geometry'],'source_url':u,'accessed_at':at,'content_sha256':csha})
  ...
  b=f'LAMBETH_LIBRARIES_ERROR:{type(ex).__name__}'; cs=[]; e={'parcel_id':p['parcel_id'],'source_url':u,'accessed_at':at,'query_sha256':qsha,'http_status':None,'content_sha256':sha(b),'sha256_basis':'bounded_error_evidence_string','relevant_record_ids_or_excerpt':b,'proven_fields':['query URL','access time','query SHA-256','bounded error type']}
...
def validate(root):
 ...
 return {'state':'VALIDATED','task_id':TASK_ID,'target_count':len(ps),'resource_class':'network_fetch','relative_script_path':'docs/chatgpt_status/parcel_label/automation/parcel_label_3_lambeth_libraries_v1.py','relative_outputs':list(OUT),'radius_metres':RADIUS,'result_limit':LIMIT,'spacing_seconds':SPACING}
