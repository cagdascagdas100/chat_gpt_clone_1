...
MANIFEST=pathlib.Path("docs/chatgpt_status/_shared/slots_21/parcel_label_3/evidence/lambeth_tree_preservation_order_boundaries_point_containment_source_manifest_20260804.json")
OUTPUTS=[pathlib.Path("docs/chatgpt_status/_shared/slots_21/parcel_label_3/lambeth_tree_preservation_order_boundaries_point_containment_result_latest.json"),pathlib.Path("england_map_web/data/aays_21_slots/parcel_label_3/lambeth_tree_preservation_order_boundaries_point_containment_latest.json")]
SERVICE_ROOTS=["https://gis.lambeth.gov.uk/arcgis/rest/services/LambethTreePreservationOrderBoundaries/MapServer"]
ALLOWED_HOST="gis.lambeth.gov.uk"; MAX_RESPONSE_BYTES=8*1024*1024
ATTRIBUTE_HINTS=("objectid","tpo","order","reference","ref","name","type","status","date","species","tree","area","group","woodland")
...
def load_manifest():
    ...
    if len(p.get("sources",[]))<5: raise RuntimeError("SOURCE_MANIFEST_INCOMPLETE")
    for s in p["sources"]:
...
def discover_layer(timeout,evidence):
    ...
    for root in SERVICE_ROOTS:
        ...
        try:
            ...
            ranked=sorted([x for x in layers if isinstance(x,dict) and isinstance(x.get("id"),int)],key=lambda x:(0 if all(t in str(x.get("name","")).casefold() for t in ("tree","preservation")) else 1,int(x["id"])))
            if not ranked: raise RuntimeError("SERVICE_METADATA_HAS_NO_VALID_LAYER")
            s=ranked[0]; evidence["metadata_response_count"]+=1; evidence["metadata_requests"].append({"service_root":root,"request_url":url,"final_url":final,"http_status":status,"bytes":len(body),"response_sha256":sha256_bytes(body),"layer_count":len(layers),"selected_layer_id":int(s["id"]),"selected_layer_name":str(s.get("name","")),"state":"RESPONSE"})
...
def synthetic_feature(row,fid,offset=0.0):
    ...
    return {"type":"Feature","id":fid,"properties":{"OBJECTID":fid,"TPO_REF":f"TPO-{fid}","TYPE":"Area"},"geometry":{"type":"Polygon","coordinates":[ring]}}
def run(rows,timeout,synthetic=False,ambiguous=False):
    ...
    if synthetic: root,layer,name=SERVICE_ROOTS[0],0,"Synthetic TPO boundaries"
    else:
    ...
    for idx,row in enumerate(rows,1):
        ...
        try:
            ...
            if len(candidates)==1:
                out.update({"state":"MATCHED_UNIQUE_LAMBETH_TREE_PRESERVATION_ORDER_BOUNDARY","official_tree_preservation_order_boundary":True,"official_tree_preservation_order_label":"Lambeth Tree Preservation Order boundary",**candidates[0]}); matched+=1
            elif len(candidates)>1: out.update({"state":"NO_DATA","reason":"AMBIGUOUS_MULTIPLE_POINT_CONTAINING_TPO_BOUNDARIES","candidate_geometry_sha256":[c["geometry_sha256"] for c in candidates]})
            else: out.update({"state":"NO_DATA","reason":"NO_POINT_CONTAINING_TPO_BOUNDARY"})
        except Exception as exc:
...
def main():
    ...
    if args.synthetic_test:
        counts=[r["candidate_count"] for r in records]
        if matched!=3 or counts!=[1,1,1]: raise RuntimeError(f"SYNTHETIC_UNIQUE_FAILED:{matched}:{counts}")
        print(json.dumps({"valid":True,"matched_rows":matched,"candidate_counts":counts,"point_query_count":ev["point_query_count"]},sort_keys=True)); return 0
    if args.synthetic_ambiguous_test:
        states=[r["state"] for r in records]
        if matched!=2 or states[1]!="NO_DATA" or records[1].get("reason")!="AMBIGUOUS_MULTIPLE_POINT_CONTAINING_TPO_BOUNDARIES": raise RuntimeError(f"SYNTHETIC_AMBIGUOUS_FAILED:{matched}:{states}")
    ...
    state="PUBLISHED" if matched else "NO_DATA_CONTINUE"; result={"schema_version":1,"workstream_id":"AAYS_21_SLOT_SAFE_PARALLEL_V1","slot_id":"parcel_label_3","task_id":"parcel-label-3-lambeth-tree-preservation-order-boundaries-point-containment-v1-20260804","state":state,"panel_status":"PUBLISHED","completed_count":len(records),"target_count":3,"previous_percent":0.0,"progress_percent":round(len(records)/3*100,6),"percent_increase":round(len(records)/3*100,6),"matched_unique_tpo_boundary_rows":matched,"evidence_records":len(records),"source_evidence":ev,"records":records,"unknown_attributes_promoted_to_label":False,"fake_data":False,"large_raw_files_committed":False,"generated_at":now()}
    text=canonical_json(result)+"\n"
    for p in OUTPUTS: atomic_write(p,text)
    print(json.dumps({"completed_count":len(records),"target_count":3,"matched_unique_tpo_boundary_rows":matched,"state":state,"output_sha256":sha256_bytes(text.encode())},sort_keys=True)); return 0
