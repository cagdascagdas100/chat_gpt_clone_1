...
TASK_ID = "parcel-label-3-nhs-hospital-postcode-v1-20260802"
PROBE = "england_map_web/data/distance_property_types/parcel_label_3_canonical_probe_latest.json"
OUT = (
    "docs/chatgpt_status/_shared/slots_21/parcel_label_3/nhs_hospital_postcode_result_latest.json",
    "england_map_web/data/aays_21_slots/parcel_label_3/nhs_hospital_postcode_latest.json",
)
LANDING = "https://www.nhs.uk/service-search/Hospital"
HOSPITALS = "https://www.nhs.uk/nhs-services/hospitals/"
ABOUT = "https://www.nhs.uk/nhs-services/hospitals/about-nhs-hospital-services/"
...
def canonical_points(base: Path) -> list[dict[str, Any]]:
    payload = json.loads((base / PROBE).read_text(encoding="utf-8"))
    ...
    for parcel_id, _postcode in POSTCODES:
        ...
        expected_longitude, expected_latitude = EXPECTED_POINTS[parcel_id]
        if abs(longitude - expected_longitude) > 1e-7 or abs(latitude - expected_latitude) > 1e-7:
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "form":
            ...
            self.current_form["inputs"].append(
                {
                    key: values.get(key, "")
                    for key in ("name", "id", "type", "value", "placeholder", "aria-label")
                }
            )
...
def fetch(url: str, timeout: float, body: bytes | None = None) -> tuple[bytes, int | None, str]:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "TerraYield-AAYS/1.0 bounded NHS hospital postcode research",
        },
    )
...
def discover_search_form(raw: bytes, base_url: str) -> dict[str, Any]:
    ...
    for form in parser.forms:
        for item in form["inputs"]:
            ...
            if item.get("name") and any(
                token in descriptor for token in ("location", "postcode", "post code", "town", "city")
            ):
                return {
    ...
    raise ValueError("NHS hospital location/postcode search form not discovered")
...
def extract_candidates(
    ...
    for link in parser.links:
        ...
        if parsed.netloc not in {"www.nhs.uk", "nhs.uk"}:
            continue
        if not path.startswith("/services/hospital/"):
            continue
        ...
        candidates.append(
            {
                ...
                "hospital_name_or_link_text": label or None,
                "hospital_profile_url": candidate_url,
                "candidate_only": True,
...
def run(timeout: float) -> dict[str, Any]:
    ...
    for parcel_id, postcode in POSTCODES:
        ...
        try:
            ...
            evidence.append(
                {
                    ...
                    "record_scope": "one official NHS hospital landing request plus one discovered postcode-search submission; maximum 20 hospital-profile candidate links and 1 MiB per response",
                    "supports_fields": [
                        "NHS hospital profile candidate URL",
                        "visible hospital name or link text",
                        "postcode-level search association",
                    ],
                    "relevant_record_ids_or_excerpt": {
                        "candidate_count": len(found),
                        "candidate_urls": [row["hospital_profile_url"] for row in found],
                        "form_method": form["method"],
                        "location_field": form["field"],
                    },
                    "hospitals_url": HOSPITALS,
                    "about_hospital_services_url": ABOUT,
            ...
            message = f"NHS_HOSPITAL_POSTCODE_ERROR:{type(exc).__name__}:{exc}"
            evidence.append(
                {
                    "parcel_id": parcel_id,
                    "searched_postcode": postcode,
                    ...
                    "record_scope": "one bounded official NHS hospital form-discovery/postcode-search attempt; maximum one landing and one search response; no hospital-profile or document crawl",
                    "supports_fields": ["NHS hospital postcode-search availability"],
    ...
    state = "NHS_HOSPITAL_CANDIDATES_FOUND" if candidates else "NO_DATA_CONTINUE"
    result = {
        "schema_version": 1,
        ...
        "blocker": {
            "code": "NONE" if candidates else "NHS_HOSPITAL_NO_USABLE_RESPONSE_OR_NO_POSTCODE_RESULTS",
            "state": state,
            "manual_action_required": False,
            "retry_unchanged_route": False,
        },
        "next_unverified_step": (
            "VALIDATE_NHS_HOSPITAL_CANDIDATES_WITHOUT_EXACT_PARCEL_OR_PROPERTY_TYPE_INFERENCE"
            if candidates
            else "SELECT_NEXT_OFFICIAL_OR_FREE_SOURCE_AFTER_NHS_HOSPITAL_POSTCODE"
        ),
        "login_or_api_key_used": False,
        "bulk_download_performed": False,
        "hospital_profile_or_document_crawl_performed": False,
    ...
    for output_path in OUT:
        atomic_write(base / output_path, result)
    return result
...
def validate() -> None:
    ...
    if LANDING != "https://www.nhs.uk/service-search/Hospital":
        raise ValueError("official NHS hospital landing required")
    ...
    print("PASS_TARGET_3_NHS_HOSPITAL_FORM_DISCOVERY_POSTCODE_MAX2_REQUESTS_EACH_MAX1MIB_20_CANDIDATES")
