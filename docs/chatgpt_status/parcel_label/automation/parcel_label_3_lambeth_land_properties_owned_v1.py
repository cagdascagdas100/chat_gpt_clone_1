...
TASK_ID = "parcel-label-3-lambeth-land-properties-owned-v1-20260802"
LAYER_URL = "https://gis.lambeth.gov.uk/arcgis/rest/services/LambethLandandPropertiesOwned/FeatureServer/0/query"
PROBE = "england_map_web/data/distance_property_types/parcel_label_3_canonical_probe_latest.json"
OUTPUTS = (
    "docs/chatgpt_status/_shared/slots_21/parcel_label_3/lambeth_land_properties_owned_result_latest.json",
    "england_map_web/data/aays_21_slots/parcel_label_3/lambeth_land_properties_owned_latest.json",
)
...
def validate(root: Path) -> str:
    ...
    return "PASS_TARGET_3_NETWORK_FETCH_RELATIVE_PATHS_LAMBETH_LAND_PROPERTIES_OWNED_25M_MAX10_MAX1MIB"
...
def run(root: Path, timeout: float) -> dict:
    ...
    for index, point in enumerate(points):
        ...
        record = {
            ...
            "http_status": status,
            "record_scope": f"one bounded 25m Lambeth Land and Properties Owned query; max {MAX_RECORDS} records",
            "proven_fields": ["query URL", "access time", "query SHA-256"],
        }
        if raw is None:
            bounded = f"LAMBETH_LAND_PROPERTIES_OWNED_ERROR:{error}"
            record.update({
    ...
    blocker = None if candidates else {
        "code": "LAMBETH_LAND_PROPERTIES_OWNED_NO_USABLE_RESPONSE",
    ...
    result = {
        ...
        "blocker": blocker,
        "next_unverified_step": "SELECT_NEXT_OFFICIAL_OR_FREE_SOURCE_AFTER_LAMBETH_LAND_PROPERTIES_OWNED",
