...
MANIFEST=pathlib.Path('docs/chatgpt_status/_shared/slots_21/parcel_label_3/evidence/lambeth_smoke_control_zone_point_containment_source_manifest_20260804.json')
OUTPUTS=[pathlib.Path('docs/chatgpt_status/_shared/slots_21/parcel_label_3/lambeth_smoke_control_zone_point_containment_result_latest.json'),pathlib.Path('england_map_web/data/aays_21_slots/parcel_label_3/lambeth_smoke_control_zone_point_containment_latest.json')]
ROOTS=['https://gis.lambeth.gov.uk/arcgis/rest/services/LambethSmokeControlZone/FeatureServer','https://gis.lambeth.gov.uk/arcgis/rest/services/LambethSmokeControlZone/MapServer']
...
def load():
 ...
 if m.get('service_roots')!=ROOTS or len(m.get('sources',[]))<5:raise RuntimeError('BAD_MANIFEST')
 for s in m['sources']:
...
def discover(t,e):
 ...
 for root in ROOTS:
  ...
  try:
   ...
   ls.sort(key=lambda x:(0 if all(q in str(x.get('name','')).casefold() for q in ('smoke','control')) else 1,x['id']));x=ls[0]
...
def named(props):
 for k,v in sorted(props.items()):
  if isinstance(v,(str,int,float)) and str(v).strip() and any(x in k.casefold() for x in ('name','zone','smoke','control','order','area')):return ' '.join(str(v).split())
 return 'Lambeth Smoke Control Zone'
def parse(b):
 p=json.loads(b)
 ...
 for i,f in enumerate(p['features'],1):
  ...
  out.append({'feature_id':f.get('id'),'feature_index':i,'official_smoke_control_zone_name':named(a),'geometry_sha256':sha(cj(g).encode()),'raw_attributes_sha256':sha(cj(a).encode())})
 return out,len(p['features'])
def synthetic(r,i,dup=False):
 def f(j):return {'type':'Feature','id':j,'properties':{'NAME':'Lambeth Smoke Control Zone'},'geometry':{'type':'Polygon','coordinates':[[[r['longitude']-.0001,r['latitude']-.0001],[r['longitude']+.0001,r['latitude']-.0001],[r['longitude']+.0001,r['latitude']+.0001],[r['longitude']-.0001,r['latitude']+.0001],[r['longitude']-.0001,r['latitude']-.0001]]]}}
...
def run(rows,t,syn=False,amb=False):
 ...
 if syn:root,lid,lname=ROOTS[0],0,'Smoke Control Zone'
 else:
 ...
 for i,r in enumerate(rows,1):
  ...
  try:
   ...
   if len(c)==1:x.update({'state':'MATCHED_UNIQUE_LAMBETH_SMOKE_CONTROL_ZONE_POLYGON','official_smoke_control_zone_designation':True,'official_smoke_control_zone_label':'Lambeth Smoke Control Zone',**c[0]});matched+=1
   elif len(c)>1:x.update({'state':'NO_DATA','reason':'AMBIGUOUS_MULTIPLE_SMOKE_CONTROL_ZONE_POLYGONS','candidate_geometry_sha256':[a['geometry_sha256'] for a in c]})
   else:x.update({'state':'NO_DATA','reason':'NO_SMOKE_CONTROL_ZONE_POLYGON'})
  except Exception as ex:
...
def main():
 ...
 if z.synthetic_ambiguous_test:
  if m!=2 or rs[1].get('reason')!='AMBIGUOUS_MULTIPLE_SMOKE_CONTROL_ZONE_POLYGONS':raise RuntimeError('SYNTH_AMBIG_FAIL')
  print(cj({'valid':True,'matched_rows':m,'ambiguous_state':rs[1]['state']}));return 0
 state='PUBLISHED' if m else 'NO_DATA_CONTINUE';result={'schema_version':1,'workstream_id':'AAYS_21_SLOT_SAFE_PARALLEL_V1','slot_id':'parcel_label_3','task_id':'parcel-label-3-lambeth-smoke-control-zone-point-containment-v1-20260804','state':state,'panel_status':'PUBLISHED','completed_count':len(rs),'target_count':3,'previous_percent':0.0,'progress_percent':round(len(rs)/3*100,6),'percent_increase':round(len(rs)/3*100,6),'matched_unique_smoke_control_zone_rows':m,'evidence_records':len(rs),'source_evidence':e,'records':rs,'unknown_attributes_promoted_to_label':False,'fake_data':False,'large_raw_files_committed':False,'generated_at':now()};text=cj(result)+'\n'
 for p in OUTPUTS:write(p,text)
 print(cj({'completed_count':len(rs),'target_count':3,'matched_unique_smoke_control_zone_rows':m,'state':state,'output_sha256':sha(text.encode())}));return 0
