...
TASK_ID = "parcel-label-3-nhs-gp-postcode-v1-20260802"
PROBE = "england_map_web/data/distance_property_types/parcel_label_3_canonical_probe_latest.json"
OUT = (
    "docs/chatgpt_status/_shared/slots_21/parcel_label_3/nhs_gp_postcode_result_latest.json",
    "england_map_web/data/aays_21_slots/parcel_label_3/nhs_gp_postcode_latest.json",
)
LANDING = "https://www.nhs.uk/service-search/find-a-gp/"
SERVICES = "https://www.nhs.uk/nhs-services/gps/"
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "form":
            ...
            self.current_form["inputs"].append(
                {key: values.get(key, "") for key in ("name", "id", "type", "value", "placeholder", "aria-label")}
            )
...
def fetch(url: str, timeout: float, body: bytes | None = None) -> tuple[bytes, int | None, str]:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "TerraYield-AAYS/1.0 bounded NHS GP postcode research",
        },
    )
...
def discover_search_form(raw: bytes, base_url: str) -> dict[str, Any]:
    ...
    raise ValueError("NHS GP location/postcode search form not discovered")
...
def extract_candidates(raw: bytes, result_url: str, parcel_id: str, postcode: str) -> list[dict[str, Any]]:
    ...
    for link in parser.links:
        ...
        if parsed.netloc not in {"www.nhs.uk", "nhs.uk"} or not path.startswith("/services/gp-surgery/"):
            continue
        ...
        candidates.append(
            {
                ...
                "gp_surgery_name_or_link_text": label or None,
                "gp_surgery_url": candidate_url,
                "candidate_only": True,
...
def run(timeout: float) -> dict[str, Any]:
    ...
    for parcel_id, postcode in POSTCODES:
        ...
        try:
            ...
            evidence.append(
                {
                    ...
                    "record_scope": "one official NHS GP landing request plus one discovered postcode-search submission; maximum 20 candidate links and 1 MiB per response",
                    "supports_fields": ["NHS GP surgery candidate URL", "visible GP surgery name or link text", "postcode-level search association"],
                    "relevant_record_ids_or_excerpt": {
                        "candidate_count": len(found),
                        "candidate_urls": [row["gp_surgery_url"] for row in found],
            ...
            message = f"NHS_GP_POSTCODE_ERROR:{type(exc).__name__}:{exc}"
            evidence.append(
                {
                    "parcel_id": parcel_id,
                    "searched_postcode": postcode,
                    ...
                    "record_scope": "one bounded official NHS GP form-discovery/postcode-search attempt; maximum one landing and one search response; no surgery-page or document crawl",
                    "supports_fields": ["NHS GP postcode-search availability"],
    ...
    state = "NHS_GP_CANDIDATES_FOUND" if candidates else "NO_DATA_CONTINUE"
    result = {
        "schema_version": 1,
        ...
        "blocker": {
            "code": "NONE" if candidates else "NHS_GP_NO_USABLE_RESPONSE_OR_NO_POSTCODE_RESULTS",
            "state": state,
            "manual_action_required": False,
            "retry_unchanged_route": False,
        },
        "next_unverified_step": (
            "VALIDATE_NHS_GP_CANDIDATES_WITHOUT_EXACT_PARCEL_OR_PROPERTY_TYPE_INFERENCE"
            if candidates
            else "SELECT_NEXT_OFFICIAL_OR_FREE_SOURCE_AFTER_NHS_GP_POSTCODE"
        ),
        "login_or_api_key_used": False,
        "bulk_download_performed": False,
        "surgery_page_or_document_crawl_performed": False,
...
def validate() -> None:
    ...
    print("PASS_TARGET_3_NHS_GP_FORM_DISCOVERY_POSTCODE_MAX2_REQUESTS_EACH_MAX1MIB_20_CANDIDATES")
