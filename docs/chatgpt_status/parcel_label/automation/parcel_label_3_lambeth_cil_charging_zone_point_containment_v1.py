...
import argparse, hashlib, json, pathlib, re, tempfile, urllib.parse, urllib.request
from datetime import datetime, timezone
from typing import Any

INPUT=pathlib.Path('docs/chatgpt_status/_shared/slots_21/parcel_label_3/mdu_status_official_result_latest.json')
MANIFEST=pathlib.Path('docs/chatgpt_status/_shared/slots_21/parcel_label_3/evidence/lambeth_cil_charging_zone_point_containment_source_manifest_20260804.json')
OUTPUTS=[pathlib.Path('docs/chatgpt_status/_shared/slots_21/parcel_label_3/lambeth_cil_charging_zone_point_containment_result_latest.json'),pathlib.Path('england_map_web/data/aays_21_slots/parcel_label_3/lambeth_cil_charging_zone_point_containment_latest.json')]
LAYER_ROOTS=['https://gis.lambeth.gov.uk/arcgis/rest/services/LambethCommunityInfrastructureLevyChargingZones/FeatureServer/0','https://gis.lambeth.gov.uk/arcgis/rest/services/LambethCommunityInfrastructureLevyChargingZones/MapServer/0']
ALLOWED_HOST='gis.lambeth.gov.uk'; MAX_RESPONSE_BYTES=8*1024*1024; MAX_METADATA_REQUESTS=2; MAX_POINT_REQUESTS=3
ZONE_RE=re.compile(r'\b(?:ZONE\s*)?([A-D])\b',re.I)
...
def load_manifest()->dict[str,Any]:
 ...
 if p.get('harvest_guid')!='c5b88683fd87450bbfbba7562ea32f20_0':raise RuntimeError('WRONG_MANIFEST_HARVEST_GUID')
...
def ring_contains_or_touches(ring,point):
 ...
 return inside
def polygon_covers(coords,point):
 return bool(coords and ring_contains_or_touches(coords[0],point) and not any(ring_contains_or_touches(h,point) for h in coords[1:]))
...
def discover_layer(timeout,evidence):
 ...
 for root in LAYER_ROOTS:
  ...
  try:
   ...
   fields={str(f.get('name','')).upper() for f in p.get('fields',[]) if isinstance(f,dict)}
   if 'CIL_ZONE' not in fields:raise RuntimeError('CIL_ZONE_FIELD_MISSING')
   evidence['metadata_response_count']+=1; evidence['metadata_requests'].append({'layer_root':root,'request_url':url,'final_url':final,'http_status':status,'bytes':len(body),'response_sha256':sha256_bytes(body),'geometry_type':p.get('geometryType'),'field_names':sorted(fields),'state':'RESPONSE'}); return root
...
def query_url(root,row):
 q={'where':'1=1','geometry':f"{float(row['longitude']):.15f},{float(row['latitude']):.15f}",'geometryType':'esriGeometryPoint','inSR':'4326','spatialRel':'esriSpatialRelIntersects','outFields':'OBJECTID,CIL_ZONE','returnGeometry':'true','outSR':'4326','f':'geojson'}
...
def parse_candidates(body,row):
 ...
 for i,f in enumerate(fs,1):
  ...
  props=f.get('properties') if isinstance(f.get('properties'),dict) else {}; raw=str(props.get('CIL_ZONE') or '').strip(); m=ZONE_RE.search(raw)
  if not m:continue
  zone=m.group(1).upper(); g=f['geometry']; out.append({'feature_id':f.get('id'),'feature_index':i,'official_cil_zone':zone,'official_cil_zone_raw':raw,'retained_official_attributes':{'OBJECTID':props.get('OBJECTID'),'CIL_ZONE':props.get('CIL_ZONE')},'raw_attributes_sha256':sha256_bytes(canonical_json(props).encode()),'geometry_sha256':sha256_bytes(canonical_json(g).encode()),'geometry':g})
 return out,len(fs)
def synthetic_feature(row,i,zone,offset=0.0):
 ...
 return {'type':'Feature','id':i,'properties':{'OBJECTID':i,'CIL_ZONE':f'Zone {zone}'},'geometry':{'type':'Polygon','coordinates':[ring]}}
def run(rows,timeout,synthetic=False,ambiguous=False):
 ...
 recs=[]; matched=0; zones=['C','D','D']
 for idx,row in enumerate(rows,1):
  url=query_url(root,row); ev['point_query_count']+=1
  try:
   if synthetic:
    fs=[synthetic_feature(row,idx,zones[idx-1])]
    if ambiguous and idx==2:fs.append(synthetic_feature(row,100+idx,'C',.00001))
   ...
   if len(candidates)==1:o.update({'state':'MATCHED_UNIQUE_LAMBETH_CIL_CHARGING_ZONE_POLYGON','official_cil_charging_zone':candidates[0]['official_cil_zone'],**candidates[0]}); matched+=1
   elif len(candidates)>1:o.update({'state':'NO_DATA','reason':'AMBIGUOUS_MULTIPLE_POINT_CONTAINING_LAMBETH_CIL_ZONE_POLYGONS','candidate_geometry_sha256':[c['geometry_sha256'] for c in candidates]})
   else:o.update({'state':'NO_DATA','reason':'NO_POINT_CONTAINING_LAMBETH_CIL_ZONE_POLYGON'})
  except Exception as exc:
...
def main():
 ...
 if a.synthetic_test:
  zones=[r.get('official_cil_charging_zone') for r in recs]
  if matched!=3 or zones!=['C','D','D']:raise RuntimeError(f'SYNTHETIC_UNIQUE_FAILED:{matched}:{zones}')
  print(json.dumps({'valid':True,'matched_rows':matched,'zones':zones,'point_query_count':ev['point_query_count']},sort_keys=True)); return 0
 if a.synthetic_ambiguous_test:
  states=[r['state'] for r in recs]
  if matched!=2 or states[1]!='NO_DATA' or recs[1].get('reason')!='AMBIGUOUS_MULTIPLE_POINT_CONTAINING_LAMBETH_CIL_ZONE_POLYGONS':raise RuntimeError(f'SYNTHETIC_AMBIGUOUS_FAILED:{matched}:{states}')
  print(json.dumps({'valid':True,'matched_rows':matched,'ambiguous_state':states[1]},sort_keys=True)); return 0
 state='PUBLISHED' if matched else 'NO_DATA_CONTINUE'; result={'schema_version':1,'workstream_id':'AAYS_21_SLOT_SAFE_PARALLEL_V1','slot_id':'parcel_label_3','task_id':'parcel-label-3-lambeth-cil-charging-zone-point-containment-v1-20260804','state':state,'panel_status':'PUBLISHED','completed_count':len(recs),'target_count':3,'previous_percent':0.0,'progress_percent':round(len(recs)/3*100,6),'percent_increase':round(len(recs)/3*100,6),'matched_unique_cil_charging_zone_rows':matched,'evidence_records':len(recs),'source_evidence':ev,'records':recs,'unknown_attributes_promoted_to_label':False,'fake_data':False,'large_raw_files_committed':False,'generated_at':now()}
 text=canonical_json(result)+'\n'
 for p in OUTPUTS:atomic_write(p,text)
 print(json.dumps({'completed_count':len(recs),'target_count':3,'matched_unique_cil_charging_zone_rows':matched,'state':state,'output_sha256':sha256_bytes(text.encode())},sort_keys=True)); return 0
