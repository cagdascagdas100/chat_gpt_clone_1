...
TASK_ID = "parcel-label-3-lambeth-metropolitan-sinc-coordinate-v1-20260803"
PROBE_BLOB_SHA = "ea8e95593a58ab6cbb9369abc30bc38ce8543ad9"
SERVICE_ROOT = "https://gis.lambeth.gov.uk/arcgis/rest/services/LambethSiteOfMetropolitanNatureConservationImportance/FeatureServer"
...
DIRECTORY_URL = "https://gis.lambeth.gov.uk/arcgis/rest/services"
...
def select_fields(metadata: dict[str, Any]) -> list[str]:
    ...
    priority_tokens = (
        "name", "site", "ref", "code", "grade", "type", "designation",
        "category", "class", "description", "address",
    )
...
def evidence_error(kind: str, url: str, error: str, scope: str, supports: list[str], accessed_at: str) -> dict[str, Any]:
    return {
        "attempt_kind": kind,
        "source_url": url,
...
def metadata_attempt(timeout: float) -> tuple[list[str], dict[str, Any]]:
    accessed_at = utc_now()
    scope = "one bounded official Lambeth metropolitan-SINC layer metadata request; maximum 1 MiB"
    try:
        ...
        return fields, {
            ...
            "relevant_record_ids_or_excerpt": json.dumps(
                {"name": payload.get("name"), "geometryType": payload.get("geometryType"), "selected_fields": fields},
                separators=(",", ":"),
            )[:1500],
            "terms_or_license_urls": [TERMS_URL, COPYRIGHT_URL, OGL_URL],
            "http_status": status,
        ...
        error = f"LAMBETH_METROPOLITAN_SINC_METADATA_ERROR:{type(exc).__name__}:{exc}"
        return ["OBJECTID"], evidence_error(
            "layer_metadata", METADATA_URL, error, scope, ["layer metadata availability"], accessed_at
        )
...
def point_attempt(point: dict[str, Any], fields: list[str], timeout: float) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    scope = f"one bounded exact-point intersection query; at most {MAX_FEATURES} features, {MAX_FIELDS} non-geometry fields and 1 MiB"
    try:
        ...
        evidence = {
            ...
            "sha256_basis": "bounded_response_bytes",
            "record_scope": scope,
            "supports_fields": ["metropolitan SINC point intersection", *fields],
            "relevant_record_ids_or_excerpt": json.dumps(
                {"feature_count": len(rows), "object_ids": [row["attributes"].get("OBJECTID") for row in rows]},
                separators=(",", ":"),
            )[:1500],
            "terms_or_license_urls": [TERMS_URL, COPYRIGHT_URL, OGL_URL],
            "http_status": status,
        ...
        error = f"LAMBETH_METROPOLITAN_SINC_POINT_ERROR:{type(exc).__name__}:{exc}"
        evidence = evidence_error(
            "exact_point_intersection", url, error, scope,
            ["metropolitan SINC point-intersection availability"], accessed_at,
        )
        evidence.update({"parcel_id": point["parcel_id"], "canonical_point": point})
        return [], evidence
...
def build_payload(points: list[dict[str, Any]], timeout: float) -> dict[str, Any]:
    ...
    return {
        ...
        "blocker": {
            "code": None if rows else "LAMBETH_METROPOLITAN_SINC_NO_USABLE_RESPONSE_OR_NO_POINT_MATCH",
            "state": "NONE" if rows else "NO_DATA_CONTINUE",
            "manual_action_required": False,
            "retry_unchanged_route": False,
        },
        "next_unverified_step": "SELECT_NEXT_OFFICIAL_OR_FREE_SOURCE_AFTER_LAMBETH_METROPOLITAN_SINC_COORDINATE",
        "service_root": SERVICE_ROOT,
        "layer_url": LAYER_URL,
        "service_directory_url": DIRECTORY_URL,
...
def validate(root: Path) -> None:
    ...
    expected = "https://gis.lambeth.gov.uk/arcgis/rest/services/LambethSiteOfMetropolitanNatureConservationImportance/FeatureServer"
    if SERVICE_ROOT != expected:
        raise ValueError("official service root mismatch")
    if MAX_FEATURES != 5 or MAX_FIELDS != 12 or MAX_BYTES != 1_048_576:
        raise ValueError("bounded limits changed")
    print("PASS_TARGET_4_LAMBETH_METROPOLITAN_SINC_1_METADATA_PLUS_3_EXACT_POINT_QUERIES_MAX5_EACH_MAX12_FIELDS_MAX1MIB_CONTEXT_ONLY")
...
def main() -> int:
    ...
    atomic_json(root / "docs/chatgpt_status/_shared/slots_21/parcel_label_3/lambeth_metropolitan_sinc_coordinate_result_latest.json", payload)
    atomic_json(root / "england_map_web/data/aays_21_slots/parcel_label_3/lambeth_metropolitan_sinc_coordinate_latest.json", payload)
