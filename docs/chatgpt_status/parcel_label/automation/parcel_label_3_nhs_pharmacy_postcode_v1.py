...
TASK_ID = "parcel-label-3-nhs-pharmacy-postcode-v1-20260802"
PROBE = "england_map_web/data/distance_property_types/parcel_label_3_canonical_probe_latest.json"
OUT = (
    "docs/chatgpt_status/_shared/slots_21/parcel_label_3/nhs_pharmacy_postcode_result_latest.json",
    "england_map_web/data/aays_21_slots/parcel_label_3/nhs_pharmacy_postcode_latest.json",
)
LANDING = "https://www.nhs.uk/service-search/pharmacy/find-a-pharmacy/"
SERVICES = "https://www.nhs.uk/nhs-services/pharmacies/"
...
def fetch(url: str, timeout: float, body: bytes | None = None) -> tuple[bytes, int | None, str]:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "TerraYield-AAYS/1.0 bounded NHS pharmacy postcode research",
        },
    )
...
def discover_search_form(raw: bytes, base_url: str) -> dict[str, Any]:
    ...
    for form in parser.forms:
        for item in form["inputs"]:
            ...
            if item.get("name") and any(
                token in descriptor
                for token in ("location", "postcode", "post code", "town", "city")
            ):
                return {
    ...
    raise ValueError("NHS pharmacy location/postcode search form not discovered")
...
def extract_candidates(raw: bytes, result_url: str, parcel_id: str, postcode: str) -> list[dict[str, Any]]:
    ...
    for link in parser.links:
        ...
        if parsed.netloc not in {"www.nhs.uk", "nhs.uk"}:
            continue
        if not (path.startswith("/services/pharmacy/") or path.startswith("/services/service-directory/")):
            continue
        ...
        candidates.append(
            {
                ...
                "pharmacy_name_or_link_text": label or None,
                "pharmacy_url": candidate_url,
                "candidate_only": True,
...
def run(timeout: float) -> dict[str, Any]:
    ...
    for parcel_id, postcode in POSTCODES:
        ...
        try:
            ...
            evidence.append(
                {
                    ...
                    "record_scope": "one official NHS pharmacy landing request plus one discovered postcode-search submission; maximum 20 candidate links and 1 MiB per response",
                    "supports_fields": [
                        "NHS pharmacy candidate URL",
                        "visible pharmacy name or link text",
                        "postcode-level search association",
                    ],
                    "relevant_record_ids_or_excerpt": {
                        "candidate_count": len(found),
                        "candidate_urls": [row["pharmacy_url"] for row in found],
            ...
            message = f"NHS_PHARMACY_POSTCODE_ERROR:{type(exc).__name__}:{exc}"
            evidence.append(
                {
                    "parcel_id": parcel_id,
                    "searched_postcode": postcode,
                    ...
                    "record_scope": "one bounded official NHS pharmacy form-discovery/postcode-search attempt; maximum one landing and one search response; no service-page or document crawl",
                    "supports_fields": ["NHS pharmacy postcode-search availability"],
    ...
    state = "NHS_PHARMACY_CANDIDATES_FOUND" if candidates else "NO_DATA_CONTINUE"
    result = {
        "schema_version": 1,
        ...
        "blocker": {
            "code": "NONE" if candidates else "NHS_PHARMACY_NO_USABLE_RESPONSE_OR_NO_POSTCODE_RESULTS",
            "state": state,
            "manual_action_required": False,
            "retry_unchanged_route": False,
        },
        "next_unverified_step": (
            "VALIDATE_NHS_PHARMACY_CANDIDATES_WITHOUT_EXACT_PARCEL_OR_PROPERTY_TYPE_INFERENCE"
            if candidates
            else "SELECT_NEXT_OFFICIAL_OR_FREE_SOURCE_AFTER_NHS_PHARMACY_POSTCODE"
        ),
        "login_or_api_key_used": False,
        "bulk_download_performed": False,
        "service_page_or_document_crawl_performed": False,
...
def validate() -> None:
    ...
    if MAX_BYTES != 1_048_576 or MAX_CANDIDATES != 20 or len(POSTCODES) != 3:
        raise ValueError("bounded guards")
    if not LANDING.startswith("https://www.nhs.uk/service-search/pharmacy/"):
        raise ValueError("official source guard")
    print("PASS_TARGET_3_NHS_PHARMACY_FORM_DISCOVERY_POSTCODE_MAX2_REQUESTS_EACH_MAX1MIB_20_CANDIDATES")
