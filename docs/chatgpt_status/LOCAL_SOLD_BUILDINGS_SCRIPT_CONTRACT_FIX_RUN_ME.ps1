...
$out = Join-Path $statusDir 'LOCAL_SOLD_BUILDINGS_SCRIPT_CONTRACT_PICKUP_LATEST.txt'
...
foreach ($dir in @($pendingDir,$runningDir)) {
  foreach ($f in @(Get-ChildItem $dir -File -Force -ErrorAction Continue)) {
    ...
    if ($isSold) {
      $dest = Join-Path $failedDir ("old_sold_script_contract_${ts}_" + $f.Name)
...
$heldDir = Join-Path $queueRoot "held_non_sold_buildings_script_contract_$ts"
...
$ackScript = Join-Path $scriptDir "sold_buildings_runner_self_ack_$ts.ps1"
$ackLines = @(
  "$" + "ErrorActionPreference = 'Continue'",
  "$" + "repo = '$repo'",
  "$" + "branch = '$branch'",
  ...
  "Set-Location `$repo",
  "'ok' | Set-Content -Path (Join-Path `$fRoot '_runner_self_ack_write_test.txt') -Encoding UTF8",
  ...
  "`$ack += ('f_drive_writable=' + (Test-Path (Join-Path `$fRoot '_runner_self_ack_write_test.txt')))",
  ...
  "`$fd += ('f_drive_writable=' + (Test-Path (Join-Path `$fRoot '_runner_self_ack_write_test.txt')))",
  ...
  "`$contract += 'pending_dir=C:\AAYS_GITHUB_BRIDGE_CLEAN2\ai-queue\pending'",
  "`$contract += 'running_dir=C:\AAYS_GITHUB_BRIDGE_CLEAN2\ai-queue\running'",
  "`$contract += 'done_dir=C:\AAYS_GITHUB_BRIDGE_CLEAN2\ai-queue\done'",
  "`$contract += 'failed_dir=C:\AAYS_GITHUB_BRIDGE_CLEAN2\ai-queue\failed'",
  "`$contract += 'script_field_required=True'",
  "`$contract += 'script_path_executed=$ackScript'",
  ...
  "git commit -m 'status: sold buildings true runner self pickup ack' 2>`$null",
  "git push origin HEAD:$branch"
)
$ackLines -join [Environment]::NewLine | Set-Content -Path $ackScript -Encoding UTF8

$taskId = "sold-buildings-script-contract-pickup-proof-$ts"
...
$task = [ordered]@{
  ...
  task = 'Execute script field to write true runner self ACK and refresh bridge/F-disk/contract reports. Do not run source-gate, V2, LOCATION_MATCH, DB write, deploy, or PPD redownload.'
}
...
'ok' | Set-Content -Path (Join-Path $fRoot '_script_contract_probe_write_test.txt') -Encoding UTF8
...
$failedListing = Get-ChildItem $failedDir -Force -ErrorAction Continue | Sort-Object LastWriteTime -Descending | Select-Object -First 100 Name,Length,LastWriteTime | Format-Table -AutoSize | Out-String

@"
status=LOCAL_SOLD_BUILDINGS_SCRIPT_CONTRACT_PICKUP
...
f_drive_writable=$(Test-Path (Join-Path $fRoot '_script_contract_probe_write_test.txt'))
script_contract=task_json_requires_script_path
...
git add docs/chatgpt_status/LOCAL_SOLD_BUILDINGS_SCRIPT_CONTRACT_PICKUP_LATEST.txt
git commit -m "status: sold buildings script contract pickup proof" 2>$null
git push origin HEAD:$branch

'SOLD_BUILDINGS_SCRIPT_CONTRACT_PICKUP_REPORT_PUSHED_OR_PUSH_ATTEMPTED'
