...
set "AAYS_BOOTSTRAP_FILE=%TEMP%\aays_contract8_%RANDOM%_%RANDOM%.ps1"
...
$tracked=(G @('status','--porcelain','--untracked-files=no'))
if(-not[string]::IsNullOrWhiteSpace($tracked)){
  throw ("BLOCKED_CANONICAL_TRACKED_WORKTREE_DIRTY: "+$tracked)
}
...
$tempV7=Join-Path $env:TEMP ("aays_contract7_core_{0}_{1}.ps1"-f $PID,[guid]::NewGuid().ToString('N'))
try{
