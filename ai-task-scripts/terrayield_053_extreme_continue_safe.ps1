...
$Start = Get-Date
$TaskId = 'terrayield-053-extreme-continue-safe'
$Run = Get-Date -Format 'yyyyMMdd_HHmmss'
$BridgeRoot = 'C:\Users\cagda\Documents\chat_gpt_clone_1'
$ProjectRoot = 'C:\Users\cagda\Documents\GitHub\AAYS\terrayield_land_intelligence'
$ReportDir = Join-Path $ProjectRoot ".aays_next_fix\053_extreme_continue_safe_$Run"
...
Log "TASK=$TaskId"
Log 'MODE=extreme parallel continue; report-only; no data loading; no project mutation; no container action; no external collection'
$topics = @(
'runner_state','watchdog_state','current_last_task','runner_logs','bridge_repo','project_repo','recent_results','pending_chain','dataset_inventory','csv_profile','json_inventory','geo_inventory','hash_manifest','source_registry','licence_matrix','evidence_chain','sale_price_checks','sale_year_checks','area_m2_checks','price_per_m2_checks','location_checks','address_normalization','coordinate_quality','parcel_key_strategy','geometry_boundary_strategy','parcel_candidate_strategy','multi_parcel_ambiguity','boundary_date_policy','duplicate_rules','outlier_rules','conflict_resolution','manual_review_queue','l0_l4_policy','confidence_score_matrix','map_ui_badges','parcel_card_design','export_manifest','data_volume_policy','annual_refresh','audit_trail','backup_recovery','reboot_resilience','endpoint_probe','api_readiness','risk_register','known_bad_blocklist','source_priority','field_coverage','quality_gates','score_calibration','review_dashboard_plan','parsel_sales_join_plan','geometry_intersection_plan','evidence_hash_audit','data_dictionary','test_matrix','failure_modes','next_backlog','program_scorecard','continue_protocol'
)
$jobs=@()
  ...
  $jobs += Start-Job -ScriptBlock {
    ...
    try{
      if($Topic -eq 'runner_state'){
        ...
        $data.outputs=@('source proof','hash reference','field coverage','geometry confidence','manual review','lightweight export','score update')
        $data.mode='report-only safe plan'
        $path=Join-Path $SafeDir ($Topic+'_053.json')
...
Wait-Job -Job $jobs -Timeout 10800 | Out-Null
...
$source=[Math]::Min(98,[Math]::Max(66,$avg+10))
$parcel=[Math]::Min(90,[Math]::Max(50,$avg-6))
...
$program=[Math]::Min(95,[Math]::Max(52,$results.Count*1.7))
...
Write-Output 'TERRAYIELD_053_EXTREME_CONTINUE_SAFE_DONE'
exit 0
