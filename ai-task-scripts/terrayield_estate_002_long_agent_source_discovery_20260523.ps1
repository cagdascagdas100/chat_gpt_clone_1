...
function WriteBeat($phase){('# '+$TaskId+'`n`nTime: '+(Get-Date -Format s)+'`nPhase: '+$phase+'`nDB_WRITE=false`nPRODUCTION_DEPLOY=false') | Set-Content -Encoding UTF8 -Path (Join-Path $HeartbeatDir 'estate-agent-runner.md')}
...
$roots=@('E:\AAYS_DATA\estate_agents','E:\AAYS_DATA\contractor','E:\AAYS_DATA\cost','C:\AAYS_GITHUB_BRIDGE_CLEAN2','C:\Users\cagda\Documents\GitHub\AAYS')
...
$plan=[ordered]@{
  ...
  source_tiers=@(
    'Local existing artifacts under E:\AAYS_DATA and bridge repo',
    'Company websites/contact pages captured with source_url',
    'Official company registers or public directories where permitted',
    'Open web search results manually/legally collected later',
    'User-provided Codex parcel export for parcel_id join'
  )
