...
$TaskId = 'cost50-003-inventory-audit-20260511'
...
Step "TASK=$TaskId"
Step 'MODE=cost50_inventory_audit'
...
} catch { try { Pop-Location } catch {}; $gitStatus = 'GIT_STATUS_ERROR=' + $_.Exception.Message }

$ReportPath = Join-Path $ResultDir "$TaskId.report.md"
$ExternalReportPath = Join-Path $ReportDir "$TaskId.report.md"
$Report = @()
$Report += '# Cost50 Step 003 Inventory Audit'
$Report += ''
...
if ($readiness -ge 80) { $Report += '- Proceed to Step 004 DB/config smoke audit.' } else { $Report += '- Repair missing Cost50 sentinel files before DB smoke.' }
$Report += ''
