...
$OutDir = Join-Path $BridgeRoot "ai-runner-outputs\rtsacc_005_first_10_batch_count_$RunId"
...
$Lines = @("# RTSACC 005 First 10 Batch Count", "", "Output dir: $OutDir", "Runs root: $RunsRoot", "Latest run: $($LatestRun.FullName)", "")
$totalRows = 0
if ($LatestRun) {
  for ($i = 1; $i -le 10; $i++) {
...
$Lines | Set-Content -LiteralPath (Join-Path $OutDir "first_10_batch_count.md") -Encoding UTF8
Write-Host "RTSACC_005_OUTPUT=$OutDir"
