...
$TaskId='ty124-parcel-table-discovery'
...
$py=Join-Path $OutDir 'ty124_parcel_discovery.py'
@'
import os, json, sys
try:
    import psycopg
...
$raw=''
$exit=0
if($dbCreds){$raw=(& python $py 2>&1 | Out-String);$exit=$LASTEXITCODE}else{$raw='{"ok":false,"error":"missing_db_credentials_in_watchdog_session"}';$exit=3;Set-Content -Encoding UTF8 -Path (Join-Path $LegalRoot 'reports\blocked_by_missing_credential_parcel_discovery.json') -Value $raw}
try{$obj=$raw|ConvertFrom-Json}catch{$obj=[ordered]@{ok=$false;error='json_parse_failed';raw=$raw}}
$progress=95
$next='Set DB env for watchdog session or use local PowerShell with credentials, then rerun parcel discovery.'
if($obj.ok -eq $true -and @($obj.candidates).Count -gt 0){$progress=97;$next='Choose a parcel table/id/geom candidate and run contractor_match_to_parcels.'}
elseif($obj.ok -eq $true){$next='No parcel-like table found; import/create PostGIS parcel table, then rerun matcher.'}
$a=[ordered]@{task_id=$TaskId;status='completed';generated_at=(Get-Date).ToString('s');db_credentials_present_in_watchdog=$dbCreds;python_exit_code=$exit;discovery=$obj;plan_progress_percent=$progress;plan_percent_remaining=(100-$progress);next_action=$next}
$a|ConvertTo-Json -Depth 10|Set-Content -Encoding UTF8 -Path (Join-Path $OutDir 'ty124-parcel-table-discovery.audit.json')
$lines=@('# TY124 Parcel Table Discovery','',('Plan completed: '+$progress+'%'),('Plan remaining: '+(100-$progress)+'%'),('DB credentials present in watchdog: '+$dbCreds),('Python exit code: '+$exit),'','## Raw discovery',$raw,'','## Next Action',$next)
$lines|Set-Content -Encoding UTF8 -Path (Join-Path $OutDir 'ty124-parcel-table-discovery.report.md')
