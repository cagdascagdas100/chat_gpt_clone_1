...
$TaskId='aays-051-gap-closure-acceptance-matrix-20260514'
...
Log "TASK=$TaskId"
Log 'MODE=read_only_gap_closure_acceptance_matrix'
...
$checks=[ordered]@{
 ...
 startup_launcher=Has $StartupCmd
...
foreach($k in @('result_md','scripts','project_py','cost_quality_reports')){if($counts[$k] -le 0){$gaps+=$k}}
$score=0;$total=0
...
$r=@('# AAYS 051 Gap Closure Acceptance Matrix','',"Generated: $(Get-Date -Format s)","Task: $TaskId",'','## Checks')
