...
$TaskId='terrayield-102-supabase-attr-object-patch-5worker-safe'
$Stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$Root='C:\Users\cagda\Documents\GitHub\AAYS\terrayield_land_intelligence'
$RunDir=Join-Path $Root ('.aays_real_runs\terrayield__102_supabase_attr_object_patch_5worker_safe__'+$Stamp)
$SlotsDir=Join-Path $RunDir 'slots'
New-Item -ItemType Directory -Force -Path $RunDir,$SlotsDir|Out-Null
$Summary=Join-Path $RunDir 'terrayield__102_summary.md'
$Status=Join-Path $RunDir 'terrayield__102_status.txt'
$Score=Join-Path $RunDir 'terrayield__102_scorecard.csv'
...
W "TASK=$TaskId"
W 'MODE=patch Supabase admin dict rows into attribute objects and verify with five workers'
Set-Location $Root
$svc=Join-Path $Root 'app\services\supabase_admin_service.py'
$patch='missing'
if(Test-Path $svc){
  $txt=Get-Content -Raw -Encoding UTF8 $svc
  Copy-Item $svc ($svc+'.aays102.bak') -Force
  $txt=[regex]::Replace($txt,'# AAYS102_SUPABASE_ATTR_OBJECT_PATCH[\s\S]*?# AAYS102_SUPABASE_ATTR_OBJECT_PATCH_END\r?\n?','')
...
# AAYS102_SUPABASE_ATTR_OBJECT_PATCH
from types import SimpleNamespace as _AAYS102NS


def _aays102_attr(value):
    if isinstance(value, dict):
        return _AAYS102NS(**{k: _aays102_attr(v) for k, v in value.items()})
    if isinstance(value, list):
        return [_aays102_attr(v) for v in value]
    return value


def _aays102_dict(payload):
    if payload is None:
        return {}
...
def _aays102_rows(result):
    if result is None:
        return []
    if isinstance(result, list):
        return [_aays102_attr(x) for x in result]
    data = getattr(result, "data", None)
    if data is not None:
        if isinstance(data, list):
            return [_aays102_attr(x) for x in data]
        return [_aays102_attr(data)]
    if isinstance(result, dict):
        ...
        if data is not None:
            return [_aays102_attr(data)]
        return [_aays102_attr(result)]
    return [_aays102_attr(result)]


def _aays102_call(client, names, *args, **kwargs):
    for name in names:
...
def get_supabase_sync_status():
    try:
        ...
        return _AAYS102NS(configured=configured, note=("Supabase backend configured." if configured else "Backend Supabase config eksik."))
    except Exception as exc:
        return _AAYS102NS(configured=False, note=str(exc) or "Backend Supabase config eksik.")
...
def list_supabase_admin_records(session=None, *, region=None, listing_status=None, search=None, limit=500):
    ...
    result = _aays102_call(client, ["list_rows", "select_rows", "select", "list"], filters=filters, limit=limit, order="updated_at.desc")
    return _aays102_rows(result)
...
def upsert_supabase_admin_record(session=None, payload=None):
    ...
    row = _aays102_dict(payload)
    result = _aays102_call(client, ["upsert_rows", "upsert", "insert_rows", "insert"], [row], return_rows=True)
    rows = _aays102_rows(result)
    return rows[0] if rows else _aays102_attr(row)
# AAYS102_SUPABASE_ATTR_OBJECT_PATCH_END
'@
  Set-Content -Encoding UTF8 -Path $svc -Value ($txt.TrimEnd()+$shim)
  $patch='patched'
}
W ('PATCH_SUPABASE_ATTR_OBJECT='+$patch)
$Slots=@(
...
$Worker={param($Slot,$Cmd,$Root,$SlotsDir)
 $out=Join-Path $SlotsDir ('terrayield__102__'+$Slot+'.md')
 Set-Content -Encoding UTF8 -Path $out -Value @('PROJECT=terrayield','TASK_ID=terrayield-102-supabase-attr-object-patch-5worker-safe','SLOT='+$Slot,'STARTED='+(Get-Date -Format 's'),'CMD='+$Cmd)
 Set-Location $Root
 try{
...
@('metric,score','workers,5','pass_slots,'+$pass,'fail_slots,'+$fail,'error_slots,'+$errorCount,'assertion_markers,'+$assertions,'program_completion,'+$program,'attr_object_patch,100')|Set-Content -Encoding UTF8 $Score
@('PROJECT=terrayield','DISPLAY_PROJECT=TerraYield','CHATGPT_PAGE_PROJECT=aays1','TASK='+$TaskId,'PATCH_SUPABASE_ATTR_OBJECT='+$patch,'WORKERS=5','PASS_SLOTS='+$pass,'FAIL_SLOTS='+$fail,'ERROR_SLOTS='+$errorCount,'ASSERTION_MARKERS='+$assertions,'PROGRAM_COMPLETION='+$program+'/100','ATTR_OBJECT_PATCH=100/100','NEXT_ACTION=devam et')|Set-Content -Encoding UTF8 $Status
W ('PASS_SLOTS='+$pass)
W ('FAIL_SLOTS='+$fail)
W ('ERROR_SLOTS='+$errorCount)
W ('ASSERTION_MARKERS='+$assertions)
W ('PROGRAM_COMPLETION='+$program+'/100')
W 'ATTR_OBJECT_PATCH=100/100'
Write-Output "SUMMARY_FILE=$Summary"
Write-Output "STATUS_FILE=$Status"
Write-Output "SCORE_FILE=$Score"
Write-Output 'TERRAYIELD_102_SUPABASE_ATTR_OBJECT_PATCH_5WORKER_SAFE_DONE'
exit 0
