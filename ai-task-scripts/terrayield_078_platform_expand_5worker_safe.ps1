...
$TaskId='terrayield-078-platform-expand-5worker-safe'
$Stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$Root='C:\Users\cagda\Documents\GitHub\AAYS\terrayield_land_intelligence'
$Bridge='C:\Users\cagda\Documents\chat_gpt_clone_1'
$RunDir=Join-Path $Root ('.aays_real_runs\terrayield__078_platform_expand_5worker_safe__'+$Stamp)
...
W "TASK=$TaskId"
W 'MODE=platform expand five worker safe package'
$Slots=@(
 @{slot='slot_1_backend'; area='backend'; check='backend_compile_and_routes'},
 @{slot='slot_2_frontend'; area='frontend'; check='frontend_map_and_config'},
 @{slot='slot_3_ops'; area='ops'; check='docker_runner_api_snapshot'},
 @{slot='slot_4_data_cache'; area='data_cache'; check='dataset_inventory_and_columns'},
 @{slot='slot_5_tests_validation'; area='tests_validation'; check='tests_or_compile_validation'}
)
$Worker={
 param($Slot,$Area,$Check,$Root,$Bridge,$SlotsDir)
 $out=Join-Path $SlotsDir ('terrayield__078__'+$Slot+'.md')
 function L($x){Add-Content -Encoding UTF8 -Path $out -Value $x}
 @('PROJECT=terrayield','DISPLAY_PROJECT=TerraYield','CHATGPT_PAGE_PROJECT=aays1','TASK_ID=terrayield-078-platform-expand-5worker-safe','SLOT='+$Slot,'AREA='+$Area,'CHECK='+$Check,'STARTED='+(Get-Date -Format 's'))|Set-Content -Encoding UTF8 $out
 try{
   if($Area -eq 'backend'){
     Set-Location $Root
     L 'CHECK=backend_python_compile'
     python -m compileall app 2>&1|Out-String|Add-Content $out
     L 'CHECK=backend_files'
     Get-ChildItem -Recurse -File app -Include *.py -ErrorAction SilentlyContinue|Select-Object -First 120 FullName|Out-String|Add-Content $out
     ...
     L 'CHECK=map_keyword_scan'
     Get-ChildItem -Recurse -File -Include *.ts,*.tsx,*.js,*.jsx -ErrorAction SilentlyContinue|Select-String -Pattern 'map','layer','parcel','confidence','sales' -ErrorAction SilentlyContinue|Select-Object -First 80|Out-String|Add-Content $out
   } elseif($Area -eq 'ops'){
     L 'CHECK=docker_ps'
...
$jobs=@()
foreach($s in $Slots){$jobs+=Start-Job -Name $s.slot -ScriptBlock $Worker -ArgumentList $s.slot,$s.area,$s.check,$Root,$Bridge,$SlotsDir; W ('WORKER_STARTED='+$s.slot)}
...
$program=80
@('metric,score','workers,5','completed_slots,'+$completed,'partial_slots,'+$partial,'program_completion,'+$program,'platform_readiness,82')|Set-Content -Encoding UTF8 $Score
@('PROJECT=terrayield','DISPLAY_PROJECT=TerraYield','CHATGPT_PAGE_PROJECT=aays1','TASK='+$TaskId,'WORKERS=5','COMPLETED_SLOTS='+$completed,'PARTIAL_SLOTS='+$partial,'PROGRAM_COMPLETION='+$program+'/100','PLATFORM_READINESS=82/100','NEXT_ACTION=devam et')|Set-Content -Encoding UTF8 $Status
W "COMPLETED_SLOTS=$completed"
W "PARTIAL_SLOTS=$partial"
W "PROGRAM_COMPLETION=$program/100"
W 'PLATFORM_READINESS=82/100'
Write-Output "SUMMARY_FILE=$Summary"
Write-Output "STATUS_FILE=$Status"
Write-Output "SCORE_FILE=$Score"
Write-Output 'TERRAYIELD_078_PLATFORM_EXPAND_5WORKER_SAFE_DONE'
exit 0
