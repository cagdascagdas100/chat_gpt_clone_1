...
try {
  ...
  Pop-Location
  if ($GitExitCode -eq 0) { if ($GitOutput) { $GitStatus = ($GitOutput -join "`n") } else { $GitStatus = 'CLEAN' } } else { $GitStatus = 'GIT_STATUS_NON_BLOCKING_FAILED' }
} catch { try { Pop-Location } catch {}; $GitStatus = 'GIT_STATUS_EXCEPTION_NON_BLOCKING' }
$Audit = [ordered]@{
  task_id = 'ty-pdi-016-clean-bridge-selftest'
  status = 'completed'
...
$Audit | ConvertTo-Json -Depth 8 | Set-Content -Path (Join-Path $ResultsDir 'ty-pdi-016-clean-bridge-selftest.audit.json') -Encoding UTF8
