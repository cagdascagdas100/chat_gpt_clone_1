...
$TaskId='cost50-032-delivery-readiness-review-20260513'
...
$checks=[ordered]@{cost_root=Has $CostRoot;project_root=Has $ProjectRoot;quality_reports=Has (Join-Path $CostRoot 'quality_reports');handoff_ready=Has (Join-Path $CostRoot 'handoff_ready');app_main=Has (Join-Path $ProjectRoot 'app\main.py');requirements=Has (Join-Path $ProjectRoot 'requirements.txt')}
$counts=[ordered]@{quality_md=Cnt (Join-Path $CostRoot 'quality_reports') '*.md';quality_json=Cnt (Join-Path $CostRoot 'quality_reports') '*.json';handoff_files=Cnt (Join-Path $CostRoot 'handoff_ready') '*';project_py=Cnt $ProjectRoot '*.py';project_sql=Cnt $ProjectRoot '*.sql';ai_result_md=Cnt $ResultDir '*.md'}
...
foreach($k in @('quality_md','handoff_files','project_py','ai_result_md')){$total++;if($counts[$k] -gt 0){$score++}}
$readiness=[int](($score/[Math]::Max($total,1))*100)
$out=Join-Path $ResultDir "$TaskId.report.md"
$r=@('# Cost50 032 Delivery Readiness Review','',"Generated: $(Get-Date -Format s)","Task: $TaskId",'','## Checks')
