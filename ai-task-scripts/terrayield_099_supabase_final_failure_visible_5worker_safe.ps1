...
$TaskId='terrayield-099-supabase-final-failure-visible-5worker-safe'
$Stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$Root='C:\Users\cagda\Documents\GitHub\AAYS\terrayield_land_intelligence'
$RunDir=Join-Path $Root ('.aays_real_runs\terrayield__099_supabase_final_failure_visible_5worker_safe__'+$Stamp)
$SlotsDir=Join-Path $RunDir 'slots'
New-Item -ItemType Directory -Force -Path $RunDir,$SlotsDir|Out-Null
$Summary=Join-Path $RunDir 'terrayield__099_summary.md'
$Status=Join-Path $RunDir 'terrayield__099_status.txt'
$Score=Join-Path $RunDir 'terrayield__099_scorecard.csv'
...
W "TASK=$TaskId"
W 'MODE=make final Supabase admin failure visible and verify controls with five workers'
$Slots=@(
 @{slot='slot_1_admin_service_firstfail_full'; cmd='python -m pytest tests/test_supabase_admin_service.py -x -vv --tb=long'},
 @{slot='slot_2_admin_service_short'; cmd='python -m pytest tests/test_supabase_admin_service.py -q -ra --tb=short'},
 ...
 @{slot='slot_4_control_passes'; cmd='python -m pytest tests/test_admin_publish_service.py tests/test_supabase_sync.py -q'},
...
$Worker={param($Slot,$Cmd,$Root,$SlotsDir)
 $out=Join-Path $SlotsDir ('terrayield__099__'+$Slot+'.md')
 Set-Content -Encoding UTF8 -Path $out -Value @('PROJECT=terrayield','DISPLAY_PROJECT=TerraYield','TASK_ID=terrayield-099-supabase-final-failure-visible-5worker-safe','SLOT='+$Slot,'STARTED='+(Get-Date -Format 's'),'CMD='+$Cmd)
 Set-Location $Root
 try{
...
Write-Output 'TERRAYIELD_099_SUPABASE_FINAL_FAILURE_VISIBLE_5WORKER_SAFE_DONE'
exit 0
