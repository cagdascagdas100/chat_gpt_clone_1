...
$TaskId='aays-112-parcel-elevation-evidence-export-20260519'
...
$DataRoot='E:\AAYS_DATA'
$CostRoot='E:\AAYS_DATA\cost'
$OutXlsx=Join-Path $ResultDir ($TaskId+'.xlsx')
$OutCsv=Join-Path $ResultDir ($TaskId+'.csv')
$OutJson=Join-Path $ResultDir ($TaskId+'.result.json')
$OutReport=Join-Path $ResultDir ($TaskId+'.report.md')
$PyPath=Join-Path $ResultDir ($TaskId+'.py')
@('# AAYS Portable Task Runner Fixed','',"Time: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')",'Status: running',"TaskId: $TaskId","BridgeRoot: $BridgeRoot","ProjectRoot: $Target","TaskFile: $(Join-Path $BridgeRoot 'ai-tasks\current-task.json')",'Message: AAYS 112 parcel elevation evidence export','Mode: no-spawn-foreground-loop','SafeScriptOnly: enabled') | Set-Content -Encoding UTF8 -Path $HeartbeatPath
$py=@'
...
OUT_XLSX = Path(r"__OUT_XLSX__")
OUT_CSV = Path(r"__OUT_CSV__")
OUT_JSON = Path(r"__OUT_JSON__")
OUT_REPORT = Path(r"__OUT_REPORT__")
COLUMNS = [
...
'@
$py=$py.Replace('__OUT_XLSX__',$OutXlsx.Replace('\','\\')).Replace('__OUT_CSV__',$OutCsv.Replace('\','\\')).Replace('__OUT_JSON__',$OutJson.Replace('\','\\')).Replace('__OUT_REPORT__',$OutReport.Replace('\','\\'))
$py | Set-Content -Encoding UTF8 -Path $PyPath
$r=RunT $ResultDir 'python' ('"'+$PyPath+'"') 1200
$runnerResultPath=Join-Path $ResultDir ((Get-Date -Format 'yyyy-MM-dd_HH-mm-ss')+'-'+$TaskId+'.md')
@('# AAYS Portable Task Runner Result','',"## Task ID",$TaskId,'',"## Exit Code",$r.exit_code,'',"## Output",'```text',$r.output,'```') | Set-Content -Encoding UTF8 -Path $runnerResultPath
@('# AAYS Portable Task Runner Fixed','',"Time: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')",'Status: finished',"TaskId: $TaskId","BridgeRoot: $BridgeRoot","ProjectRoot: $Target","TaskFile: $(Join-Path $BridgeRoot 'ai-tasks\current-task.json')","Message: exit=$($r.exit_code) aays_112_parcel_elevation_evidence_export_done",'Mode: no-spawn-foreground-loop','SafeScriptOnly: enabled') | Set-Content -Encoding UTF8 -Path $HeartbeatPath
