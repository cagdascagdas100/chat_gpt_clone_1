...
$Report = Join-Path $ResultDir "aays-083-sleepwait-wrapper-fixed-$Stamp.md"
$Src = Join-Path $PSScriptRoot 'aays_056_db_dryrun_no_pgctl_20260514.ps1'
$Patched = Join-Path $env:TEMP "aays083_$Stamp.ps1"
function AddLine { param([string]$m) $m | Add-Content -Encoding UTF8 $Report }
'# AAYS 083 SleepWait Wrapper Fixed' | Set-Content -Encoding UTF8 $Report
...
$t = $t.Replace('AAYS_056_DB_DRYRUN_NO_PGCTL_DONE=true','AAYS_083_SLEEPWAIT_WRAPPER_FIXED_DONE=true')
Set-Content -Encoding UTF8 -Path $Patched -Value $t
Start-Sleep -Seconds 2
AddLine ('patched: ' + $Patched)
$out = Join-Path $ResultDir "aays-083-child-stdout-$Stamp.log"
$err = Join-Path $ResultDir "aays-083-child-stderr-$Stamp.log"
...
if(Test-Path $out){ Get-Content $out -Tail 160 | Add-Content -Encoding UTF8 $Report }
AddLine '## child stderr tail'
if(Test-Path $err){ Get-Content $err -Tail 160 | Add-Content -Encoding UTF8 $Report }
if($p.ExitCode -eq 0){ AddLine 'AAYS_083_SLEEPWAIT_WRAPPER_FIXED_DONE=true' } else { AddLine 'AAYS_083_SLEEPWAIT_WRAPPER_FIXED_DONE=false' }
exit $p.ExitCode
