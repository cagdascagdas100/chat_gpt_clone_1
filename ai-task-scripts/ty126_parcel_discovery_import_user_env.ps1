...
$TaskId='ty126-parcel-discovery-import-user-env'
...
foreach($k in @('DATABASE_URL','PGHOST','PGPORT','PGDATABASE','PGUSER','PGPASSWORD')){if(-not $env:$k){$v=[Environment]::GetEnvironmentVariable($k,'User');if($v){Set-Item -Path ('Env:'+ $k) -Value $v}}}
$dbCreds=[bool]($env:DATABASE_URL -or ($env:PGHOST -and $env:PGDATABASE -and $env:PGUSER -and $env:PGPASSWORD))
$py=Join-Path $OutDir 'ty126_parcel_discovery.py'
@'
import os, json, sys
try:
    import psycopg
...
if not conninfo.strip():
    print(json.dumps({"ok": False, "error": "missing_db_credentials"}, ensure_ascii=False))
    sys.exit(3)
out=[]
try:
...
if($obj.ok -eq $true -and @($obj.candidates).Count -gt 0){$progress=97;$next='Choose a candidate table plus id/geom columns, then queue matcher/export.'}
elseif($obj.ok -eq $true){$next='No parcel-like table found; import/create PostGIS parcel table, then rerun matcher.'}
$a=[ordered]@{task_id=$TaskId;status='completed';generated_at=(Get-Date).ToString('s');db_credentials_present_after_user_env_import=$dbCreds;python_exit_code=$exit;discovery=$obj;plan_progress_percent=$progress;plan_percent_remaining=(100-$progress);next_action=$next}
$a|ConvertTo-Json -Depth 12|Set-Content -Encoding UTF8 -Path (Join-Path $OutDir 'ty126-parcel-discovery-import-user-env.audit.json')
$lines=@('# TY126 Parcel Discovery Import User Env','',('Plan completed: '+$progress+'%'),('Plan remaining: '+(100-$progress)+'%'),('DB credentials present after User env import: '+$dbCreds),('Python exit code: '+$exit),'','## Raw discovery',$raw,'','## Next Action',$next)
$lines|Set-Content -Encoding UTF8 -Path (Join-Path $OutDir 'ty126-parcel-discovery-import-user-env.report.md')
