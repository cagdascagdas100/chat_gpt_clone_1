...
function Safe-GitStatus($Path) {
  ...
  try {
    ...
    Pop-Location
    if ($exitCode -eq 0) {
      if ($raw) { $status = 'HAS_TRACKED_CHANGES'; $output = ($raw -join "`n") } else { $status = 'CLEAN'; $output = 'CLEAN' }
    } else {
      $status = 'GIT_STATUS_FAILED_NON_BLOCKING'; $output = ($raw -join "`n")
    }
  } catch {
    try { Pop-Location } catch {}
    $status = 'GIT_STATUS_EXCEPTION_NON_BLOCKING'; $output = $_.Exception.Message
  }
...
Write-Host "EXTERNAL_REPORT: $ExternalReport"
exit 0
