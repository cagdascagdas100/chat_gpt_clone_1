...
$Report = Join-Path $ResultDir "aays-086-final-verify-runner-$Stamp.md"
...
'# AAYS 086 Final Verify Runner' | Set-Content -Encoding UTF8 $Report
...
$t = $t.Replace("NULLIF(TRIM(verified_polygon_geojson), '') IS NOT NULL", 'verified_polygon_geojson IS NOT NULL')
$t = $t.Replace('AAYS_056_DB_DRYRUN_NO_PGCTL_DONE=true','AAYS_086_FINAL_VERIFY_DONE=true')
...
if($p.ExitCode -eq 0){ AddLine 'AAYS_086_FINAL_VERIFY_DONE=true' } else { AddLine 'AAYS_086_FINAL_VERIFY_DONE=false' }
exit $p.ExitCode
