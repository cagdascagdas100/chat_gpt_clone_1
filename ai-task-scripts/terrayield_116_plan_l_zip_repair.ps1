...
W '# TerraYield 116 Plan L ZIP Repair'
W ('TASK='+$TaskId)
...
if(!$latest){
  @('TASK='+$TaskId,'RESULT=blocked_no_final_dir','FINAL_ZIP_EXISTS=False','NEXT_COMMAND=devam et')|Set-Content -Encoding UTF8 $Status
  W 'RESULT=blocked_no_final_dir'
  exit 0
}
$zipPath=$latest.FullName+'.zip'
try{
  if(Test-Path $zipPath){Remove-Item -Force $zipPath}
  Compress-Archive -LiteralPath (Join-Path $latest.FullName '*') -DestinationPath $zipPath -Force
}catch{W ('ZIP_ERROR='+$_.Exception.Message)}
...
function CsvRows($p){try{if(Test-Path $p){return @((Import-Csv -LiteralPath $p)).Count}}catch{return -1};return 0}
function GeoCount($p){try{if(Test-Path $p){$j=Get-Content -Raw -Encoding UTF8 $p|ConvertFrom-Json;return $j.features.Count}}catch{return -1};return 0}
$rows=CsvRows $csv
$features=GeoCount $geo
$zipExists=Test-Path $zipPath
$zipBytes=0;if($zipExists){$zipBytes=(Get-Item $zipPath).Length}
$accept=($rows -eq 34864 -and $features -eq 34864 -and (Test-Path $summaryCsv) -and (Test-Path $confCsv) -and $zipExists -and $zipBytes -gt 0)
