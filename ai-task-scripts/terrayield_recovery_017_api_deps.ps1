...
$ReportDir = Join-Path $Project ".aays_next_fix\recovery_017_api_deps_$Run"
...
Log "TASK: TerraYield recovery 017 API dependency bootstrap"
Log "PROGRESS: 98%"
Log "PROJECT=$Project"
...
Log "--- patch compose API command to install deps and run python modules ---"
...
$patched=@()
foreach($file in @('docker-compose.aays-fast-start.yml','docker-compose.yml')){
  if(Test-Path $file){
    Backup $file
    $txt=Get-Content -Raw -Encoding UTF8 $file
    $orig=$txt
    if($txt -match 'alembic upgrade head'){
      $lines=$txt -split "`r?`n"
      $newLines=@()
      foreach($line in $lines){
        if($line -match 'alembic upgrade head' -or $line -match 'uvicorn app.main:app' -or $line -match 'bash -lc' -or $line -match 'sh -lc'){
          if($line -match '^\s*command\s*:'){
            $indent=($line -replace '^(\s*).*','$1')
            $newLines += ($indent + 'command: ' + $command)
          } elseif($line -match '^\s*-\s*(bash|sh)\s*$' -or $line -match '^\s*-\s*-lc\s*$' -or $line -match 'cd /app') {
            # skip list-style old command lines
          } else {
            $newLines += $line
          }
        } else {
      ...
      $txt=($newLines -join [Environment]::NewLine)
    }
    $txt=$txt.Replace('bash -lc','sh -lc')
    if($txt -ne $orig){Set-Content -Encoding UTF8 -Path $file -Value $txt;$patched += $file;Log "PATCHED $file api dependency bootstrap"}else{Log "NO_PATCH $file"}
    Log "--- CONTENT $file ---"
    Get-Content -Raw -Path $file | ForEach-Object{Add-Content -Encoding UTF8 -Path $DetailFile -Value $_}
  }else{Log "MISSING $file"}
}
...
RunCmd "remove api" { docker compose -f docker-compose.yml -f docker-compose.aays-fast-start.yml rm -sf api }
RunCmd "up stack after dependency bootstrap" { docker compose -f docker-compose.yml -f docker-compose.aays-fast-start.yml up -d --remove-orphans }
...
Write-Output "RECOVERY_017_API_DEPS_DONE"
exit 0
