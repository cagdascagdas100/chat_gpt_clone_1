...
function Invoke-Git([string[]]$Args){ try{ Push-Location $BridgeRoot; $out=(& git @Args 2>&1 | Out-String); Add-Content -Encoding UTF8 -Path $MainLog -Value ('> git '+($Args -join ' ')); Add-Content -Encoding UTF8 -Path $MainLog -Value $out; return $out } catch { return $_.Exception.Message } finally { try{ Pop-Location }catch{} } }
function Sync-Repo(){ Invoke-Git @('fetch','origin','main') | Out-Null; Invoke-Git @('reset','--hard','origin/main') | Out-Null }
...
function Push-Results([string]$TaskId){ Invoke-Git @('add','ai-results','ai-heartbeat','ai-tasks\.direct-autopilot-last-task-id','ai-runner-logs') | Out-Null; $commit=Invoke-Git @('commit','-m',('Direct autopilot result '+$TaskId)); if($commit -match 'nothing to commit'){ return }; Invoke-Git @('pull','--rebase','origin','main') | Out-Null; Invoke-Git @('push','origin','main') | Out-Null }
