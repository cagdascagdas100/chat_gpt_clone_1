...
$result=[ordered]@{ timestamp=$stamp; progress=$progress; runner_recovered=$true; windows_toolchain_ready=$winReady; wsl_toolchain_ready=$wslReady; toolchain_ready=$toolchainReady; source_ready=$sourceReady; tools=$tools; docker_images=$dockerImages; assets=$assets; dry_run_allowed=$dryRunAllowed; safety=[ordered]@{ db_write=$false; deploy=$false; fake_data=$false } }
$json=$result|ConvertTo-Json -Depth 8
$json|Set-Content (Join-Path $out 'aays-toolchain-evidence-probe-latest.json') -Encoding UTF8
"AAYS toolchain evidence probe $stamp`nprogress=$progress`nrunner_recovered=true`ntoolchain_ready=$toolchainReady`nsource_ready=$sourceReady`ndry_run_allowed=$dryRunAllowed"|Set-Content (Join-Path $out 'aays-toolchain-evidence-probe-latest.txt') -Encoding UTF8
