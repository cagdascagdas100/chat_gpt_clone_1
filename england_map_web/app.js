(async function () {
  ...
  const TURKISH_MOJIBAKE_REPLACEMENTS = [
    ...
    ["ÃƒÂ¢Ã¢â€šÂ¬Ã‹Å“", "Ã¢â‚¬Ëœ"],
    ["ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€Â¢", "Ã¢â‚¬â„¢"],
    ["ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ", "Ã¢â‚¬Å“"],
        ...
        (scenario) => `
          <div class="scenario-card">
            ...
            <div class="scenario-meta">
              ROI: ${formatNumber(scenario.roiPct, 1)}% Ãƒâ€šÃ‚Â· IRR: ${formatNumber(scenario.irrPct, 1)}%<br />
              Gelir: ${formatMoney(scenario.revenue)} Ãƒâ€šÃ‚Â· Maliyet: ${formatMoney(scenario.totalCost)}
            </div>
          </div>
        `
      )
  ...
  function renderParcelCompareAction(parcel) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma Ãƒâ€Ã‚Â°Ãƒâ€¦Ã…Â¸lemi</h3>
        <div class="workspace-row workspace-action-row">
          <span>${compared ? "Bu parsel karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesinde." : "Bu parsel henÃƒÆ’Ã‚Â¼z karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesinde deÃƒâ€Ã…Â¸il."}</span>
          <button
            class="workspace-button"
            type="button"
            data-compare-action="${compared ? "remove" : "add"}"
            data-compare-key="${encodeURIComponent(getParcelCompareKey(parcel))}"
          >${compared ? "- Listeden ÃƒÆ’Ã¢â‚¬Â¡Ãƒâ€Ã‚Â±kar" : "+ Listeye Ekle"}</button>
        </div>
      </div>
    `;
  }
  ...
  function renderComparedParcelsList(includeRemoveButton = true) {
    if (!comparedParcels.length) {
      return '<div class="workspace-empty">Haritadan bir parcel seÃƒÆ’Ã‚Â§. AÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±lan + butonuyla karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesine ekleyebiliriz.</div>';
    }
                  ...
                  <div class="compare-parcel-details">
                    ...
                    ${
                      item.bestScenario
                        ? `
                          <div class="workspace-section">
                            <h3 class="workspace-section-title">En Ãƒâ€Ã‚Â°yi Senaryo</h3>
                    ...
                    ${
                      includeRemoveButton
                        ? `<button class="workspace-button" type="button" data-remove-compare="${encodeURIComponent(item.key)}">Listeden ÃƒÆ’Ã¢â‚¬Â¡Ãƒâ€Ã‚Â±kar</button>`
                        : ""
                    }
                  </div>
                `
          ...
          .map(
            (entry) => `
              <div class="compare-selection-card">
                <div class="compare-selection-title">${entry.ref} Ãƒâ€šÃ‚Â· ${entry.scenario.name}</div>
                <div class="compare-selection-meta">${formatNumber(entry.scenario.usedM2, 0)} mÃƒâ€šÃ‚Â² Ãƒâ€šÃ‚Â· ROI %${formatNumber(entry.scenario.roiPct, 2)} Ãƒâ€šÃ‚Â· IRR %${formatNumber(entry.scenario.irrPct, 2)}</div>
                ${
                  entry.selectionId
                    ? `<button class="workspace-button" type="button" data-remove-compare-selection="${entry.selectionId}">Senaryoyu KaldÃƒâ€Ã‚Â±r</button>`
                    : `<button class="workspace-button" type="button" data-remove-compare="${encodeURIComponent(entry.key)}">Parseli KaldÃƒâ€Ã‚Â±r</button>`
                }
              </div>
            `
          )
  ...
  function renderSelectedParcelsTable(compareEntries) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">SeÃƒÆ’Ã‚Â§ilen Parseller</h3>
          ...
          <table class="settings-table">
            <thead>
              <tr>
                <th>Parsel</th>
                <th>Parsel ID</th>
                <th>Arsa alanÃƒâ€Ã‚Â± (mÃƒâ€šÃ‚Â²)</th>
  ...
  function renderCompareSignalSummaryTable(compareEntries) {
      ...
      .map((entry) => {
        ...
        if (state.loading) {
          return `
            <tr>
              <td>${entry.ref}</td>
              <td colspan="7">Kaynak sinyalleri yÃƒÆ’Ã‚Â¼kleniyor...</td>
            </tr>
          `;
        }
        if (state.error) {
        ...
        if (!state.hasFetched || !state.signals) {
          return `
            <tr>
              <td>${entry.ref}</td>
              <td colspan="7">Bu parsel iÃƒÆ’Ã‚Â§in source sinyali henÃƒÆ’Ã‚Â¼z hazÃƒâ€Ã‚Â±r deÃƒâ€Ã…Â¸il.</td>
            </tr>
          `;
        }
        return `
          <tr>
            ...
            <td>${filteredView.officialStatus || (filteredView.filterNotice ? "Filtre dÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±" : "-")}</td>
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Parsel BazlÃƒâ€Ã‚Â± Kaynak Sinyalleri</h3>
  ...
  function renderCompareSignalDetails(compareEntries) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Kaynak Linkleri ve Warning DetaylarÃƒâ€Ã‚Â±</h3>
        ${getLandFilterSummaryHtml()}
        <div class="workspace-stack">
          ${parcelEntries
            .map((entry) => {
                ...
                return `
                  <details class="workspace-section workspace-details">
                    ...
                    <div class="workspace-details-body">
                      <div class="workspace-note">${state.loading ? "Kaynak sinyalleri yÃƒÆ’Ã‚Â¼kleniyor..." : "Bu parsel iÃƒÆ’Ã‚Â§in detay henÃƒÆ’Ã‚Â¼z hazÃƒâ€Ã‚Â±r deÃƒâ€Ã…Â¸il."}</div>
                    </div>
                  </details>
                `;
              }
              const sourceRows = filteredView.filteredSources
                .map((source) => {
                  ...
                  return `<div class="workspace-row"><span>${label}</span><strong>${status || "-"} Ãƒâ€šÃ‚Â· ${confidence !== null ? formatNumber(confidence, 1) : "-"} Ãƒâ€šÃ‚Â· ${buildExternalSaleLink(link.url, link.label)}</strong></div>`;
                })
                .join("");
              ...
              return `
                <details class="workspace-section workspace-details">
                  <summary class="workspace-details-summary">${entry.ref} Ãƒâ€šÃ‚Â· ${entry.regionLabel}</summary>
                  <div class="workspace-details-body">
                    <div class="workspace-stack">
                      ${filteredView.filterNotice ? `<div class="workspace-note">${filteredView.filterNotice}</div>` : ""}
                      ${sourceRows || '<div class="workspace-note">Aktif filtreyle eÃƒâ€¦Ã…Â¸leÃƒâ€¦Ã…Â¸en kaynak linki bulunamadÃƒâ€Ã‚Â±.</div>'}
                      ${warningRows || '<div class="workspace-note">Bu parsel iÃƒÆ’Ã‚Â§in ek warning yok.</div>'}
                      <div class="workspace-row"><span>Price Paid History</span><strong>${(state.history || []).length} kayÃƒâ€Ã‚Â±t</strong></div>
                    </div>
                  </div>
                </details>
              `;
  ...
  function renderParcelFeaturesAndDistances(compareEntries) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Parsel ÃƒÆ’Ã¢â‚¬â€œzellikleri ve UzaklÃƒâ€Ã‚Â±klar</h3>
        <div class="workspace-stack">
          ${parcelEntries
            .map(
              (entry) => `
                <details class="workspace-section workspace-details">
                  ...
                  <div class="workspace-details-body">
                    <div class="metric-grid">
                      <div class="metric-card"><div class="metric-card-label">Alan</div><div class="metric-card-value">${formatNumber(entry.areaM2, 0)} mÃƒâ€šÃ‚Â²</div></div>
                      <div class="metric-card"><div class="metric-card-label">EÃƒâ€Ã…Â¸im</div><div class="metric-card-value">%${formatNumber(entry.slopePct, 1)}</div></div>
                      <div class="metric-card"><div class="metric-card-label">Arsa ÃƒÆ’Ã‚Â§evresi</div><div class="metric-card-value">${formatDistanceMeters(entry.perimeterM)}</div></div>
                      <div class="metric-card"><div class="metric-card-label">BoÃƒâ€¦Ã…Â¸ arazi</div><div class="metric-card-value">%${formatNumber(entry.emptyLandSignal * 100, 0)}</div></div>
                    </div>
          ...
          <table class="settings-table">
            <thead>
              <tr>
                <th>Parsel</th>
                <th>Senaryo</th>
                <th>KullanÃƒâ€Ã‚Â±m (mÃƒâ€šÃ‚Â²)</th>
                <th>Kapasite (mÃƒâ€šÃ‚Â²)</th>
                <th>Toplam Maliyet (TRY)</th>
                <th>Gelir (TRY)</th>
                <th>KÃƒÆ’Ã‚Â¢r (TRY)</th>
                <th>ROI</th>
                <th>IRR (Annual)</th>
              </tr>
            </thead>
  ...
  function renderScenarioDetailsByBuildingType(compareEntries) {
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Senaryo DetaylarÃƒâ€Ã‚Â± (YapÃƒâ€Ã‚Â± TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼ne GÃƒÆ’Ã‚Â¶re)</h3>
        <div class="workspace-stack">
          ${compareEntries
            .map((entry) => {
              ...
              return `
                <details class="workspace-section workspace-details">
                  <summary class="workspace-details-summary">${entry.ref} Ãƒâ€šÃ‚Â· ${entry.scenario.name}</summary>
                  <div class="workspace-details-body">
                    <div class="settings-table-wrap">
                      <table class="settings-table">
                        <thead>
                          <tr>
                            <th>YapÃƒâ€Ã‚Â± TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼</th>
                            <th>Birim SayÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â±</th>
                            <th>Birim AlanÃƒâ€Ã‚Â± (mÃƒâ€šÃ‚Â²)</th>
                            <th>KullanÃƒâ€Ã‚Â±m (mÃƒâ€šÃ‚Â²)</th>
                            <th>Birim Maliyet</th>
                            <th>Birim SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ DeÃƒâ€Ã…Â¸eri</th>
                            <th>Toplam DeÃƒâ€Ã…Â¸er</th>
                          </tr>
                        </thead>
                        <tbody>${rows}</tbody>
                      </table>
  ...
  function renderScenarioComparisonCharts(compareEntries) {
    return `
      ${renderScenarioComparisonChart(compareEntries, "profit", "KÃƒÆ’Ã‚Â¢r (TRY)", (value) => formatNumber(value, 0), "SeÃƒÆ’Ã‚Â§ili senaryolarÃƒâ€Ã‚Â±n kÃƒÆ’Ã‚Â¢r karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rmasÃƒâ€Ã‚Â±")}
      ${renderScenarioComparisonChart(compareEntries, "roiPct", "ROI", (value) => formatNumber(value, 4), "SeÃƒÆ’Ã‚Â§ili senaryolarÃƒâ€Ã‚Â±n ROI karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rmasÃƒâ€Ã‚Â±")}
      ${renderScenarioComparisonChart(compareEntries, "irrPct", "IRR (Annual)", (value) => formatNumber(value, 4), "SeÃƒÆ’Ã‚Â§ili senaryolarÃƒâ€Ã‚Â±n yÃƒâ€Ã‚Â±llÃƒâ€Ã‚Â±k IRR karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rmasÃƒâ€Ã‚Â±")}
    `;
  }
  ...
  function renderParcelAnalysisSummaryTable(entries) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma Listesindeki Parseller</h3>
        ${getLandFilterSummaryHtml()}
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
              <tr>
                <th>Parsel ID</th>
                <th>Aktif BÃƒÆ’Ã‚Â¶lge</th>
                <th>Alan (mÃƒâ€šÃ‚Â²)</th>
                <th>EÃƒâ€Ã…Â¸im (%)</th>
                <th>BoÃƒâ€¦Ã…Â¸ Arazi</th>
                <th>Tahmini Arsa Maliyeti (TRY)</th>
              </tr>
            </thead>
            <tbody>${rows}</tbody>
        ...
        return `
          <div class="workspace-section">
            ...
            <div class="metric-grid">
              ...
              <div class="metric-card"><div class="metric-card-label">Aktif BÃƒÆ’Ã‚Â¶lge</div><div class="metric-card-value">${entry.regionLabel}</div></div>
              <div class="metric-card"><div class="metric-card-label">Alan</div><div class="metric-card-value">${formatNumber(entry.areaM2, 0)} mÃƒâ€šÃ‚Â²</div></div>
            ...
            <div class="workspace-section">
              <h3 class="workspace-section-title">Parsel ÃƒÆ’Ã¢â‚¬â€œzellikleri</h3>
              <div class="workspace-stack">${detailRows || '<div class="workspace-note">Uygun parcel ÃƒÆ’Ã‚Â¶zelliÃƒâ€Ã…Â¸i bulunamadÃƒâ€Ã‚Â±.</div>'}</div>
            </div>
          ...
          .map(
            (unit) => `
              <div class="workspace-row">
                ...
                <strong>${unit.name} Ãƒâ€šÃ‚Â· ${formatDistanceMeters(unit.distanceM)}</strong>
              </div>
            `
          )
          .join("")
      : '<div class="workspace-note">Aktif bÃƒÆ’Ã‚Â¶lgede uygun POI/birim detayÃƒâ€Ã‚Â± bulunamadÃƒâ€Ã‚Â±.</div>';
  }

  function renderUnitsOverviewSection(primaryUnits, nearbyUnits) {
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">POI / Birim UzaklÃƒâ€Ã‚Â±klarÃƒâ€Ã‚Â±</h3>
  ...
  const LAND_SOURCE_LABELS = {
    ...
    market_listing_adapter: "LisanslÃƒâ€Ã‚Â± Market Feed",
  ...
  function getSalePriceTruth(record, sources = []) {
    ...
    if (!record || !hasSalePrice(record)) {
      return {
        label: "Fiyat yok",
        detail: "Kaynakta ilan fiyatÃƒâ€Ã‚Â± bulunamadÃƒâ€Ã‚Â±.",
        tone: "neutral",
        isReal: null,
      };
    }
    ...
    if (tier === "licensed") {
      return {
        label: "GerÃƒÆ’Ã‚Â§ek ilan fiyatÃƒâ€Ã‚Â±",
        detail: providerName || "Portal listing kaynaÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±",
        tone: "listing",
        isReal: true,
      };
    }
    if (tier === "official") {
      return {
        label: "Resmi kaynak fiyatÃƒâ€Ã‚Â±",
  ...
  function formatSalePriceWithTruth(record, sources = []) {
    ...
    return hasSalePrice(record) ? `${priceLabel} Ãƒâ€šÃ‚Â· ${truth.label}` : priceLabel;
  }
  ...
  function buildSupabaseSourceDetails(records) {
    return (records || []).map((record) => ({
      ...
      detail: {
        ...
        external_label: record.source_url ? "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ ilanÃƒâ€Ã‚Â±na git" : null,
  ...
  function renderParcelSaleRecords(records) {
    const rows = (records || [])
      ...
      .map((record) => {
        ...
        return `
          <tr>
            ...
            <td>${priceTruth.detail || "-"}${linkTruth?.label ? ` Ãƒâ€šÃ‚Â· ${linkTruth.label}` : ""}</td>
      ...
      .join("");
    if (!rows) {
      return '<div class="workspace-note">Bu parcel iÃƒÆ’Ã‚Â§in filtrelenmiÃƒâ€¦Ã…Â¸ satÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ kaydÃƒâ€Ã‚Â± yok.</div>';
    }
    return `<table class="data-table"><thead><tr><th>SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ KaydÃƒâ€Ã‚Â±</th><th>Kaynak</th><th>Durum</th><th>Ilan Alani</th><th>Fiyat</th><th>Fiyat TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼</th><th>Planning</th><th>Link</th><th>Updated</th></tr></thead><tbody>${rows}</tbody></table>`;
  }
  ...
  function buildFilteredLandIntelligenceView(state) {
    ...
    const filterNotice = hasActiveLandFilters() && !hasMatches ? "Aktif filtrelerle eÃƒâ€¦Ã…Â¸leÃƒâ€¦Ã…Â¸en kaynak sinyali yok." : null;
    return {
  ...
  function renderScenarioScoreCards(parcelDetail, parcelLike) {
    ...
    if (!entries.length) {
      return '<div class="workspace-note">HenÃƒÆ’Ã‚Â¼z scenario scoring hesaplanmadi.</div>';
    }
    return `
      <div class="scenario-score-grid">
        ${entries
  ...
  function renderFacilityContextSection(parcelLike, parcelDetail) {
    ...
    if (!context) {
      return `
        <div class="workspace-section">
          <h3 class="workspace-section-title">YakÃƒâ€Ã‚Â±n Cevre Profili</h3>
          <div class="workspace-note">Buildings & facilities context henÃƒÆ’Ã‚Â¼z hesaplanmadi.</div>
        </div>
      `;
    }
    const nearestRows = [
  ...
  function renderFutureDevelopmentSection(parcelDetail, futureGrowthScore, plannedAssets) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Future Development & Infrastructure</h3>
        <div class="workspace-note">Bu bolum yatÄ±rÄ±m tavsiyesi degil; resmi kaynaklara dayali evidence-backed forward-looking sinyal ozetidir.</div>
        <div class="metric-grid">
  ...
  function renderLandIntelligenceSection(parcelLike) {
    ...
    const sourceLinks = filteredView.filteredSources
      .map((source) => {
        ...
        const confidence = getLandSourceConfidence(source);
        const status = getLandSourceStatus(source);
        return `<div class="workspace-row"><span>${label}</span><strong>${status || "status yok"} Ãƒâ€šÃ‚Â· ${confidence !== null ? formatNumber(confidence, 1) : "-"} Ãƒâ€šÃ‚Â· ${buildExternalSaleLink(link.url, link.label)}</strong></div>`;
      })
      .join("");
    const summaryRows = filteredView.filteredSources
      .map((source) => {
        ...
        const confidence = getLandSourceConfidence(source);
        return `<div class="workspace-row"><span>${label}</span><strong>${status || "-"} Ãƒâ€šÃ‚Â· ${updated} Ãƒâ€šÃ‚Â· ${confidence !== null ? formatNumber(confidence, 1) : "-"}</strong></div>`;
      })
      .join("");
    ...
    const actionableSaleLink = actionableSaleRecord ? resolveSaleRecordLink(actionableSaleRecord, filteredView.filteredSources) : { url: null, label: "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ adresine git" };
    ...
    const managedSummaryBlock = filteredView.topManagedRecord
      ? `<div class="workspace-note">Supabase yÃƒÆ’Ã‚Â¶netimli kayÃƒâ€Ã‚Â±t ÃƒÆ’Ã‚Â¶ncelikli gÃƒÆ’Ã‚Â¶steriliyor: <strong>${filteredView.topManagedRecord.parcel_name || filteredView.topManagedRecord.label || filteredView.topManagedRecord.source_record_id || "-"}</strong> Ãƒâ€šÃ‚Â· ${filteredView.topManagedRecord.listing_status || filteredView.topManagedRecord.status || "durum yok"}${filteredView.topManagedRecord.ask_price ? ` Ãƒâ€šÃ‚Â· ${formatSalePriceWithTruth(filteredView.topManagedRecord, filteredView.filteredSources)}` : ""}</div>`
      : "";
    const actionableSummaryBlock = actionableSaleRecord
      ? `<div class="workspace-note">Bu parcel icin kullaniciya acik en iyi satis kaydi: <strong>${actionableSaleRecord.parcel_name || actionableSaleRecord.label || actionableSaleRecord.source_record_id || "-"}</strong> Ãƒâ€šÃ‚Â· ${actionableSaleRecord.listing_status || actionableSaleRecord.status || "durum yok"} Ãƒâ€šÃ‚Â· ${formatSalePriceWithTruth(actionableSaleRecord, filteredView.filteredSources)} Ãƒâ€šÃ‚Â· ${actionableSaleTruth.detail || "-"} Ãƒâ€šÃ‚Â· ${actionableLinkTruth.label || "Link tipi bilinmiyor"} Ãƒâ€šÃ‚Â· ${buildExternalSaleLink(actionableSaleLink.url, actionableSaleLink.label)}</div>`
      : "";
    ...
    return `
      <div class="workspace-section">
        ...
        <div class="metric-grid">
          <div class="metric-card"><div class="metric-card-label">Supabase KaydÃƒâ€Ã‚Â±</div><div class="metric-card-value">${filteredView.managedSources.length ? 'Var' : 'Yok'}</div></div>
        ...
        <details class="workspace-section workspace-details" open>
          <summary class="workspace-details-summary">Parcel Ãƒâ€Ã‚Â°ÃƒÆ’Ã‚Â§ine YazÃƒâ€Ã‚Â±lan SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ KayÃƒâ€Ã‚Â±tlarÃƒâ€Ã‚Â±</summary>
          <div class="workspace-details-body">
            ${renderParcelSaleRecords(filteredSaleRecords)}
          </div>
        </details>
        <details class="workspace-section workspace-details" open>
          <summary class="workspace-details-summary">FiltrelenmiÃƒâ€¦Ã…Â¸ Kaynak ÃƒÆ’Ã¢â‚¬â€œzeti</summary>
          <div class="workspace-details-body workspace-stack">${summaryRows || '<div class="workspace-note">Aktif filtreyle eÃƒâ€¦Ã…Â¸leÃƒâ€¦Ã…Â¸en kaynak ÃƒÆ’Ã‚Â¶zeti yok.</div>'}</div>
        </details>
        <details class="workspace-section workspace-details" open>
          <summary class="workspace-details-summary">FiltrelenmiÃƒâ€¦Ã…Â¸ Source Links</summary>
          <div class="workspace-details-body workspace-stack">${sourceLinks || '<div class="workspace-note">Aktif filtreyle eÃƒâ€¦Ã…Â¸leÃƒâ€¦Ã…Â¸en kaynak linki yok.</div>'}</div>
        </details>
        <details class="workspace-section workspace-details">
          <summary class="workspace-details-summary">Price Paid History</summary>
          <div class="workspace-details-body">
  ...
  function renderParcelDetailsAccordion(parcelLike) {
    return `
      <details class="workspace-section workspace-details">
        <summary class="workspace-details-summary">Parsel AyrÃƒâ€Ã‚Â±ntÃƒâ€Ã‚Â±larÃƒâ€Ã‚Â±</summary>
        <div class="workspace-details-body">
          <div class="metric-grid">
            <div class="metric-card"><div class="metric-card-label">Parsel Ref</div><div class="metric-card-value">${parcelLike.ref}</div></div>
            <div class="metric-card"><div class="metric-card-label">Alan</div><div class="metric-card-value">${formatNumber(parcelLike.areaM2, 0)} mÃƒâ€šÃ‚Â²</div></div>
            <div class="metric-card"><div class="metric-card-label">EÃƒâ€Ã…Â¸im</div><div class="metric-card-value">${formatNumber(parcelLike.slopePct, 1)}%</div></div>
            <div class="metric-card"><div class="metric-card-label">Arsa ÃƒÆ’Ã‚Â§evresi</div><div class="metric-card-value">${formatDistanceMeters(parcelLike.perimeterM)}</div></div>
            <div class="metric-card"><div class="metric-card-label">BoÃƒâ€¦Ã…Â¸ arazi</div><div class="metric-card-value">%${formatNumber(parcelLike.emptyLandSignal * 100, 0)}</div></div>
            <div class="metric-card"><div class="metric-card-label">Aktif BÃƒÆ’Ã‚Â¶lge</div><div class="metric-card-value">${parcelLike.regionLabel}</div></div>
          </div>
        </div>
      </details>
    `;
  ...
  const PARCEL_ANALYSIS_FACTORS = [
    ...
    { factor: "Metro (UlaÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±m)", unitKey: "metro" },
    { factor: "OtobÃƒÆ’Ã‚Â¼s DuraÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±", unitKey: "otobus" },
    { factor: "AVM", unitKey: "avm" },
    { factor: "Deniz Ãƒâ€Ã‚Â°skelesi / Su KenarÃƒâ€Ã‚Â±", unitKey: "iskele" },
    { factor: "Nehir (Su KaynaklarÃƒâ€Ã‚Â±)", unitKey: "nehir" },
    { factor: "Ãƒâ€Ã‚Â°mar PlanÃƒâ€Ã‚Â± & Hukuki Durum", unitKey: null },
    { factor: "Topografya & Zemin ÃƒÆ’Ã¢â‚¬â€œzellikleri", unitKey: null },
  ...
  function renderParcelAnalysisDistanceTable(parcel, factorRows) {
      ...
      .map(
        (row) => `
          <tr>
            ...
            <td>${row.score ?? "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</td>
          </tr>
        `
      )
      .join("");
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">Alternatif YapÃƒâ€Ã‚Â± TÃƒÆ’Ã‚Â¼rleri (KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rmalÃƒâ€Ã‚Â±)</h3>
        <div class="workspace-note">Parsel ${parcel.ref}</div>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
              <tr>
                <th>FaktÃƒÆ’Ã‚Â¶r / YapÃƒâ€Ã‚Â± TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼</th>
                <th>Mesafe</th>
                <th>Skor (1-10)</th>
              </tr>
            </thead>
  ...
  function renderBuildingTypeCoefficientTable() {
    ...
    const rows = supportedTypes
      .map((type) => {
        const cells = planningSettings.factorMatrix.map((row) => `<td>${row[type.key] ?? "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</td>`).join("");
        return `
          <tr>
            <td>${type.label}</td>
            ${cells}
    ...
    return `
      <div class="workspace-section">
        <div class="workspace-note">Kurulum ekranÃƒâ€Ã‚Â±ndaki katsayÃƒâ€Ã‚Â± tablosuna gÃƒÆ’Ã‚Â¶re yapÃƒâ€Ã‚Â± tÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼ faktÃƒÆ’Ã‚Â¶r daÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±lÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â±</div>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
              <tr>
  ...
  function renderGeometricAverageDistributionTable(parcel, factorRows, options = {}) {
      ...
      .map((type) => {
        ...
        return `
          <tr>
            <td>${type.label}</td>
            ${cells}
            <td>${geometricMean !== null ? formatNumber(geometricMean, 2) : "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</td>
            <td>${adjustedSaleMap.has(type.key) ? formatNumber(adjustedSaleMap.get(type.key), 0) : "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</td>
          </tr>
        `;
      })
      .join("");
    return `
      <div class="workspace-section">
        ${showTitle ? '<h3 class="workspace-section-title">YapÃƒâ€Ã‚Â± TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼ FaktÃƒÆ’Ã‚Â¶r DaÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±lÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â± (Geometrik Ortalama)</h3>' : ""}
        <div class="workspace-note">Parsel ${parcel.ref}</div>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
              <tr>
                ...
                ${header}
                <th>Toplam DeÃƒâ€Ã…Â¸er</th>
                <th>Toplam Maliyet DeÃƒâ€Ã…Â¸eri</th>
              </tr>
            </thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
        ${footerNote ? '<div class="analysis-footnote">MVP Notu: Bu analiz, mesafe skorlarÃƒâ€Ã‚Â± ile kurulum katsayÃƒâ€Ã‚Â±larÃƒâ€Ã‚Â±nÃƒâ€Ã‚Â± geometrik ortalama mantÃƒâ€Ã‚Â±Ãƒâ€Ã…Â¸Ãƒâ€Ã‚Â±nda birleÃƒâ€¦Ã…Â¸tirir.</div>' : ""}
      </div>
    `;
  }
  ...
  function renderRecommendationGeometricTables(parcel, insights) {
    ...
    return `
      <div class="workspace-section">
        <h3 class="workspace-section-title">YapÃƒâ€Ã‚Â± TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼ FaktÃƒÆ’Ã‚Â¶r DaÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±lÃƒâ€Ã‚Â±mÃƒâ€Ã‚Â± (Geometrik Ortalama)</h3>
        ${tableEntries
          .map(
            (entry) => `
          ...
          .join("")}
        <div class="analysis-footnote">MVP notu: Bu ÃƒÆ’Ã‚Â¶neri motoru heuristik ve demo veriye dayanÃƒâ€Ã‚Â±r. ÃƒÆ’Ã…â€œretimde veri gÃƒÆ’Ã‚Â¼ncelliÃƒâ€Ã…Â¸i, belirsizlik raporu ve mevzuat doÃƒâ€Ã…Â¸rulamasÃƒâ€Ã‚Â± zorunludur.</div>
      </div>
    `;
  }
  ...
  function calculateScenarioCapacity(parcel) {
    ...
    return planningSettings.capacityMethod === "SatÃƒâ€Ã‚Â±labilir Alan"
  ...
  function buildScenarioSelectionEntryFromDraft(sourceEntry, draftEntries, selectionId = null) {
    ...
    return {
      ...
      scenario: {
        name: components
          .map((component, index) => `${index + 1}. ${component.label} (${formatNumber(component.surfaceArea, 0)} mÃƒâ€šÃ‚Â², ${component.floors} kat)`)
          .join(" + "),
  ...
  function renderScenarioAutoTable(rows) {
    if (!rows.length) {
      return '<div class="workspace-empty">Bu ayarlarla ÃƒÆ’Ã‚Â¼retilebilen senaryo bulunamadÃƒâ€Ã‚Â±.</div>';
    }
    const header = `
      <div class="scenario-table-row scenario-table-head">
        <span>Senaryo</span>
        <span>KullanÃƒâ€Ã‚Â±m</span>
        <span>Doluluk</span>
        <span>Toplam Maliyet</span>
        <span>Gelir</span>
        <span>KÃƒÆ’Ã‚Â¢r</span>
        <span>ROI</span>
      </div>
    `;
    const body = rows
      .map(
        (scenario) => `
          <div class="scenario-table-row">
            ...
            <span>${formatNumber(scenario.usedM2, 0)} mÃƒâ€šÃ‚Â²</span>
          ...
          .map((draft, index) => {
            ...
            return `
              <div class="manual-entry-card">
                ...
                <label class="field-label">${typeConfig.label} yuzey alani (mÃƒâ€šÃ‚Â²)</label>
  ...
  function renderScenarioSelectionSummaryCards(selectionEntry) {
    ...
    const warning = selectionEntry.overCapacity
      ? `<div class="settings-warning">Toplam kullanim ${formatNumber(selectionEntry.scenario.usedM2, 0)} mÃƒâ€šÃ‚Â², parcel kapasitesi olan ${formatNumber(
          selectionEntry.scenario.capacityM2,
          0
        )} mÃƒâ€šÃ‚Â² degerini asiyor. Karsilastirmaya eklemeden once kullanim dusurulmeli.</div>`
      : "";
    return `
      ${warning}
      <div class="workspace-note">Toplam kullanim: ${formatNumber(selectionEntry.scenario.usedM2, 0)} mÃƒâ€šÃ‚Â² / ${formatNumber(
        selectionEntry.scenario.capacityM2,
        0
      )} mÃƒâ€šÃ‚Â²</div>
      <div class="metric-grid">
      ...
      <div class="workspace-note">ROI: ${formatNumber(selectionEntry.scenario.roiPct, 2)} Ãƒâ€šÃ‚Â· IRR: ${formatNumber(
        selectionEntry.scenario.irrPct,
        2
      )} Ãƒâ€šÃ‚Â· Doluluk: %${formatNumber(selectionEntry.scenario.utilization * 100, 0)}</div>
    `;
  }
  ...
  function renderPlanningSettingsPanel(candidates) {
    ...
    const parcelRows = candidates.length
      ? candidates
          ...
          .join("")
      : '<tr><td colspan="4">HenÃƒÆ’Ã‚Â¼z seÃƒÆ’Ã‚Â§ilebilir parcel bulunamadÃƒâ€Ã‚Â±.</td></tr>';
    ...
    const factorHeader = `
      <tr>
        ...
        <th>Ofis</th>
        <th>Karma</th>
        <th>EndÃƒÆ’Ã‚Â¼striyel</th>
        <th>TarÃƒâ€Ã‚Â±msal</th>
      </tr>
    `;
    ...
    return `
      <div class="workspace-section settings-hero">
        <div class="workspace-note settings-warning">Uygulama, aÃƒâ€¦Ã…Â¸aÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±daki varsayÃƒâ€Ã‚Â±mlar ile ÃƒÆ’Ã‚Â§alÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±r. Ãƒâ€Ã‚Â°stersen buradan deÃƒâ€Ã…Â¸iÃƒâ€¦Ã…Â¸tirip kaydedebilirsin.</div>
        <h3 class="workspace-section-title settings-title">Uygulama aÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±lmadan ÃƒÆ’Ã‚Â¶nce temel varsayÃƒâ€Ã‚Â±mlarÃƒâ€Ã‚Â± girin ve onaylayÃƒâ€Ã‚Â±n.</h3>
        <div class="workspace-note">Devam ederseniz, sistem varsayÃƒâ€Ã‚Â±lan deÃƒâ€Ã…Â¸erlerle baÃƒâ€¦Ã…Â¸lar. Ãƒâ€Ã‚Â°sterseniz burada deÃƒâ€Ã…Â¸iÃƒâ€¦Ã…Â¸tirebilirsiniz.</div>
        <div class="settings-grid">
          <div>
            <label class="field-label" for="planningUsableRatio">KullanÃƒâ€Ã‚Â±labilir arsa ÃƒÆ’Ã‚Â§arpanÃƒâ€Ã‚Â±</label>
            <input id="planningUsableRatio" type="number" min="0.1" max="1" step="0.05" value="${planningSettings.usableRatio}" />
          </div>
          <div>
            <label class="field-label" for="planningCapacityMethod">Kapasite hesabÃƒâ€Ã‚Â± yÃƒÆ’Ã‚Â¶ntemi</label>
            <select id="planningCapacityMethod">
              <option value="Taban AlanÃƒâ€Ã‚Â±" ${planningSettings.capacityMethod === "Taban AlanÃƒâ€Ã‚Â±" ? "selected" : ""}>Taban AlanÃƒâ€Ã‚Â±</option>
              <option value="SatÃƒâ€Ã‚Â±labilir Alan" ${planningSettings.capacityMethod === "SatÃƒâ€Ã‚Â±labilir Alan" ? "selected" : ""}>SatÃƒâ€Ã‚Â±labilir Alan</option>
            </select>
          </div>
          <div>
          ...
          <div>
            <label class="field-label" for="planningProjectMonths">Proje sÃƒÆ’Ã‚Â¼resi (ay) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â IRR iÃƒÆ’Ã‚Â§in</label>
            <input id="planningProjectMonths" type="number" min="6" max="60" step="1" value="${planningSettings.projectMonths}" />
          </div>
        </div>
      </div>
      <div class="workspace-section">
        <h3 class="workspace-section-title">Parsel deÃƒâ€Ã…Â¸erleri (alan ve arsa maliyeti)</h3>
        <div class="workspace-note">Web sÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼mÃƒÆ’Ã‚Â¼nde sadece eriÃƒâ€¦Ã…Â¸ilebilir seÃƒÆ’Ã‚Â§ili parselleri dÃƒÆ’Ã‚Â¼zenliyoruz.</div>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
              <tr><th>Parsel ID</th><th>BÃƒÆ’Ã‚Â¶lge</th><th>Arsa alanÃƒâ€Ã‚Â± (mÃƒâ€šÃ‚Â²)</th><th>Tahmini arsa maliyeti (TRY)</th></tr>
            </thead>
      ...
      <div class="workspace-section">
        <h3 class="workspace-section-title">YapÃƒâ€Ã‚Â± tÃƒÆ’Ã‚Â¼rleri (birim alan / maliyet / satÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ deÃƒâ€Ã…Â¸eri)</h3>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead>
              <tr><th>key</th><th>YapÃƒâ€Ã‚Â± tÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼</th><th>Birim alanÃƒâ€Ã‚Â± (mÃƒâ€šÃ‚Â²)</th><th>Min (mÃƒâ€šÃ‚Â²)</th><th>Max (mÃƒâ€šÃ‚Â²)</th><th>Birim maliyet (TRY)</th><th>Birim satÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ deÃƒâ€Ã…Â¸eri (TRY)</th></tr>
            </thead>
      ...
      <div class="workspace-section">
        <h3 class="workspace-section-title">Mesafe ve YapÃƒâ€Ã‚Â± TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼ DeÃƒâ€Ã…Â¸er Analiz Tablosu</h3>
        <div class="workspace-note">AÃƒâ€¦Ã…Â¸aÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±daki katsayÃƒâ€Ã‚Â±larÃƒâ€Ã‚Â± deÃƒâ€Ã…Â¸iÃƒâ€¦Ã…Â¸tirmezseniz, sistem varsayÃƒâ€Ã‚Â±lan deÃƒâ€Ã…Â¸erlerle devam eder.</div>
        <div class="settings-table-wrap">
          <table class="settings-table">
      ...
      <div class="workspace-section">
        <h3 class="workspace-section-title">Ortalama katsayÃƒâ€Ã‚Â±lar (bilgilendirme)</h3>
        <div class="settings-table-wrap">
          <table class="settings-table">
            <thead><tr><th></th><th>YapÃƒâ€Ã‚Â± TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼</th><th>Ortalama KatsayÃƒâ€Ã‚Â±</th></tr></thead>
            <tbody>${averageTableRows}</tbody>
          </table>
        </div>
        <div class="settings-actions">
          <button class="workspace-button" type="button" data-settings-reset="true">VarsayÃƒâ€Ã‚Â±lanlara DÃƒÆ’Ã‚Â¶n</button>
  ...
  function renderWorkspace() {
    ...
    if (!parcel && !["portfolio", "parcel", "compare", "contact", "scenario_select"].includes(activeWorkspaceScreen)) {
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-empty">Bu paneli doldurmak icin haritadan bir parcel secelim. Sonra ayni ekranda PortfÃƒÆ’Ã‚Â¶y, ÃƒÆ’Ã¢â‚¬â€œneri, Senaryo ve KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma akÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±nÃƒâ€Ã‚Â± web uygulamada kullanabiliriz.</div>
        </div>
      `);
      return;
    }
    ...
    if (activeWorkspaceScreen === "portfolio") {
        ...
        <div class="workspace-panel">
          ${
            parcel
              ? `
                <div class="metric-grid">
                  <div class="metric-card"><div class="metric-card-label">SeÃƒÆ’Ã‚Â§ili Parcel</div><div class="metric-card-value">${parcel.ref}</div></div>
                  <div class="metric-card"><div class="metric-card-label">Aktif BÃƒÆ’Ã‚Â¶lge</div><div class="metric-card-value">${parcel.regionLabel}</div></div>
                  <div class="metric-card"><div class="metric-card-label">Alan</div><div class="metric-card-value">${formatNumber(parcel.areaM2, 0)} mÃƒâ€šÃ‚Â²</div></div>
              ...
              `
              : `<div class="workspace-note">Detay kartlarini doldurmak icin haritadan bir parcel sec. KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesi ise aÃƒâ€¦Ã…Â¸aÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±da gÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼nmeye devam eder.</div>`
          }
          <div class="workspace-section">
            <h3 class="workspace-section-title">KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma Listesine Eklenen Parseller</h3>
            <div class="workspace-note">Haritadan parcel seÃƒÆ’Ã‚Â§ince popup ve saÃƒâ€Ã…Â¸ panel ÃƒÆ’Ã‚Â¼zerinde + / - aksiyonu gÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼nÃƒÆ’Ã‚Â¼r. KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rmadaki parseller koyu gÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼nÃƒÆ’Ã‚Â¼r.</div>
      ...
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <h2 class="workspace-page-title">ÃƒÆ’Ã¢â‚¬â€œneri SonuÃƒÆ’Ã‚Â§larÃƒâ€Ã‚Â± ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Parsel Analizi</h2>
            ${
              analysisEntries.length
                ? `<div class="workspace-note">Bu ekran artÃƒâ€Ã‚Â±k karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesindeki gÃƒÆ’Ã‚Â¼ncel parselleri birlikte gÃƒÆ’Ã‚Â¶sterir. Listeden ÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±karÃƒâ€Ã‚Â±lan parseller burada gÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼nmez.</div>`
                : '<div class="workspace-note">Parsel Analizi ekranÃƒâ€Ã‚Â±nÃƒâ€Ã‚Â± doldurmak iÃƒÆ’Ã‚Â§in ÃƒÆ’Ã‚Â¶nce haritadan parselleri karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesine ekleyelim.</div>'
            }
          </div>
          ${
            analysisEntries.length
              ...
              `
              : '<div class="workspace-empty">KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesi boÃƒâ€¦Ã…Â¸. Haritadan bir parsele tÃƒâ€Ã‚Â±klayÃƒâ€Ã‚Â±p + ile ekledikten sonra bu ekran ÃƒÆ’Ã‚Â§oklu karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rmalÃƒâ€Ã‚Â± dolacaktÃƒâ€Ã‚Â±r.</div>'
          }
        </div>
      `);
      ...
      const financeRows = best
        ? `
          <div class="workspace-stack">
            ...
            <div class="workspace-row"><span>SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ Geliri</span><strong>${formatMoney(best.revenue)}</strong></div>
            <div class="workspace-row"><span>Tahmini Arsa Maliyeti</span><strong>${formatMoney(parcel.landCost)}</strong></div>
            <div class="workspace-row"><span>Ãƒâ€Ã‚Â°nÃƒâ€¦Ã…Â¸aat KÃƒÆ’Ã‚Â¢rlÃƒâ€Ã‚Â±lÃƒâ€Ã‚Â±Ãƒâ€Ã…Â¸Ãƒâ€Ã‚Â±</span><strong>${formatMoney(best.profit)}</strong></div>
          </div>
        `
        : '<div class="workspace-note">Finansal ÃƒÆ’Ã‚Â¶zet ÃƒÆ’Ã‚Â¼retilemedi.</div>';
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <h2 class="workspace-page-title">ÃƒÆ’Ã¢â‚¬â€œneri Motoru ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ÃƒÆ’Ã¢â‚¬â€œneri ÃƒÆ’Ã¢â‚¬â€œzeti</h2>
            <div class="workspace-stack">
          ...
          <div class="workspace-section">
            <h3 class="workspace-section-title">ÃƒÆ’Ã¢â‚¬â€œnerilen YapÃƒâ€Ã‚Â± TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼: ${best ? best.name : "-"}</h3>
            <div class="workspace-note"><strong>Model GÃƒÆ’Ã‚Â¼ven DÃƒÆ’Ã‚Â¼zeyi:</strong> ${formatNumber(scoreCard.confidence, 2)}</div>
          </div>
          <div class="workspace-section">
            <h3 class="workspace-section-title">ÃƒÆ’Ã¢â‚¬Â¡ok Kriterli DeÃƒâ€Ã…Â¸erlendirme ÃƒÆ’Ã¢â‚¬â€œzeti</h3>
            <div class="workspace-stack">
              <div class="workspace-note">Ãƒâ€Ã‚Â°mar uygunluÃƒâ€Ã…Â¸u (stub) ve pazar sinyali birlikte ele alÃƒâ€Ã‚Â±nÃƒâ€Ã‚Â±r.</div>
              <div class="workspace-note">Konum/eriÃƒâ€¦Ã…Â¸im, piyasa ve arsa ÃƒÆ’Ã‚Â¶zellikleri ayrÃƒâ€Ã‚Â± skorlanÃƒâ€Ã‚Â±r.</div>
              <div class="workspace-note">AÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±rlÃƒâ€Ã‚Â±klÃƒâ€Ã‚Â± skor ile alternatifler kÃƒâ€Ã‚Â±yaslanÃƒâ€Ã‚Â±r.</div>
            </div>
          </div>
          <div class="workspace-section">
            <h3 class="workspace-section-title">ÃƒÆ’Ã¢â‚¬Â¡ok Kriterli Skor KartÃƒâ€Ã‚Â±</h3>
            ${renderScoreBar("Konum", scoreCard.location)}
            ${renderScoreBar("Piyasa", scoreCard.market)}
            ${renderScoreBar("Arsa ÃƒÆ’Ã¢â‚¬â€œzellikleri", scoreCard.land)}
            <div class="metric-grid">
          ...
          <div class="workspace-section">
            ...
            <div class="workspace-stack">
              <div class="workspace-row"><span>Alan</span><strong>${formatNumber(parcel.areaM2, 2)} mÃƒâ€šÃ‚Â²</strong></div>
              <div class="workspace-row"><span>EÃƒâ€Ã…Â¸im</span><strong>%${formatNumber(parcel.slopePct, 1)}</strong></div>
              <div class="workspace-row"><span>ÃƒÆ’Ã¢â‚¬Â¡evre Bilgisi</span><strong>Arsa ÃƒÆ’Ã‚Â§evresi ${formatDistanceMeters(parcel.perimeterM)}</strong></div>
              <div class="workspace-row"><span>BoÃƒâ€¦Ã…Â¸ arazi sinyali</span><strong>%${formatNumber(parcel.emptyLandSignal * 100, 0)}</strong></div>
            </div>
          </div>
          ...
          ${renderLandIntelligenceSection(parcel)}
          <div class="workspace-section">
            <h3 class="workspace-section-title">Finansal ÃƒÆ’Ã¢â‚¬â€œzet</h3>
            ${financeRows}
          </div>
          <div class="workspace-section">
            <h3 class="workspace-section-title">DiÃƒâ€Ã…Â¸er Senaryolar</h3>
      ...
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <div class="workspace-row workspace-action-row">
              <h2 class="workspace-page-title">Parsel ve Senaryo SeÃƒÆ’Ã‚Â§imi (Parsel BazlÃƒâ€Ã‚Â±)</h2>
              <button class="workspace-button" type="button" data-toggle-scenario-settings="true">${scenarioSelectState.showSettings ? "AyarlarÃƒâ€Ã‚Â± Gizle" : "VarsayÃƒâ€Ã‚Â±mlarÃƒâ€Ã‚Â± DÃƒÆ’Ã‚Â¼zenle"}</button>
            </div>
          </div>
          ${scenarioSelectState.showSettings ? renderPlanningSettingsPanel(candidates) : ""}
          <div class="workspace-section">
            <h3 class="workspace-section-title">Parsel SeÃƒÆ’Ã‚Â§imi</h3>
            ${
              candidates.length
                ? `
                  <label class="field-label" for="scenarioSelectParcel">Parsel seÃƒÆ’Ã‚Â§ (KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rmaya eklenenler)</label>
                  <select id="scenarioSelectParcel">
                    ${candidates
                      .map(
                        ...
                        }>${item.parcel.ref} Ãƒâ€šÃ‚Â· ${formatNumber(item.parcel.areaM2, 0)} mÃƒâ€šÃ‚Â² Ãƒâ€šÃ‚Â· ${item.parcel.regionLabel}</option>`
                      )
                      .join("")}
                  </select>
                `
                : '<div class="workspace-note">KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rmaya eklenmiÃƒâ€¦Ã…Â¸ parcel yok. Haritadan bir parcel seÃƒÆ’Ã‚Â§ip + ile ekleyebilir ya da mevcut seÃƒÆ’Ã‚Â§ili parcel ÃƒÆ’Ã‚Â¼zerinden ilerleyebilirsin.</div>'
            }
            <label class="field-label" for="scenarioSelectManual">Parsel ID ile seÃƒÆ’Ã‚Â§</label>
            <input id="scenarioSelectManual" type="text" value="${manualRef}" placeholder="Parsel ID gir" />
            ${
              selectedCandidate
                ? `<div class="workspace-note">${selectedCandidate.parcel.ref} Ãƒâ€šÃ‚Â· ${selectedCandidate.parcel.regionLabel} Ãƒâ€šÃ‚Â· ${formatNumber(
                    selectedCandidate.parcel.areaM2,
                    0
                  )} mÃƒâ€šÃ‚Â²</div>`
                : '<div class="workspace-note">HenÃƒÆ’Ã‚Â¼z kullanÃƒâ€Ã‚Â±labilir bir parcel bulunamadÃƒâ€Ã‚Â±.</div>'
            }
          </div>
          <div class="workspace-section">
            <h3 class="workspace-section-title">Otomatik Senaryo ÃƒÆ’Ã…â€œretimi</h3>
            <div class="metric-grid">
              <div class="metric-card">
                <div class="metric-card-label">Senaryo sayÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â± (otomatik)</div>
            ...
            <div class="workspace-section-title">Dahil edilecek yapÃƒâ€Ã‚Â± tÃƒÆ’Ã‚Â¼rleri</div>
            ...
            ${
              scenarioParcel
                ? `<div class="workspace-note"><strong>Parsel alanÃƒâ€Ã‚Â±:</strong> ${formatNumber(scenarioParcel.areaM2, 0)} mÃƒâ€šÃ‚Â² Ãƒâ€šÃ‚Â· <strong>KullanÃƒâ€Ã‚Â±labilir kapasite:</strong> ${formatNumber(
                    Math.max(scenarioParcel.areaM2 * 0.9, 1200),
                    0
                  )} mÃƒâ€šÃ‚Â² Ãƒâ€šÃ‚Â· <strong>Tahmini arsa maliyeti:</strong> ${formatMoney(scenarioParcel.landCost)}</div>`
                : ""
            }
            ${
              bestAuto
                ? `<div class="workspace-note"><strong>En kÃƒÆ’Ã‚Â¢rlÃƒâ€Ã‚Â± otomatik senaryo:</strong> ${bestAuto.name} Ãƒâ€šÃ‚Â· KÃƒÆ’Ã‚Â¢r: ${formatMoney(bestAuto.profit)} Ãƒâ€šÃ‚Â· ROI: %${formatNumber(
                    bestAuto.roiPct,
                    1
                  )}</div>`
                : ""
          ...
          <div class="workspace-section">
            <h3 class="workspace-section-title">Senaryo SeÃƒÆ’Ã‚Â§imi (KullanÃƒâ€Ã‚Â±cÃƒâ€Ã‚Â±)</h3>
          ...
          <div class="workspace-section">
            <h3 class="workspace-section-title">SonuÃƒÆ’Ã‚Â§</h3>
            ${renderScenarioSelectionSummaryCards(manualSelectionPreview)}
          </div>
          ${scenarioParcel ? renderLandIntelligenceSection(scenarioParcel) : ""}
          <div class="workspace-section">
            <h3 class="workspace-section-title">KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma Listesi</h3>
            <div class="workspace-note">En az 2 ve en fazla 10 senaryo karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rabiliriz.</div>
            <div class="workspace-row workspace-action-row">
              <button
                class="workspace-button"
                type="button"
              ...
              >SeÃƒÆ’Ã‚Â§imi KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma Listesine Ekle</button>
              <button
                class="workspace-button subtle"
                type="button"
                data-reset-manual-selection="true"
                ${manualDraftEntries.length ? "" : "disabled"}
              >SeÃƒÆ’Ã‚Â§imi Temizle</button>
            </div>
            ${
      ...
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <h2 class="workspace-page-title">KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma (ROI/IRR) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â SeÃƒÆ’Ã‚Â§ilen Senaryolar</h2>
            ${
              !compareEntries.length
                ? '<div class="workspace-empty">Arsa & Senaryo SeÃƒÆ’Ã‚Â§imi ekraninda bir senaryo kurup karsilastirma listesine ekleyelim.</div>'
                : `
                  <div class="workspace-note">Streamlit akÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±ndaki gibi seÃƒÆ’Ã‚Â§ilen parseller, seÃƒÆ’Ã‚Â§ilen senaryolar ve detay tablolarÃƒâ€Ã‚Â± aÃƒâ€¦Ã…Â¸aÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±da birlikte gÃƒÆ’Ã‚Â¶steriliyor.</div>
                  <div class="workspace-row workspace-action-row">
                    <span>SeÃƒÆ’Ã‚Â§ilen Senaryolar</span>
                    <button class="workspace-button" type="button" data-clear-scenario-selections="true">TÃƒÆ’Ã‚Â¼m listeyi temizle</button>
                  </div>
      ...
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <h2 class="workspace-page-title">Senaryo KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â SeÃƒÆ’Ã‚Â§ilen Senaryolar</h2>
            ${
              !compareEntries.length
                ? '<div class="workspace-empty">Senaryo karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rmasÃƒâ€Ã‚Â± iÃƒÆ’Ã‚Â§in once birkac kullanici senaryosu ekleyelim.</div>'
                : `
                  <div class="workspace-row workspace-action-row">
                    <span>SeÃƒÆ’Ã‚Â§ilen Senaryolar</span>
                    <button class="workspace-button" type="button" data-clear-scenario-selections="true">TÃƒÆ’Ã‚Â¼m listeyi temizle</button>
      ...
      setWorkspaceHtml(`
        <div class="workspace-panel">
          <div class="workspace-section">
            <h2 class="workspace-page-title">Ãƒâ€Ã‚Â°letiÃƒâ€¦Ã…Â¸im MenÃƒÆ’Ã‚Â¼sÃƒÆ’Ã‚Â¼ ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Arsa SatÃƒâ€Ã‚Â±n Alma KanallarÃƒâ€Ã‚Â±</h2>
            ${
              !compareEntries.length
                ? '<div class="workspace-empty">Ãƒâ€Ã‚Â°letiÃƒâ€¦Ã…Â¸im akÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â± iÃƒÆ’Ã‚Â§in ÃƒÆ’Ã‚Â¶nce birkaÃƒÆ’Ã‚Â§ parceli karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesine ekleyelim.</div>'
                : `
                  <div class="workspace-note">SeÃƒÆ’Ã‚Â§ilen parseller iÃƒÆ’Ã‚Â§in satÃƒâ€Ã‚Â±n alma kanallarÃƒâ€Ã‚Â± ve yapÃƒâ€Ã‚Â± tÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼ne gÃƒÆ’Ã‚Â¶re yÃƒÆ’Ã‚Â¼klenici iletiÃƒâ€¦Ã…Â¸imleri aÃƒâ€¦Ã…Â¸aÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±da listelenir.</div>
                  <div class="workspace-section">
                    <h3 class="workspace-section-title">KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesindeki parseller</h3>
    ...
    workspaceContentEl.querySelectorAll("[data-remove-compare]").forEach((buttonEl) => {
      buttonEl.addEventListener("click", () => {
        ...
        if (result.changed) {
          showStatus(`${result.entry.ref} karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesinden ÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±karÃƒâ€Ã‚Â±ldÃƒâ€Ã‚Â±.`);
        }
      });
    });
    ...
    workspaceContentEl.querySelectorAll("[data-remove-compare-selection]").forEach((buttonEl) => {
      buttonEl.addEventListener("click", () => {
        ...
        if (result.changed) {
          showStatus(`${result.entry.ref} senaryosu karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesinden ÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±karÃƒâ€Ã‚Â±ldÃƒâ€Ã‚Â±.`);
        }
      });
    });
    ...
    if (clearCompareEl) {
      clearCompareEl.addEventListener("click", () => {
        ...
        showStatus("KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesi temizlendi.");
      });
    }
    ...
    if (clearScenarioSelectionsEl) {
      clearScenarioSelectionsEl.addEventListener("click", () => {
        ...
        showStatus("Senaryo karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesi temizlendi.");
      });
    }
    ...
    workspaceContentEl.querySelectorAll("[data-compare-action]").forEach((buttonEl) => {
      buttonEl.addEventListener("click", () => {
        ...
        if (result.changed) {
          showStatus(`${result.entry.ref} karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesine eklendi.`);
          refreshActiveParcelPopup();
        }
      });
    });
    ...
    if (settingsConfirmEl) {
      settingsConfirmEl.addEventListener("click", () => {
        ...
        showStatus("VarsayÃƒâ€Ã‚Â±mlar kaydedildi, program gÃƒÆ’Ã‚Â¼ncel ayarlarla devam ediyor.");
        renderWorkspace();
      });
    }
    ...
    if (addScenarioSelectionEl) {
      addScenarioSelectionEl.addEventListener("click", () => {
        ...
        if (result.reason === "capacity") {
          showStatus("Bu senaryo parcel kapasitesini aÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±yor. Eklenmeden ÃƒÆ’Ã‚Â¶nce kullanim azaltÃƒâ€Ã‚Â±lmalÃƒâ€Ã‚Â±.", true);
          return;
        }
        if (result.changed) {
          showStatus(`${result.entry.ref} iÃƒÆ’Ã‚Â§in kullanÃƒâ€Ã‚Â±cÃƒâ€Ã‚Â± senaryosu karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesine eklendi.`);
        }
      });
    }
  ...
  let scenarioSelectState = {
    ...
    maxScenarios: 200,
    minUtil: 0.4,
    allowedTypes: ["Konut ApartmanÃƒâ€Ã‚Â±", "Site", "Karma KullanÃƒâ€Ã‚Â±m", "Ticari", "Ofis", "Konut", "EndÃƒÆ’Ã‚Â¼striyel", "Prefabrik", "TarÃƒâ€Ã‚Â±msal"],
  ...
  const WORKSPACE_SCENARIOS = ["Konut ApartmanÃƒâ€Ã‚Â±", "Site", "Karma KullanÃƒâ€Ã‚Â±m", "Ticari", "Ofis", "Konut", "EndÃƒÆ’Ã‚Â¼striyel", "Prefabrik", "TarÃƒâ€Ã‚Â±msal"];
  ...
  const WORKSPACE_SCREENS = [
    { id: "portfolio", label: "PortfÃƒÆ’Ã‚Â¶y Analizi" },
    { id: "parcel", label: "Parsel Analizi" },
    { id: "recommendation", label: "ÃƒÆ’Ã¢â‚¬â€œneri Motoru" },
    { id: "scenario_select", label: "Arsa & Senaryo SeÃƒÆ’Ã‚Â§imi" },
    { id: "compare", label: "KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma (ROI/IRR)" },
    { id: "scenario_compare", label: "Senaryo KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma" },
    { id: "contact", label: "Ãƒâ€Ã‚Â°letiÃƒâ€¦Ã…Â¸im MenÃƒÆ’Ã‚Â¼sÃƒÆ’Ã‚Â¼" },
  ];

  const DEFAULT_PLANNING_SETTINGS = {
    usableRatio: 0.6,
    capacityMethod: "Taban AlanÃƒâ€Ã‚Â±",
    far: 1.5,
    projectMonths: 18,
    buildingTypes: [
      { key: "mustakil", label: "MÃƒÆ’Ã‚Â¼stakil Ev", scenarioName: "Konut", unitArea: 150, minArea: 120, maxArea: 220, unitCost: 2000000, unitSaleValue: 3000000 },
      { key: "apartman", label: "Apartman", scenarioName: "Konut ApartmanÃƒâ€Ã‚Â±", unitArea: 400, minArea: 250, maxArea: 650, unitCost: 6500000, unitSaleValue: 9000000 },
      ...
      { key: "karma", label: "Karma", scenarioName: "Karma KullanÃƒâ€Ã‚Â±m", unitArea: 500, minArea: 300, maxArea: 900, unitCost: 7000000, unitSaleValue: 10000000 },
      { key: "endustriyel", label: "EndÃƒÆ’Ã‚Â¼striyel", scenarioName: "EndÃƒÆ’Ã‚Â¼striyel", unitArea: 800, minArea: 500, maxArea: 1400, unitCost: 10000000, unitSaleValue: 14000000 },
      { key: "tarimsal", label: "TarÃƒâ€Ã‚Â±msal", scenarioName: "TarÃƒâ€Ã‚Â±msal", unitArea: 500, minArea: 250, maxArea: 900, unitCost: 3000000, unitSaleValue: 4200000 },
    ],
    factorMatrix: [
      ...
      { factor: "Metro (UlaÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±m)", mustakil: 6, apartman: 9, site: 9, prefab: 5, perakende: 8, ofis: 9, karma: 9, endustriyel: 10, tarimsal: 3 },
      { factor: "OtobÃƒÆ’Ã‚Â¼s DuraÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±", mustakil: 7, apartman: 8, site: 8, prefab: 6, perakende: 9, ofis: 8, karma: 9, endustriyel: 8, tarimsal: 5 },
      { factor: "AVM", mustakil: 8, apartman: 9, site: 10, prefab: 6, perakende: 10, ofis: 9, karma: 8, endustriyel: 4, tarimsal: 4 },
      { factor: "Deniz Ãƒâ€Ã‚Â°skelesi / Su KenarÃƒâ€Ã‚Â±", mustakil: 5, apartman: 7, site: 7, prefab: 5, perakende: 6, ofis: 5, karma: 6, endustriyel: 8, tarimsal: 7 },
      { factor: "Nehir (Su KaynaklarÃƒâ€Ã‚Â±)", mustakil: 7, apartman: 8, site: 8, prefab: 6, perakende: 7, ofis: 7, karma: 6, endustriyel: 9, tarimsal: 9 },
      { factor: "Ãƒâ€Ã‚Â°mar PlanÃƒâ€Ã‚Â± & Hukuki Durum", mustakil: 9, apartman: 10, site: 10, prefab: 8, perakende: 8, ofis: 7, karma: 7, endustriyel: 3, tarimsal: 6 },
      { factor: "Topografya & Zemin ÃƒÆ’Ã¢â‚¬â€œzellikleri", mustakil: 7, apartman: 8, site: 7, prefab: 7, perakende: 7, ofis: 8, karma: 7, endustriyel: 9, tarimsal: 8 },
  ...
  function updateStats() {
    ...
    const topographySuffix = hasTopographyOverlayData()
      ? ` Ãƒâ€šÃ‚Â· Topografya ${currentTopographyVisible ? "acik" : "kapali"}`
      : "";
    const futureGrowthSuffix = futureGrowthLayerConfig.enabled
      ? ` Ãƒâ€šÃ‚Â· Gelecek Gelisim ${currentFutureGrowthVisible ? "acik" : "kapali"}`
      : "";
    setSanitizedText(renderModeEl, currentBaseMapMode === "satellite"
      ? `MapLibre + PMTiles + GeoJSON + Satellite Ãƒâ€šÃ‚Â· ${getMapViewModeLabel(currentMapViewMode)}${topographySuffix}${futureGrowthSuffix}`
      : `MapLibre + PMTiles + GeoJSON Ãƒâ€šÃ‚Â· ${getMapViewModeLabel(currentMapViewMode)}${topographySuffix}${futureGrowthSuffix}`);
    updateDebug();
  }
  ...
  function updateLandFilterSummary() {
    ...
    if (landOverlayFilters.saleReadyQuickFilter !== "all") {
      parts.push(`hÃƒâ€Ã‚Â±zlÃƒâ€Ã‚Â± gÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼nÃƒÆ’Ã‚Â¼m: ${getSaleReadyQuickFilterLabel(landOverlayFilters.saleReadyQuickFilter)}`);
    }
    if (landOverlayFilters.reviewOnly) {
      parts.push("sadece review gereken kayÃƒâ€Ã‚Â±tlar");
    }
    setSanitizedText(landFilterSummaryEl, parts.length
      ? `Aktif filtreler: ${parts.join(" Ãƒâ€šÃ‚Â· ")}`
      : "Harita sinyalleri tÃƒÆ’Ã‚Â¼m confidence seviyeleriyle gÃƒÆ’Ã‚Â¶steriliyor.");
  ...
  function getLandFilterSummaryHtml() {
    ...
    if (landOverlayFilters.reviewOnly) {
      parts.push("Review-only gÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼nÃƒÆ’Ã‚Â¼m");
    }
    if (!parts.length) {
      return '<div class="workspace-note">Aktif filtre yok. Harita tÃƒÆ’Ã‚Â¼m uygun sinyalleri gÃƒÆ’Ã‚Â¶steriyor.</div>';
    }
    return `<div class="workspace-note">${parts.join(" Ãƒâ€šÃ‚Â· ")}</div>`;
  }
  ...
  function buildManifestStatusSummaryHtml(sourceName) {
    ...
    return `
      <div class="workspace-note">
        <strong>${sourceLabel}</strong> Ãƒâ€šÃ‚Â· ${formatManifestAvailabilityLabel(manifestStatus.availability)}<br />
        ${details.join(" Ãƒâ€šÃ‚Â· ")}
      </div>
    `;
  }
      ...
      ? `
        <div class="global-sale-panel">
          <div class="global-sale-panel-header">
            <div class="nearest-sale-panel-title">Program Genelindeki SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸a Uygun Parcel Dizini</div>
            <div class="nearest-sale-panel-note">
              ${hasActiveLandFilters()
                ? `Aktif filtrelerle ${landOverlayState.globalSaleReadyVisibleCount} parcel gÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼nÃƒÆ’Ã‚Â¼yor. Temel dizinde ${landOverlayState.globalSaleReadyBaseCount} parcel var.`
                : `TÃƒÆ’Ã‚Â¼m yÃƒÆ’Ã‚Â¼klÃƒÆ’Ã‚Â¼ satÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸a uygun parcel seti burada. Uzak zoomda turuncu cluster olarak gÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼nÃƒÆ’Ã‚Â¼r; bir kayda tÃƒâ€Ã‚Â±klayÃƒâ€Ã‚Â±nca harita ilgili parsele uÃƒÆ’Ã‚Â§ar.`}
            </div>
          </div>
          <div class="global-sale-metrics">
            <span class="global-sale-metric"><strong>${landOverlayState.globalSaleReadyVisibleCount}</strong> toplam satÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ sinyali</span>
            <span class="global-sale-metric"><strong>${landOverlayState.globalRealPriceCount}</strong> gerÃƒÆ’Ã‚Â§ek fiyatlÃƒâ€Ã‚Â± kayÃƒâ€Ã‚Â±t</span>
            <span class="global-sale-metric"><strong>${landOverlayState.globalNonDemoVisibleCount}</strong> demo hariÃƒÆ’Ã‚Â§ gÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼nen</span>
          </div>
              ...
              .map(
                (group) => `
                  ...
                  >
                    ...
                    <span>${group.realPrice} gerÃƒÆ’Ã‚Â§ek fiyat</span>
                  </button>
                `
              )
              .join("")}
              ...
              .map(
                (item) => `
                  <div class="nearest-sale-card">
                    ...
                    <div class="nearest-sale-link-row">${buildExternalSaleLink(item.saleUrl, item.saleLinkLabel || "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ adresine git")}</div>
                  </div>
                `
              )
              .join("") || '<div class="workspace-note">Aktif filtrelerle program genelinde gÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼nÃƒÆ’Ã‚Â¼r satÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ parceli kalmadÃƒâ€Ã‚Â±.</div>'}
          </div>
          <div class="nearest-sale-panel-note">Son global yenileme: ${formatLandOverlayDate(landOverlayState.globalSaleReadyLoadedAt)}</div>
        </div>
      `
      : '<div class="workspace-note">Program genelindeki satÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸a uygun parcel dizini yÃƒÆ’Ã‚Â¼kleniyor...</div>';
    ...
    const currentViewSaleBlock = currentViewSaleItems.length
      ? `
        <div class="nearest-sale-panel">
          <div class="nearest-sale-panel-title">Bu GÃƒÆ’Ã‚Â¶rÃƒÆ’Ã‚Â¼nÃƒÆ’Ã‚Â¼mdeki SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸a Uygun Parceller</div>
          <div class="nearest-sale-panel-note">
            Haritada hareket ettikÃƒÆ’Ã‚Â§e bu liste gÃƒÆ’Ã‚Â¼ncellenir. Su anki gorunumde ${currentViewSaleVisibleCount} parcel var${currentViewSaleVisibleCount > currentViewSaleItems.length ? `; ilk ${currentViewSaleItems.length} kayit listeleniyor.` : "."}
          </div>
      ...
      `
      : `<div class="workspace-note">Bulundugun gorunumde henuz satÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸a uygun parcel bulunmadi. Haritayi kaydirinca veya biraz yakinlasinca bu mini panel dolacak.</div>`;
    const historicModeHintBlock = currentHistoricSalesVisible && currentViewHistoricSaleCount === 0
      ? '<div class="workspace-note">GeÃƒÂ§miÃ…Å¸ satÃ„Â±Ã…Å¸ modu aÃƒÂ§Ã„Â±k: bu gÃƒÂ¶rÃƒÂ¼nÃƒÂ¼mde eÃ…Å¸leÃ…Å¸me yok veya backend/veritabanÃ„Â± baÃ„Å¸lantÃ„Â±sÃ„Â± hazÃ„Â±r deÃ„Å¸il.</div>'
      : "";
  ...
  function buildSaleReadyFeatureSummary(feature, options = {}) {
    ...
    return {
      ...
      saleLinkLabel: actionableLink?.label || "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ adresine git",
  ...
  async function syncGlobalSaleReadyOverview(force = false) {
    ...
    try {
      ...
      landOverlayState = {
        ...
        globalSaleReadyLoadedAt: new Date().toISOString(),
        error: error?.message || (currentHistoricSalesVisible ? "Program geneli gecmis satis parcel dizini yuklenemedi." : "Program geneli satÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ parcel dizini yÃƒÆ’Ã‚Â¼klenemedi."),
      };
    }
    updateLandSourceStatusPanel();
  }
  ...
  function buildExternalMarketEvidenceHtml(props) {
    ...
    const sampleRows = samples.slice(0, 5).map((item) => {
      ...
      return `<div class="workspace-row"><span>${link}</span><strong>overlap ${formatNumber(overlap * 100, 0)}% Ã‚Â· conf ${formatNumber(conf * 100, 0)}%</strong></div>`;
    }).join("");
  ...
  function buildVerifiedSalesHistoryHtml(props) {
    ...
    const rows = records.slice(0, 10).map((r) => {
      return '<div class="workspace-row"><span>' +
        escapeHtml(String(r.sale_year || "-")) + ' Ã‚Â· ' + escapeHtml(String(r.property_type || "-")) +
        '</span><strong>' +
        (r.price_paid_gbp ? ('GBP ' + formatNumber(Number(r.price_paid_gbp), 0)) : '-') +
        ' Ã‚Â· ' + (r.building_area_m2 ? (formatNumber(Number(r.building_area_m2), 0) + ' mÃ‚Â²') : '-') +
        ' Ã‚Â· ' + (r.price_per_m2_gbp ? ('GBP ' + formatNumber(Number(r.price_per_m2_gbp), 0) + '/mÃ‚Â²') : '-') +
        '</strong></div>';
    }).join("");
    ...
    return '<div class="workspace-section verified-sales-history-card">' +
      ...
      '<div class="metric-card"><div class="metric-card-label">Sale area</div><div class="metric-card-value">' + (p.latest_sale_area_m2 ? (formatNumber(Number(p.latest_sale_area_m2), 0) + ' mÃ‚Â²') : '-') + '</div></div>' +
      '<div class="metric-card"><div class="metric-card-label">Price per mÃ‚Â²</div><div class="metric-card-value">' + (p.latest_sale_price_per_m2_gbp ? ('GBP ' + formatNumber(Number(p.latest_sale_price_per_m2_gbp), 0)) : '-') + '</div></div>' +
      ...
      '<div class="workspace-note">Official verified sale history only. Populated by HMLR PPD Ã¢â€ â€™ AddressBase UPRN Ã¢â€ â€™ Title_No Ã¢â€ â€™ NPS polygon Ã¢â€ â€™ parcel chain.</div>' +
      '</div>';
  }
  ...
  function renderCompactLandIntelligenceCard(parcelRef) {
    ...
    const sourceLinks = filteredView.filteredSources
      ...
      .map((source) => {
        ...
        const link = resolveLandSourceLink(source);
        const status = getLandSourceStatus(source);
        return `<div class="workspace-row"><span>${label}</span><strong>${status || "-"} Ãƒâ€šÃ‚Â· ${buildExternalSaleLink(link.url, link.label)}</strong></div>`;
      })
      .join("");
    ...
    return `
      <div class="workspace-section">
        ...
        <div class="workspace-note">Son kaynak gÃƒÆ’Ã‚Â¼ncellemesi: ${formatLandIntelligenceDate(filteredView.latestUpdatedAt || state.signals.latest_source_updated_at)}</div>
        ${warnings ? `<div class="workspace-stack">${warnings}</div>` : '<div class="workspace-note">Bu parsel icin ekstra warning yok.</div>'}
        <details class="workspace-section workspace-details">
          <summary class="workspace-details-summary">Kaynak Linkleri ve History ÃƒÆ’Ã¢â‚¬â€œzeti</summary>
          <div class="workspace-details-body workspace-stack">
            ${sourceLinks || '<div class="workspace-note">Aktif filtreyle eÃƒâ€¦Ã…Â¸leÃƒâ€¦Ã…Â¸en kaynak linki bulunamadi.</div>'}
  ...
  async function focusCity(city) {
    ...
    if (city.status === "needs_alias_qa") {
      ...
      showStatus(`${city.city} icin en yakin parcel odaÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±na gidildi. Merkez nokta parcel disinda kalabiliyor.`, true);
    } else {
  ...
  function formatHistoryPropertyType(value) {
    ...
    if (upper === "D") return "MÃƒÂ¼stakil Konut (Detached)";
    if (upper === "S") return "YarÃ„Â± MÃƒÂ¼stakil Konut (Semi-Detached)";
    if (upper === "T") return "SÃ„Â±ra Ev (Terraced)";
    if (upper === "F") return "Daire / Apartman (Flat)";
    if (upper === "O") return "DiÃ„Å¸er (Other)";
    return raw;
  }
  ...
  function classifyHistoryBuildingType(value) {
    ...
    if (["d", "s", "t", "f", "o"].includes(normalized)) {
      const coded = {
        d: "Konut / MÃƒÂ¼stakil",
        s: "Konut / YarÃ„Â± MÃƒÂ¼stakil",
        t: "Konut / SÃ„Â±ra Ev",
        f: "Konut / Apartman-Daire",
        o: "DiÃ„Å¸er YapÃ„Â±",
      };
      return coded[normalized] || "DiÃ„Å¸er YapÃ„Â±";
    }
    if (/(industrial|factory|warehouse|logistics|depo|sanayi)/i.test(normalized)) return "Sanayi BinasÃ„Â±";
    if (/(retail|shop|store|market|perakende)/i.test(normalized)) return "Perakende BinasÃ„Â±";
    if (/(office|ofis)/i.test(normalized)) return "Ofis BinasÃ„Â±";
    if (/(flat|apartment|apartman|maisonette)/i.test(normalized)) return "Apartman / Daire";
    if (/(detached|house|villa|mustakil)/i.test(normalized)) return "MÃƒÂ¼stakil Konut";
    if (/(residential|konut|dwelling)/i.test(normalized)) return "Konut";
    return "DiÃ„Å¸er YapÃ„Â±";
  }
  ...
  function formatMarketOriginalSiteStatusLabel(value) {
    ...
    if (normalized === "synthetic_unverified" || normalized === "irregular" || normalized === "duzensiz" || normalized === "dÃƒÂ¼zensiz") return "Duzensiz/uydurma - gercek poligon degil";
    return value;
  }
  ...
  function normalizeMarketPolygonStatus(value) {
    ...
    if (["derived", "derived_parcel", "derived_location", "derived_from_parcel_match", "turetilmis", "tÃƒÂ¼retilmiÃ…Å¸"].includes(normalized)) {
      return "derived";
    }
    if (["synthetic", "synthetic_unverified", "fabricated", "irregular", "duzensiz", "dÃƒÂ¼zensiz", "uydurma"].includes(normalized)) {
  ...
  function buildExternalSaleLink(url, label = "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ adresine git") {
  ...
  function getListingFeatureLinkLabel(feature) {
    ...
    return "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ adresine git";
  }

  function resolveSaleRecordLink(record, sources = []) {
    if (!record) {
      return { url: null, label: "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ adresine git" };
    }
    if (record.external_url) {
      return { url: record.external_url, label: record.external_label || "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ adresine git" };
    }
    ...
    return {
      ...
      label: detail?.external_label || record.external_label || "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ adresine git",
    };
  }
  ...
  function resolveLandSourceLink(source) {
    ...
    return {
      ...
      label: detail?.external_label || "KaynaÃƒâ€Ã…Â¸a git",
    };
  }
  ...
  function formatDistanceMeters(value) {
    if (!Number.isFinite(value) || value <= 0) return "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â";
    if (value >= 1000) {
  ...
  function assumptionsForScenario(base, scenarioName) {
    const map = {
      "Konut ApartmanÃƒâ€Ã‚Â±": { rev: 1.0, area: 1.0, cost: 1.0, extraMonths: 0 },
      Site: { rev: 1.08, area: 1.05, cost: 1.05, extraMonths: 2 },
      "Karma KullanÃƒâ€Ã‚Â±m": { rev: 1.05, area: 0.98, cost: 1.0, extraMonths: 0 },
      ...
      "EndÃƒÆ’Ã‚Â¼striyel": { rev: 0.88, area: 1.08, cost: 1.08, extraMonths: 2 },
      Prefabrik: { rev: 0.9, area: 0.92, cost: 0.82, extraMonths: 0 },
      "TarÃƒâ€Ã‚Â±msal": { rev: 0.82, area: 1.05, cost: 0.72, extraMonths: 1 },
    };
  ...
  function generateScenarioModels(parcel) {
    ...
    return ["Konut ApartmanÃƒâ€Ã‚Â±", "Site", "Karma KullanÃƒâ€Ã‚Â±m", "Ticari", "Ofis", "Konut", "EndÃƒÆ’Ã‚Â¼striyel", "Prefabrik", "TarÃƒâ€Ã‚Â±msal"].map((name) => {
    ...
    return [
      {
        ...
        note: "BÃƒÆ’Ã‚Â¶lgesel arsa portfÃƒÆ’Ã‚Â¶yleri ve ekspertiz raporu sunar.",
      },
      {
        site: "YatirimEmlak",
        url: `https://www.yatirimemlak.net/arsa/${slug}`,
        note: "YatÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±mcÃƒâ€Ã‚Â± odaklÃƒâ€Ã‚Â± listeleme ve finansman danÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸manlÃƒâ€Ã‚Â±Ãƒâ€Ã…Â¸Ãƒâ€Ã‚Â± saÃƒâ€Ã…Â¸lar.",
      },
      {
        site: "ParselBorsa",
        url: `https://www.parselborsa.com/listing/${slug}`,
        note: "KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rmalÃƒâ€Ã‚Â± emsal analizi ve tapu doÃƒâ€Ã…Â¸rulama akÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â± saÃƒâ€Ã…Â¸lar.",
      },
      {
        site: "NovaArazi",
        url: `https://www.novaarazi.org/portfoy/${slug}`,
        note: "Mimari ÃƒÆ’Ã‚Â¶n ÃƒÆ’Ã‚Â§alÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ma ve yatÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±m dÃƒÆ’Ã‚Â¶nÃƒÆ’Ã‚Â¼Ãƒâ€¦Ã…Â¸ simÃƒÆ’Ã‚Â¼lasyonu sunar.",
      },
    ];
  }
  ...
  function contractorContactsForEntry(entry) {
    ...
    const contacts = {
      mustakil: {
        buildingType: "MÃƒÆ’Ã‚Â¼stakil",
        company: "Arda MÃƒÆ’Ã‚Â¼stakil YapÃƒâ€Ã‚Â±",
        contact: "Arda YÃƒâ€Ã‚Â±ldÃƒâ€Ã‚Â±z",
        phone: "+90 212 555 1122",
        email: "iletisim@ardamustakil.com",
        note: "MÃƒÆ’Ã‚Â¼stakil konut ve bahÃƒÆ’Ã‚Â§eli yaÃƒâ€¦Ã…Â¸am projeleri iÃƒÆ’Ã‚Â§in uygulama ekibi.",
      },
      apartman: {
        ...
        note: "ÃƒÆ’Ã¢â‚¬Â¡ok katlÃƒâ€Ã‚Â± apartman ve site bloklarÃƒâ€Ã‚Â±nda tasarÃƒâ€Ã‚Â±m + yapÃƒâ€Ã‚Â±m desteÃƒâ€Ã…Â¸i.",
      },
      site: {
        buildingType: "Site",
        company: "Toplu Konut GeliÃƒâ€¦Ã…Â¸tirme",
        contact: "Mert AkÃƒâ€Ã‚Â±n",
        phone: "+90 212 555 1455",
        email: "site@toplukonut.com",
        note: "BÃƒÆ’Ã‚Â¼yÃƒÆ’Ã‚Â¼k ÃƒÆ’Ã‚Â¶lÃƒÆ’Ã‚Â§ekli toplu konut ve site masterplan ÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â¶zÃƒÆ’Ã‚Â¼mÃƒÆ’Ã‚Â¼ sunar.",
      },
      perakende: {
        buildingType: "Perakende",
        company: "Perakende Construction & Project",
        contact: "Perakende UzmanÃƒâ€Ã‚Â±",
        phone: "+90 212 555 1943",
        email: "iletisim@perakendeyapi.com",
        note: "Anahtar teslim perakende ve maÃƒâ€Ã…Â¸aza kullanÃƒâ€Ã‚Â±m tiplerinde destek verir.",
      },
      ofis: {
        buildingType: "Ofis",
        company: "WorkHub Office Build",
        contact: "Selin KoÃƒÆ’Ã‚Â§",
        phone: "+90 212 555 1620",
        email: "office@workhubbuild.com",
        note: "Ofis kampÃƒÆ’Ã‚Â¼sÃƒÆ’Ã‚Â¼ ve ticari ofis bloklarÃƒâ€Ã‚Â± iÃƒÆ’Ã‚Â§in yÃƒÆ’Ã‚Â¼klenici aÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â± saÃƒâ€Ã…Â¸lar.",
      },
      karma: {
        buildingType: "Karma",
        ...
        note: "Karma kullanÃƒâ€Ã‚Â±m senaryolarÃƒâ€Ã‚Â±nda fazlama ve uygulama eÃƒâ€¦Ã…Â¸leÃƒâ€¦Ã…Â¸tirmesi yapar.",
      },
      endustriyel: {
        buildingType: "EndÃƒÆ’Ã‚Â¼striyel",
        company: "EndÃƒÆ’Ã‚Â¼stri YapÃƒâ€Ã‚Â± Sistemleri",
        contact: "Burak Tanel",
        phone: "+90 212 555 1880",
        email: "endustri@tesisproje.com",
        note: "Lojistik, depo ve ÃƒÆ’Ã‚Â¼retim yapÃƒâ€Ã‚Â±larÃƒâ€Ã‚Â± iÃƒÆ’Ã‚Â§in mÃƒÆ’Ã‚Â¼hendislik desteklidir.",
      },
      prefab: {
        ...
        note: "HÃƒâ€Ã‚Â±zlÃƒâ€Ã‚Â± kurulumlu modÃƒÆ’Ã‚Â¼ler ve prefabrik yapÃƒâ€Ã‚Â± ÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â¶zÃƒÆ’Ã‚Â¼mleri sunar.",
      },
      tarimsal: {
        buildingType: "TarÃƒâ€Ã‚Â±msal",
        company: "AgriBuild Projects",
        contact: "Murat Toprak",
        phone: "+90 212 555 1458",
        email: "tarim@agribuild.com",
        note: "TarÃƒâ€Ã‚Â±msal depolama, sera ve kÃƒâ€Ã‚Â±rsal yapÃƒâ€Ã‚Â± projelerinde ÃƒÆ’Ã‚Â§alÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±r.",
      },
    };
      ...
      <div class="settings-table-wrap">
        <table class="settings-table">
          <thead>
            <tr>
              <th>Website</th>
              <th>EriÃƒâ€¦Ã…Â¸im URL</th>
              <th>AÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±klama</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
        </table>
        ...
        (entry) => {
          ...
          const signalSummary = signalState.signals
            ? `
              ...
              ${filteredView.topManagedRecord ? `<div class="workspace-note">YÃƒÆ’Ã‚Â¶netimli kayÃƒâ€Ã‚Â±t: <strong>${filteredView.topManagedRecord.parcel_name || filteredView.topManagedRecord.label || "-"}</strong> Ãƒâ€šÃ‚Â· ${filteredView.topManagedRecord.listing_status || filteredView.topManagedRecord.status || "-"}</div>` : ""}
            `
            : `<div class="workspace-note">${signalState.loading ? "Kaynak sinyalleri yÃƒÆ’Ã‚Â¼kleniyor..." : "Kaynak sinyali henÃƒÆ’Ã‚Â¼z hazÃƒâ€Ã‚Â±r deÃƒâ€Ã…Â¸il."}</div>`;
          return `
            <div class="workspace-section">
              <h3 class="workspace-section-title">Parsel ${entry.ref}</h3>
              <div class="metric-grid">
                <div class="metric-card"><div class="metric-card-label">Parsel ID</div><div class="metric-card-value">${entry.ref}</div></div>
                <div class="metric-card"><div class="metric-card-label">Arsa alanÃƒâ€Ã‚Â± (mÃƒâ€šÃ‚Â²)</div><div class="metric-card-value">${formatNumber(entry.areaM2, 0)}</div></div>
              ...
              ${signalSummary}
              <div class="workspace-note">Bu parsel aÃƒâ€¦Ã…Â¸aÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±daki kanallarda listelenebilir. Her platform farklÃƒâ€Ã‚Â± eriÃƒâ€¦Ã…Â¸im ve fiyatlandÃƒâ€Ã‚Â±rma/finansman akÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±na sahip olabilir.</div>
              <h4 class="workspace-section-title">SatÃƒâ€Ã‚Â±n alma kanallarÃƒâ€Ã‚Â± ve fiyat ÃƒÆ’Ã‚Â¶zetleri</h4>
              ${renderPurchaseChannelsTable(entry)}
              <h4 class="workspace-section-title">YapÃƒâ€Ã‚Â± tÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼ne gÃƒÆ’Ã‚Â¶re yÃƒÆ’Ã‚Â¼klenici iletiÃƒâ€¦Ã…Â¸imleri</h4>
  ...
  function buildParcelInsights(parcel) {
    ...
    const labelMap = {
      ...
      avm: "Market",
      hastane: "Hastane",
      otobus: "OtobÃƒÆ’Ã‚Â¼s",
      polis: "Polis",
      iskele: "Ãƒâ€Ã‚Â°skele",
    };
  ...
  function formatParcelFeature(feature) {
    ...
    return entries.length ? entries.join("\n") : "SeÃƒÆ’Ã‚Â§ili parcel ÃƒÆ’Ã‚Â¶zelliÃƒâ€Ã…Â¸i bulunamadÃƒâ€Ã‚Â±.";
  }
  ...
  function buildParcelPopupContent(feature, lngLat) {
    ...
    const topSaleLinkData = topSaleRecord ? resolveSaleRecordLink(topSaleRecord, filteredView?.filteredSources || []) : { url: null, label: "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ adresine git" };
      ...
      ${
        filteredView
          ? `<div class="parcel-popup-meta" style="margin-top:8px;">
              ...
              ${topSaleRecord ? `En ust kayÃƒâ€Ã‚Â±t: ${topSaleRecord.parcel_name || topSaleRecord.label || topSaleRecord.listing_id || "-"} Ãƒâ€šÃ‚Â· ${topSaleRecord.listing_status || topSaleRecord.status || "-"}<br />Fiyat: ${topSalePrice} Ãƒâ€šÃ‚Â· ${topSaleTruth?.label || "Fiyat tipi bilinmiyor"}<br />Kaynak: ${topSaleTruth?.detail || "-"}<br />Link: ${topSaleLinkTruth?.label || "Link tipi bilinmiyor"} Ãƒâ€šÃ‚Â· ${topSaleLinkTruth?.detail || "-"}<br />${buildExternalSaleLink(topSaleLinkData.url, topSaleLinkData.label)}` : "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ kaydÃƒâ€Ã‚Â±: yok"}
            </div>`
          : ""
      }
    ...
    actionButton.setAttribute("aria-label", compared ? "KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesinden ÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±kar" : "KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesine ekle");
    actionButton.title = compared ? "KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesinden ÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±kar" : "KarÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesine ekle";
    actionButton.addEventListener("click", (event) => {
      ...
      if (result.changed) {
        showStatus(`${parcel.ref} ${currentCompared ? "karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesinden ÃƒÆ’Ã‚Â§Ãƒâ€Ã‚Â±karÃƒâ€Ã‚Â±ldÃƒâ€Ã‚Â±" : "karÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±laÃƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rma listesine eklendi"}.`);
      }
  ...
  function openLandUseGridPopup(feature, lngLat) {
    ...
    parcelPopup = new maplibregl.Popup({ closeButton: true, maxWidth: "320px" })
      .setLngLat(lngLat)
      .setDOMContent(buildSignalPopupContent(feature, `Yapi Turu Ãƒâ€šÃ‚Â· ${categoryLabel}`, rows, [categoryLabel]))
      .addTo(map);
    parcelPopup.on("close", () => {
      parcelPopup = null;
      activeParcelPopupState = null;
  ...
  function openSignalPopup(feature, lngLat, kind) {
    ...
    const actionableSaleLink = actionableSaleRecord
      ...
      : { url: null, label: "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ adresine git" };
    ...
    if (kind === "official_sale") {
      ...
      rows = [
        ...
        { label: "Fiyat TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼", value: actionableSaleTruth.label },
        { label: "Fiyat Notu", value: actionableSaleTruth.detail },
        { label: "Link TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼", value: actionableSaleLinkTruth.label },
      ...
      title = props.parcel_name || props.listing_id || "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ AlanÃƒâ€Ã‚Â±";
      rows = [
        { label: "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ alanÃƒâ€Ã‚Â±", value: props.parcel_name || props.listing_id || "-" },
        { label: "EÃƒâ€¦Ã…Â¸leÃƒâ€¦Ã…Â¸en parcel", value: props.matched_parcel_ref || props.matched_inspire_id || "-" },
        { label: "Authority", value: props.local_authority || "-" },
        ...
        { label: "Link TÃƒÆ’Ã‚Â¼rÃƒÆ’Ã‚Â¼", value: actionableSaleLinkTruth.label },
        { label: "Link Notu", value: actionableSaleLinkTruth.detail },
        { label: "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ adresi", value: buildExternalSaleLink(actionableSaleLink.url || getListingFeatureLink(feature), actionableSaleLink.label || getListingFeatureLinkLabel(feature)) },
        { label: "Confidence", value: props.confidence_score ? formatNumber(props.confidence_score, 1) : "-" },
        { label: "Last Updated", value: formatLandOverlayDate(props.source_updated_at) },
        { label: "Review", value: props.requires_review ? "Gerekli" : "Yok" },
      ];
      tags = [
        "SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ AlanÃƒâ€Ã‚Â±",
        "Parcel-first seÃƒÆ’Ã‚Â§im",
        actionableSaleTruth?.label || null,
        actionableSaleLinkTruth?.label || null,
        actionableSaleRecord?.source_name === "market_listing_adapter" ? "Portal fiyat/link" : null,
        actionableSaleRecord?.source_name === "supabase_admin" ? "Managed Sales fiyat/link" : null,
    ...
    return {
      ...
      sources: {
        ...
        satellite: {
          ...
          tileSize: 256,
          attribution: "Tiles Ãƒâ€šÃ‚Â© Esri",
        },
      },
      layers: [
        {
      ...
      const filteredMarketListings = filterLandOverlayFeatures({
        ...
        features: (marketListings.features || []).map((feature) => {
          ...
          const hiddenNote = !marketPolygonDisplayable
            ? "Bu kayit nokta olarak kalir; dogrulanmamis/dÃƒÂ¼zensiz satis poligonu gercek parsel gibi cizilmez."
            : "";
  ...
  async function switchRegion(region, reason = "auto") {
    ...
    showStatus(`${region.label} yÃƒÆ’Ã‚Â¼kleniyor...`, true);
    if (parcelPopup) {
      parcelPopup.remove();
      parcelPopup = null;
      activeParcelPopupState = null;
...
(function () {
  ...
  function tyFmtM2(value) {
    const n = Number(value);
    return Number.isFinite(n) ? n.toLocaleString(undefined, { maximumFractionDigits: 2 }) + " mÃƒâ€šÃ‚Â²" : tyEsc(value);
  }
  ...
  function tyCard(id) {
    ...
    return `
      <div class="ty-candidate-metrics-card" data-ty-candidate-metrics="${tyEsc(id)}">
        <div class="ty-candidate-metrics-title">SatÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ Geometrisi Aday Plan Bilgisi</div>
        <div class="ty-candidate-metrics-badge">Durum: derived / estimated candidate Ãƒâ€šÃ‚Â· GÃƒÆ’Ã‚Â¼ven: ${tyConfidenceLabel(m.confidence)}</div>
        <table class="ty-candidate-metrics-table">
          <tr><td>Alan</td><td>${tyFmtM2(m.area_m2)}</td></tr>
          <tr><td>Hesaplanan poligon alanÃƒâ€Ã‚Â±</td><td>${tyFmtM2(m.computed_area_m2)}</td></tr>
          <tr><td>ÃƒÆ’Ã¢â‚¬Â¡evre</td><td>${tyFmtM(m.perimeter_m)}</td></tr>
          <tr><td>Alan kaynaÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±</td><td>${tyEsc(m.area_source)} ${m.area_raw ? "(" + tyEsc(m.area_raw) + ")" : ""}</td></tr>
          <tr><td>YÃƒÆ’Ã‚Â¶ntem</td><td>${tyEsc(m.method)}</td></tr>
          <tr><td>Belge/gÃƒÆ’Ã‚Â¶rsel kanÃƒâ€Ã‚Â±tÃƒâ€Ã‚Â±</td><td>${tyEsc(m.doc_evidence || "unknown")}</td></tr>
        </table>
        <div class="ty-candidate-metrics-subtitle">Kenar uzunluklarÃƒâ€Ã‚Â±</div>
        ...
        <div class="ty-candidate-metrics-note">
          Bu geometri gerÃƒÆ’Ã‚Â§ek/original verified boundary deÃƒâ€Ã…Â¸ildir. Alan, konum, kaynak sinyalleri ve oranlardan tÃƒÆ’Ã‚Â¼retilmiÃƒâ€¦Ã…Â¸ satÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ geometrisi adayÃƒâ€Ã‚Â±dÃƒâ€Ã‚Â±r.
        </div>
      </div>
    `;
  }
  ...
  function tyBuildInfoHtml(context, feature) {
    ...
    return `
      <div class="ty-sale-geometry-card" data-ty-sale-geometry-card="1">
        <div class="title">SatÃ„Â±Ã…Å¸ Geometrisi</div>
        <div class="badge">GÃƒÂ¼ven: ${tyEsc(meta.confidence || "-")}</div>
        ...
        <table>
          ...
          <tr><td>Ãƒâ€¡evre</td><td>${tyFmtM(meta.perimeter_m)}</td></tr>
          ${sideRows}
        </table>
        <div class="note">
          <div><strong>Brownfield kaynaklarÃ„Â±:</strong> ${tyEsc(context?.brownfield?.text || "-")}</div>
  ...
  function tyEnsureOverlayLayers() {
    ...
    if (!map.getLayer(TY_LABEL_ID)) {
      map.addLayer({
        ...
        source: TY_SOURCE_ID,
        layout: {
          "text-field": ["coalesce", ["get", "label"], "SatÃ„Â±Ã…Å¸"],
          "text-size": 12,
          "text-offset": [0, 0]
        },
        paint: {
      ...
      features: [
        {
          ...
          properties: {
            label: `SatÃ„Â±Ã…Å¸ ${meta.confidence || ""}`.trim()
          }
        }
      ]
    });
