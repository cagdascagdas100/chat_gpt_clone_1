...
try {
  if (-not (Test-Path -Path $TaskFile)) {
    ...
    Commit-And-Push -Message "chore(ai): oneshot task file missing"
    exit 1
  }
  ...
  if (-not $ScriptName -or $ScriptName.Trim() -eq "") {
    ...
    Write-Handoff -Status "idle" -TaskId $TaskId -Message "No script_path provided." -ResultPath $ResultPath
    Commit-And-Push -Message "chore(ai): oneshot idle $TaskId"
    exit 0
  }
  ...
  if (-not $ScriptPath.StartsWith($AllowedRoot, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "Blocked unsafe script path: $ScriptPath"
  }
  ...
  Write-Handoff -Status "running" -TaskId $TaskId -Message "One-shot executor running task."
  ...
  Commit-And-Push -Message "chore(ai): oneshot result $TaskId"
  exit 0
} catch {
  $TaskIdForError = ""
  ...
  Commit-And-Push -Message "chore(ai): oneshot error $TaskIdForError"
  exit 1
}
