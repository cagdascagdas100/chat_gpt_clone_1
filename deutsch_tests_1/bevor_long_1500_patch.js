...
function wrapGeneratedLessonExamples(original){
  ...
  var wrapped=function(level){var key=currentBevorKey();if(isBevorSourceLocked(key)){clearBevorSourceCache(key);return false;}return original.apply(this,arguments);};
  wrapped.__bevorSourceLockedDisabled=true;wrapped.__original=original;return wrapped;
}
function installGeneratedLessonExamplesTrap(){
  if(window.__AAYS_APPEND_EXAMPLES_TRAP_INSTALLED){if(typeof window.appendGeneratedLessonExamples==='function')window.appendGeneratedLessonExamples=wrapGeneratedLessonExamples(window.appendGeneratedLessonExamples);return;}
  try{var stored=wrapGeneratedLessonExamples(window.appendGeneratedLessonExamples);Object.defineProperty(window,'appendGeneratedLessonExamples',{configurable:true,enumerable:true,get:function(){return stored;},set:function(fn){stored=wrapGeneratedLessonExamples(fn);}});window.__AAYS_APPEND_EXAMPLES_TRAP_INSTALLED=true;}catch(e){if(typeof window.appendGeneratedLessonExamples==='function')window.appendGeneratedLessonExamples=wrapGeneratedLessonExamples(window.appendGeneratedLessonExamples);}
}
...
function cleanBevorGeneratedBlocks(){
  ...
  if(!root)return;
  Array.prototype.slice.call(root.querySelectorAll('section')).forEach(function(sec){var text=String(sec.textContent||'');if(blockedHeadings.some(function(h){return text.indexOf(h)!==-1;})){sec.remove();}});
}
...
function enforceBevorSourceRender(){
  applySourceLong1500();cleanBevorGeneratedBlocks();
  var key=currentBevorKey();if(!isBevorSourceLocked(key))return;
  var content=document.getElementById('lessonContent'),lesson=document.getElementById('lesson');
