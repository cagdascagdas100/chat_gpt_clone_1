...
if(!window.AAYS_EHRENAMT_VORTEILE_LOADER_OK){
...
function clearBevorSourceCache(key){if(!key)return;try{localStorage.removeItem('AAYS_CANONICAL_LONG_V3_'+key);}catch(e){}}
function levelName(level){return level==='short'?'Kısa':level==='medium'?'Orta':'Uzun';}
function buttonLevel(btn){if(!btn)return '';if(btn.id==='btnLessonShort')return 'short';if(btn.id==='btnLessonMedium')return 'medium';if(btn.id==='btnLessonLong')return 'long';return '';}
var blockedHeadings=['Mini paragraf örnekleri','Ezberlenecek kalıplar','Gramer ve Rektion','C1/C2 örnek cümleler','Hata uyarıları','Yazma görevi'];
...
function sourceLong1500(key,heading,orderText){
  ...
  var html='<h3>'+esc(heading)+' · ausführliche Quellenfassung</h3>'+'<p><b>Prinzip:</b> Diese lange Fassung nutzt nur die im Repository vorhandenen Quellen zu '+esc(key)+'. Die Informationen werden nicht als technische Übungsliste angezeigt, sondern als geordnetes Material für eine flüssige C1/C2-Konu anlatımı. Fremde Beispiele und erfundene Zusatzinformationen werden nicht ergänzt.</p>'+'<section>'+base+'</section>'+'<h4>Zentraler Wortschatz aus der Quelle</h4>'+list((T.words||[]).map(esc))+'<h4>Satzbausteine aus dem Quellenmaterial</h4>'+fillList(T)+'<h4>Kernformulierungen für die Argumentation</h4>'+mcList(T)+'<h4>Inhaltliche Grenzen und stilistische Kontrolle</h4>'+tfList(T)+'<h4>Begriffe und Bedeutungen</h4>'+pairList(T.wordMatch)+'<h4>Feste Verbindungen für einen natürlichen Stil</h4>'+pairList(T.phraseMatch)+'<h4>Präpositionen und Satzanschlüsse</h4>'+prepList(T)+'<h4>Formulierungen, die im Absatz wiederverwendet werden können</h4>'+list((T.hang||[]).map(esc))+'<h4>Schreiblogik ohne Wiederholung und ohne Erfindung</h4><p>'+esc(orderText)+'</p>';
...
function wrapGeneratedLessonExamples(original){if(typeof original!=='function')return original;if(original.__bevorSourceLockedDisabled)return original;var wrapped=function(level){var key=currentBevorKey();if(isBevorSourceLocked(key)){clearBevorSourceCache(key);return false;}return original.apply(this,arguments);};wrapped.__bevorSourceLockedDisabled=true;wrapped.__original=original;return wrapped;}
function installGeneratedLessonExamplesTrap(){if(window.__AAYS_APPEND_EXAMPLES_TRAP_INSTALLED){if(typeof window.appendGeneratedLessonExamples==='function')window.appendGeneratedLessonExamples=wrapGeneratedLessonExamples(window.appendGeneratedLessonExamples);return;}try{var stored=wrapGeneratedLessonExamples(window.appendGeneratedLessonExamples);Object.defineProperty(window,'appendGeneratedLessonExamples',{configurable:true,enumerable:true,get:function(){return stored;},set:function(fn){stored=wrapGeneratedLessonExamples(fn);}});window.__AAYS_APPEND_EXAMPLES_TRAP_INSTALLED=true;}catch(e){if(typeof window.appendGeneratedLessonExamples==='function')window.appendGeneratedLessonExamples=wrapGeneratedLessonExamples(window.appendGeneratedLessonExamples);}}
function patchGeneratedLessonExamples(){installGeneratedLessonExamplesTrap();}
function cleanBevorGeneratedBlocks(){var key=currentBevorKey();if(!isBevorSourceLocked(key))return;clearBevorSourceCache(key);var root=document.getElementById('lessonContent');if(!root)return;Array.prototype.slice.call(root.querySelectorAll('section')).forEach(function(sec){var text=String(sec.textContent||'');if(blockedHeadings.some(function(h){return text.indexOf(h)!==-1;})){sec.remove();}});}
function renderBevorSourceLesson(level){applySourceLong1500();var key=currentBevorKey();var L=(window.DEUTSCH_LESSONS||{})[key]||{};var T=(window.DEUTSCH_TESTS||{})[key]||{};if(!isBevorSourceLocked(key)||!L[level])return false;clearBevorSourceCache(key);var lesson=document.getElementById('lesson'),title=document.getElementById('lessonTitle'),meta=document.getElementById('lessonMeta'),content=document.getElementById('lessonContent');if(!lesson||!title||!meta||!content)return false;try{if(typeof hide==='function')hide();}catch(e){}lesson.classList.remove('hide');title.textContent='Konu anlatımı: '+(T.title||key);meta.textContent='Seviye: '+levelName(level)+' · Word kaynaklı içerik';content.dataset.bevorSourceLevel=level;content.innerHTML='<section style="padding:14px;border:1px solid #d8d3ca;border-radius:14px;background:#fff;margin-bottom:18px"><h2>Konu açıklaması</h2>'+L[level]+'</section>';try{lesson.scrollIntoView({behavior:'smooth'});}catch(e){}return true;}
function enforceBevorSourceRender(){applySourceLong1500();cleanBevorGeneratedBlocks();var key=currentBevorKey();if(!isBevorSourceLocked(key))return;var content=document.getElementById('lessonContent'),lesson=document.getElementById('lesson');if(!content||!lesson||lesson.classList.contains('hide'))return;var level=content.dataset.bevorSourceLevel||window.AAYS_BEVOR_LAST_LEVEL||'long';var text=String(content.textContent||'');var bad=blockedHeadings.some(function(h){return text.indexOf(h)!==-1;});if(bad){renderBevorSourceLesson(level);}}
...
applySourceLong1500();installGeneratedLessonExamplesTrap();
document.addEventListener('click',function(ev){var btn=ev.target&&ev.target.closest&&ev.target.closest('#btnLessonShort,#btnLessonMedium,#btnLessonLong');var level=buttonLevel(btn);if(level&&isBevorSourceLocked(currentBevorKey())){window.AAYS_BEVOR_LAST_LEVEL=level;if(renderBevorSourceLesson(level)){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation();setTimeout(cleanBevorGeneratedBlocks,0);return false;}}setTimeout(installBevorSourceLock,0);setTimeout(installBevorSourceLock,150);setTimeout(installBevorSourceLock,600);},true);
