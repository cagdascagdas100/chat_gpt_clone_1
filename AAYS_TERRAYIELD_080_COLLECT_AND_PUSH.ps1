...
$TaskId='terrayield-080-patch-admin-supabase-5worker-local'
...
$RunLog=Join-Path $LogDir ('terrayield-080-collect-'+$Stamp+'.log')
...
$LatestRun=Get-ChildItem -LiteralPath (Join-Path $ProjectRoot '.aays_real_runs') -Directory -ErrorAction SilentlyContinue | Where-Object { $_.Name -like 'terrayield__080_patch_admin_supabase_5worker__*' } | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
if($LatestRun){
  ...
  foreach($f in Get-ChildItem -LiteralPath $LatestRun.FullName -Recurse -File -ErrorAction SilentlyContinue | Select-Object -First 50){
