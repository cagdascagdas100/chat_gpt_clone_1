...
$TaskId = 'terrayield-077-permission-safe-5worker-direct'
$ScriptPath = Join-Path $BridgeRoot 'ai-task-scripts\terrayield_077_permission_safe_5worker.ps1'
...
$RunLog = Join-Path $LogDir ('terrayield-077-direct-' + $Stamp + '.log')
...
$LatestRun = Get-ChildItem -LiteralPath (Join-Path $ProjectRoot '.aays_real_runs') -Directory -ErrorAction SilentlyContinue |
  Where-Object { $_.Name -like 'terrayield__077_permission_safe_5worker__*' } |
...
} else { Add-Content -Encoding UTF8 -Path $ResultFile -Value 'LATEST_RUN_DIR=not_found' }
