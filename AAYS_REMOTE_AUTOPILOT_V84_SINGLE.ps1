...
$HeartbeatFile = Join-Path $HeartbeatDir 'remote-autopilot-v84.md'
$LastTaskFile = Join-Path $StateDir 'remote-autopilot-v84.last-task-id'
$RunnerLog = Join-Path $LogsDir ('remote-autopilot-v84-' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.log')
$PollSeconds = 30
...
function HB([string]$Status,[string]$TaskId,[string]$Message) {
  Set-Content -Encoding UTF8 -Path $HeartbeatFile -Value @(
    '# TerraYield Remote Autopilot V8.4','',
    ('status: ' + $Status),
    ('task_id: ' + $TaskId),
...
function GitRun {
  ...
  try {
    ...
    if ($null -eq $GitArgs -or $GitArgs.Count -eq 0) {
      Log 'GIT_EMPTY_ARGS_BLOCKED'
      return @{ Code = 997; Out = 'empty git args' }
    }
    $txt = $GitArgs -join ' '
    Log ('GIT_START ' + $txt)
    ...
    Log ('GIT_EXCEPTION ' + ($GitArgs -join ' ') + ' :: ' + $_.Exception.Message)
...
function CommitLocalState([string]$Label) {
  ...
  if ([string]::IsNullOrWhiteSpace($s.Out)) {
    Log ('NO_LOCAL_STATE_TO_COMMIT ' + $Label)
    return $false
  }
  $c = GitRun commit -m ("autopilot-v84 state " + $Label)
  return ($c.Code -eq 0)
}

function PushLocalState([string]$Label) {
  CommitLocalState $Label | Out-Null
  for ($i = 1; $i -le 8; $i++) {
    EnsureMain
    GitRun fetch origin main | Out-Null
    $rb = GitRun pull --rebase --autostash origin main
    if ($rb.Code -ne 0) {
      GitRun rebase --abort | Out-Null
      GitRun pull --rebase --autostash origin main | Out-Null
    }
    $p = GitRun push origin main
    if ($p.Code -eq 0) {
      Log ('PUSH_OK attempt=' + $i)
      return $true
    }
...
function PullLatestPreserveLocal {
...
function WriteResult([string]$Id,[string]$Title,[int]$Code,[string]$Stdout,[string]$Stderr,[string]$Message) {
  ...
  Set-Content -Encoding UTF8 -Path $result -Value @('# TerraYield Autopilot V8.4 Result','',('task_id: ' + $Id),('title: ' + $Title),('exit_code: ' + $Code),('message: ' + $Message),('time: ' + (Get-Date -Format 's')),'','## Output','```text')
...
function RunTask($Task) {
  ...
  if ([string]::IsNullOrWhiteSpace($sp)) {
    Set-Content -Encoding UTF8 -Path $stderr -Value 'missing script_path'
  ...
  WriteResult $id $title $code $stdout $stderr 'completed_by_v84'
  Set-Content -Encoding UTF8 -Path $LastTaskFile -Value $id
...
Log 'Autopilot V8.4 started'
HB 'polling' '' 'started'
PushLocalState 'v84-started' | Out-Null
while ($true) {
  try {
    HB 'polling' '' 'pulling'
    if (-not (PullLatestPreserveLocal)) {
      HB 'error' '' 'pull failed; will retry'
      PushLocalState 'pull-failed' | Out-Null
      Start-Sleep -Seconds $PollSeconds
      continue
    }
    if (-not (Test-Path $TaskFile)) {
      HB 'polling' '' 'no task file'
      PushLocalState 'no-task-file' | Out-Null
      Start-Sleep -Seconds $PollSeconds
      continue
    }
    ...
    PushLocalState 'loop-error' | Out-Null
  }
  Start-Sleep -Seconds $PollSeconds
}
